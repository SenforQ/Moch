...
fileprivate let dataRowKey:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "More" :*/
fileprivate let main_stopMsg:String = "Morein list title false"

/*: "icon_details_arrow" :*/
fileprivate let showSexAfterSubMessage:String = "iblock"
fileprivate let showBeautyData:String = "self popon_det"
fileprivate let show_dataPath:String = "RROW"

/*: " (%ld)" :*/
fileprivate let notiInstanceContent:String = "add height head (%ld)"

/*: "Post" :*/
fileprivate let constGiftMessage:String = "Postsize top style make"

/*: "btn_dynamic_stop_nor" :*/
fileprivate let show_centerUrl:String = "else hiddenbtn_d"
fileprivate let k_viewKey:String = "_stop_nomodel model"
fileprivate let constPathKey:String = "add"
...
class BuildUserThen: UITableViewCell {
    //: var userId: String = ""
    var userId: String = ""
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataRowKey.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var maskMoreView: UIView = {
        ...
        view.backgroundColor = .showViewCreate(0, 0, 0, 0.4)
    ...
    lazy var moreButton: BlockButton = .init().then {
        //: $0.titleLabel?.font = .pingfangFont(type: .Regular, fontSize: 17)
        $0.titleLabel?.font = .photographicEquipment(type: .Regular, fontSize: 17)
        //: $0.setTitle("More".localized, for: .normal)
        $0.setTitle((String(main_stopMsg.prefix(4))).localized, for: .normal)
        //: $0.setImage(UIImage(named: "icon_details_arrow"), for: .normal)
        $0.setImage(UIImage(named: (showSexAfterSubMessage.replacingOccurrences(of: "block", with: "c") + String(showBeautyData.suffix(6)) + "ails_a" + show_dataPath.lowercased())), for: .normal)
...
extension BuildUserThen {
    ...
    func back(userModel: GainDetailsMeasurable) {
        ...
        strA = [(String(constGiftMessage.prefix(4))).localized, str]
        //: colorA = [UIColor.appTitleColor(), UIColor.appValueColor()]
        colorA = [UIColor.towardOf(), UIColor.heatherMixture()]
        //: fontA = [UIFont.pingfangMediumFont(fontSize: 18), UIFont.pingfangMediumFont(fontSize: 16)]
        fontA = [UIFont.outView(fontSize: 18), UIFont.outView(fontSize: 16)]
        ...
        for index in 0 ..< itemCount {
            ...
            if index == 0 {
                ...
                let model: ReportCardTransformable = (userModel.momentPic?.list)![index]
                ...
                firstImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.warning())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: firstImgView)
                    endTo(btn: firstImgView)
                }
                ...
                secondImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.warning())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: secondImgView)
                    endTo(btn: secondImgView)
                }
                //: } else {
            } else {
                //: let model: TalkingUserInfoMomentItemModel = (userModel.momentPic?.list)![index]
                ...
                thirdImgView.kf.setImage(with: URL(string: model.img_url), placeholder: UIImage.warning())

                //: if model.moment_type == 2 {
                if model.moment_type == 2 {
                    //: setPlayView(btn: thirdImgView)
                    endTo(btn: thirdImgView)
                }
            }
        }
    }
    ...
    @objc func post() {
        ...
        let vc = ConstraintEmptyViewDelegate(uid: userId)
    ...
    func endTo(btn: UIImageView) {
        ...
        let imag = UIImageView(image: UIImage(named: (String(show_centerUrl.suffix(5)) + "ynamic" + String(k_viewKey.prefix(8)) + constPathKey.replacingOccurrences(of: "add", with: "r"))))
...
extension BuildUserThen {
    ...
    private func sumernal() {
        ...
        let height = (constExplainViewMsg - actualWidth(w: 15 * 4)) / 3.0
