...
fileprivate let mainMicTitle:[UInt8] = [0xb0,0xb7,0xb0,0xad,0xf1,0xba,0xb6,0xbd,0xbc,0xab,0xe3,0xf0,0xf9,0xb1,0xb8,0xaa,0xf9,0xb7,0xb6,0xad,0xf9,0xbb,0xbc,0xbc,0xb7,0xf9,0xb0,0xb4,0xa9,0xb5,0xbc,0xb4,0xbc,0xb7,0xad,0xbc,0xbd]

private func eventVideo(enable num: UInt8) -> UInt8 {
    return num ^ 217
}

/*: "icon_userdetail_header_shadow" :*/
fileprivate let kEquipmentUrl:String = "icon_top medium path common source"
fileprivate let const_indexColorData:String = "detview"
fileprivate let notiAvailableText:[Character] = ["_"]
fileprivate let user_screenId:String = "shadosubmit"

/*: "icon_lounge" :*/
fileprivate let user_shUrl:String = "value button app labelicon_l"
fileprivate let showEvaluateText:String = "oungnumber"

/*: "Official" :*/
fileprivate let user_tableVideoKey:[Character] = ["O","f","f","i","c","i"]
fileprivate let noti_sizeMessage:String = "afatal"

/*: "Reviewing" :*/
fileprivate let mainBlockPath:String = "list self matchReview"
fileprivate let constColorUrl:String = "popng"

/*: "#FF506D" :*/
fileprivate let noti_documentTitle:[Character] = ["#","F","F","5","0","6"]
fileprivate let constCurrentGreetData:String = "d"

/*: "icon_user_detail_intimate_header" :*/
fileprivate let kLabelFormat:[UInt8] = [0x50,0x5a,0x56,0x57,0x66,0x4c,0x4a,0x5c,0x4b,0x66,0x5d,0x5c,0x4d,0x58,0x50,0x55,0x66,0x50,0x57,0x4d,0x50,0x54,0x58,0x4d,0x5c,0x66,0x51,0x5c,0x58,0x5d,0x5c,0x4b]

private func willTable(edge num: UInt8) -> UInt8 {
    return num ^ 57
}

/*: "?toUid= :*/
fileprivate let data_screenUrl:String = "bird let insert follow m?toUid="

/*: "PortionView" :*/
fileprivate let mainWindowFormat:String = "TUserDeaccept pop center style"
fileprivate let app_toFormat:String = "drop"
fileprivate let constFinishFirstEqualValue:[Character] = ["a","i","l","H","e","a","d","e","r","I","t","e","m","C","e","l","l"]

/*: "#FFFFFF" :*/
fileprivate let mainSendName:[Character] = ["#","F"]
fileprivate let app_contentData:[Character] = ["F","F","F","F","F"]

/*: "#BDB8B5" :*/
fileprivate let main_logKey:String = "#BDB8B5false size manager self"

/*: "head_detail_bottom" :*/
fileprivate let show_partyStr:String = "head_else language let effect"
fileprivate let show_diskId:String = "l_bottomlet view"

/*: "#2DF2FF" :*/
fileprivate let notiIndexMsg:[Character] = ["#","2","D","F","2"]
fileprivate let constLabelValue:[Character] = ["F","F"]

/*: "Live" :*/
fileprivate let appSelectId:[Character] = ["L","i","v","e"]

/*: "#48FF1E" :*/
fileprivate let show_eyeTitle:[Character] = ["#","4","8","F","F","1","E"]

/*: "Online" :*/
fileprivate let mainTagMsg:[Character] = ["O","n","l","i","n"]
fileprivate let const_closeFormat:String = "action"

/*: "#FF4B7F" :*/
fileprivate let userContentText:String = "display how#FF4B7F"

/*: "busy" :*/
fileprivate let main_rowViewPath:String = "bposty"
...
let mainInformationPath = (302.0 + appImageLimitPath)

//: class TalkingUserDetailHeaderView: UIView {
class VanguardBannerDelegate: UIView {
    //: var VideoCallBlock: (() -> Void)?
    var VideoCallBlock: (() -> Void)?
    //: private var userInfoModel = TalkingUserInfoModel()
    private var userInfoModel = GainDetailsMeasurable()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainMicTitle.map{eventVideo(enable: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var shadowBgView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.atomicCount85(name: (String(kEquipmentUrl.prefix(5)) + "user" + const_indexColorData.replacingOccurrences(of: "view", with: "a") + "il_header" + String(notiAvailableText) + user_screenId.replacingOccurrences(of: "submit", with: "w"))))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var vipBgView: UIImageView = {
        ...
        v.frame = CGRect(x: 0, y: mainInformationPath - 15, width: constExplainViewMsg, height: 15)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var loungeImgV: UIImageView = {
        ...
        img.image = UIImage.atomicCount85(name: (String(user_shUrl.suffix(6)) + showEvaluateText.replacingOccurrences(of: "number", with: "e")))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var officalImgV: UIButton = {
        ...
        img.forClick(color: UIColor.tips(), forState: .normal)
        //: img.setTitle("Official".localized, for: .normal)
        img.setTitle((String(user_tableVideoKey) + noti_sizeMessage.replacingOccurrences(of: "fatal", with: "l")).localized, for: .normal)
        //: img.setTitleColor(.white, for: .normal)
        img.setTitleColor(.white, for: .normal)
        //: img.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 11)
        img.titleLabel?.font = UIFont.labelWith(fontSize: 11)
    ...
    private lazy var onlineImgV: BlockButton = {
    ...
    private lazy var liveView: ThreadMiniView = {
        //: let v = UserLivePullStreamsView()
        let v = ThreadMiniView()
        //: v.isHidden = true
        v.isHidden = true
        //: v.playEndBlock = {
        v.playEndBlock = {
        ...
        v.tapGestureBlock = { [weak self] in
            ...
            guard let self = self else { return }
            //: TotalReactiveCompatible.share.func_audiencePushToLiveRoomVC(uid: self.userInfoModel.uid, enterType: .userDetail)
            TotalReactiveCompatible.share.runQuantityo(uid: self.userInfoModel.uid, enterType: .userDetail)
        }
        //: return v
        return v
        //: }()
    ...
    private lazy var reViewingLB: UILabel = {
        ...
        lb.text = (String(mainBlockPath.suffix(6)) + constColorUrl.replacingOccurrences(of: "pop", with: "i")).localized
        //: lb.textColor = .white
        lb.textColor = .white
        //: lb.backgroundColor = UIColor.init(hex: "#FF506D")
        lb.backgroundColor = UIColor(hex: (String(noti_documentTitle) + constCurrentGreetData.uppercased()))
    ...
    lazy var lastLoginLab: UILabel = {
        ...
        lb.font = .photographicEquipment(type: .Medium, fontSize: 12)
        //: lb.textColor = .white
        lb.textColor = .white
        //: return lb
        return lb
    ...
    private lazy var intimateBtn: BlockButton = {
        //: let btn = TalkingButton()
        let btn = BlockButton()
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_user_detail_intimate_header"), for: .normal)
        btn.setImage(UIImage.atomicCount85(name: String(bytes: kLabelFormat.map{willTable(edge: $0)}, encoding: .utf8)!), for: .normal)
        //: btn.isHidden = true
        btn.isHidden = true
        //: btn.addTarget(self, action: #selector(intimateBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(sumimateLikeClick), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension VanguardBannerDelegate {
    /// 查看视频
    //: private func goToVideo() {
    private func appropriate() {
        //: let vc = TalkingMomentVideoVC.init(videoPath: self.userInfoModel.videoUrl)
        let vc = CurrentVideoVc(videoPath: self.userInfoModel.videoUrl)
        //: if NowInfoThen.share.loginUid != self.userInfoModel.uid {
        if NowInfoThen.share.loginUid != self.userInfoModel.uid {
    ...
    private func titlePhoto(_ index: Int) {
        ...
        let vc = MomentScreenVc(momentModel: currenModel, index: index, type: .normal)
        //: self.currentViewController()?.navigationController?.pushViewController(vc, animated: true)
        self.buttonController()?.navigationController?.pushViewController(vc, animated: true)
    }

    //: @objc func intimateBtnClick() {
    @objc func sumimateLikeClick() {
        //: let url = TalkingWebManager.getFullUrl(urlType: .Intimate) + "?toUid=\(self.userInfoModel.uid)"
        let url = ViewProduceThen.nearOfType(urlType: .Intimate) + (String(data_screenUrl.suffix(7))) + "\(self.userInfoModel.uid)"
...
extension VanguardBannerDelegate: JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: PortionView.self, reuseIdentifier: (String(mainWindowFormat.prefix(7)) + app_toFormat.replacingOccurrences(of: "drop", with: "t") + String(constFinishFirstEqualValue)))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        if index == 0 && userInfoModel.sex == PointExpressionConvertible.female.rawValue && userInfoModel.videoUrl.count > 1 { // 视频
            ...
            bannerCell.targetTitleErase(videoUrl: userInfoModel.videoUrl, imgUrl: userInfoModel.cover)
            ...
            bannerCell.bgImgView.luculentUpwardly(urlStr: urlStr, placeImg: UIImage.towardLineFrom())
        }
    ...
    func jxBanner(_: JXBannerType, centerIndex: Int, centerCell _: UICollectionViewCell) {
        //: guard userInfoModel.uid == NowInfoThen.share.loginUid else {
        guard userInfoModel.uid == NowInfoThen.share.loginUid else {
            //: return
            return
        }
    ...
    func jxBanner(pageControl _: JXBannerType, numberOfPages _: Int, coverView: UIView, builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder {
        ...
        pageControl.activeColor = UIColor(hex: (String(mainSendName) + String(app_contentData)))!
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(main_logKey.prefix(7))))!
...
extension VanguardBannerDelegate: JXBannerDelegate {
    ...
    public func jxBanner(_: JXBannerType, didSelectItemAt index: Int) {
        //: if index == 0 && userInfoModel.sex == Gender.female.rawValue && userInfoModel.videoUrl.count>1 {
        if index == 0 && userInfoModel.sex == PointExpressionConvertible.female.rawValue && userInfoModel.videoUrl.count > 1 {
            //: goToVideo()
            appropriate()
            ...
            if userInfoModel.sex == PointExpressionConvertible.female.rawValue && userInfoModel.videoUrl.count > 1 && curren > 0 {
                //: curren -= 1
                curren -= 1
            }
            //: goToPhoto(curren)
...
extension VanguardBannerDelegate {
    ...
    func afterView(_ userModel: GainDetailsMeasurable) {
        ...
        if userModel.sex == PointExpressionConvertible.female.rawValue && userModel.videoUrl.count > 1 {
        ...
        let normalImg = UIImage.atomicCount85(name: (String(show_partyStr.prefix(5)) + "detai" + String(show_diskId.prefix(8)))).withTintColor(.white)
        //: if userModel.loungePlus {
        if userModel.loungePlus {
            //: if let vipModel = NowInfoThen.share.appUserConfigMode.vipSetting.filter({$0.vipSkinId == userModel.vipSkinId}).first {
            if let vipModel = NowInfoThen.share.appUserConfigMode.vipSetting.filter({ $0.vipSkinId == userModel.vipSkinId }).first {
                //: vipBgView.setUrlImage(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
                vipBgView.luculentUpwardly(urlStr: vipModel.vipSkinTop, placeImg: normalImg)
            }
            //: } else {
        } else {
        ...
        self.nicknameLab.textColor = userModel.loungePlus ? .equalColor() : .white
        ...
        if userModel.intimateUrl.count > 0 {
            ...
            intimateImgV.viewFinish(urlStr: userModel.intimateUrl)
        }
        // 在线状态
        ...
        if userModel.livePlayUrl.count > 0 {
            ...
            let image = UIImage.colorDetail(color: UIColor(hex: (String(notiIndexMsg) + String(constLabelValue)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.closingUniform(), for: .normal)
            onlineImgV.setImage(image.closingUniform(), for: .normal)
            //: onlineImgV.setTitle("Live".localized, for: .normal)
            onlineImgV.setTitle((String(appSelectId)).localized, for: .normal)
            ...
            let image = UIImage.colorDetail(color: UIColor(hex: (String(show_eyeTitle)))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.closingUniform(), for: .normal)
            onlineImgV.setImage(image.closingUniform(), for: .normal)
            //: onlineImgV.setTitle("Online".localized, for: .normal)
            onlineImgV.setTitle((String(mainTagMsg) + const_closeFormat.replacingOccurrences(of: "action", with: "e")).localized, for: .normal)
            ...
            let image = UIImage.colorDetail(color: UIColor(hex: (String(userContentText.suffix(7))))!, size: CGSize(width: 8, height: 8))
            //: onlineImgV.setImage(image.closingUniform(), for: .normal)
            onlineImgV.setImage(image.closingUniform(), for: .normal)
            //: onlineImgV.setTitle("busy".localized, for: .normal)
            onlineImgV.setTitle((main_rowViewPath.replacingOccurrences(of: "post", with: "us")).localized, for: .normal)
            //: } else {
        } else {
        ...
        // 直播
        //: if NowInfoThen.share.loginUid != userModel.uid &&
        if NowInfoThen.share.loginUid != userModel.uid,
        ...
        if liveView.isHidden == false {
            ...
            liveView.nameAdd(liveUrl: userModel.livePlayUrl)
        }
    }
    ...
    private func pushConstraint() {
        ...
        liveView.snp.makeConstraints { make in
            ...
            make.top.equalTo(appImageLimitPath + 80)
        ...
        reViewingLB.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(appImageLimitPath + 15)
