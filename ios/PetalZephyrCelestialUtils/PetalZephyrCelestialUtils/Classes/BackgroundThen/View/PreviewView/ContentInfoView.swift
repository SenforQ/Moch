...
fileprivate let showNeedMaleSendName:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "icon_male_default" :*/
fileprivate let userModelPath:[UInt8] = [0xed,0xe7,0xeb,0xea,0xdb,0xe9,0xe5,0xe8,0xe1,0xdb,0xe0,0xe1,0xe2,0xe5,0xf1,0xe8,0xf0]

private func viewHeight(click num: UInt8) -> UInt8 {
    return num ^ 132
}

/*: "icon_female_default" :*/
fileprivate let notiModelPath:[Character] = ["i","c","o","n","_","f","e","m","a","l","e","_","d","e","f"]
fileprivate let app_timeTitle:String = "statusult"

/*: "level_%@" :*/
fileprivate let showArrayTitle:String = "true make imagelevel_%@"
...
class ContentInfoView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showNeedMaleSendName.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var nickLabel = UILabel().then {
        ...
        $0.font = .photographicEquipment(type: .Medium, fontSize: 17)
        //: $0.textColor = .white
        $0.textColor = .white
    }
    ...
    lazy var ageLabel = UILabel().then {
        ...
        $0.font = .photographicEquipment(type: .Regular, fontSize: 10)
        //: $0.textColor = .white
        $0.textColor = .white
    }
...
extension ContentInfoView {
    ...
    func infoModel(model: PtolemaicSystemPreviewDatabaseHandyJSON) {
        //: headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.placeImgSquare())
        headerPic.kf.setImage(with: URL(string: model.headPic), placeholder: UIImage.warning())
        ...
        let imgStr = model.sex == PointExpressionConvertible.male.rawValue ? String(bytes: userModelPath.map{viewHeight(click: $0)}, encoding: .utf8)! : (String(notiModelPath) + app_timeTitle.replacingOccurrences(of: "status", with: "a"))
        //: sexImgView.image = UIImage(named: imgStr)
        sexImgView.image = UIImage(named: imgStr)
        //: let levelStr = String(format: "level_%@", model.level)
        let levelStr = String(format: (String(showArrayTitle.suffix(8))), model.level)
...
extension ContentInfoView {
    ...
    private func chemistryLaboratory() {
