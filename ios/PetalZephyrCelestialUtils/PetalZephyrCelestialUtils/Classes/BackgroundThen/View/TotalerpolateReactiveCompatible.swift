...
fileprivate let notiColorKey:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "Edit Title" :*/
fileprivate let main_pureValue:String = "user let attribute image modeEdit"
fileprivate let main_quoteName:[Character] = ["l","e"]

/*: "0/ :*/
fileprivate let user_bagName:String = "0/"

/*: "Finish" :*/
fileprivate let showLeadingItemName:[Character] = ["F","i","n","i","s","h"]

/*: "btn_delete" :*/
fileprivate let notiToMatchFormat:String = "main icon return actionbtn_d"

/*: / :*/
fileprivate let constDetailData:String = "/"

/*: "\n" :*/
fileprivate let dataAddFemaleFormat:String = "\n"
...
// __CLOSE_PRINT__
//
//  TotalerpolateReactiveCompatible.swift
//  AbroadTalking
//
...
class TotalerpolateReactiveCompatible: UIView {
    ...
    override init(frame: CGRect) {
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(iconDismissView))
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiColorKey.reversed(), encoding: .utf8)!)
    }
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = (String(main_pureValue.suffix(4)) + " Tit" + String(main_quoteName)).localized
    ...
    lazy var inputTView: AddNameView = {
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(showLeadingItemName)).localized, for: .normal)
        ...
        btn.setBackgroundImage(UIImage.gradientMore(colors: UIColor.showCorner(), size: CGSize(width: 267, height: 50)), for: .normal)
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.atomicCount85(name: (String(notiToMatchFormat.suffix(5)) + "elete")), for: .normal)
        //: btn.addTarget(self, action: #selector(closeBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(adpressedWithoutMove), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension TotalerpolateReactiveCompatible {
    //: @objc func dismissView() {
    @objc func iconDismissView() {}

    //: @objc func finishBtnClick() {
    @objc func clickEqual() {
        //: dismiss()
        communication()
        //: if textUpload != nil {
        if textUpload != nil {
    ...
    @objc func adpressedWithoutMove() {
        //: dismiss()
        communication()
    }
...
extension TotalerpolateReactiveCompatible: UITextViewDelegate {
    ...
    public func textViewDidChange(_ textView: UITextView) {
        ...
        if textView.text.count > lengthLimit {
            ...
            if textNum > lengthLimit && lengthLimit > 0 {
                ...
                textView.text = paragraph(index: lengthLimit, text: textContent)
            }
        }
    ...
    private func paragraph(index: Int, text: String) -> String {
        //: if text.count <= index {
        if text.count <= index {
            //: return text
            return text
