...
fileprivate let showRankKey:[UInt8] = [0x54,0x53,0x54,0x49,0x15,0x5e,0x52,0x59,0x58,0x4f,0x7,0x14,0x1d,0x55,0x5c,0x4e,0x1d,0x53,0x52,0x49,0x1d,0x5f,0x58,0x58,0x53,0x1d,0x54,0x50,0x4d,0x51,0x58,0x50,0x58,0x53,0x49,0x58,0x59]

private func customText(image num: UInt8) -> UInt8 {
    return num ^ 61
}

/*: "#333333" :*/
fileprivate let app_leadingFormat:[Character] = ["#","3"]
fileprivate let showManagerGiftName:String = "33333"

/*: "#999999" :*/
fileprivate let noti_infoMessage:[Character] = ["#","9","9","9","9","9","9"]

/*: "isCupid" :*/
fileprivate let data_listText:[Character] = ["i","s","C","u","p","i","d"]

/*: "autoGreetAuth" :*/
fileprivate let noti_objectPath:[Character] = ["a","u","t","o","G","r","e","e","t"]
fileprivate let const_viewScreenTitlePath:[Character] = ["A","u","t","h"]

/*: "isLeaderboardAnonymous" :*/
fileprivate let noti_finishText:String = "user femaleisLea"
fileprivate let mainPicMessage:String = "center view separateardAnon"
fileprivate let constLabMsg:[Character] = ["y","m","o","u","s"]

/*: "isSameGendered" :*/
fileprivate let app_filterMessage:String = "isSamefilter color"
fileprivate let noti_valueFormat:String = "eretask"

/*: "type" :*/
fileprivate let k_sectionTitle:String = "tykeye"

/*: "value" :*/
fileprivate let data_sharedUrl:[Character] = ["v","a","l","u","e"]
...
class SmilerView: UITableViewCell {
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(doingRecent), name: UIApplication.didBecomeActiveNotification, object: nil)
    }

    //: required init?(coder aDecoder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: showRankKey.map{customText(image: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var titleBLB: UILabel = {
        ...
        lb.textColor = UIColor(hex: (String(app_leadingFormat) + showManagerGiftName.capitalized))
        //: return lb
        return lb
        //: }()
    }()
    ...
    lazy var switchView: UISwitch = {
        ...
        swit.addTarget(self, action: #selector(sizeFinish), for: .valueChanged)
        //: return swit
        return swit
        //: }()
    }()
...
extension SmilerView {
    //: func setCellMessage(titile: String, row: Int, isLast: Bool) {
    func later(titile: String, row: Int, isLast: Bool) {
...
//        if titile == MaterielCollection.General.rawValue {
//            LengthPermissionTool.checkPushNotification { isShow in
...
//        }
    ...
    func messageStore(detail: String) {
    ...
    func alongImageShow(isShow: Int) {
        //: if isShow == 1 {
        if isShow == 1 {
    ...
    @objc func sizeFinish() {
        //: if titleBLB.text == SettingsUniversalType.Turnon_Notif.rawValue.localized {
        if titleBLB.text == DataConverterKeyRepresentable.Turnon_Notif.rawValue.localized {
        ...
        } else if titleBLB.text == DataConverterKeyRepresentable.Cupid.rawValue.localized {
            //: setConfig(type: "isCupid", value: self.switchView.isOn)
            module(type: (String(data_listText)), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Automatic.rawValue.localized {
        } else if titleBLB.text == DataConverterKeyRepresentable.Automatic.rawValue.localized {
            //: setConfig(type: "autoGreetAuth", value: self.switchView.isOn)
            module(type: (String(noti_objectPath) + String(const_viewScreenTitlePath)), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Ranking.rawValue.localized {
        } else if titleBLB.text == DataConverterKeyRepresentable.Ranking.rawValue.localized {
            //: setConfig(type: "isLeaderboardAnonymous", value: self.switchView.isOn)
            module(type: (String(noti_finishText.suffix(5)) + "derbo" + String(mainPicMessage.suffix(7)) + String(constLabMsg)), value: self.switchView.isOn)
            //: } else if titleBLB.text == SettingsUniversalType.Same_gendered.rawValue.localized {
        } else if titleBLB.text == DataConverterKeyRepresentable.Same_gendered.rawValue.localized {
            //: setConfig(type: "isSameGendered", value: self.switchView.isOn)
            module(type: (String(app_filterMessage.prefix(6)) + "Gend" + noti_valueFormat.replacingOccurrences(of: "task", with: "d")), value: self.switchView.isOn)
        }
    }

    //: func setConfig(type: String, value: Bool) {
    func module(type: String, value: Bool) {
        //: let va = value ? 1 : -1
        let va = value ? 1 : -1
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["type"] = type
        dict[(k_sectionTitle.replacingOccurrences(of: "key", with: "p"))] = type
        //: dict["value"] = va
        dict[(String(data_sharedUrl))] = va

        //: ProgressHUD.show()
        DeleteReactiveCompatible.fileAdd()
        //: TalkingMeRequestTool.req_SettingChange(params: dict) { succeed, result, errorModel in
        ValueThen.hiddenView(params: dict) { succeed, _, _ in
            //: ProgressHUD.dismiss()
    ...
    @objc func doingRecent(notification _: NSNotification) {
//        if titleBLB.text == MaterielCollection.General.rawValue {
//            LengthPermissionTool.checkPushNotification { isShow in
//                DispatchQueue.main.sync {
//                    self.switchView.isOn = isShow
//                }
