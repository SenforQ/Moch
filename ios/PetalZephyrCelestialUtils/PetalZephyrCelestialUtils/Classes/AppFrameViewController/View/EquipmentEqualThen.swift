...
fileprivate let appObjectData:[UInt8] = [0x81,0x86,0x81,0x8c,0x40,0x7b,0x87,0x7c,0x7d,0x8a,0x52,0x41,0x38,0x80,0x79,0x8b,0x38,0x86,0x87,0x8c,0x38,0x7a,0x7d,0x7d,0x86,0x38,0x81,0x85,0x88,0x84,0x7d,0x85,0x7d,0x86,0x8c,0x7d,0x7c]

fileprivate func fillIn(voice num: UInt8) -> UInt8 {
...
/*: "8075F5" :*/
fileprivate let show_pageFormat:String = "table against center8075F"
fileprivate let const_messageData:String = "remove"

/*: "9A9B9C" :*/
fileprivate let user_photoTitle:String = "9A9B9Cto info domain data extension"

/*: "kOpacityAnimation" :*/
fileprivate let constNameValue:[Character] = ["k","O","p","a","c","i","t","y","A","n"]
fileprivate let app_systemNameUrl:[Character] = ["i"]
fileprivate let const_emptyTitle:String = "shareation"

/*: "opacity" :*/
fileprivate let kTodayId:[Character] = ["o","p","a","c","i","t","y"]

/*: "forwards" :*/
fileprivate let mainCanKey:String = "make"
fileprivate let constMePath:String = "oformatwaformatds"

/*: "easeIn" :*/
fileprivate let user_viewContent:String = "text manager click view eyeeaseIn"

/*: "8C7AF8" :*/
fileprivate let constEqualValue:String = "8"
fileprivate let noti_keyStr:String = "C7user8"
...
class EquipmentEqualThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appObjectData.map{fillIn(voice: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var textField: UITextField = {
        ...
        view.addTarget(self, action: #selector(sizeAdd(_:)), for: .editingChanged)
        //: return view
        return view
        //: }()
    }()
...
extension EquipmentEqualThen {
    ...
    func video() {
        ...
        for index in 0 ..< codeNumber {
            ...
            if index < verStr.count {
                ...
                layer.shadowColor = UIColor(hex: (String(show_pageFormat.suffix(5)) + const_messageData.replacingOccurrences(of: "remove", with: "5")))!.withAlphaComponent(0.6).cgColor
            }
            //: else {
            else {
                //: label.text = ""
                ...
                if index == verStr.count {
                    ...
                    layer.shadowColor = UIColor(hex: (String(user_photoTitle.prefix(6))))!.withAlphaComponent(0.25).cgColor
            ...
            gestureFile(index: index, hidden: index == verStr.count ? false : true)
        }
    }

    // 当输入内容等于验证码的长度时候，把输入的验证码回调
    //: @objc func textChage(_ textField: UITextField) {
    @objc func sizeAdd(_: UITextField) {
        //: func__changeCodeInputView()
        video()
    }

    //: func func__addLabelTapGesture(label: UILabel) {
    func requestError(label: UILabel) {
    ...
    fileprivate func gestureFile(index: Int, hidden: Bool) {
        //: let line = shapeArray[index]
        let line = shapeArray[index]
        //: if hidden {
        if hidden {
            //: line.removeAnimation(forKey: "kOpacityAnimation")
            line.removeAnimation(forKey: (String(constNameValue) + String(app_systemNameUrl) + const_emptyTitle.replacingOccurrences(of: "share", with: "m")))
            //: } else {
        } else {
            //: line.add(opacityAnimation(), forKey: "kOpacityAnimation")
            line.add(will(), forKey: (String(constNameValue) + String(app_systemNameUrl) + const_emptyTitle.replacingOccurrences(of: "share", with: "m")))
        }
    ...
    fileprivate func will() -> CABasicAnimation {
        //: let animation = CABasicAnimation.init(keyPath: "opacity")
        let animation = CABasicAnimation(keyPath: (String(kTodayId)))
        ...
        animation.fillMode = CAMediaTimingFillMode(rawValue: (mainCanKey.replacingOccurrences(of: "make", with: "f") + constMePath.replacingOccurrences(of: "format", with: "r")))
        //: animation.timingFunction = CAMediaTimingFunction.init(name: CAMediaTimingFunctionName.init(rawValue: "easeIn"))
        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName(rawValue: (String(user_viewContent.suffix(6)))))
        //: return animation
        return animation
    }
}
...
extension EquipmentEqualThen {
    ...
    private func lockObject() {
        ...
        for index in 0 ..< codeNumber {
            ...
            line.fillColor = UIColor(hex: (constEqualValue.capitalized + noti_keyStr.replacingOccurrences(of: "user", with: "AF")))?.cgColor
