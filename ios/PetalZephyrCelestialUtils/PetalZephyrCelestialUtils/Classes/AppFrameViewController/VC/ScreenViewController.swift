...
class ScreenViewController: TailRecognizerDelegate {
    //: var type = LoginType.PhoneLogin
    var type = InsertHashable.PhoneLogin // 修改类型
    //: var content = ""                // 手机号
    var content = "" // 手机号
    //: var isNewPwd = false
    ...
    private lazy var pwdView: HiddenView = {
        //: let v = TalkingSetNewPwdView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
        let v = HiddenView(frame: .zero, isNewPwd: self.isNewPwd, loginType: self.type)
...
extension ScreenViewController {
    ...
    private func error(_ pwd: String, _ vcode: String?) {
        //: ProgressHUD.show()
        ...
        BoundaryReactiveCompatible.weltanschauung(pwd: pwd, vcode: vcode) { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
    ...
    private func script() {
        ...
        BoundaryReactiveCompatible.displace { [weak self] succeed, _, _ in
            //: ProgressHUD.dismiss()
            DeleteReactiveCompatible.deflector()
            //: guard let self = self else { return }
            guard let self = self else { return }
            ...
            self.pwdView.solicitationVoice()
        }
    }
}
