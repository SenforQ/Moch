...
/*: "position.x" :*/
fileprivate let appToId:String = "posileading"

/*: "slideIn" :*/
fileprivate let mainColorValue:String = "model party new extensionslideI"
fileprivate let main_hiddenPath:String = "N"

/*: "slideOut" :*/
fileprivate let dataTitleContent:[Character] = ["s","l","i","d","e","O","u","t"]
...
// __CLOSE_PRINT__
//
//  RiverbankManagingDirectorThen.swift
...
class RiverbankManagingDirectorThen: NSObject {
    //: static let shared = GiftFloatViewManager()
    static let shared = RiverbankManagingDirectorThen()
    //: private var giftArr = [SocialBroadcastModel]()
    private var giftArr = [CommentBroadcastModel]()
    ...
    private lazy var giftView: FuturismThen = {
        //: let v = GiftFloatView()
        let v = FuturismThen()
        //: LineThen.getWindow().addSubview(v)
        LineThen.getName().addSubview(v)
        //: return v
        return v
        //: }()
    }()
...
extension RiverbankManagingDirectorThen {
    ...
    private func isServer(_ model: CommentBroadcastModel) {
        ...
        let isRTL = WithLanguageManager.shared.direction == .rightToLeft
        // 起始位置：屏幕外
        //: let startCenterX = isRTL ? -centerX : ScreenWidth + centerX
        let startCenterX = isRTL ? -centerX : constExplainViewMsg + centerX
        // 目标位置：屏幕中间
        //: let targetCenterX = ScreenWidth / 2
        let targetCenterX = constExplainViewMsg / 2
        // 结束位置：屏幕另一侧
        //: let endCenterX = isRTL ? ScreenWidth + centerX : -centerX
        let endCenterX = isRTL ? constExplainViewMsg + centerX : -centerX
        ...
        let slideIn = CABasicAnimation(keyPath: (appToId.replacingOccurrences(of: "leading", with: "ti") + "on.x"))
        ...
        self.giftView.layer.add(slideIn, forKey: (String(mainColorValue.suffix(6)) + main_hiddenPath.lowercased()))
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) { [weak self] in
            ...
            let slideOut = CABasicAnimation(keyPath: (appToId.replacingOccurrences(of: "leading", with: "ti") + "on.x"))
            ...
            self.giftView.layer.add(slideOut, forKey: (String(dataTitleContent)))
...
extension RiverbankManagingDirectorThen {
    ...
    func liveSize() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.makeShow(noti:)),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.withForeground),
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(self.phenomAcrossOf),
    ...
    @objc private func makeShow(noti: Notification) {
        ...
        guard let data = noti.object,
              ...
              let gift = JSONDeserializer<CommentBroadcastModel>.deserializeFrom(dict: data as? [String: Any], designatedPath: nil),
              //: gift.broadType != BroadcastType.Unknow.rawValue
              gift.broadType != AddConstantTarget.Unknow.rawValue
        //: else {
        else {
            //: return
            return
        }
        //: let currUserType = NowInfoThen.share.appUserConfigMode.userType
        let currUserType = NowInfoThen.share.appUserConfigMode.userType
        // 过滤接口异常
        ...
        // 非特殊用户 不展示 不同地区用户礼物
        //: if currUserType != 0, gift.userCountryType != NowInfoThen.share.appUserConfigMode.userCountryType {
        if currUserType != 0, gift.userCountryType != NowInfoThen.share.appUserConfigMode.userCountryType {
            //: return
            return
        }
        ...
        self.objectDoing(gift: gift)
    }

    /// 应用退到后台
    //: @objc private func enterBackground() {
    @objc private func toName() {
    ...
    @objc private func withForeground() {
        //: self.activeWhenShow()
        self.addShow()
    }
    ...
    @objc private func phenomAcrossOf() {
