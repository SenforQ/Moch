...
fileprivate let main_dataName:[UInt8] = [0x75,0x72,0x75,0x68,0x34,0x7f,0x73,0x78,0x79,0x6e,0x26,0x35,0x3c,0x74,0x7d,0x6f,0x3c,0x72,0x73,0x68,0x3c,0x7e,0x79,0x79,0x72,0x3c,0x75,0x71,0x6c,0x70,0x79,0x71,0x79,0x72,0x68,0x79,0x78]

/*: "JXBannerCellID" :*/
fileprivate let kFatalUrl:[Character] = ["J","X","B","a","n","n","e","r"]
fileprivate let show_pushId:[Character] = ["C","e","l","l","I","D"]

/*: "#FFFFFF" :*/
fileprivate let userTableMsg:String = "#FFFFFFkey as"

/*: "#BDB8B5" :*/
fileprivate let constAddNorViewName:[Character] = ["#","B"]
fileprivate let k_successPath:String = "hiddenB8B5"
...
protocol EraseViewProtocol: NSObjectProtocol {
    // 加载更多数据
...
class PanoramicViewThen: UICollectionReusableView {
    //: weak var delegate: SocialPopularListFooterViewProtocol?
    weak var delegate: EraseViewProtocol?
    //: var listData: [PremiumStarListModel]?
    var listData: [HearingListModel]? // 巨星计划数据
    //: var adBannderListData: [SocialAdBannerModel]?
    var adBannderListData: [SocietalTitleDivinatoryRecitalModelType]? // 广告banner数据

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: main_dataName.map{$0^28}, encoding: .utf8)!)
    }
...
extension PanoramicViewThen {
    /// 获取当前视图高度
    /// - Returns: 高度
    //: func getFooterViewHeight() -> CGFloat {
    func smallHeight() -> CGFloat {
    ...
    func neutronStarMode(data: [HearingListModel]) {
        //: self.listData = data
        self.listData = data
    }
    ...
    func pastEvent(data: [SocietalTitleDivinatoryRecitalModelType]) {
...
extension PanoramicViewThen: JXBannerDataSource, JXBannerDelegate {
    //: func jxBanner(_ banner: JXBannerType) -> (JXBannerCellRegister) {
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        //: return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: "JXBannerCellID")
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(kFatalUrl) + String(show_pushId)))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        tempCell.imageView.luculentUpwardly(urlStr: model?.pic ?? "", placeImg: UIImage.labWrite())
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(userTableMsg.prefix(7))))!
        //: pageControl.inactiveSize = CGSize(width: 6, height: 6)
        pageControl.inactiveSize = CGSize(width: 6, height: 6)
        //: pageControl.inactiveColor = UIColor(hex: "#BDB8B5")!
        pageControl.inactiveColor = UIColor(hex: (String(constAddNorViewName) + k_successPath.replacingOccurrences(of: "hidden", with: "D")))!
                         ...
                         didSelectItemAt index: Int)
    {
        ...
        delegate?.cellSuccess(model: model)
    }
}
...
extension PanoramicViewThen {
    ...
    private func substitute() {
        ...
        bannerView.snp.makeConstraints { make in
            ...
            make.size.equalTo(CGSize(width: constExplainViewMsg - 30, height: 84))
        }
    }
}
