...
/*: / :*/
fileprivate let app_viewId:String = "show"

/*: "\n" :*/
fileprivate let data_yearMsg:[Character] = ["\n"]
...
struct RequestCellContainerInset {
    //: let top: CGFloat = 5.0
    let top: CGFloat = 5.0
...
class AtViewDelegate: UIView {
    ...
    convenience init(placeholder: String?, placeholderColor: UIColor?, frame: CGRect) {
        ...
        colorPhoto(placeholder: placeholder, placeholderColor: placeholderColor)
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        colorPhoto(placeholder: nil, placeholderColor: nil)
    }
}
...
extension AtViewDelegate {
    /// placeholder Label Setup
    //: private func setup(placeholder: String?, placeholderColor: UIColor?) {
    private func colorPhoto(placeholder: String?, placeholderColor: UIColor?) {
        ...
        plaleLabel.snp.remakeConstraints { make in
            ...
            make.leading.equalTo(NameMusicLabelOrigin().x)
            //: make.top.equalTo(PlaceholderLabelOrigin().y)
            make.top.equalTo(NameMusicLabelOrigin().y)
            //: make.trailing.equalTo(-PlaceholderLabelOrigin().x)
            make.trailing.equalTo(-NameMusicLabelOrigin().x)
        }
        ...
        placeholdertextView.textContainerInset = UIEdgeInsets(top: RequestCellContainerInset().top, left: RequestCellContainerInset().left, bottom: RequestCellContainerInset().bottom, right: RequestCellContainerInset().right)
...
extension AtViewDelegate: UITextViewDelegate {
    //: func textViewDidChange(_ textView: UITextView) {
    func textViewDidChange(_ textView: UITextView) {
        //: checkShowHiddenPlaceholder()
        sleepingDraughtPlaceholder()
...
extension AtViewDelegate {
    ...
    private func sleepingDraughtPlaceholder() {
