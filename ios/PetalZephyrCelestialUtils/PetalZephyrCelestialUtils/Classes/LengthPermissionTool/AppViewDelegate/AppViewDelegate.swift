...
protocol BlockNavigationDelegate: AnyObject {
...
class AppViewDelegate: UIViewController {
    ...
    weak var delegate: BlockNavigationDelegate?
    ...
    private lazy var controllerScrollView: UIScrollView = {
        ...
        let count = min(self.controllerItems.count, self.titleItems.count)
        //: var height = LineThen.getScreenHeight() - (topDistance + kTitle_H)
        var height = LineThen.registerUp() - (topDistance + kTitle_H)
        //: if self.hasBttomTabBar {
        if self.hasBttomTabBar {
            //: height -= TabBarViewHeight
            height -= userQualityContent
        }
        ...
        //: let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: LineThen.getScreenWidth(), height: height)
        let rect = CGRect(x: 0, y: topDistance + kTitle_H, width: LineThen.dataWidth(), height: height)
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        //: scrollView.contentSize = CGSize(width: count * Int(LineThen.getScreenWidth()), height: 0)
        scrollView.contentSize = CGSize(width: count * Int(LineThen.dataWidth()), height: 0)
        ...
        scrollView.semanticContentAttribute = WithLanguageManager.shared.direction == .leftToRight ? .forceLeftToRight:.forceRightToLeft
        //: return scrollView
        return scrollView
    //: }()
    }()
    ...
    private lazy var titleScrollView: UIScrollView = {
        //: var rect = CGRect(x: 15, y: topDistance, width: LineThen.getScreenWidth()-rightSpace, height: kTitle_H)
        var rect = CGRect(x: 15, y: topDistance, width: LineThen.dataWidth()-rightSpace, height: kTitle_H)
        //: if isBisect {
        if isBisect {
            //: rect = CGRect(x: 0, y: topDistance, width: LineThen.getScreenWidth()-rightSpace, height: kTitle_H)
            rect = CGRect(x: 0, y: topDistance, width: LineThen.dataWidth()-rightSpace, height: kTitle_H)
        }
        //: let scrollView = UIScrollView(frame: rect)
        let scrollView = UIScrollView(frame: rect)
        ...
        var originX: CGFloat = WithLanguageManager.shared.direction == .leftToRight ? 0:25
        ...
        self.titleItems.enumerated().forEach { [weak self] index, obj in
            ...
            titleBtn.addTarget(self, action: #selector(executeIn), for: .touchDown)
            //: var titleWidth = 0.0
            var titleWidth = 0.0
            //: if isBisect {
            if isBisect {
                //: titleWidth = LineThen.getScreenWidth() / Double(self.titleItems.count)
                titleWidth = LineThen.dataWidth() / Double(self.titleItems.count)
            //: } else {
            } else {
                //: titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
                titleWidth = (titleBtn.titleLabel?.sizeThatFits(CGSize(width: CGFLOAT_MAX, height: kTitle_H)).width ?? 0) + 22
            }
            //: let x = LanguageManager.shared.direction == .leftToRight ? originX:(LineThen.getScreenWidth()-originX-titleWidth)
            let x = WithLanguageManager.shared.direction == .leftToRight ? originX:(LineThen.dataWidth()-originX-titleWidth)
...
extension AppViewDelegate {
    ...
    @objc private func executeIn(_ sender: UIButton) {
        //: currentSelectedTitleButton(sender)
        observerButton(sender)
        //: let index = sender.tag - 1000
        let index = sender.tag - 1000
        //: let offset_x = CGFloat(index) * LineThen.getScreenWidth()
        let offset_x = CGFloat(index) * LineThen.dataWidth()
        //: controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        controllerScrollView.contentOffset = CGPoint(x: offset_x, y: 0)
        //: addControllerView(withIndex: index)
        butylateLockScale(withIndex: index)
        //: seletedViewController(withIndex: index)
        behindSeleted(withIndex: index)
    }
    
    //: private func addControllerView(withIndex index: Int) {
    private func butylateLockScale(withIndex index: Int) {
        //: if index >= children.count {
        if index >= children.count {
            //: return
            return
        }
        //: let origin_x = CGFloat(index) * LineThen.getScreenWidth()
        let origin_x = CGFloat(index) * LineThen.dataWidth()
        ...
        //: vc.view.frame = CGRect(x: origin_x, y: 0, width: LineThen.getScreenWidth(), height: controllerScrollView.frame.size.height)
        vc.view.frame = CGRect(x: origin_x, y: 0, width: LineThen.dataWidth(), height: controllerScrollView.frame.size.height)
        //: controllerScrollView.addSubview(vc.view)
        controllerScrollView.addSubview(vc.view)
    }
    
    //: private func currentSelectedTitleButton(_ button: UIButton) {
    private func observerButton(_ button: UIButton) {
    ...
    func areaApp(_ sender: UIButton) {
        //: if titleScrollView.contentSize.width <= LineThen.getScreenWidth() {
        if titleScrollView.contentSize.width <= LineThen.dataWidth() {
            //: return
            return
        }
        //: var offset_x = sender.center.x - LineThen.getScreenWidth() * 0.5
        var offset_x = sender.center.x - LineThen.dataWidth() * 0.5
        //: if offset_x < 0 {
        if offset_x < 0 {
            //: offset_x = 0
            offset_x = 0
        }
        //: let maxOffset = titleScrollView.contentSize.width - LineThen.getScreenWidth()
        let maxOffset = titleScrollView.contentSize.width - LineThen.dataWidth()
...
extension AppViewDelegate {
    ...
    func behindSeleted(withIndex index: Int) {
        //: guard index < children.count else { return }
        guard index < children.count else { return }
        //: delegate?.selectedViewController(children[index], index: index)
        delegate?.countimateColor(children[index], index: index)
    }
}

// MARK: - UIScrollViewDelegate
//: extension QYSlideNavigationViewController: UIScrollViewDelegate {
extension AppViewDelegate: UIScrollViewDelegate {
    //: func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
    func scrollViewDidEndDecelerating(_ scrollView: UIScrollView) {
        //: let index = Int(controllerScrollView.contentOffset.x / LineThen.getScreenWidth())
        let index = Int(controllerScrollView.contentOffset.x / LineThen.dataWidth())
        //: if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
        if let selectedBtn = titleScrollView.viewWithTag(index + 1000) as? UIButton {
            //: currentSelectedTitleButton(selectedBtn)
            observerButton(selectedBtn)
        }
        //: addControllerView(withIndex: index)
        butylateLockScale(withIndex: index)
        //: seletedViewController(withIndex: index)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        //: let index = Int(scrollView.contentOffset.x / LineThen.getScreenWidth())
        let index = Int(scrollView.contentOffset.x / LineThen.dataWidth())
        ...
        //: let scaleR = scrollView.contentOffset.x / LineThen.getScreenWidth() - CGFloat(index)
        let scaleR = scrollView.contentOffset.x / LineThen.dataWidth() - CGFloat(index)
        //: let scaleL = 1 - scaleR
        let scaleL = 1 - scaleR
