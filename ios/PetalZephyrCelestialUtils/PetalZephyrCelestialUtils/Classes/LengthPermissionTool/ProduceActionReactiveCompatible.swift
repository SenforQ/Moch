...
fileprivate let user_blockName:String = "AppRin height"
fileprivate let noti_toLabelLetPath:String = "rcesVmy title type number file"
fileprivate let const_menuMsg:[Character] = ["e"]
fileprivate let constCellUrl:String = "rsionKeyraw false"

/*: ".zip" :*/
fileprivate let mainActionKey:[Character] = [".","z","i","p"]

/*: "pic" :*/
fileprivate let show_toId:String = "PIC"

/*: "nor" :*/
fileprivate let mainBarStr:String = "nranger"

/*: "svga" :*/
fileprivate let showMakeMsg:String = "svglay"

/*: "mp3" :*/
fileprivate let appFirstStr:String = "MP3"

/*: "model" :*/
fileprivate let main_touchCenterMessage:String = "modviewl"

/*: "language" :*/
fileprivate let dataContainerMessage:[Character] = ["l","a","n","g","u","a","g","e"]

/*: "@2x.png" :*/
fileprivate let showBurnSessionTitle:[Character] = ["@","2"]
fileprivate let app_viewId:[Character] = ["x",".","p","n","g"]

/*: "png" :*/
fileprivate let constPlayReportKey:[Character] = ["p","n","g"]
...
// __CLOSE_PRINT__
//
//  ProduceActionReactiveCompatible.swift
//  AbroadTalking
//
...
private let noti_sessionName = (String(user_blockName.prefix(4)) + "esou" + String(noti_toLabelLetPath.prefix(5)) + String(const_menuMsg) + String(constCellUrl.prefix(8)))

//: enum SVGAType: String {
enum VideoMirrorPath: String {
...
class ProduceActionReactiveCompatible: NSObject {
    //: static let `default` = SVGAEffectTool()
    static let `default` = ProduceActionReactiveCompatible()
    ...
    private lazy var AppResPath: String = {
        ...
        let res_Dir = (docuPath as NSString).appendingPathComponent(k_imagePath)
        //: let version = Defaults.string(forKey: AppResourcesVersionKey) ?? ""
        let version = main_clickSenseKey.string(forKey: noti_sessionName) ?? ""
        //: if version == AppVersion, FileManager.default.fileExists(atPath: res_Dir) {
        if version == notiErrTurnUrl, FileManager.default.fileExists(atPath: res_Dir) {
            //: return res_Dir
            return res_Dir
        }
        //: let path = PodspecBundle.bundle.path(forResource: PodspecName, ofType: ".zip") ?? ""
        let path = EngineCoolingSystemPodspecBundle.bundle.path(forResource: k_imagePath, ofType: (String(mainActionKey))) ?? ""
        ...
        let zipSuc = SSZipArchive.unzipFile(atPath: path,
                                            ...
                                            password: k_imagePath,
                                            //: progressHandler: nil)
                                            progressHandler: nil)
        //: if zipSuc {
        if zipSuc {
            //: Defaults.set(AppVersion, forKey: AppResourcesVersionKey)
            main_clickSenseKey.set(notiErrTurnUrl, forKey: noti_sessionName)
            //: return res_Dir
            return res_Dir
        }
        //: return ""
    ...
    private lazy var imageResPath: String = //: return (AppResPath as NSString).appendingPathComponent("pic")
        (AppResPath as NSString).appendingPathComponent((show_toId.lowercased()))
    //: }()
    ...
    private lazy var norImageResPath: String = {
        ...
        return (imageResPath as NSString).appendingPathComponent((mainBarStr.replacingOccurrences(of: "range", with: "o")))
        //: }()
    }()
    ...
    private lazy var svgaResPath: String = //: return (AppResPath as NSString).appendingPathComponent("svga")
        (AppResPath as NSString).appendingPathComponent((showMakeMsg.replacingOccurrences(of: "lay", with: "a")))
    //: }()
    ...
    private lazy var mp3ResPath: String = //: return (AppResPath as NSString).appendingPathComponent("mp3")
        (AppResPath as NSString).appendingPathComponent((appFirstStr.lowercased()))
    //: }()
    ...
    public lazy var modelResPath: String = //: return (AppResPath as NSString).appendingPathComponent("model")
        (AppResPath as NSString).appendingPathComponent((main_touchCenterMessage.replacingOccurrences(of: "view", with: "e")))
    //: }()
    ...
    public lazy var languagePath: String = //: return (AppResPath as NSString).appendingPathComponent("language")
        (AppResPath as NSString).appendingPathComponent((String(dataContainerMessage)))
    //: }()
}

//: extension SVGAEffectTool {
extension ProduceActionReactiveCompatible {
    /// 直接获取图片
    //: func getImage(name: String) -> UIImage? {
    func exceptTop(name: String) -> UIImage? {
        //: let languagePath = (languageImageResPath as NSString).appendingPathComponent(name).appending("@2x.png")
        let languagePath = (languageImageResPath as NSString).appendingPathComponent(name).appending((String(showBurnSessionTitle) + String(app_viewId)))
        ...
        let path = (norImageResPath as NSString).appendingPathComponent(name).appending((String(showBurnSessionTitle) + String(app_viewId)))
    ...
    func distaffType(type: VideoMirrorPath) -> URL {
        //: var path = (svgaResPath as NSString).appendingPathComponent(type.rawValue)
        var path = (svgaResPath as NSString).appendingPathComponent(type.rawValue)
        //: path = (path as NSString).appendingPathExtension("svga")!
        path = (path as NSString).appendingPathExtension((showMakeMsg.replacingOccurrences(of: "lay", with: "a")))!
    ...
    func isothermal(name: String, type: String = (appFirstStr.lowercased())) -> String {
