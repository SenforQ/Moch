...
fileprivate let appPartyName:String = "Therfirst data price"
fileprivate let kToolFormat:String = "no daby view icon view"
fileprivate let constViewMsg:[Character] = [" ","u","p","l","o","a","d"]

/*: "Upload Log successfully!" :*/
fileprivate let kRequestName:[Character] = ["U","p","l","o","a","d"," "]
fileprivate let constLabId:String = "view selectedLog s"
fileprivate let data_lineName:String = "sfully!break view"

/*: "UID:  :*/
fileprivate let notiFollowCornerTitle:[Character] = ["U","I","D",":"," "]

/*: "/com_tencent_imsdk_log" :*/
fileprivate let app_equalStr:[Character] = ["/","c","o","m","_","t","e","n","c"]
fileprivate let constToKey:[Character] = ["e","n","t","_","i","m","s","d","k","_","l","o","g"]

/*: "/log" :*/
fileprivate let user_howValue:String = "/logcell child detail error"

/*: "/TempLogZip" :*/
fileprivate let data_centerMsg:[Character] = ["/","T","e","m","p","L","o","g","Z","i"]
fileprivate let mainClearSizeValue:String = "selected"

/*: "/LogZip.zip" :*/
fileprivate let appActionMsg:[Character] = ["/","L","o","g","Z"]
fileprivate let dataManagerId:[Character] = ["i"]
fileprivate let app_mainFormat:[Character] = ["p",".","z","i","p"]
...
class WoodLogTool: NSObject {
    ...
    func playMode(_ showTips: Bool = true) {
        ...
        if let path = WoodLogTool.getInsideTouchFullLivingPic() {
            //: copyLogToTempPath(logPath: path)
            cardCourseOfAction(logPath: path)
        }
        // trtc
        //: if let path = UploadLogTool.getTRTCLogPath() {
        if let path = WoodLogTool.logComeUp() {
            //: copyLogToTempPath(logPath: path)
            cardCourseOfAction(logPath: path)
        }
        // xlog
        ...
        cardCourseOfAction(logPath: DXMarsXLogger.getLogPath())

        // 压缩上传
        //: if let tempPath = UploadLogTool.getTempUploadLogPath(), let zipPath = UploadLogTool.getZipLogPath() {
        if let tempPath = WoodLogTool.themeGift(), let zipPath = WoodLogTool.uncolored() {
            ...
            if let file = NSData(contentsOfFile: zipPath) {
                ...
                if file.length < 10 {
                    ...
                    if showTips {
                        ...
                        self.addMsg(showMsg: (String(appPartyName.prefix(4)) + "e is " + String(kToolFormat.prefix(5)) + "ta to" + String(constViewMsg)).localized)
                    }
                    //: return
                    return
                }
                ...
                TailSignatureProvider.shared.objectCreate(filePath: zipPath) { succeed, errorMsg in
                    //: ProgressHUD.dismiss()
                    DeleteReactiveCompatible.deflector()
                    //: guard succeed else {
                    guard succeed else {
                    ...
                    if showTips {
                        ...
                        self.beforeDemonstrate(showMsg: (String(kRequestName) + String(constLabId.suffix(5)) + "ucces" + String(data_lineName.prefix(7))).localized)
                    }
                    ...
                    self.sinceFirstPath(atPath: DXMarsXLogger.getLogPath())
                }
            }
        }
    }
...
extension WoodLogTool {
    /// 写入用户行为日志到本地文件
    /// - Parameter msg: 日志
    //: static func writeLog<T>(msg: T,
    static func knowHow<T>(msg: T,
                           //: file: String = #file,
                           file: String = #file,
                           ...
                           line: Int = #line)
    {
        //: DXMarsXLogger.logInfo("UID: \(NowInfoThen.share.loginUserMode.userID)",
        DXMarsXLogger.logInfo((String(notiFollowCornerTitle)) + "\(NowInfoThen.share.loginUserMode.userID)",
                              //: message: "\(msg)",
                              message: "\(msg)",
                              //: file: file,
                              file: file,
...
extension WoodLogTool {
    ...
    private func cardCourseOfAction(logPath: String) {
        ...
        subpaths.forEach { fileName in
            ...
            if fileDate > daysAgo,
               ...
               let tempPath = WoodLogTool.themeGift()
            {
    ...
    private func sinceFirstPath(atPath path: String) {
...
extension WoodLogTool {
    ...
    private static func getInsideTouchFullLivingPic() -> String? {
        //: return NSSearchPathForDirectoriesInDomains(.cachesDirectory, .userDomainMask, true).first?.appending("/com_tencent_imsdk_log")
        return NSSearchPathForDirectoriesInDomains(.cachesDirectory, .userDomainMask, true).first?.appending((String(app_equalStr) + String(constToKey)))
    }

    /// 获取腾讯TRTC日志路径
    //: private static func getTRTCLogPath() -> String? {
    private static func logComeUp() -> String? {
        //: return NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first?.appending("/log")
        return NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first?.appending((String(user_howValue.prefix(4))))
    }

    /// copy临时路径
    //: private static func getTempUploadLogPath() -> String? {
    private static func themeGift() -> String? {
        //: if let logPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first?.appending("/TempLogZip") {
        if let logPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first?.appending((String(data_centerMsg) + mainClearSizeValue.replacingOccurrences(of: "selected", with: "p"))) {
    ...
    private static func uncolored() -> String? {
        //: return NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first?.appending("/LogZip.zip")
        return NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first?.appending((String(appActionMsg) + String(dataManagerId) + String(app_mainFormat)))
    }
}
