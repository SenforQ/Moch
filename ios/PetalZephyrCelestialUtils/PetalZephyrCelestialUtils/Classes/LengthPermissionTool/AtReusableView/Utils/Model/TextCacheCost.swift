...
/*: "filterSuite" :*/
fileprivate let kValueBlockAsName:String = "FILTER"
fileprivate let dataModeEqualViewUrl:[Character] = ["S","u","i","t","e"]

/*: "customSuite" :*/
fileprivate let notiProgressStr:[Character] = ["c","u","s","t","o","m","S","u","i","t","e"]

/*: "App/TextCacheCost" :*/
fileprivate let dataToMsg:String = "try arrayApp/ST"
fileprivate let notiViewMsg:String = "erCachemake create"
fileprivate let user_contentMsg:String = "Costguard at time list"
...
//: @objcMembers public class TextCacheCost: NSObject, NSSecureCoding {
@objcMembers public class TextCacheCost: NSObject, NSSecureCoding {
    //: var curBeautySuite: BeautyItemType?
    var curBeautySuite: PracticeInsetTarget?
    //: var curFeature: STEffectsType?
    var curFeature: ImageQuantity?
    /// 用户的滤镜设置
    //: public var filterSuite: AttractorModel?
    public var filterSuite: AttractorModel?
    /// 滤镜列表模型数组
    //: var filterModelArray = Array<FiltrateModel>.init()
    var filterModelArray = Array<FiltrateModel>.init()

    //: var customSuite =  AttractorModel.init()
    var customSuite = AttractorModel() // 用户自定义的美颜

    //: static public let share = TextCacheCost()
    public static let share = TextCacheCost()
    ...
    public required init?(coder: NSCoder) {
        ...
        let filDict = coder.decodeObject(forKey: (kValueBlockAsName.lowercased() + String(dataModeEqualViewUrl)))
        //: let custDict =  coder.decodeObject(forKey: "customSuite")
        let custDict = coder.decodeObject(forKey: (String(notiProgressStr)))
        //: if filDict != nil {
        if filDict != nil {
            //: filterSuite = try! AttractorModel.init(dictionary: filDict as? [AnyHashable: Any])
            filterSuite = try! AttractorModel(dictionary: filDict as? [AnyHashable: Any])
        }
        //: if custDict != nil {
        if custDict != nil {
            //: customSuite = try! AttractorModel.init(dictionary: custDict as? [AnyHashable: Any])
            customSuite = try! AttractorModel(dictionary: custDict as? [AnyHashable: Any])
        }
    }
    ...
    public func encode(with coder: NSCoder) {
        ...
        coder.encode(filDict, forKey: (kValueBlockAsName.lowercased() + String(dataModeEqualViewUrl)))
        //: coder.encode(custDict, forKey: "customSuite")
        coder.encode(custDict, forKey: (String(notiProgressStr)))
    }
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(redact(sender:)), name: show_barSuccessMsg, object: nil)
    }
...
//: @objc extension TextCacheCost {
@objc extension TextCacheCost {
    /// 获取缓存
    //: public func getLocalModel() {
    public func someConversation() {
        //: let path = self.cachePath()
        let path = self.curiosity()
        //: if FileManager.default.fileExists(atPath: path) {
        if FileManager.default.fileExists(atPath: path) {
            //: var obj: TextCacheCost?
            var obj: TextCacheCost?
            ...
            if #available(iOS 11.0, *) {
                ...
                do {
                    ...
                    let data = try Data(contentsOf: URL(fileURLWithPath: path))
                    //: if let model: TextCacheCost =  try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data) as? TextCacheCost {
                    if let model: TextCacheCost = try NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data) as? TextCacheCost {
                        //: obj  = model
                        obj = model
                    }
                ...
                //: let model: TextCacheCost? = NSKeyedUnarchiver.unarchiveObject(withFile: path) as? TextCacheCost
                let model: TextCacheCost? = NSKeyedUnarchiver.unarchiveObject(withFile: path) as? TextCacheCost
                //: if  model != nil {
                if model != nil {
                    //: obj  = model
    ...
    //: public func currentBeautySuite() -> AttractorModel {
    public func ptolemaicSystemSuite() -> AttractorModel {
    ...
    func dayOfRemembrance() {
        //: customSuite  = AttractorModel.init().initWith(suiteType: .Face)
        customSuite = AttractorModel().renderView(suiteType: .Face)

        //: if filterSuite == nil {
        if filterSuite == nil {
            //: filterSuite  = AttractorModel.init()
            filterSuite = AttractorModel()
        }
    }
    ...
    func curiosity() -> String {
        //: let userId = 0
        let userId = 0

        //: let documentDictPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let documentDictPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        //: let appDirectoryPath = (documentDictPath as NSString).appendingPathComponent("App/TextCacheCost")
        let appDirectoryPath = (documentDictPath as NSString).appendingPathComponent((String(dataToMsg.suffix(6)) + "Filt" + String(notiViewMsg.prefix(7)) + String(user_contentMsg.prefix(4))))
    ...
    func logOnSuite(curBeautySuite: PracticeInsetTarget) {
    ...
    func use() {
        ...
        self.customSuite.searchUpEnable()
        //: self.curBeautySuite = .Face
        self.curBeautySuite = .Face
        //: self.filterSuite = AttractorModel.init()
        self.filterSuite = AttractorModel()
    }

    // notification
    //: func didReceiveFeatureValueChange(sender: NSNotification) {
    func redact(sender: NSNotification) {
        //: var cache: AttractorModel?
        var cache: AttractorModel?
        //: cache = self.customSuite
        cache = self.customSuite
        //: let featureModel: FiltrateModel = sender.object as! FiltrateModel
        let featureModel: FiltrateModel = sender.object as! FiltrateModel
        //: handleFeatureValueChangeWith(beautySuite: cache ?? AttractorModel.init(), featureModel: featureModel)
        dismissAttractorIndex(beautySuite: cache ?? AttractorModel(), featureModel: featureModel)
    }

    //: func handleFeatureValueChangeWith(beautySuite: AttractorModel, featureModel: FiltrateModel) {
    func dismissAttractorIndex(beautySuite: AttractorModel, featureModel: FiltrateModel) {
        //: if (self.curBeautySuite == .Face) {
        if self.curBeautySuite == .Face {
            //: if featureModel.isKind(of: FiltrateModel.self) == true {
            if featureModel.isKind(of: FiltrateModel.self) == true {
                // [0,1]
    ...
    /// -------初始化滤镜资源
    //: func reloadFilterData(model: FiltrateModel) {
    func theoreticalAccountModel(model: FiltrateModel) {
    ...
    //: public func getFilterDataModel() -> FiltrateModel? {
    public func positionTo() -> FiltrateModel? {
    ...
    func rawCamera(rawValue: Float) -> Float {
    ...
    //: func initFilterData() {
    func digitiserData() {
        //: let orignArray  = FiltrateModel.init().arrayOfFeatureModelWith(categoryType: .Filter)
        let orignArray = FiltrateModel().coldShoulder(categoryType: .Filter)

        //: if (orignArray.count > 0) {
        if orignArray.count > 0 {
            //: let model = orignArray.first
            let model = orignArray.first
            //: self.filterModelArray = NSMutableArray.init(array: model?.subFilterItemsArr ?? [FiltrateModel].init()) as! [FiltrateModel]
            self.filterModelArray = NSMutableArray(array: model?.subFilterItemsArr ?? [FiltrateModel].init()) as! [FiltrateModel]
        }
    }
}
