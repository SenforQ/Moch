...
//: STDisplayShader
Byte showViewPath[] = {58, 15, 97, 10, 82, 253, 242, 214, 31, 83, 242, 243, 227, 8, 18, 15, 11, 0, 24, 242, 7, 0, 3, 4, 17, 221};
...
- (instancetype)initWithSample:(CGRect)frame session:(EAGLContext *)context
{
    ...
    if (self) {
        ...
        [self streetSmart:StringFromArrowData(showViewPath)
                //: fragmentShader:@"STDisplayShader"
                indexAcrossProgram:StringFromArrowData(showViewPath)
                    //: forProgram:&stDisplayProgram];
                    center:&stDisplayProgram];
...
- (BOOL)streetSmart:(NSString *)vertexShaderName
          //: fragmentShader:(NSString *)fragmentShaderName
          indexAcrossProgram:(NSString *)fragmentShaderName
              //: forProgram:(GLuint *)programPointer;
              center:(GLuint *)programPointer;
{
    ...
    *programPointer = glCreateProgram();
    ...
    if (![self compile:&vertexShader version:0x8B31 afterActiveTo:userDelayName]) {
    ...
    if (![self compile:&fragShader version:0x8B30 afterActiveTo:noti_countStr]) {
...
- (BOOL)compile:(GLuint *)shader version:(GLenum)type afterActiveTo:(const char *)str
{
    //: GLint status;
    GLint status;
    //: const GLchar *source;
...
Byte * ArrowDataToCache(Byte *data) {
    int to = data[0];
    int managerTension = data[1];
    Byte schedule = data[2];
    int video = data[3];
    if (!to) return data + video;
    for (int i = video; i < video + managerTension; i++) {
    ...
    data[0] = 0;
    data[video + managerTension] = 0;
    return data + video;
}

NSString *StringFromArrowData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)ArrowDataToCache(data)];
}
