...
//: Width
@property (nonatomic, copy) NSString *mainScheduleStr;

//: Height
@property (nonatomic, copy) NSString *data_streamArrowMessage;

//: 请打开相机权限
@property (nonatomic, copy) NSString *appWillText;
...
- (Byte *)SizeDataToByte:(StructSizeData *)data {
    for (int i = 0; i < data->withSource; i++) {
        data->row[i] ^= data->quality;
    }
    data->row[data->withSource] = 0;
    return data->row;
}

- (NSString *)StringFromSizeData:(StructSizeData *)data {
    return [NSString stringWithUTF8String:(char *)[self SizeDataToByte:data]];
}

//: 请打开相机权限
- (NSString *)appWillText {
    if (!_appWillText) {
        StructSizeData value = (StructSizeData){37, (Byte []){205, 138, 146, 195, 172, 182, 192, 153, 165, 194, 190, 157, 195, 185, 159, 195, 184, 166, 204, 188, 181, 236}, 21};
        _appWillText = [self StringFromSizeData:&value];
    }
    return _appWillText;
}

//: Height
- (NSString *)data_streamArrowMessage {
    if (!_data_streamArrowMessage) {
        StructSizeData value = (StructSizeData){81, (Byte []){25, 52, 56, 54, 57, 37, 153}, 6};
        _data_streamArrowMessage = [self StringFromSizeData:&value];
    }
    return _data_streamArrowMessage;
}

//: Width
- (NSString *)mainScheduleStr {
    if (!_mainScheduleStr) {
        StructSizeData value = (StructSizeData){109, (Byte []){58, 4, 9, 25, 5, 64}, 5};
        _mainScheduleStr = [self StringFromSizeData:&value];
    }
    return _mainScheduleStr;
}
...
@interface EqualLoad () <AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate>
                         ...
                         object:(BOOL)needYuvOutput
{
    ...
    if (self) {
        ...
        self.videoDevice = [self along:iDevicePosition];
...
- (void)load:(CGPoint)point user:(CGRect)frame
{
    ...
    [self active:AVCaptureFocusModeAutoFocus
           //: exposureMode:AVCaptureExposureModeContinuousAutoExposure
           streetwise:AVCaptureExposureModeContinuousAutoExposure
                //: atPoint:CGPointMake(fX, fY)];
                fit:CGPointMake(fX, fY)];

}

//: - (void)focusWithMode:(AVCaptureFocusMode)focusMode exposureMode:(AVCaptureExposureMode)exposureMode atPoint:(CGPoint)point
- (void)active:(AVCaptureFocusMode)focusMode streetwise:(AVCaptureExposureMode)exposureMode fit:(CGPoint)point
{
...
- (void)setDevicePosition:(AVCaptureDevicePosition)devicePosition
{
    ...
    if (_devicePosition != devicePosition && devicePosition != AVCaptureDevicePositionUnspecified) {
        ...
        if (_session) {
            ...
            AVCaptureDevice *targetDevice = [self along:devicePosition];

            //: if (targetDevice && [self judgeCameraAuthorization]) {
            if (targetDevice && [self counter]) {
...
- (void)setIExpectedFPS:(int)iExpectedFPS
{
    ...
    CGFloat fWidth = [[_dataOutput.videoSettings objectForKey:[SizeData sharedInstance].mainScheduleStr] floatValue];
    //: CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:@"Height"] floatValue];
    CGFloat fHeight = [[_dataOutput.videoSettings objectForKey:[SizeData sharedInstance].data_streamArrowMessage] floatValue];
...
- (CGRect)instance:(CGRect)rect languageUnit:(BOOL)bScaleToFit
{
    ...
    if (self.dataOutput.videoSettings) {
        ...
        CGFloat fWidth = [[self.dataOutput.videoSettings objectForKey:[SizeData sharedInstance].mainScheduleStr] floatValue];
        //: CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:@"Height"] floatValue];
        CGFloat fHeight = [[self.dataOutput.videoSettings objectForKey:[SizeData sharedInstance].data_streamArrowMessage] floatValue];
...
- (BOOL)counter
{
    ...
    if (authStatus == AVAuthorizationStatusRestricted || authStatus == AVAuthorizationStatusDenied) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            UIAlertController *alertView = [UIAlertController alertControllerWithTitle:@""
                                                                               //: message:@"请打开相机权限"
                                                                               message:[SizeData sharedInstance].appWillText
...
- (AVCaptureDevice *)along:(AVCaptureDevicePosition)position
{
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection
{
    ...
    if (!self.bSessionPause) {
        ...
        if (self.delegate && [self.delegate respondsToSelector:@selector(input:streetwise:at:)]) {
            //: [self.delegate captureOutput:captureOutput didOutputSampleBuffer:sampleBuffer fromConnection:connection];
            [self.delegate input:captureOutput streetwise:sampleBuffer at:connection];
        }
    }
