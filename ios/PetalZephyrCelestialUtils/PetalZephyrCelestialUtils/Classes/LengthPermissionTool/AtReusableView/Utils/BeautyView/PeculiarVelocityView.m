...
+ (instancetype)sharedInstance {
    static CameraData *instance = nil;
...
- (Byte *)CameraDataToCache:(Byte *)data {
    int timeTension = data[0];
    Byte countOn = data[1];
    int sceneLine = data[2];
    for (int i = sceneLine; i < sceneLine + timeTension; i++) {
        int value = data[i] + countOn;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    }
    data[sceneLine + timeTension] = 0;
    return data + sceneLine;
}

- (NSString *)StringFromCameraData:(Byte *)data {
    return [NSString stringWithUTF8String:(char *)[self CameraDataToCache:data]];
}

//: FaceActionEvent_Interval_12_frame
- (NSString *)main_schedulePublishFormat {
    /* static */ NSString *main_schedulePublishFormat = nil;
    if (!main_schedulePublishFormat) {
        Byte value[] = {33, 76, 4, 197, 250, 21, 23, 25, 245, 23, 40, 29, 35, 34, 249, 42, 25, 34, 40, 19, 253, 34, 40, 25, 38, 42, 21, 32, 19, 229, 230, 19, 26, 38, 21, 33, 25, 39};
        main_schedulePublishFormat = [self StringFromCameraData:value];
    }
    return main_schedulePublishFormat;
}

//: hasFace
- (NSString *)mainQualityFinishId {
    /* static */ NSString *mainQualityFinishId = nil;
    if (!mainQualityFinishId) {
        Byte value[] = {7, 92, 3, 12, 5, 23, 234, 5, 7, 9, 144};
        mainQualityFinishId = [self StringFromCameraData:value];
    }
    return mainQualityFinishId;
}

//: FaceActionEvent_Interval_1_second
- (NSString *)dataVideoStr {
    /* static */ NSString *dataVideoStr = nil;
    if (!dataVideoStr) {
        Byte value[] = {33, 66, 5, 235, 92, 4, 31, 33, 35, 255, 33, 50, 39, 45, 44, 3, 52, 35, 44, 50, 29, 7, 44, 50, 35, 48, 52, 31, 42, 29, 239, 29, 49, 35, 33, 45, 44, 34, 31};
        dataVideoStr = [self StringFromCameraData:value];
    }
    return dataVideoStr;
}

//: model
- (NSString *)const_searchVersionMessage {
    /* static */ NSString *const_searchVersionMessage = nil;
    if (!const_searchVersionMessage) {
        Byte value[] = {5, 54, 10, 32, 107, 207, 22, 9, 210, 189, 55, 57, 46, 47, 54, 2};
        const_searchVersionMessage = [self StringFromCameraData:value];
    }
    return const_searchVersionMessage;
}

//: camera_focus_red
- (NSString *)data_addGroupPath {
    /* static */ NSString *data_addGroupPath = nil;
    if (!data_addGroupPath) {
        Byte value[] = {16, 89, 5, 247, 167, 10, 8, 20, 12, 25, 8, 6, 13, 22, 10, 28, 26, 6, 25, 12, 11, 55};
        data_addGroupPath = [self StringFromCameraData:value];
    }
    return data_addGroupPath;
}

//: key.Notification.Tap.To.Modify.Exposure.Point
- (NSString *)constWithTitle {
    /* static */ NSString *constWithTitle = nil;
    if (!constWithTitle) {
        Byte value[] = {45, 29, 12, 31, 169, 141, 109, 214, 107, 240, 132, 123, 78, 72, 92, 17, 49, 82, 87, 76, 73, 76, 70, 68, 87, 76, 82, 81, 17, 55, 68, 83, 17, 55, 82, 17, 48, 82, 71, 76, 73, 92, 17, 40, 91, 83, 82, 86, 88, 85, 72, 17, 51, 82, 76, 81, 87, 80};
        constWithTitle = [self StringFromCameraData:value];
    }
    return constWithTitle;
}

//: M_SenseME_Face_Video_5.3.4
- (NSString *)noti_selectTensionMessage {
    /* static */ NSString *noti_selectTensionMessage = nil;
    if (!noti_selectTensionMessage) {
        Byte value[] = {26, 43, 8, 56, 157, 201, 46, 30, 34, 52, 40, 58, 67, 72, 58, 34, 26, 52, 27, 54, 56, 58, 52, 43, 62, 57, 58, 68, 52, 10, 3, 8, 3, 9, 179};
        noti_selectTensionMessage = [self StringFromCameraData:value];
    }
    return noti_selectTensionMessage;
}
...
// __CLOSE_PRINT__
//
//  PeculiarVelocityView.m
//  FZIphone
//
//  Created by Jack on 2018/8/3.
//  Copyright © 2018年 GuoJiang. All rights reserved.
//
//#import "TextCacheCost.h"
//#import "FiltrateModel.h"
//ST_MOBILE

// __M_A_C_R_O__
//: #import "PeculiarVelocityView.h"
#import "PeculiarVelocityView.h"
...
//: @interface PeculiarVelocityView ()<STFilterCameraDelegate,UIGestureRecognizerDelegate>
@interface PeculiarVelocityView ()<MagnitudeDelegate,UIGestureRecognizerDelegate>
{
...
//: @property (nonatomic, strong) LabLicHelper * lichelper;
@property (nonatomic, strong) LabLicHelper * lichelper;
//: @property (nonatomic, strong) MinimumThen * ParamUtil;
@property (nonatomic, strong) MinimumThen * ParamUtil;
...
//: @implementation PeculiarVelocityView
@implementation PeculiarVelocityView
...
- (instancetype)init {
    ...
    if (self) {
        ...
        //: self.lichelper = [LabLicHelper share];
        self.lichelper = [LabLicHelper share];
        //: self.ParamUtil = [[MinimumThen alloc]init];
        self.ParamUtil = [[MinimumThen alloc]init];
        //: _currentDropCount = 5;
        ...
        if ([self.lichelper countLicense]) {
            ...
            //: [[TextCacheCost share] getLocalModel];
            [[TextCacheCost share] someConversation];
            ...
            [NSNotificationCenter.defaultCenter addObserver:self
                                                   ...
                                                   selector:@selector(generating:)
                                                       //: name:@"key.Notification.Tap.To.Modify.Exposure.Point"
                                                       name:[[CameraData sharedInstance] constWithTitle]
                                                     //: object:nil];
                                                     object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(pageDownNotice:) name:self.ParamUtil.kNotificationSelectBeautyFilterStr object:nil];
            ...
            [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(absorptionned:) name:self.ParamUtil.kNotificationBeautyFilterValueChangeStr object:nil];
...
- (instancetype)initWithVideo:(NSString*)sessionPreset {
    ...
    if (self = [super init]) {
        ...
        if ([self.lichelper countLicense]) {
            ...
            [self initHopedForView:sessionPreset];

        //: } else {
        } else {
            //: [LabLicHelper.share checkRemoteLicInfoWithCompletion:^(BOOL x) {
            [LabLicHelper.share instilWithCompletion:^(BOOL x) {
                //: if (x) {
                if (x) {
                    //: [self initFilterView:sessionPreset];
                    [self initHopedForView:sessionPreset];
...
- (void)initHopedForView:(NSString*)sessionPreset {
    //: self.layer.masksToBounds = YES;
    self.layer.masksToBounds = YES;
    //: self.clipsToBounds = YES;
    self.clipsToBounds = YES;
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(pageDownNotice:)
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(absorptionned:)
...
- (void)isCustom {
    ...
    //: if ([TextCacheCost share].filterSuite.filterModelPath.length > 0) {
    if ([TextCacheCost share].filterSuite.filterModelPath.length > 0) {
        //: FiltrateModel* model = [[TextCacheCost share] getFilterDataModel];
        FiltrateModel* model = [[TextCacheCost share] positionTo];
        //: if (model) {
        if (model) {
...
- (void)setCurrentSessionPreset:(NSString *)currentSessionPreset {
    ...
    [self.stCamera load:self.previewCenter user:self.previewFrame];
}
...
- (void)frame:(BOOL)isFullScreen {
    ...
    [self viewImage:width changeOnSession:height];
}
/// 设置视频通话预览的采集大小
...
- (void)error {
    ...
    _stCamera = [[EqualLoad alloc] initWithSession:AVCaptureDevicePositionFront
                                          //: sessionPresset:self.currentSessionPreset
                                          need:self.currentSessionPreset
                                                     //: fps:fps
                                                     title:fps
                                           //: needYuvOutput:NO];
                                           object:NO];
...
- (void)viewImage:(CGFloat)width changeOnSession:(CGFloat)height {
    //: CGRect previewRect = [self.stCamera getScaleRect:CGRectMake(0 , 0 , width, height)
    CGRect previewRect = [self.stCamera instance:CGRectMake(0 , 0 , width, height)
                                                   //: scaleToFit:NO];
                                                   languageUnit:NO];
    ...
    if (self.glPreview.superview == nil) {
        ...
        @synchronized (self) {
           ...
           _glPreview = [[MaleView alloc] initWithSample:previewRect session:previewContext];
        }
...
- (void)actionDown {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(secOf) name:UIApplicationWillResignActiveNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appDidEnterBackground) name:UIApplicationDidEnterBackgroundNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(messageName) name:UIApplicationDidEnterBackgroundNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appWillEnterForeground) name:UIApplicationWillEnterForegroundNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(videoHeight) name:UIApplicationWillEnterForegroundNotification object:nil];

    //: [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(appDidBecomeActive) name:UIApplicationDidBecomeActiveNotification object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(runProperty) name:UIApplicationDidBecomeActiveNotification object:nil];
}
...
- (void)sign:(CMAccelerometerData *)accelerometerData {
...
- (st_rotate_type)load {
    ...
    [self sign:self.motionManager.accelerometerData];
...
- (void)parameterStart {
    ...
    //初始化检测模块句柄
    //: NSString *strModelPath = [LineThen.getModelFileName stringByAppendingPathComponent:@"M_SenseME_Face_Video_5.3.4"];
    NSString *strModelPath = [LineThen.startText stringByAppendingPathComponent:[[CameraData sharedInstance] noti_selectTensionMessage]];
    //: strModelPath = [strModelPath stringByAppendingPathExtension:@"model"];
    strModelPath = [strModelPath stringByAppendingPathExtension:[[CameraData sharedInstance] const_searchVersionMessage]];
...
- (void)initOrientationPath {
    // 创建结果纹理
    //: [self setupTextureWithPixelBuffer:&_cvBeautifyBuffer
    [self faceOf:&_cvBeautifyBuffer
                                    //: w:self.imageWidth
                                    nearEnable:self.imageWidth
                                    //: h:self.imageHeight
                                    with:self.imageHeight
                            //: glTexture:&_textureBeautifyOutput
                            jobDescription:&_textureBeautifyOutput
                            //: cvTexture:&_cvTextureBeautify];
                            sessionTexture:&_cvTextureBeautify];

    //: [self setupTextureWithPixelBuffer:&_cvStickerBuffer
    [self faceOf:&_cvStickerBuffer
                                    //: w:self.imageWidth
                                    nearEnable:self.imageWidth
                                    //: h:self.imageHeight
                                    with:self.imageHeight
                            //: glTexture:&_textureStickerOutput
                            jobDescription:&_textureStickerOutput
                            //: cvTexture:&_cvTextureSticker];
                            sessionTexture:&_cvTextureSticker];

    //: [self setupTextureWithPixelBuffer:&_cvFilterBuffer
    [self faceOf:&_cvFilterBuffer
                                    //: w:self.imageWidth
                                    nearEnable:self.imageWidth
                                    //: h:self.imageHeight
                                    with:self.imageHeight
                            //: glTexture:&_textureFilterOutput
                            jobDescription:&_textureFilterOutput
                            //: cvTexture:&_cvTextureFilter];
                            sessionTexture:&_cvTextureFilter];

    //: [self setupTextureWithPixelBuffer:&_cvMakeUpBuffer
    [self faceOf:&_cvMakeUpBuffer
                                    //: w:self.imageWidth
                                    nearEnable:self.imageWidth
                                    //: h:self.imageHeight
                                    with:self.imageHeight
                            //: glTexture:&_textureMakeUpOutput
                            jobDescription:&_textureMakeUpOutput
                            //: cvTexture:&_cvTextureMakeup];
                            sessionTexture:&_cvTextureMakeup];

}

//: - (BOOL)setupTextureWithPixelBuffer:(CVPixelBufferRef *)pixelBufferOut
- (BOOL)faceOf:(CVPixelBufferRef *)pixelBufferOut
                                  //: w:(int)iWidth
                                  nearEnable:(int)iWidth
                                  //: h:(int)iHeight
                                  with:(int)iHeight
                          //: glTexture:(GLuint *)glTexture
                          jobDescription:(GLuint *)glTexture
                          //: cvTexture:(CVOpenGLESTextureRef *)cvTexture {
                          sessionTexture:(CVOpenGLESTextureRef *)cvTexture {
...
- (BOOL)temple:(CVPixelBufferRef)pixelBuffer {
...
- (void)generating:(NSNotification *)sender {
    ...
    if ([tapGesture isKindOfClass:UITapGestureRecognizer.class]) {
        ...
        [self table:tapGesture];
    }

}

//: - (void)tapScreen:(UITapGestureRecognizer *)tapGesture {
- (void)table:(UITapGestureRecognizer *)tapGesture {
    ...
    [self.stCamera load:point user:self.frame];
...
- (UIImageView *)focusImageView {
    ...
    if (!_focusImageView) {
        ...
        _focusImageView.image = [UIImage imageNamed:[[CameraData sharedInstance] data_addGroupPath]];
...
- (void)pageDownNotice:(NSNotification*) notice {
    //: if ([notice.object isKindOfClass:[FiltrateModel class]]) {
    if ([notice.object isKindOfClass:[FiltrateModel class]]) {
        //: FiltrateModel* model = (FiltrateModel*)notice.object;
        FiltrateModel* model = (FiltrateModel*)notice.object;
        //: [self handleFilterChanged:model];
        [self first:model];
    }
...
//: - (void)handleFilterChanged:(FiltrateModel *)model {
- (void)first:(FiltrateModel *)model {
...
- (void)absorptionned:(NSNotification*) notice {
    //: FiltrateModel* model = (FiltrateModel*)notice.object;
    FiltrateModel* model = (FiltrateModel*)notice.object;
    //: if (_hFilter) {
    if (_hFilter) {
...
- (void)title {
    ...
    //: AttractorModel *beautyValueCache = TextCacheCost.share.currentBeautySuite;
    AttractorModel *beautyValueCache = TextCacheCost.share.ptolemaicSystemSuite;

    // 设置美白参数
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_WHITEN_STRENGTH value:beautyValueCache.fWhitenStrength];
    // 设置默认红润参数
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_REDDEN_STRENGTH value:beautyValueCache.fReddenStrength];
    // 设置默认磨皮参数
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SMOOTH_STRENGTH value:beautyValueCache.fSmoothStrength];
    // 设置默认大眼参数
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ENLARGE_EYE_RATIO value:beautyValueCache.fEnlargeEyeStrength];
    // 设置默认瘦脸参数
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_FACE_RATIO value:beautyValueCache.fShrinkFaceStrength];
    // 设置小脸参数
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_JAW_RATIO value:beautyValueCache.fShrinkJawStrength];

    //瘦脸型
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_THIN_FACE_SHAPE_RATIO value:beautyValueCache.fThinFaceShapeStrength];
    //窄脸
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_NARROW_FACE_STRENGTH value:beautyValueCache.fNarrowFaceStrength];
    //圆眼
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_ROUND_EYE_RATIO value:beautyValueCache.fRoundEyeStrength];
    //下巴
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_CHIN_LENGTH_RATIO value:beautyValueCache.fChinStrength];
    //额头
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_HAIRLINE_HEIGHT_RATIO value:beautyValueCache.fHairLineStrength];
    //瘦鼻翼
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NARROW_NOSE_RATIO value:beautyValueCache.fNarrowNoseStrength];
    //长鼻
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_NOSE_LENGTH_RATIO value:beautyValueCache.fLongNoseStrength];
    //嘴形
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fMouthStrength]];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_MOUTH_SIZE_RATIO value:[self min:beautyValueCache.fMouthStrength]];
    //缩人中
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fPhiltrumStrength]];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PHILTRUM_LENGTH_RATIO value:[self min:beautyValueCache.fPhiltrumStrength]];

    //设置对比度参数
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fContrastStrength]];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_CONTRAST_STRENGTH value:[self min:beautyValueCache.fContrastStrength]];

    //设置饱和度参数
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSaturationStrength]];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SATURATION_STRENGTH value:[self min:beautyValueCache.fSaturationStrength]];

    //锐化参数
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self rangeMappedValueFrom:beautyValueCache.fSharpenStrength]];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHARPEN_STRENGTH value:[self min:beautyValueCache.fSharpenStrength]];

    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self rangeMappedValueFrom:beautyValueCache.fAppleMusleStrength]];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_APPLE_MUSLE_RATIO value:[self min:beautyValueCache.fAppleMusleStrength]];

    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_PROFILE_RHINOPLASTY_RATIO value:beautyValueCache.fProfileRhinoplastyStrength];

    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_DISTANCE_RATIO value:beautyValueCache.fEyeDistanceStrength];

    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_EYE_ANGLE_RATIO value:beautyValueCache.fEyeAngleStrength];

    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_OPEN_CANTHUS_RATIO value:beautyValueCache.fOpenCanthusStrength];

    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_BRIGHT_EYE_RATIO value:beautyValueCache.fBrightEyeStrength];
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_DARK_CIRCLES_RATIO value:beautyValueCache.fRemoveDarkCirclesStrength];
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_REMOVE_NASOLABIAL_FOLDS_RATIO value:beautyValueCache.fRemoveNasolabialFoldsStrength];
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_3D_WHITE_TEETH_RATIO value:beautyValueCache.fWhiteTeethStrength];
    //: [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];
    [self.ParamUtil viewNeedWithBeautifyHandle:_hBeautify type:ST_BEAUTIFY_SHRINK_CHEEKBONE_RATIO value:beautyValueCache.fShrinkCheekboneStrength];

}

//: - (float)rangeMappedValueFrom:(float) rawValue {
- (float)min:(float) rawValue {
...
void resultTranslate(st_mobile_animal_face_t *src, int faceCount, st_mobile_animal_face_t *dst) {
...
void visageFormat(st_mobile_animal_face_t *src, int faceCount) {
    //: if (faceCount > 0) {
    if (faceCount > 0) {
        //: for (int i = 0; i < faceCount; ++i) {
        for (int i = 0; i < faceCount; ++i) {
...
- (void)mail:(CMSampleBufferRef)originalSampleBuffer
                       //: resultPixelBuffer:(CVPixelBufferRef)resultPixelBufffer {
                       countCreate:(CVPixelBufferRef)resultPixelBufffer {
    ...
    if ( originalSampleBuffer && resultPixelBufffer ) {
        ...
        CVPixelBufferRef copyBuffer = [self valueData:resultPixelBufffer];

        //: id<PeculiarVelocityViewDelegete> tmpDelegate = self.delegate;
        id<ImageError> tmpDelegate = self.delegate;

        //
        //: if ([tmpDelegate respondsToSelector:@selector(stVideoBeautyView:didProcessVideoSampleBuffer:)]) {
        if ([tmpDelegate respondsToSelector:@selector(active:totalimate:)]) {
            //: [tmpDelegate stVideoBeautyView:self didProcessVideoSampleBuffer:copyBuffer];
            [tmpDelegate active:self totalimate:copyBuffer];
        }

        //
        //: if (tmpDelegate && [tmpDelegate respondsToSelector:@selector(PeculiarVelocityView:didProcessSampleBuffer:)]) {
        if (tmpDelegate && [tmpDelegate respondsToSelector:@selector(user:processMessageFilterSampleParadigmBuffer:)]) {
            ...
            //: if (sampleBuffer) {
            if (sampleBuffer) {
                //: [tmpDelegate PeculiarVelocityView:self didProcessSampleBuffer:sampleBuffer];
                [tmpDelegate user:self processMessageFilterSampleParadigmBuffer:sampleBuffer];
            }
...
- (CVPixelBufferRef)valueData:(CVPixelBufferRef)pixelBuffer {
...
- (void)input:(AVCaptureOutput *)captureOutput streetwise:(CMSampleBufferRef)sampleBuffer at:(AVCaptureConnection *)connection {
    ...
    st_rotate_type stMobileRotate = [self load];
    ...
    if (_hDetector) {
        ...
        if (self.iCurrentAction > 0) {
            ...
            if (_faceFrameCount == 12) { // 第12帧时
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:[[CameraData sharedInstance] main_schedulePublishFormat] object:nil userInfo: @{[[CameraData sharedInstance] mainQualityFinishId]: @(hasFace)}];
            }
            ...
            if (_faceFrameCount%18 == 0) { // 每秒发送一次
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:[[CameraData sharedInstance] dataVideoStr] object:nil userInfo: @{[[CameraData sharedInstance] mainQualityFinishId]: @(hasFace)}];
            }
        }
    }
    ...
    if (faceCount > 0) {
        ...
        resultTranslate(_detectResult1, faceCount, newDetectResult1);
    }
    ...
    dispatch_async(self.renderQueue, ^{
       ...
       BOOL isTextureOriginReady = [self temple:pixelBuffer];
       ...
       // ST_MOBILE 以下为滤镜部分
        ...
        if (callBackFlag) [self mail:sampleBuffer countCreate:resultPixelBufffer];
        ...
        if (faceCount > 0) {
            ...
            visageFormat(newDetectResult1, faceCount);
        }
        //: if (callBackFlag) {
        if (callBackFlag) {
            //: @synchronized (self) {
            @synchronized (self) {
                ...
                [self.glPreview texture:textureResult];
            }
        }
