...
open class DeleteReactiveCompatible: UIView {
    ...
    static var shared = DeleteReactiveCompatible()
    ...
    class func fileAdd() {
        //: show(superView: nil)
        withMethod(superView: nil)
    }

    //: class func show(superView: UIView?) {
    class func withMethod(superView: UIView?) {
        //: if superView != nil {
        if superView != nil {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                //: ProgressHUD.shared.frame = superView!.bounds
                DeleteReactiveCompatible.shared.frame = superView!.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                DeleteReactiveCompatible.shared.activityIndicator.center = DeleteReactiveCompatible.shared.center
                //: superView!.addSubview(ProgressHUD.shared)
                superView!.addSubview(DeleteReactiveCompatible.shared)
            }
            //: } else {
        } else {
            //: DispatchQueue.main.async {
            DispatchQueue.main.async {
                ...
                DeleteReactiveCompatible.shared.frame = UIScreen.main.bounds
                //: ProgressHUD.shared.activityIndicator.center = ProgressHUD.shared.center
                DeleteReactiveCompatible.shared.activityIndicator.center = DeleteReactiveCompatible.shared.center
                //: LineThen.getWindow().addSubview(ProgressHUD.shared)
                LineThen.getName().addSubview(DeleteReactiveCompatible.shared)
            }
        }
        //: ProgressHUD.shared.hud_startAnimating()
        DeleteReactiveCompatible.shared.commencePickAnimating()
    }

    //: class func dismiss() {
    class func deflector() {
        //: ProgressHUD.shared.hud_stopAnimating()
        DeleteReactiveCompatible.shared.third()
    }

    //: private func hud_startAnimating() {
    private func commencePickAnimating() {
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: self.backgroundColor = UIColor(white: 0, alpha: 0)
            self.backgroundColor = UIColor(white: 0, alpha: 0)
            //: self.activityIndicator.transform = CGAffineTransform(scaleX: kTransformScale, y: kTransformScale)
            self.activityIndicator.transform = CGAffineTransform(scaleX: main_packageShowUrl, y: main_packageShowUrl)
            //: self.activityIndicator.alpha = 0
            self.activityIndicator.alpha = 0
            //: UIView.animate(withDuration: kAnimationInterval) {
            UIView.animate(withDuration: constMaxUserMsg) {
                //: self.backgroundColor = UIColor(white: 0, alpha: kBackgroundView_alpha)
                self.backgroundColor = UIColor(white: 0, alpha: showDetailText)
                //: self.activityIndicator.transform = CGAffineTransform(scaleX: 1, y: 1)
                self.activityIndicator.transform = CGAffineTransform(scaleX: 1, y: 1)
                //: self.activityIndicator.alpha = kProgressHUD_alpha
                self.activityIndicator.alpha = kBarText
    ...
    private func third() {
        ...
        DispatchQueue.main.async {
            ...
            UIView.animate(withDuration: constMaxUserMsg) {
                ...
                DeleteReactiveCompatible.shared.removeFromSuperview()
            }
        }
    }
    ...
    private lazy var activityIndicator: UIActivityIndicatorView = {
        ...
        indicator.bounds = CGRect(x: 0, y: 0, width: user_infoKey, height: user_infoKey)
        //: indicator.center = self.center
        indicator.center = self.center
        //: indicator.backgroundColor = .black
        indicator.backgroundColor = .black
        //: indicator.layer.cornerRadius = kProgressHUD_cornerRadius
        indicator.layer.cornerRadius = main_ageMsg
...
extension DeleteReactiveCompatible {
    ...
    class func snapToast(_ str: String?) {
        //: toast(str, showTime: 1)
        addAction(str, showTime: 1)
    }

    //: class func toast(_ str: String?, showTime: CGFloat) {
    class func addAction(_ str: String?, showTime: CGFloat) {
        ...
        titleLab.textColor = .white
        //: LineThen.getWindow().addSubview(titleLab)
        LineThen.getName().addSubview(titleLab)
        //: let size = titleLab.sizeThatFits(CGSize(width: UIScreen.main.bounds.width - 40, height: CGFloat(MAXFLOAT)))
        let size = titleLab.sizeThatFits(CGSize(width: UIScreen.main.bounds.width - 40, height: CGFloat(MAXFLOAT)))
        //: titleLab.center = LineThen.getWindow().center
        titleLab.center = LineThen.getName().center
