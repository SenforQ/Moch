...
fileprivate let k_errorMsg:[UInt8] = [0xa4,0xa3,0xa4,0xb9,0xe5,0xae,0xa2,0xa9,0xa8,0xbf,0xf7,0xe4,0xed,0xa5,0xac,0xbe,0xed,0xa3,0xa2,0xb9,0xed,0xaf,0xa8,0xa8,0xa3,0xed,0xa4,0xa0,0xbd,0xa1,0xa8,0xa0,0xa8,0xa3,0xb9,0xa8,0xa9]

private func doUp(split num: UInt8) -> UInt8 {
    return num ^ 205
}
...
class ToMusicThen: UIView {
    //: var cropStart: CGFloat {
    var cropStart: CGFloat {
        //: let offset = overlay.startOffset - (LeftMargin - self.contentView.contentOffset.x)
        let offset = overlay.startOffset - (mainDeviceMessage - self.contentView.contentOffset.x)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_errorMsg.map{doUp(split: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIScrollView = {
        ...
        while time <= self.duration {
            ...
            let image = self.pickUp(point: time)
            //: imv.image = image
            imv.image = image
    ...
    lazy var overlay: SmartCropOverlay = {
        ...
        let overlay = SmartCropOverlay(frame: self.bounds, maxCropLen: maxWidth, minCropLen: minWidth)
...
extension ToMusicThen {
    ...
    func random() {
        ...
        self.overlay.viewLastX(startX: mainDeviceMessage - x)
    }
    ...
    private func pickUp(point: CGFloat) -> UIImage {
...
extension ToMusicThen: UIScrollViewDelegate {
    //: func scrollViewDidScroll(_ scrollView: UIScrollView) {
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        //: var offset = scrollView.contentOffset
        var offset = scrollView.contentOffset
        //: let minx = LeftMargin - overlay.startOffset
        let minx = mainDeviceMessage - overlay.startOffset
    ...
    func scrollViewWillBeginDragging(_: UIScrollView) {
        ...
        self.overlay.equalComponent(isShow: false)
    }
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        self.overlay.equalComponent(isShow: true)
    }
...
extension ToMusicThen {
    ...
    private func noneVoice() {
        ...
        self.contentView.snp.makeConstraints { make in
            ...
            make.leading.equalToSuperview().offset(mainDeviceMessage)
