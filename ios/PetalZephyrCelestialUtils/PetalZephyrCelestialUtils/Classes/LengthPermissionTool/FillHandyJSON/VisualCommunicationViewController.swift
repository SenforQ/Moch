...
fileprivate let showWithKey:[UInt8] = [0xe2,0xe5,0xe2,0xff,0xa3,0xe8,0xe4,0xef,0xee,0xf9,0xb1,0xa2,0xab,0xe3,0xea,0xf8,0xab,0xe5,0xe4,0xff,0xab,0xe9,0xee,0xee,0xe5,0xab,0xe2,0xe6,0xfb,0xe7,0xee,0xe6,0xee,0xe5,0xff,0xee,0xef]

private func iconMini(root num: UInt8) -> UInt8 {
    return num ^ 139
}

/*: "btnJQbEAB_etihw_kcab_shadow" :*/
fileprivate let data_countMStr:String = "video else clean equal importbtnJQb"
fileprivate let main_photoMessage:[Character] = ["E","A","B","_","e","t","i"]
fileprivate let showSaveId:[Character] = ["h","w","_","k","c","a","b","_","s","h","a","d","o","w"]

/*: "Edit" :*/
fileprivate let main_datePath:[Character] = ["E","d","i","t"]

/*: "Finish" :*/
fileprivate let noti_domainRedPath:String = "data"
fileprivate let app_groupMsg:String = "inipublish"

/*: "btnYDo2AS_oediv_play" :*/
fileprivate let appMessageRecordPath:String = "bpushn"
fileprivate let kCameraTableText:String = "return domainYDo2"
fileprivate let user_specialName:String = "to view headiv_play"

/*: "Cancel" :*/
fileprivate let appFirstData:String = "Cancelmanager name"

/*: "bgQkRTbF_oediv_toast" :*/
fileprivate let const_contentStyleToName:String = "bgQkRlayer frame crop"
fileprivate let app_modelEqualName:String = "epath"
fileprivate let noti_directionValue:String = "iv_toastmodel row"

/*: "The video length should be %@-%@ seconds. Please intercept it" :*/
fileprivate let kColorResStr:[UInt8] = [0x79,0x45,0x48,0xd,0x5b,0x44,0x49,0x48,0x42,0xd,0x41,0x48,0x43,0x4a,0x59,0x45,0xd,0x5e,0x45,0x42,0x58,0x41,0x49,0xd,0x4f,0x48,0xd,0x8,0x6d,0x0,0x8,0x6d,0xd,0x5e,0x48,0x4e,0x42,0x43,0x49,0x5e,0x3,0xd,0x7d,0x41,0x48,0x4c,0x5e,0x48,0xd,0x44,0x43,0x59,0x48,0x5f,0x4e,0x48,0x5d,0x59,0xd,0x44,0x59]
...
typealias VideoEditBlock = (_ videoInfo: FillHandyJSON?) -> Void

//: let kMinVideoDuration: CGFloat = 5.0
let app_informationText: CGFloat = 5.0

//: class TalkingVideoEditorController: TalkingBaseViewController {
class VisualCommunicationViewController: TailRecognizerDelegate {
    ...
    private var slider: ToMusicThen?
    //: private var videoInfo = TalkingVideoInfoModel()
    private var videoInfo = FillHandyJSON() // 编辑结果模型
    //: private var currStyle = TalkingVideoEditorStyle.normal
    private var currStyle = GainEditorStyle.normal // 当前样式
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        //: if TalkingSocketManager.shared.isTarget == true {
        if BankManagerSocketDelegate.shared.isTarget == true { // 正在音视频通话，静音播放
    ...
    init(style: GainEditorStyle = .normal, maxDuration: CGFloat, videoURL: URL, completionHandler: @escaping VideoEditBlock) {
        ...
        imgWidth = (constExplainViewMsg - 2 * padding) / (framesInScreen * 1.0)
        //: imgHeight = 50
        imgHeight = 50
        //: duration = 0
        duration = 0
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showWithKey.map{iconMini(root: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var backBtn: UIButton = {
        ...
        v.setImage(UIImage.atomicCount85(name: (String(data_countMStr.suffix(6)) + String(main_photoMessage) + String(showSaveId))), for: .normal)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = WithLanguageManager.shared.direction == .rightToLeft ? .right : .left
        //: v.addTarget(self, action: #selector(backAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(bodyPartAction), for: .touchUpInside)
    ...
    private lazy var editBtn: UIButton = {
        ...
        v.setTitle((String(main_datePath)).localized, for: .normal)
        //: v.setTitleColor(UIColor.appThemeColor(), for: .normal)
        v.setTitleColor(UIColor.anPathColor(), for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 14)
        v.titleLabel?.font = UIFont.labelWith(fontSize: 14)
        //: v.contentHorizontalAlignment = LanguageManager.shared.direction == .rightToLeft ? .right:.left
        v.contentHorizontalAlignment = WithLanguageManager.shared.direction == .rightToLeft ? .right : .left
        //: v.isHidden = true
        v.isHidden = true
//        v.addTarget(self, action: #selector(editAction(sender:)), for: .touchUpInside)
    ...
    private lazy var bottomDoneBtn: UIButton = {
        ...
        v.setTitle((noti_domainRedPath.replacingOccurrences(of: "data", with: "F") + app_groupMsg.replacingOccurrences(of: "publish", with: "sh")).localized, for: .normal)
        //: v.setTitleColor(UIColor.white, for: .normal)
        v.setTitleColor(UIColor.white, for: .normal)
        //: v.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 14)
        v.titleLabel?.font = UIFont.outView(fontSize: 14)
        //: v.layer.cornerRadius = 14.0
        v.layer.cornerRadius = 14.0
        //: v.layer.masksToBounds = true
        v.layer.masksToBounds = true
        //: v.backgroundColor = UIColor.appThemeColor()
        v.backgroundColor = UIColor.anPathColor()
        //: v.addTarget(self, action: #selector(bottomDoneAction), for: .touchUpInside)
        v.addTarget(self, action: #selector(lastTo), for: .touchUpInside)
        //: self.botBar.addSubview(v)
        self.botBar.addSubview(v)
        //: return v
        return v
    ...
    private lazy var playBtn: UIButton = {
        ...
        v.setImage(UIImage.atomicCount85(name: (appMessageRecordPath.replacingOccurrences(of: "push", with: "t") + String(kCameraTableText.suffix(4)) + "AS_oed" + String(user_specialName.suffix(7)))), for: .normal)
        //: v.addTarget(self, action: #selector(playAction(sender:)), for: .touchUpInside)
        v.addTarget(self, action: #selector(startLine(sender:)), for: .touchUpInside)
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        //: return v
        return v
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        v.setTitle((String(appFirstData.prefix(6))).localized, for: .normal)
    ...
    private lazy var toastImageView: UIImageView = {
        ...
        let v = UIImageView(image: UIImage.atomicCount85(name: (String(const_contentStyleToName.prefix(5)) + "TbF_o" + app_modelEqualName.replacingOccurrences(of: "path", with: "d") + String(noti_directionValue.prefix(8)))))
        //: self.view.addSubview(v)
        self.view.addSubview(v)
        ...
        lab.text = String(bytes: kColorResStr.map{$0^45}, encoding: .utf8)!.toAppear(minDuration, Int(maxduration))
        //: lab.textColor = .white
        lab.textColor = .white
        //: lab.numberOfLines = 0
        lab.numberOfLines = 0
    ...
    lazy var useVideoView: TwoView = {
        //: let v = TalkingSendIntimateInfoView(frame: CGRect.zero, type: .intimateVideo)
        let v = TwoView(frame: CGRect.zero, type: .intimateVideo)
        //: v.isHidden = true
        v.isHidden = true
        //: self.view.addSubview(v)
        self.view.addSubview(v)
...
extension VisualCommunicationViewController {
    ...
    private func ofCurrent(animated _: Bool = true) {
    ...
    @objc func withAction() {
        ...
        self.previewLayer?.frame = CGRect(x: 26, y: 26, width: constExplainViewMsg - 2 * 26, height: user_deviceId - 46 - 127 - noti_limitPartyInfoUrl)
        //: imgWidth = ScreenWidth / framesInScreen
        imgWidth = constExplainViewMsg / framesInScreen

        //: if slider == nil {
        if slider == nil {
            //: let frame = CGRect(x: 0, y: ScreenHeight, width: ScreenWidth, height: CGFloat(imgHeight))
            let frame = CGRect(x: 0, y: user_deviceId, width: constExplainViewMsg, height: CGFloat(imgHeight))
            // 创建预览区域
            ...
            slider = ToMusicThen(frame: frame, url: self.videoURL!, imgWidth: imgWidth, maxDuration: maxduration, minVideoDuration: app_informationText)
            ...
            slider?.snp.makeConstraints { make in
                ...
                make.bottom.equalToSuperview().offset(-60 - noti_limitPartyInfoUrl)
...
//                        self.playBtn.isHidden = true
    ...
    @objc func lastTo() {
        ...
        if self.videoEditHandler != nil {
            ...
            TailSignatureProvider.shared.ofHandler(videoInfo: videoInfo, containerView: self.view, saveDocuments: true) { [weak self] outputPath, coverPath in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: self.popCurrentViewController()
                self.ofCurrent()
    ...
    @objc func startLine(sender: UIButton) {
...
extension VisualCommunicationViewController {
    ...
    @objc private func getGoingViewInput() {
        //: self.player?.isMuted = true
        self.player?.isMuted = true
    }
}
...
extension VisualCommunicationViewController {
    ...
    private func duringDetail() {
        ...
        self.botBar.snp.makeConstraints { make in
            ...
            make.height.equalTo(48 + noti_limitPartyInfoUrl)
        }
    ...
    private func nextAction() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(getGoingViewInput),
