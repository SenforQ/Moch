...
fileprivate let show_skinData:[UInt8] = [0x6f,0x68,0x6f,0x72,0x2e,0x65,0x69,0x62,0x63,0x74,0x3c,0x2f,0x26,0x6e,0x67,0x75,0x26,0x68,0x69,0x72,0x26,0x64,0x63,0x63,0x68,0x26,0x6f,0x6b,0x76,0x6a,0x63,0x6b,0x63,0x68,0x72,0x63,0x62]

private func cancelCell(attention num: UInt8) -> UInt8 {
    return num ^ 6
}

/*: "FFD855" :*/
fileprivate let constDataStr:[Character] = ["F","F","D","8","5","5"]
...
//: let LeftMargin = 43.0
let mainDeviceMessage = 43.0
//: let kSliderWidth = 20.0
let app_userStatusScreenMessage = 20.0
...
class SmartCropOverlay: UIView {
    ...
    var defaultWidth: CGFloat = 0.0 {
        ...
        didSet {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.2) {
                ...
                let minX = self.leftSilder.frame.origin.x + self.defaultWidth - app_userStatusScreenMessage
                //: self.rightSlider.frame = CGRect(x: minX,
                self.rightSlider.frame = CGRect(x: minX,
                                                //: y: self.rightSlider.frame.origin.y,
                                                y: self.rightSlider.frame.origin.y,
                                                //: width: kSliderWidth,
                                                width: app_userStatusScreenMessage,
                ...
                self.viewLastX(startX: self.videoStartX)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_skinData.map{cancelCell(attention: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var indicatorView: UIView = {
        ...
        indicator.backgroundColor = UIColor(hex: (String(constDataStr)))
        //    indicator.alpha = 0.5
    ...
    private lazy var leftSilder: UIView = {
        ...
        leftSilder.frame = CGRect(x: mainDeviceMessage, y: 0, width: app_userStatusScreenMessage, height: self.frame.size.height)
    ...
    private lazy var rightSlider: UIView = {
        ...
        rightSlider.frame = CGRect(x: self.frame.size.width - app_userStatusScreenMessage, y: 0, width: app_userStatusScreenMessage, height: self.frame.size.height)
    ...
    private lazy var leftCoverView: UIView = {
        ...
        leftCoverView.backgroundColor = UIColor.showViewCreate(0, 0, 0, 0.6)
        //: leftCoverView.frame = CGRect(x: 0, y: 0, width: LeftMargin, height: self.frame.size.height)
        leftCoverView.frame = CGRect(x: 0, y: 0, width: mainDeviceMessage, height: self.frame.size.height)
    ...
    private lazy var rightCoverView: UIView = {
        ...
        rightCoverView.backgroundColor = UIColor.showViewCreate(0, 0, 0, 0.6)
...
extension SmartCropOverlay {
    /// 重置
    //: func resetOriginalState() {
    func memory() {
        //: self.leftSilder.frame = CGRect(x: LeftMargin, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.leftSilder.frame = CGRect(x: mainDeviceMessage, y: 0, width: app_userStatusScreenMessage, height: self.frame.size.height)
        //: self.rightSlider.frame = CGRect(x: self.frame.size.width - kSliderWidth, y: 0, width: kSliderWidth, height: self.frame.size.height)
        self.rightSlider.frame = CGRect(x: self.frame.size.width - app_userStatusScreenMessage, y: 0, width: app_userStatusScreenMessage, height: self.frame.size.height)
        //: resetIndicator()
        noCurrent()
    }

    //: func needShowIndicator(isShow: Bool) {
    func equalComponent(isShow: Bool) {
        //: self.indicatorView.isHidden = !isShow
        self.indicatorView.isHidden = !isShow
    }

    //: func refreshSliderValidScope(startX: CGFloat) {
    func viewLastX(startX: CGFloat) {
        ...
        rightScope = CGVector(dx: leftSilder.frame.minX + minDurationGap - app_userStatusScreenMessage,
                              //: dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - kSliderWidth)
                              dy: min(leftSilder.frame.minX + maxDurationGap, self.width) - app_userStatusScreenMessage)
    }
}
...
extension SmartCropOverlay {
    ...
    @objc func cleat(gesture: UIPanGestureRecognizer) {
        ...
        case .ended:
            ...
            self.viewLastX(startX: videoStartX)
    ...
    @objc func primaryColour(gesture: UIPanGestureRecognizer) {
        //: let thisTimePoint = gesture.location(in: self)
        let thisTimePoint = gesture.location(in: self)
        //: let state = gesture.state
        let state = gesture.state
...
extension SmartCropOverlay {
    ...
    private func nowOn() {
        ...
        let leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(cleat(gesture:)))
        ...
        let rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(primaryColour(gesture:)))
