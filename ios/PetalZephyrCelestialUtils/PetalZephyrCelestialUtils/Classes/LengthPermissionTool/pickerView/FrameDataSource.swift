...
/*: "Select a date" :*/
fileprivate let dataAppearFormat:String = "Select a self add app intimate now"
fileprivate let userLogKey:[Character] = ["d","a","t","e"]

/*: M" :*/
fileprivate let app_listPath:[Character] = ["M"]

/*: D" :*/
fileprivate let dataAllowMessage:String = "progress"

/*: Y" :*/
fileprivate let appCenterSelectNameMsg:String = "plus"
...
class FrameDataSource: BuildEmptyReactiveCompatible, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    weak var delegate: CellObjectProtocol?

    //: override func initPickView() {
    override func translationView() {
        //: super.initPickView()
        super.translationView()

        //: titleString = "Select a date".localized
        titleString = (String(dataAppearFormat.prefix(9)) + String(userLogKey)).localized
        ...
        pushTo(year: defaultYear!, month: defaultMonth!, day: defaultDay!)
    }

    //: func setDefaultDate(year: Int, month: Int, day: Int) {
    func pushTo(year: Int, month: Int, day: Int) {
        //: currentYear = year
        currentYear = year
        //: if year == defaultYear {
        if year == defaultYear {
...
//        }
    ...
    override func dateButton(button: UIButton) {
        //: super.confirmClick(button: button)
        super.dateButton(button: button)
        //: refreshPickViewData()
        pickMessage()
        ...
        if let day = selectDay {
            ...
            delegate?.selectToDay(year: selectYear!, month: selectMonth!, day: day)
            //: } else {
        } else {
            //: delegate?.pickDateView(year: selectYear!, month: selectMonth!, day: 0)
            delegate?.selectToDay(year: selectYear!, month: selectMonth!, day: 0)
        }
    }
        ...
        if isShowDay {
            ...
            if component == 0 {
                ...
                if currentYear == yearSum + minShowYear - 1 {
                    ...
                    return calendarYearStart(year: year, month: month)
                }
                //: } else {
            } else {
                //: return self.yearSum
    ...
    func calendarYearStart(year: Int, month: Int) -> Int {
