...
fileprivate let app_largeMessage:String = "section to topbtn_"
fileprivate let constBindFileStr:String = "alertBpic class constraint"

/*: "btn_alertBackground_false" :*/
fileprivate let kScriptStr:String = "with return color manager viewbtn_"
fileprivate let noti_resultMessageStr:String = "image let make unionBackg"
fileprivate let show_labelText:String = "return inround_"
...
public class PileThen: NSObject {
    /// 通用回调
...
public extension PileThen {
    /// 适配器回调,用于给适配器参数赋值
    //: public typealias ConfigAlert = ((_ config: ShowAlertConfig) -> Void)
    typealias ConfigAlert = (_ config: MagnitudeAlertConfig) -> Void
    ...
    internal class func downCrosshead(title: String? = nil,
                                      //: message: String?  = nil,
                                      message: String? = nil,
                                      ...
                                      config: MagnitudeAlertConfig? = nil)
    {
        //: customAlert(title: title,
        refuseLay(title: title,
                  //: message: message,
                  message: message,
    ...
    @objc internal class func vanguardMale(title: String? = nil,
    ...
    class func refuseLay(title: String? = nil,
                         ...
                         config: MagnitudeAlertConfig? = nil)
    {
        ...
        let alertView = StatuteTextReactiveCompatible(title: title,
        ...
        //: LineThen.getWindow().addSubview(alertView)
        LineThen.getName().addSubview(alertView)
    ...
    class func obstruct() {
        //: LineThen.getWindow().subviews.forEach { (view) in
        LineThen.getName().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: StatuteTextReactiveCompatible.self) {
    ...
    class func hunt(title: String, isTouch: Bool) {
        //: LineThen.getWindow().subviews.forEach { (view) in
        LineThen.getName().subviews.forEach { view in
            //: if view.isKind(of: TalkingAlertView.self) {
            if view.isKind(of: StatuteTextReactiveCompatible.self) {
                //: let cunrenView: TalkingAlertView = view as! TalkingAlertView
                let cunrenView: StatuteTextReactiveCompatible = view as! StatuteTextReactiveCompatible
                //: cunrenView.changeRightBtn(title: title, isTouch: isTouch)
                cunrenView.addColor(title: title, isTouch: isTouch)
            }
        }
    }
}
...
public class MagnitudeAlertConfig {
    /// 执行动画时间
    ...
    public var maskType: DishMaskType = .color
    /// 阴影
    ...
    public var titleFont: UIFont = .photographicEquipment(type: .Medium, fontSize: 18)
    /// alert标题字体颜色
    //: public var titleColor  = UIColor.appTitleColor()
    public var titleColor = UIColor.towardOf()
    /// alert信息字体
    //: public var textFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var textFont: UIFont = .photographicEquipment(type: .Medium, fontSize: 16)
    //: public var textColor  = UIColor.appTitleColor()
    public var textColor = UIColor.towardOf()
    ...
    public var buttonFont: UIFont = .photographicEquipment(type: .Medium, fontSize: 16)
    /// alert按钮字体颜色
    //: public var leftColor  = UIColor.appThemeColor()
    public var leftColor = UIColor.anPathColor()
    //: public var rightColor = UIColor.white
    public var rightColor = UIColor.white
    //: public var rightBackgroundImage = "btn_show_alertBackground"
    public var rightBackgroundImage = (String(app_largeMessage.suffix(4)) + "show_" + String(constBindFileStr.prefix(6)) + "ackground")

    //: public var rightBackgroundImagefalse = "btn_alertBackground_false"
    public var rightBackgroundImagefalse = (String(kScriptStr.suffix(4)) + "alert" + String(noti_resultMessageStr.suffix(5)) + String(show_labelText.suffix(6)) + "false")
    //: public var rightFont: UIFont = .pingfangFont(type: .Medium, fontSize: 16)
    public var rightFont: UIFont = .photographicEquipment(type: .Medium, fontSize: 16)
