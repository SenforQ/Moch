...
- (UIImage *)length {
    ...
    //: return [UIImage imageWithData:[self thinData]];
    return [UIImage imageWithData:[self thinData]];
}


//: - (UIImage *)wcSessionCompress {
- (UIImage *)cameraGesture {
    //: return [self wcCompres:YES];
    return [self size:YES];
}
...
- (UIImage *)size:(Boolean)isSession {
    //: CGSize size = [self wxImageSize:isSession];
    CGSize size = [self imageInsideSizeFit:isSession];
    //: UIImage *reImage = [self resizedImage:size];
    UIImage *reImage = [self finder:size];
...
- (CGSize)imageInsideSizeFit:(Boolean)isSession {
...
+ (NSData *)gift:(UIImage *)image origin:(NSInteger)fileSize{
    ...
    while (outIMGData.length>(fileSize*1024)) {
        ...
        thumIMG = [thumIMG layerBurn:scalesize resizeValueScale:NO];

        //: outIMGData = [thumIMG compressToFileSize:(fileSize*1024)];
        outIMGData = [thumIMG scale:(fileSize*1024)];
    }
...
- (UIImage *)layerBurn:(CGSize)targetSize resizeValueScale:(BOOL)isScale{
...
- (NSData *)scale:(NSInteger)fileSize{
