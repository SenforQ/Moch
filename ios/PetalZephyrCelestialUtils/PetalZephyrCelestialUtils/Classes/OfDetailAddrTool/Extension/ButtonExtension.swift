...
/*: "recharge" :*/
fileprivate let notiQueryId:String = "RECHARGE"

/*: "recharge_ :*/
fileprivate let userViewId:String = "recintimate"
fileprivate let constTableSizePath:[Character] = ["_"]

/*: ".png" :*/
fileprivate let dataClearModelStr:String = "text voice for session.png"

/*: "_ :*/
fileprivate let main_sharedValue:[Character] = ["_"]

/*: .png" :*/
fileprivate let app_indexUrl:String = ".pngview send"

/*: ".jpg" :*/
fileprivate let main_mediumMsg:[Character] = [".","j","p","g"]

/*: .jpg" :*/
fileprivate let noti_toTitleMessage:[Character] = [".","j","p","g"]
...
class BlockButton: UIButton {
    ...
    var imageAlignment: PointOutTotal = .left
...
extension UIButton {
    ...
    func toObject(_ interval: CGFloat) {
        //: verticalAlignImageAndTitle(interval: interval, topOffset: 0)
        sinceTime(interval: interval, topOffset: 0)
    }
    ...
    func sinceTime(interval: CGFloat, topOffset: CGFloat) {
    ...
    func viewVerticalCloseset(interval: CGFloat, imageSize: CGSize, topOffset: CGFloat) {
...
extension UIButton {
    ...
    func forClick(color: UIColor, forState: UIControl.State) {
    ...
    func equalButton(urlStr: String,
                     //: placeImg: UIImage? = UIImage.placeImgSquare(),
                     placeImg: UIImage? = UIImage.warning(),
                     ...
                     loadFinish: ((_ isSucceed: Bool) -> Void)? = nil)
    {
        ...
        let imgUrl = urlStr.replacingOccurrences(of: dataSessionUrl.increase(), with: main_bottomStr)
    ...
    func tingWith(urlStr: String) {
        //: guard !urlStr.isEmptyString else { return }
        guard !urlStr.isEmptyString else { return }
        //: var newUrl = ""
        var newUrl = ""
        //: if urlStr.contains("recharge") {
        if urlStr.contains((notiQueryId.lowercased())) {
            //: newUrl = urlStr.replacingOccurrences(of: "recharge", with: "recharge_\(PackageID)")
            newUrl = urlStr.replacingOccurrences(of: (notiQueryId.lowercased()), with: (userViewId.replacingOccurrences(of: "intimate", with: "h") + "arge" + String(constTableSizePath)) + "\(main_maxTitle)")
            //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
            self.equalButton(urlStr: newUrl, placeImg: nil)
            //: return
            return
                //: } else if urlStr.contains(".png") {
        } else if urlStr.contains((String(dataClearModelStr.suffix(4)))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".png", with: "_\(PackageID).png")
            newUrl = urlStr.replacingOccurrences(of: (String(dataClearModelStr.suffix(4))), with: "_\(main_maxTitle)" + (String(app_indexUrl.prefix(4))))

            //: } else if urlStr.contains(".jpg") {
        } else if urlStr.contains((String(main_mediumMsg))) {
            //: newUrl = urlStr.replacingOccurrences(of: ".jpg", with: "_\(PackageID).jpg")
            newUrl = urlStr.replacingOccurrences(of: (String(main_mediumMsg)), with: "_\(main_maxTitle)" + (String(noti_toTitleMessage)))
        }
        //: self.setUrlImage(urlStr: newUrl, placeImg: nil)
        self.equalButton(urlStr: newUrl, placeImg: nil)
    }
