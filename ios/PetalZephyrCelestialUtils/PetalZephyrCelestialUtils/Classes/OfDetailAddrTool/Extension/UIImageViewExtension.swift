...
/*: "add_header_cookies" :*/
fileprivate let app_enterTitle:[Character] = ["a","d","d","_","h","e","a","d","e","r","_","c","o","o","k","i"]
fileprivate let constFlagTitle:[Character] = ["e","s"]
...
public extension UIImageView {
    ...
    @objc func viewFinish(urlStr: String,
    ...
    internal func luculentUpwardly(urlStr: String,
                                   //: placeImg: UIImage = UIImage.placeImgSquare(),
                                   placeImg: UIImage = UIImage.warning(),
    ...
    @objc func transitionFreebie(urlStr: String) {
        //: let imgUrl = urlStr.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let imgUrl = urlStr.replacingOccurrences(of: dataSessionUrl.increase(), with: main_bottomStr)
        //: self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.giftPlaceHolderImage())
        self.kf.setImage(with: URL(string: imgUrl), placeholder: UIImage.upImage())
    }
    ...
    @objc func sexDown(urlStr: String, completion: ((_ image: UIImage?) -> Void)? = nil) {
        // 允许请求携带cookie
        ...
        if var urlComponents = URLComponents(string: urlStr),
           ...
           let currURL = URL(string: OfDetailAddrTool.share.serverUrlStr)
        {
        ...
        let downloader = ImageDownloader(name: (String(app_enterTitle) + String(constFlagTitle)))
        ...
        self.kf.setImage(with: URL(string: newImgUrl), placeholder: UIImage.warning(), options: optionsInfo) { result in
            //: if completion != nil {
            if completion != nil {
                //: let image = try? result.get().image
                let image = try? result.get().image
