...
/*: "http://stat. :*/
fileprivate let dataNameStatusId:String = "how back ifhttp:/"
fileprivate let show_filePicName:String = "/stat.slide button guard"

/*: .com" :*/
fileprivate let dataAttentionMsg:[Character] = [".","c","o","m"]

/*: "http://" :*/
fileprivate let notiFormatText:String = "name array icon withhttp://"

/*: "https://" :*/
fileprivate let kTitleUrl:String = "show addhttps:"
fileprivate let notiPriceData:[Character] = ["/","/"]

/*: "http://m. :*/
fileprivate let data_infoTitle:String = "http://mview i self self"
fileprivate let constModelRequestFromUrl:[Character] = ["."]

/*: "http:// :*/
fileprivate let app_bottomFillKey:String = "hname"
fileprivate let showHiddenMakePath:[Character] = ["t","p",":","/","/"]

/*: "-1" :*/
fileprivate let mainSucceedMessage:[Character] = ["-","1"]

/*: "3,90" :*/
fileprivate let constFromData:String = "3,90"

/*: "1990-01-01" :*/
fileprivate let user_imageText:String = "view90-"

/*: "MM-dd-yyyy" :*/
fileprivate let mainGiftKey:[Character] = ["M","M","-","d","d","-","y","y"]
fileprivate let app_hungToFormat:[Character] = ["y","y"]

/*: "id" :*/
fileprivate let constPathFormat:String = "itill"
...
class FrameRequestModel: NSObject {
    ...
    var requestType: ImageRequestType?
    ...
    override init() {
        ...
        self.requestServer = OfDetailAddrTool.share.serverUrlStr
        //: self.showErrorStatusBar = true
        self.showErrorStatusBar = true
    }

    //: func paramsContainBinaryObject() -> Bool {
    func currentNameObject() -> Bool {
...
struct CellHandyJSON: HandyJSON {
    //: var errno: Int!
...
public struct WhiteErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
...
struct MTheoryMeasurable: HandyJSON {
    ...
    var CSConfig = PointConfigModel() // 私聊官方客服提示消息模型
//    var jumpRandomVideo: Bool = false       // true表示可以跳转到随机视频匹配
    ...
    var reportDomain: String {
        ...
        if statDomain.isEmptyString {
            ...
            return (String(dataNameStatusId.suffix(6)) + String(show_filePicName.prefix(6))) + "\(main_bottomStr)" + (String(dataAttentionMsg))
            //: } else {
        } else {
            //: if !statDomain.hasPrefix("http://") && !statDomain.hasPrefix("https://") {
            if !statDomain.hasPrefix((String(notiFormatText.suffix(7)))) && !statDomain.hasPrefix((String(kTitleUrl.suffix(6)) + String(notiPriceData))) {
                //: return "http://".appending(statDomain)
                return (String(notiFormatText.suffix(7))).appending(statDomain)
                //: }else {
            } else {
                //: return statDomain
                return statDomain
    ...
    var urlH5Domain: String {
        ...
        if urlDomain.isEmpty {
            ...
            return (String(data_infoTitle.prefix(8)) + String(constModelRequestFromUrl)) + "\(main_bottomStr)" + (String(dataAttentionMsg))
            //: } else {
        } else {
            //: return "http://\(urlDomain)"
            return (app_bottomFillKey.replacingOccurrences(of: "name", with: "t") + String(showHiddenMakePath)) + "\(urlDomain)"
        }
    }
...
public class ValueMeasurable: NSObject, HandyJSON {
    ...
    var chatPriceSettings: [ExecutiveSessionPriceModel] = [] // 收费设置
    //: var videoPriceSettings: Array<ChatPriceModel> = []
    var videoPriceSettings: [ExecutiveSessionPriceModel] = [] // 视频收费设置
    //: var voicePriceSettings: Array<ChatPriceModel> = []
    var voicePriceSettings: [ExecutiveSessionPriceModel] = [] // 语音收费设置
    ...
    var vipSetting = [SurroundModelType]() // vip皮肤
    ...
    var tabPageBanner = [SocietalTitleDivinatoryRecitalModelType]() // V1.6.4 一级页面广告
    ...
    var intimateLimit = AppSendMeasurable() // 1.7.6 各种操作的亲密度限制
...
@objcMembers public class PointTransformable: NSObject, HandyJSON {
    //: required public override init() {
    override public required init() {}
    ...
    var interest: [CreateHandyJSON]?
    //: var aboutMe: Array<UserSeleteTagModel>?
    var aboutMe: [CreateHandyJSON]?
    //: var partner: String?
    var partner: String?
    //: var full: Int?
    var full: Int?
    ...
    var gallery: [LookAlikeGalleryModel]?
    //: var status: Int = -1
    ...
    var callPopupSetting = (constFromData.capitalized) // 表示来电弹窗配置，如”3,90”表示3s后弹第一次，后续90s，返回”0,0”表示不弹窗
    //: var callTabSwitch = 0
    ...
    public lazy var age: String = {
        ...
        let birthdayDate = NSDate.noontide(self.birthday ?? (user_imageText.replacingOccurrences(of: "view", with: "19") + "01-01"), dateFormat: (String(mainGiftKey) + String(app_hungToFormat)))
        //: guard let currentYear = NSDate.getCurrentDateComponents().year else { return "0" }
        guard let currentYear = NSDate.tableOfContents().year else { return "0" }
...
struct LookAlikeGalleryModel: HandyJSON {
    //: var id: String?
    var id: String?
    //: var url: String?
    var url: String?
...
public struct CreateHandyJSON: HandyJSON, Equatable {
    //: var tag_id: String?
    var tag_id: String?
    //: var tag_name = ""
    var tag_name = ""
    ...
    func replacementTo(compareTo: CreateHandyJSON) -> Bool {
        //: return
        return
...
struct EndUserMeasurable: HandyJSON {
    ...
    var aboutMe: [BackgroundTransformable]!
    //: var interest: Array<UserTagTypeModel>!
    var interest: [BackgroundTransformable]!
}

//: struct UserTagTypeModel: HandyJSON {
struct BackgroundTransformable: HandyJSON {
    //: var name = ""
    var name = ""
    //: var name_ar = ""
    var name_ar = ""
    ...
    var list: [CreateHandyJSON]!
}

/// 官方客服提示消息模型
//: struct CSConfigModel: HandyJSON {
struct PointConfigModel: HandyJSON {
...
struct SurroundModelType: HandyJSON {
...
struct AppSendMeasurable: HandyJSON {
    //: var sendImg = 10
    var sendImg = 10 // 发送图片
    //: var sendVideo = 10
