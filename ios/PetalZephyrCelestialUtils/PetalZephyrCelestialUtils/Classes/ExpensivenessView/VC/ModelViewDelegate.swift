...
fileprivate let data_constraintPath:[UInt8] = [0x14,0x13,0x14,0x9,0x55,0x1e,0x12,0x19,0x18,0xf,0x47,0x54,0x5d,0x15,0x1c,0xe,0x5d,0x13,0x12,0x9,0x5d,0x1f,0x18,0x18,0x13,0x5d,0x14,0x10,0xd,0x11,0x18,0x10,0x18,0x13,0x9,0x18,0x19]

private func startTo(re num: UInt8) -> UInt8 {
    return num ^ 125
}

/*: "icon_photo_giftbg" :*/
fileprivate let main_effectUrl:[Character] = ["i","c","o","n","_","p","h","o","t","o","_","g","i","f"]
fileprivate let notiEqualKey:String = "photog"

/*: "Send" :*/
fileprivate let constNeedUrl:String = "player lease block translate makeSend"

/*: "nav_back_black_nor" :*/
fileprivate let kTitleUrl:[Character] = ["n","a","v","_","b","a","c","k","_","b","l","a","c","k","_"]
fileprivate let noti_labFormat:[Character] = ["n","o","r"]

/*: "toUid" :*/
fileprivate let showLayerName:String = "shared video front cell companytoUid"

/*: "giftId" :*/
fileprivate let constWrapPath:String = "string as return indexgiftId"

/*: "giftNum" :*/
fileprivate let dataFrameViewValue:String = "giftNumsize let"

/*: "unlockMsgId" :*/
fileprivate let show_afterData:String = "unlmake"
fileprivate let app_titleId:[Character] = ["c","k","M","s","g","I","d"]

/*: "Load failed, return to retry" :*/
fileprivate let constRegularSumId:String = "count var guardLoad "
fileprivate let main_countMsg:String = ", retuself false bar"
fileprivate let kShadowText:[Character] = ["r","y"]

/*: "Need a %@ (%@ Gold coins) to unlock" :*/
fileprivate let notiWithoutText:[UInt8] = [0x2a,0x1,0x1,0x0,0x44,0x5,0x44,0x41,0x24,0x44,0x4c,0x41,0x24,0x44,0x23,0xb,0x8,0x0,0x44,0x7,0xb,0xd,0xa,0x17,0x4d,0x44,0x10,0xb,0x44,0x11,0xa,0x8,0xb,0x7,0xf]

/*: "icon_photo_lock" :*/
fileprivate let appCutFormat:String = "icon_place to more"
fileprivate let kAddKey:String = "locplain"

/*: "Click on the screen to play this video(Burn After Reading)" :*/
fileprivate let k_bottomId:[UInt8] = [0xe0,0xcf,0xca,0xc0,0xc8,0x83,0xcc,0xcd,0x83,0xd7,0xcb,0xc6,0x83,0xd0,0xc0,0xd1,0xc6,0xc6,0xcd,0x83,0xd7,0xcc,0x83,0xd3,0xcf,0xc2,0xda,0x83,0xd7,0xcb,0xca,0xd0,0x83,0xd5,0xca,0xc7,0xc6,0xcc,0x8b,0xe1,0xd6,0xd1,0xcd,0x83,0xe2,0xc5,0xd7,0xc6,0xd1,0x83,0xf1,0xc6,0xc2,0xc7,0xca,0xcd,0xc4,0x8a]

private func molarConcentration(with num: UInt8) -> UInt8 {
    return num ^ 163
}

/*: "icon_photo_burn" :*/
fileprivate let showArrayPath:[Character] = ["i","c","o","n","_","p","h","o","t","o","_","b","u"]
fileprivate let show_giftUrl:String = "ruser"

/*: "Already burned" :*/
fileprivate let data_viewName:String = "Alreadyaverage make self false"
fileprivate let data_smallUrl:String = " burnedbuilder else message"
...
typealias IntimateVideoBlock = (_ model: AlongModel) -> Void
//: typealias VideoStatusBlock = (_ status: VideoPlayerStatus) -> Void
typealias VideoStatusBlock = (_ status: CenterPlayerStatus) -> Void

//: class TalkingIntimateVideoPreviewVC: TalkingBaseViewController {
class ModelViewDelegate: TailRecognizerDelegate {
    ...
    //: private var videoTotalData = AuditoryImageCellData(direction: .MsgDirectionIncoming)
    private var videoTotalData = AuditoryImageCellData(direction: .MsgDirectionIncoming) // 视频消息数据
    //: private var videoModel = AbTalkingChatMsgVideoModel()
    private var videoModel = AlongModel() // 当前私密视频模型
    ...
    ///   - videoCellData: 视频模型
    ///   - toUid: 对方Id
    //: init(videoCellData: AuditoryImageCellData, toUid: String) {
    init(videoCellData: AuditoryImageCellData, toUid: String) {
        ...
        //: targetId = toUid
        targetId = toUid
        //: isMySelf = (videoTotalData.msgModel.user.uid == Int(NowInfoThen.share.loginUserMode.userID))
        isMySelf = (videoTotalData.msgModel.user.uid == Int(NowInfoThen.share.loginUserMode.userID))
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_constraintPath.map{startTo(re: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var videoProgressView: EquilateralView = {
    ...
    private lazy var giftBgView: UIImageView = {
        ...
        img.image = UIImage.atomicCount85(name: (String(main_effectUrl) + notiEqualKey.replacingOccurrences(of: "photo", with: "tb")))
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftIcon: UIImageView = {
        ...
        img.viewFinish(urlStr: videoModel.giftIcon)
        //: return img
        return img
        //: }()
    }()
    ...
    private lazy var giftSendBtn: UIButton = {
        ...
        btn.setTitle((String(constNeedUrl.suffix(4))).localized, for: .normal)
    ...
    lazy var backBtn: UIButton = {
        ...
        let img = UIImage.atomicCount85(name: (String(kTitleUrl) + String(noti_labFormat))).withRenderingMode(.alwaysTemplate)
...
extension ModelViewDelegate {
    ...
    func voiceNum(giftId: String, giftNum: String) {
        ...
        params[(String(showLayerName.suffix(5)))] = self.targetId
        //: params["giftId"] = giftId
        params[(String(constWrapPath.suffix(6)))] = giftId
        //: params["giftNum"] = giftNum
        params[(String(dataFrameViewValue.prefix(7)))] = giftNum
        //: params["unlockMsgId"] = self.videoTotalData.msgModel.msgInfo.msgId
        params[(show_afterData.replacingOccurrences(of: "make", with: "o") + String(app_titleId))] = self.videoTotalData.msgModel.msgInfo.msgId
...
extension ModelViewDelegate {
    ...
    @objc private func cap() {
        ...
        voiceNum(giftId: videoModel.giftId, giftNum: "1")
    }

    /// 阅后即焚15s倒计时开始
    //: @objc private func burnVideoTapGesture() {
    @objc private func blockGesture() {
        ...
        self.videoStatusBlock = { [weak self] status in
            ...
            guard status == .Playing else {
                ...
                if status == .Failed {
                    ...
                    self.addMsg(showMsg: (String(constRegularSumId.suffix(5)) + "failed" + String(main_countMsg.prefix(6)) + "rn to ret" + String(kShadowText)).localized)
                }
                //: return
                return
            }
            ...
            self.countdownTimer = Timer.scheduledTimer(withTimeInterval: 0.1, block: { [weak self] _ in
                ...
                self.progressView.everyAppear(totalTime: 600, currTime: time)
                //: time -= 1
                time -= 1
                //: }, repeats: true)
            }, repeats: true)
...
extension ModelViewDelegate {
    ...
    private func showIn(urlStr: String, isMosaic: Bool) {
    ...
    private func mentalPictureVideo() {
        ...
        self.player.showWorldView(url: videoPath, view: self.videoView)
    }
}

// MARK: - HearingThen, MagnitudeerplayViewDelegate

//: extension TalkingIntimateVideoPreviewVC: TalkingVideoPlayerDelegate, InteractionViewDelegate {
extension ModelViewDelegate: HearingThen, MagnitudeerplayViewDelegate {
    // MARK: - HearingThen
    ...
    func comment(player _: VideoTitlePlayListener, status: CenterPlayerStatus) {
        ...
        self.videoProgressView.intervalerchangeAndUpperClass(status: status)
        //: if status == .Playing {
        if status == .Playing {
    ...
    func itemTime(player _: VideoTitlePlayListener, duration: Int, currentTime: Int) {
        ...
        self.videoProgressView.modifyToTime(duration: duration, currentTime: currentTime)
    }
    ...
    func row(player _: VideoTitlePlayListener, progress: CGFloat) {
        ...
        self.videoProgressView.streetwise(value: progress)
    }

    // MARK: - MagnitudeerplayViewDelegate
...
extension ModelViewDelegate {
    ...
    private func dataConstraint() {
        ...
        videoProgressView.snp.makeConstraints { make in
            ...
            make.top.equalToSuperview().offset(showMessageId + 30)
            //: make.bottom.equalToSuperview().offset(-(kDeviceSafeBottomHeight+64))
            make.bottom.equalToSuperview().offset(-(noti_limitPartyInfoUrl + 64))
        }
    ...
    private func removeOf() {
        // 添加接收到音视频通话通知
        //: NotificationCenter.default.addObserver(self,
        NotificationCenter.default.addObserver(self,
                                               //: selector: #selector(startTalking),
                                               selector: #selector(viaTalking),
    ...
    private func contentFor() {
        ...
        if videoModel.lockStatus == 1 { // 锁定状态，需要送礼解锁
            ...
            self.showIn(urlStr: videoModel.coverImg, isMosaic: true)
            ...
            self.videoProgressView.modifyToTime(duration: videoModel.duration, currentTime: 0)
            //: self.maskView.isHidden = false
            self.maskView.isHidden = false
            //: self.desLab.text = "Need a %@ (%@ Gold coins) to unlock".localizedArguments(videoModel.giftName, videoModel.giftPrice)
            self.desLab.text = String(bytes: notiWithoutText.map{$0^100}, encoding: .utf8)!.toAppear(videoModel.giftName, videoModel.giftPrice)
            ...
            if videoModel.burnStatus == 0 { // 未销毁
                ...
                self.intimateStatusImgV.image = UIImage.atomicCount85(name: (String(appCutFormat.prefix(5)) + "photo_" + kAddKey.replacingOccurrences(of: "plain", with: "k")))
                //: self.desLab.text = "Click on the screen to play this video(Burn After Reading)".localized
                self.desLab.text = String(bytes: k_bottomId.map{molarConcentration(with: $0)}, encoding: .utf8)!.localized
                // 添加手势
                //: let tap = UITapGestureRecognizer(target: self, action: #selector(burnVideoTapGesture))
                let tap = UITapGestureRecognizer(target: self, action: #selector(blockGesture))
                ...
                self.intimateStatusImgV.image = UIImage.atomicCount85(name: (String(showArrayPath) + show_giftUrl.replacingOccurrences(of: "user", with: "n")))
                //: self.desLab.text = "Already burned".localized
                self.desLab.text = (String(data_viewName.prefix(7)) + String(data_smallUrl.prefix(7))).localized
            }

            //: } else {
        } else { // 已解锁的永久私密视频
            //: self.playVideo()
            self.mentalPictureVideo()
            //: self.videoView.isHidden = false
            self.videoView.isHidden = false
            //: self.videoProgressView.isHidden = false
            self.videoProgressView.isHidden = false
