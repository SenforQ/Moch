...
fileprivate let mainToContent:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "move" :*/
fileprivate let mainGiftPath:String = "movnow"
...
class NameRecognizerDelegate: UIView {
    ...
    var danumuMsgArray: [WorstedScreenModel] = []
    //: var danumuItemViewArray: [TalkingFloatScreenItemView] = []
    var danumuItemViewArray: [AliasItemView] = []

    //: let TapAction: PublishSubject<TalkingFloatingScreenModel?> = PublishSubject()
    let TapAction: PublishSubject<WorstedScreenModel?> = PublishSubject()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainToContent.reversed(), encoding: .utf8)!)
    }
    ...
    func compute(_ danumuMsgModel: WorstedScreenModel?) {
    ...
    @objc func emptyCut(_: UITapGestureRecognizer) {}
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(emptyCut(_:)))
        //: t.delegate = self
        t.delegate = self
        //: return t
        return t
...
extension NameRecognizerDelegate: UIGestureRecognizerDelegate {
    ...
    func dismissView(for danumuMsgModel: WorstedScreenModel?, isFirst: Bool) {
        //: let itemView = TalkingFloatScreenItemView(model: danumuMsgModel)
        let itemView = AliasItemView(model: danumuMsgModel)
        ...
        if let anim = POPBasicAnimation(propertyNamed: kPOPViewFrame) {
            ...
            itemView.pop_add(anim, forKey: (mainGiftPath.replacingOccurrences(of: "now", with: "e")))
    ...
    func custom(_ isFirst: Bool) {
        ...
        if danumuMsgArray.count > 0 {
            ...
            if let model {
                ...
                dismissView(for: model, isFirst: isFirst)
