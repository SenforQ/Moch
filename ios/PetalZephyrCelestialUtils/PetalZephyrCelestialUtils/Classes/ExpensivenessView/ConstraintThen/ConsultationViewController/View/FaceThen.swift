...
/*: "99+" :*/
fileprivate let noti_lightKey:[Character] = ["9","9","+"]

/*: "init(coder:) has not been implemented" :*/
fileprivate let data_contentLineId:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#2DF2FF" :*/
fileprivate let dataModelUrl:[Character] = ["#","2","D","F","2","F","F"]

/*: "Live" :*/
fileprivate let mainFlexibleAppValue:String = "hidden collect start homeLive"

/*: "img_home_shadow_icon" :*/
fileprivate let constEdgeStr:[Character] = ["i","m","g","_","h","o","m"]
fileprivate let showSharedKey:[Character] = ["e","_","s","h","a","d","o","w","_","i","c","o","n"]

/*: "get json error" :*/
fileprivate let appAddObjectKey:[Character] = ["g","e","t"," ","j","s","o","n"," ","e","r","r"]
fileprivate let showDataFormat:[Character] = ["o","r"]

/*: "#FF2348" :*/
fileprivate let mainTingTargetMessage:[Character] = ["#","F","F","2","3","4","8"]

/*: "#35C759" :*/
fileprivate let constStyleTitle:[Character] = ["#","3","5","C","7","5","9"]

/*: "mfBean" :*/
fileprivate let kDatePath:String = "mfBeanbottom view share"

/*: "%.1f" :*/
fileprivate let app_picModelValue:String = "%.1farray index show video"

/*: "points\n≈ $" :*/
fileprivate let app_shareId:[Character] = ["p","o","i","n","t","s","\n","≈"," ","$"]
...
public var main_identityData: Int {
    //: return NowInfoThen.share.appConfigMode.countdown1 + NowInfoThen.share.appConfigMode.countdown2
    return NowInfoThen.share.appConfigMode.countdown1 + NowInfoThen.share.appConfigMode.countdown2
}

//: class TalkingLiveMiniView: TalkingBaseMiniView {
class FaceThen: ModelTailMiniView {
    // 未检测出人脸时长（间隔1秒）
    ...
    var noFaceTime: Int = 0 {
        ...
        didSet {
            ...
            if noFaceTime >= NowInfoThen.share.appConfigMode.countdown1 { // 展示倒计时
                ...
                svgaNumLab.text = "\(main_identityData - noFaceTime)"
                //: } else {
            } else {
                //: refreshMiniView()
                detect()
    ...
    override init(frame: CGRect) {
        ...
        //: if Double(NowInfoThen.share.loginUserMode.mf_bean) > 0 {
        if Double(NowInfoThen.share.loginUserMode.mf_bean) > 0 {
            //: setPointText(mfBean: 0.0)
            untilPoint(mfBean: 0.0)
        }
    }
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_contentLineId.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var liveStatusImgV: BlockButton = {
        ...
        let image = UIImage.colorDetail(color: UIColor(hex: (String(dataModelUrl)))!, size: CGSize(width: 8, height: 8))
        //: img.setImage(image.closingUniform(), for: .normal)
        img.setImage(image.closingUniform(), for: .normal)
        //: img.setTitle("Live".localized, for: .normal)
        img.setTitle((String(mainFlexibleAppValue.suffix(4))).localized, for: .normal)
        //: img.isHidden = true
        img.isHidden = true
        //: self.addSubview(img)
        self.addSubview(img)
    ...
    private lazy var shadowImg: UIImageView = {
        ...
        img.image = UIImage.atomicCount85(name: (String(constEdgeStr) + String(showSharedKey)))
        //: img.layer.cornerRadius = 8
        img.layer.cornerRadius = 8
        //: img.layer.masksToBounds = true
        img.layer.masksToBounds = true
    ...
    private lazy var svgaPlayer: SVGAPlayer = {
        ...
        do {
            ...
            printLog(message: (String(appAddObjectKey) + String(showDataFormat)))
        }
        //: return player
        return player
        //: }()
    ...
    private lazy var svgaNumLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(mainTingTargetMessage)))
        //: self.addSubview(lab)
        self.addSubview(lab)
        //: return lab
        return lab
    ...
    private lazy var msgCountLab: UILabel = {
        ...
        lab.backgroundColor = UIColor(hex: (String(constStyleTitle)))
...
extension FaceThen {
    ...
    static func themeEqual() -> FaceThen {
        //: let view = TalkingLiveMiniView(frame: .zero)
        let view = FaceThen(frame: .zero)
        //: let position = TalkingSocketManager.shared.liveMiniPosition
        let position = BankManagerSocketDelegate.shared.liveMiniPosition
    ...
    func detect() {
        ...
        liveStatusImgV.isHidden = !ProduceViewPushListener.decide().isLive
...
extension FaceThen {
    ...
    private func gestureView() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(addOld(notification:)),
                                               //: name: LIVE_USER_POINT_CHANGE,
                                               name: constBarNetPath,
                                               //: object: nil)
                                               object: nil)
    }

    //: @objc private func pointchange(notification: NSNotification) {
    @objc private func addOld(notification: NSNotification) {
        //: let userinfo = notification.userInfo as! [String: AnyObject]
        let userinfo = notification.userInfo as! [String: AnyObject]
        //: guard let mfBean = userinfo["mfBean"] as? Double else { return }
        guard let mfBean = userinfo[(String(kDatePath.prefix(6)))] as? Double else { return }
        //: setPointText(mfBean: mfBean)
        untilPoint(mfBean: mfBean)
    }

    //: func setPointText(mfBean: Double) {
    func untilPoint(mfBean: Double) {
        //: let nowBean = NowInfoThen.share.loginUserMode.mf_bean + mfBean
        let nowBean = NowInfoThen.share.loginUserMode.mf_bean + mfBean
        //: NowInfoThen.share.loginUserMode.mf_bean = nowBean
        NowInfoThen.share.loginUserMode.mf_bean = nowBean
        ...
        mfBeanLB.text = "\(Int(nowBean))\((String(app_shareId)).localized) \(divisor)"
