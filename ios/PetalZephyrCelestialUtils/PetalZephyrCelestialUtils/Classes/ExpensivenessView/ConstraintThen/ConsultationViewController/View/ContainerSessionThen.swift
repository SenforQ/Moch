...
fileprivate let data_sourcePath:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "no data available" :*/
fileprivate let data_viewMessage:[Character] = ["n","o"," ","d","a","t","a"]
fileprivate let user_commitTitle:[Character] = [" ","a","v","a","i","l","a","b","l","e"]

/*: "list" :*/
fileprivate let dataEqualName:String = "liattentiont"
...
class ContainerSessionThen: UIView {
    ...
    override init(frame: CGRect) {
        ...
        confinementSetupWriteSubConstraint()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: data_sourcePath.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        collect.register(TargetliveMinimumCompartmentItemCell.self, forCellWithReuseIdentifier: TargetliveMinimumCompartmentItemCell.className())
        //: return collect
        return collect
        //: }()
    }()
    ...
    private lazy var emptyView: UILabel = {
        ...
        lab.text = (String(data_viewMessage) + String(user_commitTitle)).localized
        //: lab.textColor = .white
        lab.textColor = .white
    ...
    private lazy var dataArr: [BackgroundListModel] = //: return [BackgroundListModel]()
        .init()
    //: }()
}
...
extension ContainerSessionThen {
    /// 请求游戏列表数据
    ...
    private func constraintList() {
        ...
        ValueThen.someone(category: 2) { [weak self] _, result, _ in
            //: ProgressHUD.dismiss()
            ...
            json[(dataEqualName.replacingOccurrences(of: "attention", with: "s"))].arrayObject?.forEach { dict in
                //: if let model = TalkingGameListModel.deserialize(from: dict as? Dictionary) {
                if let model = BackgroundListModel.deserialize(from: dict as? Dictionary) {
...
extension ContainerSessionThen {
    ...
    private func timeStop() -> CGFloat {
        ...
        return lineNum * noti_keyIndexButtonUrl + 24.0 + noti_limitPartyInfoUrl
    }
}
...
extension ContainerSessionThen: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: TargetliveMinimumCompartmentItemCell.className(), for: indexPath) as! TargetliveMinimumCompartmentItemCell
        //: cell.refreshView(model: dataArr[indexPath.row])
        cell.nameReloadModelViewColor(model: dataArr[indexPath.row])
        //: return cell
        return cell
    }
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, sizeForItemAt _: IndexPath) -> CGSize {
        ...
        return CGSize(width: (constExplainViewMsg - 30) / 4, height: noti_keyIndexButtonUrl)
    }
}
...
extension ContainerSessionThen {
    ...
    func vanguardApp(from: FillEmptyFromType) {
        //: switch from {
        switch from {
        //: case .LiveRoom:
        case .LiveRoom:
            //: dataArr = TalkingLiveManager.shared().liveRoomModel.gameList
            dataArr = ProduceViewPushListener.decide().liveRoomModel.gameList
            //: showAnimation()
            nose()
    ...
    private func nose() {
        ...
        self.contentView.frame = CGRect(x: 0, y: user_deviceId, width: constExplainViewMsg, height: timeStop())
        //: UIView.animate(withDuration: 0.3) {
        UIView.animate(withDuration: 0.3) {
            //: self.contentView.frame.origin.y -= self.contentView.height
            self.contentView.frame.origin.y -= self.contentView.height
...
extension ContainerSessionThen {
    ...
    private func confinementSetupWriteSubConstraint() {
        ...
        contentView.frame = CGRect(x: 0, y: user_deviceId, width: constExplainViewMsg, height: timeStop())
