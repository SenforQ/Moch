...
fileprivate let const_recordKey:[UInt8] = [0xb1,0xb6,0xb1,0xac,0xf0,0xbb,0xb7,0xbc,0xbd,0xaa,0xe2,0xf1,0xf8,0xb0,0xb9,0xab,0xf8,0xb6,0xb7,0xac,0xf8,0xba,0xbd,0xbd,0xb6,0xf8,0xb1,0xb5,0xa8,0xb4,0xbd,0xb5,0xbd,0xb6,0xac,0xbd,0xbc]

private func viewBy(to num: UInt8) -> UInt8 {
    return num ^ 216
}

/*: "Online Users" :*/
fileprivate let mainFileValue:String = "type data filterOnline U"
fileprivate let userCommitStyleName:String = "SERS"
...
class ShitlistView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: const_recordKey.map{viewBy(to: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var dismissBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(infoEqual), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var tipsLb: UILabel = {
        ...
        lb.text = (String(mainFileValue.suffix(8)) + userCommitStyleName.lowercased()).localized
        //: return lb
        return lb
        //: }()
    }()
...
extension ShitlistView {
    //: private func reqData() {
    private func doingReceive() {
        //: TalkingAudienceManager().req_liveRoomOnlineList(chatGroupId: self.chatGroupId) { succeed, result, errorModel in
        MoviegoerAudienceManager().yellowJack(chatGroupId: self.chatGroupId) { succeed, result, _ in
            ...
            if succeed {
                ...
                if let datas = Array<ButtonModelType>.deserialize(from: array as? Array) {
...
extension ShitlistView: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.upshot(model: self.DataSource[indexPath.row], index: indexPath.row)
        //: return cell!
        return cell!
    }
...
extension ShitlistView {
    ...
    func beginDownStatus(chatGroupId: String) {
