...
fileprivate let data_labStr:[UInt8] = [0x31,0x36,0x31,0x2c,0x70,0x3b,0x37,0x3c,0x3d,0x2a,0x62,0x71,0x78,0x30,0x39,0x2b,0x78,0x36,0x37,0x2c,0x78,0x3a,0x3d,0x3d,0x36,0x78,0x31,0x35,0x28,0x34,0x3d,0x35,0x3d,0x36,0x2c,0x3d,0x3c]

/*: "icon_live_vipsubscribe_back" :*/
fileprivate let k_regularId:String = "from trueicon_l"
fileprivate let appHiddenViewId:String = "vivalues"
fileprivate let data_pathMessage:String = "self extension bind picbe_back"

/*: "icon_live_vipsubscribe_topback" :*/
fileprivate let kSlightTitle:String = "icon_lplayer menu data to size"
fileprivate let appNeverMsg:[Character] = ["u","b","s","c","r"]
fileprivate let user_videoKey:[Character] = ["i","b","e","_","t","o","p","b","a","c","k"]

/*: "Open VIP\nto give Exclusive gifts" :*/
fileprivate let dataImageValue:[UInt8] = [0x29,0x16,0x3,0x8,0x46,0x30,0x2f,0x36,0x6c,0x12,0x9,0x46,0x1,0xf,0x10,0x3,0x46,0x23,0x1e,0x5,0xa,0x13,0x15,0xf,0x10,0x3,0x46,0x1,0xf,0x0,0x12,0x15]

/*: "Giving an Exclusive gift will appeal to her" :*/
fileprivate let dataBlockBoardTitle:[UInt8] = [0x48,0x66,0x79,0x66,0x61,0x68,0x2f,0x6e,0x61,0x2f,0x4a,0x77,0x6c,0x63,0x7a,0x7c,0x66,0x79,0x6a,0x2f,0x68,0x66,0x69,0x7b,0x2f,0x78,0x66,0x63,0x63,0x2f,0x6e,0x7f,0x7f,0x6a,0x6e,0x63,0x2f,0x7b,0x60,0x2f,0x67,0x6a,0x7d]

/*: "Subscribe" :*/
fileprivate let app_sizeDirectFormat:String = "Subscview hidden"

/*: "Cancel" :*/
fileprivate let userManagerText:[Character] = ["C","a","n","c","e","l"]
...
class InsertPopView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_labStr.map{$0^88}, encoding: .utf8)!)
    }
    ...
    lazy var contentImg: UIImageView = {
        ...
        img.image = UIImage.atomicCount85(name: (String(k_regularId.suffix(6)) + "ive_" + appHiddenViewId.replacingOccurrences(of: "value", with: "p") + "ubscri" + String(data_pathMessage.suffix(7))))
    ...
    lazy var topImg: UIImageView = {
        ...
        iamg.image = UIImage.atomicCount85(name: (String(kSlightTitle.prefix(6)) + "ive_vips" + String(appNeverMsg) + String(user_videoKey)))
    ...
    lazy var titleLabel: UILabel = {
        ...
        lab.text = String(bytes: dataImageValue.map{$0^102}, encoding: .utf8)!.localized
        //: return lab
        return lab
        //: }()
    }()
    ...
    lazy var messageView: UILabel = {
        ...
        lb.text = String(bytes: dataBlockBoardTitle.map{$0^15}, encoding: .utf8)!.localized
    ...
    private lazy var finishBtn: UIButton = {
        ...
        btn.setTitle((String(app_sizeDirectFormat.prefix(5)) + "ribe").localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(viewTouchText), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setTitle((String(userManagerText)).localized, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Medium, fontSize: 16)
        btn.titleLabel?.font = UIFont.photographicEquipment(type: .Medium, fontSize: 16)
        ...
        btn.addTarget(self, action: #selector(graduate), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension InsertPopView {
    //: @objc func finishBtnClick() {
    @objc func viewTouchText() {
        //: dismiss()
        tillAddCell()
