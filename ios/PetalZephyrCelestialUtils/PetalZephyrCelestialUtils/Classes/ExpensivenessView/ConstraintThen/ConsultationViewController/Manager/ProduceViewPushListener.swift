...
fileprivate let app_toData:String = "LIVquick"
fileprivate let data_togetherMessage:String = "OPEN_Ncover normal add true value"
fileprivate let data_awakeId:String = "ICATIpath"

/*: "LIVE_STATUS_CHANGE_NOTIFICATION" :*/
fileprivate let appCountUrl:[UInt8] = [0x3c,0x39,0x26,0x35,0x2f,0x23,0x24,0x31,0x24,0x25,0x23,0x2f,0x33,0x38,0x31,0x3e,0x37,0x35,0x2f,0x3e,0x3f,0x24,0x39,0x36,0x39,0x33,0x31,0x24,0x39,0x3f,0x3e]

private func showEquipment(main num: UInt8) -> UInt8 {
    return num ^ 112
}

/*: "live/startCheck" :*/
fileprivate let user_leadingMsg:String = "livtalk"
fileprivate let app_buttonId:String = "Checkkey let array let any"

/*: "live/start" :*/
fileprivate let const_sizeKey:String = "LIVE"
fileprivate let showCellId:String = "remove content type as/start"

/*: "live/stop" :*/
fileprivate let dataValueText:[Character] = ["l","i","v","e","/","s","t","o"]
fileprivate let show_errorToValue:[Character] = ["p"]

/*: "type" :*/
fileprivate let noti_equipmentMsg:[UInt8] = [0x65,0x70,0x79,0x74]

/*: "mf/user/dataCard" :*/
fileprivate let noti_picTitle:String = "self manager permissionmf/u"
fileprivate let noti_endName:String = "bag playtaCard"

/*: "uid" :*/
fileprivate let user_taskValue:[UInt8] = [0x56,0x4a,0x47]

private func hiddenName(file num: UInt8) -> UInt8 {
    return num ^ 35
}

/*: "streamerUid" :*/
fileprivate let const_styleAppearKey:[UInt8] = [0x0,0x7,0x1,0x16,0x12,0x1e,0x16,0x1,0x26,0x1a,0x17]

private func sectionUp(selected num: UInt8) -> UInt8 {
    return num ^ 115
}

/*: "roomId" :*/
fileprivate let mainFrontName:[Character] = ["r","o","o","m","I","d"]

/*: "Try again later, if you have any questions please contact customer service" :*/
fileprivate let dataIndexUrl:[UInt8] = [0x85,0xa3,0xa8,0xf1,0xb0,0xb6,0xb0,0xb8,0xbf,0xf1,0xbd,0xb0,0xa5,0xb4,0xa3,0xfd,0xf1,0xb8,0xb7,0xf1,0xa8,0xbe,0xa4,0xf1,0xb9,0xb0,0xa7,0xb4,0xf1,0xb0,0xbf,0xa8,0xf1,0xa0,0xa4,0xb4,0xa2,0xa5,0xb8,0xbe,0xbf,0xa2,0xf1,0xa1,0xbd,0xb4,0xb0,0xa2,0xb4,0xf1,0xb2,0xbe,0xbf,0xa5,0xb0,0xb2,0xa5,0xf1,0xb2,0xa4,0xa2,0xa5,0xbe,0xbc,0xb4,0xa3,0xf1,0xa2,0xb4,0xa3,0xa7,0xb8,0xb2,0xb4]

private func targetButton(make num: UInt8) -> UInt8 {
    return num ^ 209
}

/*: "redirectUrl" :*/
fileprivate let k_identityFormat:[Character] = ["r"]
fileprivate let const_languageFormat:[Character] = ["e","d","i","r","e","c","t","U","r","l"]

/*: "livePushUrl" :*/
fileprivate let mainPlayerBackUrl:String = "self model on viewlivePu"

/*: "FaceActionEvent_Interval_1_second" :*/
fileprivate let constMediumUrl:[UInt8] = [0xc8,0xef,0xed,0xeb,0xcf,0xed,0xfa,0xe7,0xe1,0xe0,0xcb,0xf8,0xeb,0xe0,0xfa,0xd1,0xc7,0xe0,0xfa,0xeb,0xfc,0xf8,0xef,0xe2,0xd1,0xbf,0xd1,0xfd,0xeb,0xed,0xe1,0xe0,0xea]

private func viewApp(label num: UInt8) -> UInt8 {
    return num ^ 142
}

/*: "hasFace" :*/
fileprivate let userNameBottomId:[Character] = ["h","a","s","F","a","c","e"]

/*: "Opening failed" :*/
fileprivate let noti_toPath:String = "Openingcount in"
fileprivate let kModelStr:String = " failedwill share dismiss"

/*: "LIVE 状态码： :*/
fileprivate let kMakeTargetFormat:String = "LIVE 状\u{6001}码："

/*: ." :*/
fileprivate let app_extraText:String = "label"

/*: "Network busy!" :*/
fileprivate let k_faceKey:String = "Netas in action"
fileprivate let constMainGiftPath:String = "self cancel title style addwork "

/*: "LIVE 异常下播，evtID: :*/
fileprivate let constDetailMessage:String = "else kitLIVE "
fileprivate let const_choiceTitle:String = "，evtID:count dismiss information accept"

/*: "Live room exception" :*/
fileprivate let showTowardName:String = "observer show make letLive "
fileprivate let constStorageData:String = "xcebuttonti"
fileprivate let notiMaximumUrl:String = "obelow"
...
public var show_detailIndexScreenValue = false
// 唤起直播功能
//: public let LIVE_NEED_OPEN_NOTIFICATION = NSNotification.Name(rawValue: "LIVE_NEED_OPEN_NOTIFICATION")
public let data_liveId = NSNotification.Name(rawValue: (app_toData.replacingOccurrences(of: "quick", with: "E") + "_NEED_" + String(data_togetherMessage.prefix(6)) + "OTIF" + data_awakeId.replacingOccurrences(of: "path", with: "ON")))
// 直播状态变更通知
//: public let LIVE_STATUS_CHANGE_NOTIFICATION = NSNotification.Name(rawValue: "LIVE_STATUS_CHANGE_NOTIFICATION")
public let kCoverUrl = NSNotification.Name(rawValue: String(bytes: appCountUrl.map{showEquipment(main: $0)}, encoding: .utf8)!)

/// 直播渲染配置
//: class TalkingLiveConfig: NSObject {
class SocioEconomicClassLiveConfig: NSObject {
...
class ProduceViewPushListener: NSObject {
    // 直播间消息
    //: var liveRoomModel = TalkingLiveRoomModel()
    var liveRoomModel = CenterMeasurable()
    // 是否正在直播中（状态变更时发送通知）
    ...
    private(set) var isLive: Bool = false {
        ...
        didSet {
            ...
            if NowInfoThen.share.appStatus == PointRelatableTarget.normal.rawValue {
                ...
                NotificationCenter.default.post(name: kCoverUrl, object: self)
            }
        }
    }
    ...
    var config = SocioEconomicClassLiveConfig() // 直播渲染配置类
    ...
    private var localPreviewView: WithThen? // 本地预览view（使用商汤美颜）
    //: private static var _instance: TalkingLiveManager?
    private static var _instance: ProduceViewPushListener? // singleton

    //: private override init() {}
    override private init() {}
    //: class func shared() -> TalkingLiveManager {
    class func decide() -> ProduceViewPushListener {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = TalkingLiveManager()
            _instance = ProduceViewPushListener()
            //: return _instance!
            return _instance!
        }
        //: return _instance!
    ...
    class func purview() {
        ...
        if ProduceViewPushListener._instance != nil {
            //: TalkingLiveManager._instance = nil
            ProduceViewPushListener._instance = nil
        }
    }
    ...
    private lazy var liveMiniView: FaceThen = {
        //: let mini = TalkingLiveMiniView.buildLiveMiniView()
        let mini = FaceThen.themeEqual()
        ...
        mini.tapGestureBlock = { [weak self] in
            ...
            self.duringObject(self.liveRoomVC.renderView, smallWindow: false)
            ...
            if currentVC.isKind(of: LengthViewController.self) {
    ...
    private lazy var liveRoomVC: ConsultationViewController = {
    ...
    private lazy var noFaceAlert: CharacteristicRootOfASquareMatrixView = //: return CharacteristicRootOfASquareMatrixView(frame: .zero, type: .noFace)
...
extension ProduceViewPushListener {
    /// 检测是否被禁用“开播功能”
    //: class func req_liveStartCheck(completion: @escaping FinishBlock) {
    class func equalView(completion: @escaping FinishBlock) {
        //: let reqModel = TalkingRequestModel()
        let reqModel = FrameRequestModel()
        //: reqModel.requestPath = "live/startCheck"
        reqModel.requestPath = (user_leadingMsg.replacingOccurrences(of: "talk", with: "e") + "/start" + String(app_buttonId.prefix(5)))
    ...
    class func snapUser(completion: @escaping FinishBlock) {
        //: let reqModel = TalkingRequestModel()
        let reqModel = FrameRequestModel()
        //: reqModel.requestPath = "live/start"
        reqModel.requestPath = (const_sizeKey.lowercased() + String(showCellId.suffix(6)))
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: GJ.startRequest(model: reqModel) { succeed, result, errorModel in
        dataVoiceKey.titleCompletion(model: reqModel) { succeed, result, errorModel in
    ...
    class func nearSubCompletion(type: Int, completion: FinishBlock? = nil) {
        //: let reqModel = TalkingRequestModel()
        let reqModel = FrameRequestModel()
        //: reqModel.requestPath = "live/stop"
        reqModel.requestPath = (String(dataValueText) + String(show_errorToValue))
        //: reqModel.requestType = .GET
        reqModel.requestType = .GET
        //: reqModel.params = ["type": type]
        reqModel.params = [String(bytes: noti_equipmentMsg.reversed(), encoding: .utf8)!: type]
        //: GJ.startRequest(model: reqModel) { succeed, result, errorModel in
        dataVoiceKey.titleCompletion(model: reqModel) { succeed, result, errorModel in
            //: completion?(succeed, result, errorModel)
            completion?(succeed, result, errorModel)
    ...
    class func even(uid: String, streamerUid: String, roomId: String? = nil, completion: @escaping FinishBlock) {
        //: let reqModel = TalkingRequestModel.init()
        let reqModel = FrameRequestModel()
        //: reqModel.requestPath = "mf/user/dataCard"
        reqModel.requestPath = (String(noti_picTitle.suffix(4)) + "ser/da" + String(noti_endName.suffix(6)))
        //: var params = ["uid": uid, "streamerUid": streamerUid]
        var params = [String(bytes: user_taskValue.map{hiddenName(file: $0)}, encoding: .utf8)!: uid, String(bytes: const_styleAppearKey.map{sectionUp(selected: $0)}, encoding: .utf8)!: streamerUid]
        //: if let roomId = roomId {
        if let roomId = roomId {
            //: params["roomId"] = roomId
            params[(String(mainFrontName))] = roomId
        }
...
extension ProduceViewPushListener {
    ...
    func conversationPath(completionHandler: ((_ isSucceed: Bool) -> Void)? = nil) {
        ...
        guard notiAppPath == true else {
            ...
            self.addMsg(showMsg: String(bytes: dataIndexUrl.map{targetButton(make: $0)}, encoding: .utf8)!.localized)
        ...
        LengthPermissionTool.nearGift { isOpen in
            ...
            ProduceViewPushListener.equalView { succeed, result, errorModel in
                //: guard succeed else {
                guard succeed else {
                    //: self.config.isRequest = false
                    self.config.isRequest = false
                    ...
                    if errorModel?.errorCode == 302 { // 失败，跳转H5
                        ...
                        if let result = result as? [String: Any] {
                            //: TotalReactiveCompatible.share.func__pushToWebVC(urlStr: result["redirectUrl"] as? String)
                            TotalReactiveCompatible.share.constraintImage(urlStr: result[(String(k_identityFormat) + String(const_languageFormat))] as? String)
                        }
                ...
                self.buttonController()?.navigationController?.pushViewController(self.liveRoomVC, animated: true)
                //: self.live_createRoomAndStartPush(livePushUrl: json["livePushUrl"].stringValue)
                self.uncoloredSum(livePushUrl: json[(String(mainPlayerBackUrl.suffix(6)) + "shUrl")].stringValue)
            }
        }
    }
}
...
extension ProduceViewPushListener {
    ...
    static func nowadays(content: String, reason: String, time: String) {
        //: let alert = TalkingLiveWarningView()
        let alert = TitleThen()
        //: alert.refreshUI(content: content, reason: reason, time: time)
        alert.messageTime(content: content, reason: reason, time: time)
        //: alert.show()
        alert.deadlineAt()
    }
    ...
    func uncoloredSum(livePushUrl: String) {
        ...
        self.anyMakeUrl(livePushUrl: livePushUrl)
    }
    ...
    func duringObject(_ randerView: UIView? = nil, smallWindow: Bool) {
        //: if randerView == nil {
        if randerView == nil {
        ...
        swaddlingClothes(smallWindow: smallWindow)
    }

    /// 前后摄像头切换
    /// - Parameter front: 是否前置
    //: func switchTRTCCamer(front: Bool) {
    func upCamer(front: Bool) {
        ...
        self.localPreviewView?.side(isFront: front)
    }
}
...
extension ProduceViewPushListener {
    ...
    private func isNameNotifications() {
        // 监听是否捕获到人脸
        ...
        if notiAppPath == true {
            ...
            NotificationCenter.default.addObserver(self,
                                                   ...
                                                   selector: #selector(notificationFeeObject(notification:)),
                                                   //: name: NSNotification.Name(rawValue: "FaceActionEvent_Interval_1_second"),
                                                   name: NSNotification.Name(rawValue: String(bytes: constMediumUrl.map{viewApp(label: $0)}, encoding: .utf8)!),
                                                   //: object: nil)
                                                   object: nil)
        }
    ...
    @objc private func blockEqual() {
        //: guard self.isLive == true else { return }
        guard self.isLive == true else { return }
        //: TalkingLiveManager.req_liveStop(type: 1)
        ProduceViewPushListener.nearSubCompletion(type: 1)
    }
}
...
extension ProduceViewPushListener {
    ...
    @objc private func notificationFeeObject(notification: NSNotification) {
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: guard self.isLive == true else { return }
            guard self.isLive == true else { return }
            ...
            let hasFace = (userInfo[(String(userNameBottomId))] as! NSNumber).description.boolValue
            //: if hasFace == true {
            if hasFace == true {
            ...
            if self.liveMiniView.noFaceTime == main_identityData {
    ...
    private func overdoFrom() {
        //: self.perform(#selector(interval_noFace), with: nil, afterDelay: 30)
        self.perform(#selector(counterval), with: nil, afterDelay: 30)
    }
    ...
    @objc private func counterval() {
        //: guard self.isLive == true else { return }
        guard self.isLive == true else { return }
        //: TalkingLiveManager.req_liveStop(type: 2)
        ProduceViewPushListener.nearSubCompletion(type: 2)
        //: self.live_releaseAllResource()
        self.blockFemale()
        //: self.noFaceAlert.show()
        self.noFaceAlert.drogue()
    }
}
...
extension ProduceViewPushListener {
    ...
    private func swaddlingClothes(smallWindow: Bool) {
        //: guard SenseTime_Use == true else {
        guard notiAppPath == true else {
        ...
        if self.localPreviewView == nil {
            ...
            self.localPreviewView = WithThen()
        }
        //: self.localPreviewView?.startPreView(isSmallWindow: smallWindow, livePusher: livePusher)
        self.localPreviewView?.topIndex(isSmallWindow: smallWindow, livePusher: livePusher)
    ...
    private func anyMakeUrl(livePushUrl: String) {
        //: if SenseTime_Use {
        if notiAppPath {
            //: } else {
        } else {
        ...
        recordTalk(Int(result))
    }

    /// 销毁tx直播
    //: private func destoryTXLiveView() {
    private func manageress() {
        //: if SenseTime_Use {
        if notiAppPath {
            //: self.localPreviewView?.stopPreView()
            self.localPreviewView?.pullUpShortFuturism()
            //: } else {
        } else {
    ...
    func recordTalk(_ result: Int) {
        //: guard result >= 0 else {
        guard result >= 0 else {
            //: self.func__showStatusBarErrorMsg(showMsg: "Opening failed".localized)
            self.addMsg(showMsg: (String(noti_toPath.prefix(7)) + String(kModelStr.prefix(7))).localized)
            //: self.live_releaseAllResource()
            self.blockFemale()
            //: return
            return
        ...
        ProduceViewPushListener.snapUser { succeed, result, _ in
            //: guard succeed else {
            guard succeed else {
                //: self.live_releaseAllResource()
                self.blockFemale()
            ...
            if let model = CenterMeasurable.deserialize(from: result as? Dictionary) {
            ...
            BankManagerEnableReactiveCompatible.anSufficient(groupId: self.liveRoomModel.chatGroupId, announcementMsg: self.liveRoomModel.notification)
            // 开播成功
            //: self.isLive = true
            self.isLive = true
            //: self.liveMiniView.refreshMiniView()
            self.liveMiniView.detect()
            //: self.liveRoomVC.refreshLiveRoomView()
            self.liveRoomVC.dinette()
            //: TalkingSocketManager.shared.startLivePing()
            BankManagerSocketDelegate.shared.belowKeyCrop()
            //: self.addNotifications()
            self.isNameNotifications()
            // 销毁上传busy状态定时器
            //: (TalkingApplication.shared as! TalkingApplication).destroyTimer()
            (SocialKingfisherCompatible.shared as! SocialKingfisherCompatible).appeal()
        }
    }
}
...
extension ProduceViewPushListener: TXLivePushListener {
    ...
    func onPushEvent(_ evtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        WoodLogTool.knowHow(msg: (kMakeTargetFormat) + "\(evtID).")
        //: if evtID == PUSH_WARNING_NET_BUSY.rawValue {
        if evtID == PUSH_WARNING_NET_BUSY.rawValue {
            //: self.func__showStatusBarErrorMsg(showMsg: "Network busy!".localized)
            self.addMsg(showMsg: (String(k_faceKey.prefix(3)) + String(constMainGiftPath.suffix(5)) + "busy!").localized)
            ...
            evtID == PUSH_ERR_SERVER_REFUSED.rawValue
        {
            ...
            WoodLogTool.knowHow(msg: (String(constDetailMessage.suffix(5)) + "异常下播" + String(const_choiceTitle.prefix(7))) + "\(evtID).")
            //: self.func__showStatusBarErrorMsg(showMsg: "Live room exception".localized)
            self.addMsg(showMsg: (String(showTowardName.suffix(5)) + "room e" + constStorageData.replacingOccurrences(of: "button", with: "p") + notiMaximumUrl.replacingOccurrences(of: "below", with: "n")).localized)
            //: TalkingLiveManager.req_liveStop(type: 1)
            ProduceViewPushListener.nearSubCompletion(type: 1)
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.5) {
                ...
                let alert = CharacteristicRootOfASquareMatrixView(frame: .zero, type: .turnOnLive)
                //: alert.rightBlcok = {
                alert.rightBlcok = {
