
//: Declare String Begin

/*: "init(coder:) has not been implemented" :*/
fileprivate let userSizeTemporaryName:[UInt8] = [0x9c,0x9b,0x9c,0x81,0xdd,0x96,0x9a,0x91,0x90,0x87,0xcf,0xdc,0xd5,0x9d,0x94,0x86,0xd5,0x9b,0x9a,0x81,0xd5,0x97,0x90,0x90,0x9b,0xd5,0x9c,0x98,0x85,0x99,0x90,0x98,0x90,0x9b,0x81,0x90,0x91]

/*: "Followers" :*/
fileprivate let mainLiveKey:[Character] = ["F","o","l","l","o","w","e"]
fileprivate let userModelPath:String = "rshow"

/*: "Following" :*/
fileprivate let kLocalValue:String = "Follself if"
fileprivate let app_toMsg:String = "otitleing"

/*: "#E6E6E6" :*/
fileprivate let k_transformMessage:[Character] = ["#","E","6","E","6","E","6"]

/*: "btn_message_more" :*/
fileprivate let dataVideoErrorStr:String = "lab"
fileprivate let notiPresentationStr:[Character] = ["t","n","_","m","e","s","s","a","g","e","_","m","o","r","e"]

/*: "UID:  :*/
fileprivate let mainModelValue:String = "in device share colorUID: "

/*: "level_ :*/
fileprivate let notiPhotoFormat:String = "level_view time cell"

/*: "Gift" :*/
fileprivate let app_resumeName:[UInt8] = [0x74,0x66,0x69,0x47]

/*: "Mike" :*/
fileprivate let main_errorValue:[UInt8] = [0xf6,0x12,0x14,0xe]

fileprivate func dataNow(gender num: UInt8) -> UInt8 {
    let value = Int(num) - 169
    if value < 0 {
        return UInt8(value + 256)
    } else {
        return UInt8(value)
    }
}

/*: "Follow" :*/
fileprivate let const_beautyStr:[UInt8] = [0x77,0x6f,0x6c,0x6c,0x6f,0x46]

/*: "gift_voice_card" :*/
fileprivate let dataScaleVerticalMsg:String = "gift_votop count"
fileprivate let showDomainAppText:String = "image fatal contentice_card"

/*: "btn_party_voice_nor" :*/
fileprivate let userAddViewTitle:String = "btn_pfor equal"
fileprivate let appPostId:[Character] = ["i","c","e","_","n","o","r"]

/*: "btn_party_voice_pre" :*/
fileprivate let kListTitle:String = "btn_pvar shared"
fileprivate let main_toTitle:[Character] = ["v","o","i"]
fileprivate let user_valueTitle:[Character] = ["c","e","_","p","r","e"]

/*: "attentionUid" :*/
fileprivate let kAfterValue:[Character] = ["a","t","t","e","n","t","i","o","n","U"]
fileprivate let dataCountMessage:String = "make"

/*: "source" :*/
fileprivate let notiFadeUpText:[Character] = ["s","o","u","r","c","e"]

/*: "Kick out" :*/
fileprivate let userViewTitle:String = "Kickdata block"

/*: "Unmute" :*/
fileprivate let k_managerMsg:[Character] = ["U"]
fileprivate let userLocationStr:String = "nmvaluete"

/*: "Mute" :*/
fileprivate let user_kitTitle:String = "view self if pathMute"

/*: "Kick off" :*/
fileprivate let noti_allPathViewText:[Character] = ["K","i"]
fileprivate let userNoContent:[Character] = ["c","k"," ","o","f","f"]

/*: "Block" :*/
fileprivate let const_toKey:String = "party list lab viewBlock"

/*: "Report" :*/
fileprivate let showEventMsg:String = "Reporcontent style"
fileprivate let app_transitionMessage:[Character] = ["t"]

/*: "#F6F6F6" :*/
fileprivate let userSocialData:[Character] = ["#","F","6","F","6","F","6"]

/*: "Confirm to kick out the room?" :*/
fileprivate let showWhiteKey:[Character] = ["C","o","n","f","i","r","m"," ","t","o"," ","k","i","c","k"," ","o","u"]
fileprivate let app_addContent:String = "let return at maket the"

/*: "Cancel" :*/
fileprivate let appTargetDefineTitle:String = "c"
fileprivate let noti_pathMsg:String = "ANCEL"

/*: "Sure" :*/
fileprivate let const_progressId:String = "manager model mixtureSure"

/*: "Are you sure you want to cancel the mute?" :*/
fileprivate let k_appDataIndexId:[UInt8] = [0x94,0xa7,0xb0,0xf5,0xac,0xba,0xa0,0xf5,0xa6,0xa0,0xa7,0xb0,0xf5,0xac,0xba,0xa0,0xf5,0xa2,0xb4,0xbb,0xa1,0xf5,0xa1,0xba,0xf5,0xb6,0xb4,0xbb,0xb6,0xb0,0xb9,0xf5,0xa1,0xbd,0xb0,0xf5,0xb8,0xa0,0xa1,0xb0,0xea]

private func limitSize(pic num: UInt8) -> UInt8 {
    return num ^ 213
}

/*: "Confirm" :*/
fileprivate let noti_contentStopName:[Character] = ["C"]
fileprivate let k_socialTouchImageData:String = "ONFIRM"

/*: "The mute has been lifted~" :*/
fileprivate let main_pairTitleValue:[Character] = ["T","h","e"," ","m","u","t","e"," ","h","a","s"," ","b","e"]
fileprivate let dataMMessageKitMsg:String = "en ltrue self"

/*: "Confirm to kick off the seat?" :*/
fileprivate let appWithValue:[Character] = ["C","o","n","f","i","r","m"," ","t","o"," ","k","i","c","k"," ","o","f","f"," ","t","h"]
fileprivate let noti_arrayStr:[Character] = ["e"," "]
fileprivate let k_itemKey:[Character] = ["s","e","a","t","?"]

/*: "OK" :*/
fileprivate let app_lineTitle:String = "ok"

/*: "Shielding Success" :*/
fileprivate let user_sinceTitle:String = "color"
fileprivate let kModelKey:String = "HIELDI"
fileprivate let dataShowText:String = "uindicatorindicatoress"

//: Declare String End

// __DEBUG__
// __CLOSE_PRINT__
//
//  ForefrontCardView.swift
//  AbroadTalking
//

//: import UIKit
import UIKit

//: @objc protocol TalkingVoiceRoomCardViewDelegate: NSObjectProtocol {
@objc protocol ContentViewDelegate: NSObjectProtocol {
    // @
    //: func func__atUserClick(uid: String, nickname: String)
    func objectNickname(uid: String, nickname: String)

    //: func func__giftUserClick(uid: String)
    func untilRelease(uid: String)

    //: func func__muteUserMickClick(uid: String, mikeStatus: Int, position: Int)
    func mickMikeTopUserMiddleClassMovePositionMute(uid: String, mikeStatus: Int, position: Int)

    //: func func__followUserSucess(uid: String)
    func sucess(uid: String)
}

//: class TalkingVoiceRoomCardView: UIView {
class ForefrontCardView: UIView {
    //: var popView: TalkingPopView?
    var popView: CurrentReactiveCompatible?
    //: var data =  [String: Any]()
    var data = [String: Any]()
    //: var type: LiveUserCardType?
    var type: ViewBlockInsetTarget?
    //: var uid = ""
    var uid = ""
    //: var liveUserId = String(TalkingVoiceRoomManager.shared().partyModel.streamerInfo.uid)
    var liveUserId = String(ConstraintEventHandler.bindTo().partyModel.streamerInfo.uid) // 主播ID

    //: var userModel = TalkingUserInfoModel.init()
    var userModel = GainDetailsMeasurable()

    //: open weak var delegate: TalkingVoiceRoomCardViewDelegate?
    open weak var delegate: ContentViewDelegate?

    //: init(frame: CGRect, uid: String) {
    init(frame: CGRect, uid: String) {
        //: super.init(frame: frame)
        super.init(frame: frame)

        //: if uid == NowInfoThen.share.loginUserMode.userID {
        if uid == NowInfoThen.share.loginUserMode.userID {
            //: self.type = .user_me
            self.type = .user_me
            //: } else {
        } else {
            //: self.type = .user_other
            self.type = .user_other
        }
        //: self.uid = uid
        self.uid = uid
        //: setupSubviews()
        addMakeMagnitudery()
        //: setupSubViewsConstraint()
        fire()
        //: reqUserCardInfo()
        nameToThen()
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: userSizeTemporaryName.map{$0^245}, encoding: .utf8)!)
    }

    //: deinit {
    deinit {}

    // 懒加载初始化

    //: lazy var contentView: UIView = {
    lazy var contentView: UIView = {
        //: let view = UIView.init()
        let view = UIView()
        //: view.backgroundColor = UIColor.white
        view.backgroundColor = UIColor.white
        //: view.layer.cornerRadius = 8
        view.layer.cornerRadius = 8
        //: return view
        return view
        //: }()
    }()

    //: lazy var uidLb: UILabel = {
    lazy var uidLb: UILabel = {
        //: let label = UILabel.init()
        let label = UILabel()
        //: label.font = .pingfangFont(type: .Regular, fontSize: 12)
        label.font = .photographicEquipment(type: .Regular, fontSize: 12)
        //: label.textColor = UIColor.appValueDetailColor()
        label.textColor = UIColor.someRegard()
        //: return label
        return label
        //: }()
    }()

    //: lazy var topIcon: UIButton = {
    lazy var topIcon: UIButton = {
        //: let btn = UIButton.init()
        let btn = UIButton()
        //: btn.addTarget(self, action: #selector(iconBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(upBy), for: .touchUpInside)
        //: btn.layer.cornerRadius = 40
        btn.layer.cornerRadius = 40
        //: btn.layer.masksToBounds = true
        btn.layer.masksToBounds = true
        //: btn.imageView?.contentMode = .scaleAspectFill
        btn.imageView?.contentMode = .scaleAspectFill
        //: btn.layer.borderWidth = 2
        btn.layer.borderWidth = 2
        //: btn.layer.borderColor = UIColor.white.cgColor
        btn.layer.borderColor = UIColor.white.cgColor
        //: return btn
        return btn

        //: }()
    }()

    //: lazy var nameLb: UILabel = {
    lazy var nameLb: UILabel = {
        //: let label = UILabel.init()
        let label = UILabel()
        //: label.font = .pingfangFont(type: .Semibold, fontSize: 15)
        label.font = .photographicEquipment(type: .Semibold, fontSize: 15)
        //: label.textColor = UIColor.appTitleColor()
        label.textColor = UIColor.towardOf()
        //: return label
        return label
        //: }()
    }()

    //: lazy var levelIcon: UIImageView = {
    lazy var levelIcon: UIImageView = {
        //: let imag = UIImageView.init()
        let imag = UIImageView()
        //: imag.contentMode = .scaleAspectFill
        imag.contentMode = .scaleAspectFill
        //: return imag
        return imag
        //: }()
    }()

    //: lazy var signLb: UILabel = {
    lazy var signLb: UILabel = {
        //: let label = UILabel.init()
        let label = UILabel()
        //: label.font = .pingfangFont(type: .Regular, fontSize: 13)
        label.font = .photographicEquipment(type: .Regular, fontSize: 13)
        //: label.textColor = UIColor.appValueDetailColor()
        label.textColor = UIColor.someRegard()
        //: label.textAlignment = .center
        label.textAlignment = .center
        //: label.numberOfLines = 2
        label.numberOfLines = 2
        //: return label
        return label
        //: }()
    }()

    //: private lazy var followersLab: UILabel = {
    private lazy var followersLab: UILabel = {
        //: let lab = UILabel()
        let lab = UILabel()
        //: lab.textColor = .appValueDetailColor()
        lab.textColor = .someRegard()
        //: lab.textAlignment = .center
        lab.textAlignment = .center
        //: lab.text = "Followers".localized
        lab.text = (String(mainLiveKey) + userModelPath.replacingOccurrences(of: "show", with: "s")).localized
        //: lab.font = UIFont.pingfangFont(type: .Semibold, fontSize: 12)
        lab.font = UIFont.photographicEquipment(type: .Semibold, fontSize: 12)
        //: return lab
        return lab
        //: }()
    }()

    //: private lazy var followersNum: UILabel = {
    private lazy var followersNum: UILabel = {
        //: let lab = UILabel()
        let lab = UILabel()
        //: lab.textColor = .appTitleColor()
        lab.textColor = .towardOf()
        //: lab.textAlignment = .center
        lab.textAlignment = .center
        //: lab.text = "0"
        lab.text = "0"
        //: lab.font = UIFont.pingfangFont(type: .Semibold, fontSize: 15)
        lab.font = UIFont.photographicEquipment(type: .Semibold, fontSize: 15)
        //: return lab
        return lab
        //: }()
    }()

    //: private lazy var followingLab: UILabel = {
    private lazy var followingLab: UILabel = {
        //: let lab = UILabel()
        let lab = UILabel()
        //: lab.textColor = .appValueDetailColor()
        lab.textColor = .someRegard()
        //: lab.textAlignment = .center
        lab.textAlignment = .center
        //: lab.text = "Following".localized
        lab.text = (String(kLocalValue.prefix(4)) + app_toMsg.replacingOccurrences(of: "title", with: "w")).localized
        //: lab.font = UIFont.pingfangFont(type: .Semibold, fontSize: 12)
        lab.font = UIFont.photographicEquipment(type: .Semibold, fontSize: 12)
        //: return lab
        return lab
        //: }()
    }()

    //: private lazy var followingNum: UILabel = {
    private lazy var followingNum: UILabel = {
        //: let lab = UILabel()
        let lab = UILabel()
        //: lab.textColor = .appTitleColor()
        lab.textColor = .towardOf()
        //: lab.textAlignment = .center
        lab.textAlignment = .center
        //: lab.text = "0"
        lab.text = "0"
        //: lab.font = UIFont.pingfangFont(type: .Semibold, fontSize: 15)
        lab.font = UIFont.photographicEquipment(type: .Semibold, fontSize: 15)
        //: return lab
        return lab
        //: }()
    }()

    //: private lazy var lineView: UIView = {
    private lazy var lineView: UIView = {
        //: let view = UIView()
        let view = UIView()
        //: view.backgroundColor = UIColor.init(hex: "#E6E6E6")
        view.backgroundColor = UIColor(hex: (String(k_transformMessage)))
        //: return view
        return view
        //: }()
    }()

    //: private lazy var stackView: UIStackView = {
    private lazy var stackView: UIStackView = {
        //: let v = UIStackView()
        let v = UIStackView()
        //: v.axis = .horizontal
        v.axis = .horizontal
        //: v.alignment = .fill
        v.alignment = .fill
        //: v.distribution = .fillEqually
        v.distribution = .fillEqually
        //: return v
        return v
        //: }()
    }()

    //: private lazy var moreBtn: UIButton = {
    private lazy var moreBtn: UIButton = {
        //: let btn = UIButton.init()
        let btn = UIButton()
        //: btn.imageView?.contentMode = .scaleAspectFill
        btn.imageView?.contentMode = .scaleAspectFill
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_message_more"), for: .normal)
        btn.setImage(UIImage.atomicCount85(name: (dataVideoErrorStr.replacingOccurrences(of: "lab", with: "b") + String(notiPresentationStr))), for: .normal)
        //: btn.addTarget(self, action: #selector(moreBtnClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(makeEvent), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()

    //: private lazy var muteView: TalkingLiveRoomMuteView = {
    private lazy var muteView: MusicMuteView = {
        //: let v = TalkingLiveRoomMuteView()
        let v = MusicMuteView()
        //: v.viewType  = .voiceRoom
        v.viewType = .voiceRoom
        //: return v
        return v
        //: }()
    }()
}

//: extension TalkingVoiceRoomCardView {
extension ForefrontCardView {
    //: func reqUserCardInfo() {
    func nameToThen() {
        //: TalkingLiveManager.req__userCardInfo(uid: self.uid, streamerUid: liveUserId, roomId: TalkingVoiceRoomManager.shared().partyModel.roomId) { succeed, result, errorModel in
        ProduceViewPushListener.even(uid: self.uid, streamerUid: liveUserId, roomId: ConstraintEventHandler.bindTo().partyModel.roomId) { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }

            //: if let userModel = JSONDeserializer<TalkingUserInfoModel>.deserializeFrom(dict: result as? Dictionary<String, Any>, designatedPath: nil) {
            if let userModel = JSONDeserializer<GainDetailsMeasurable>.deserializeFrom(dict: result as? [String: Any], designatedPath: nil) {
                //: self.userModel = userModel
                self.userModel = userModel
                //: self.setUserCardData()
                self.endUserByData()
            }
        }
    }

    //: func setUserCardData() {
    func endUserByData() {
        //: uidLb.text = "UID: \(self.userModel.uid)"
        uidLb.text = (String(mainModelValue.suffix(5))) + "\(self.userModel.uid)"
        //: topIcon.setUrlImage(urlStr: self.userModel.headPic)
        topIcon.equalButton(urlStr: self.userModel.headPic)
        //: nameLb.text = self.userModel.nickname
        nameLb.text = self.userModel.nickname
        //: levelIcon.image = UIImage.BundleImageNamed(name: "level_\(self.userModel.level)")
        levelIcon.image = UIImage.atomicCount85(name: (String(notiPhotoFormat.prefix(6))) + "\(self.userModel.level)")
        //: signLb.text = self.userModel.signature
        signLb.text = self.userModel.signature
        //: followersNum.text = self.userModel.attentionNum
        followersNum.text = self.userModel.attentionNum
        //: followingNum.text = self.userModel.fansNum
        followingNum.text = self.userModel.fansNum

        //: if self.type == .user_other {
        if self.type == .user_other {
            //: setBottomView(sex: self.userModel.sex)
            attributeTo(sex: self.userModel.sex)
        }
    }

    //: func setBottomView(sex: String) {
    func attributeTo(sex _: String) {
        //: var dataSource = Array<String>()
        var dataSource = [String]()
        //: if self.userModel.isOnMike {
        if self.userModel.isOnMike {
            //: dataSource.append("Gift")
            dataSource.append(String(bytes: app_resumeName.reversed(), encoding: .utf8)!)
            //: if liveUserId == NowInfoThen.share.loginUserMode.userID && self.userModel.mikeStatus != 0  {
            if liveUserId == NowInfoThen.share.loginUserMode.userID, self.userModel.mikeStatus != 0 { // 当前用户是主播非自己闭麦  闭麦/开麦 按钮
                //: dataSource.append("Mike")
                dataSource.append(String(bytes: main_errorValue.map{dataNow(gender: $0)}, encoding: .utf8)!)
            }
        }
        //: dataSource.append("@")
        dataSource.append("@")

        //: if self.userModel.isAttention == false {
        if self.userModel.isAttention == false {
            //: dataSource.append("Follow")
            dataSource.append(String(bytes: const_beautyStr.reversed(), encoding: .utf8)!)
        }

        //: for i in 0 ..< dataSource.count {
        for i in 0 ..< dataSource.count {
            //: let btnStr = dataSource[i]
            let btnStr = dataSource[i]
            //: let btn = UIButton()
            let btn = UIButton()
            //: btn.accessibilityLabel = btnStr
            btn.accessibilityLabel = btnStr
            //: btn.setTitleColor(UIColor.appThemeColor(), for: UIControl.State.normal)
            btn.setTitleColor(UIColor.anPathColor(), for: UIControl.State.normal)
            //: if btnStr == "Gift" {
            if btnStr == String(bytes: app_resumeName.reversed(), encoding: .utf8)! {
                //: btn.setImage(UIImage.BundleImageNamed(name: "gift_voice_card"), for: .normal)
                btn.setImage(UIImage.atomicCount85(name: (String(dataScaleVerticalMsg.prefix(7)) + String(showDomainAppText.suffix(8)))), for: .normal)
                //: }else if btnStr == "Mike"{
            } else if btnStr == String(bytes: main_errorValue.map{dataNow(gender: $0)}, encoding: .utf8)! {
                //: btn.setImage(UIImage.BundleImageNamed(name: "btn_party_voice_nor"), for: .normal)
                btn.setImage(UIImage.atomicCount85(name: (String(userAddViewTitle.prefix(5)) + "arty_vo" + String(appPostId))), for: .normal)
                //: btn.setImage(UIImage.BundleImageNamed(name: "btn_party_voice_pre"), for: .selected)
                btn.setImage(UIImage.atomicCount85(name: (String(kListTitle.prefix(5)) + "arty_" + String(main_toTitle) + String(user_valueTitle))), for: .selected)
                //: if self.userModel.mikeStatus != 2 {
                if self.userModel.mikeStatus != 2 {
                    //: btn.isSelected = true
                    btn.isSelected = true
                }
                //: }else{
            } else {
                //: btn.titleLabel?.font = UIFont.pingfangMediumFont(fontSize: 15)
                btn.titleLabel?.font = UIFont.outView(fontSize: 15)
                //: btn.setTitle(btnStr.localized, for: UIControl.State.normal)
                btn.setTitle(btnStr.localized, for: UIControl.State.normal)
            }
            //: btn.addTarget(self, action: #selector(itemTouch(sender:)), for: UIControl.Event.touchUpInside)
            btn.addTarget(self, action: #selector(mic(sender:)), for: UIControl.Event.touchUpInside)
            //: self.stackView.addArrangedSubview(btn)
            self.stackView.addArrangedSubview(btn)

            //: if i != 0 {
            if i != 0 {
                //: let lineView = UIView.init()
                let lineView = UIView()
                //: lineView.backgroundColor = UIColor.init(hex: "#E6E6E6")
                lineView.backgroundColor = UIColor(hex: (String(k_transformMessage)))
                //: btn.addSubview(lineView)
                btn.addSubview(lineView)
                //: lineView.snp.makeConstraints { make in
                lineView.snp.makeConstraints { make in
                    //: make.top.equalTo(10)
                    make.top.equalTo(10)
                    //: make.leading.equalToSuperview()
                    make.leading.equalToSuperview()
                    //: make.width.equalTo(1)
                    make.width.equalTo(1)
                    //: make.height.equalTo(24)
                    make.height.equalTo(24)
                }
            }
        }
    }

    //: @objc func itemTouch(sender: UIButton) {
    @objc func mic(sender: UIButton) {
        //: let accessLabel = sender.accessibilityLabel ?? ""
        let accessLabel = sender.accessibilityLabel ?? ""
        //: switch accessLabel {
        switch accessLabel {
        //: case "Gift":
        case String(bytes: app_resumeName.reversed(), encoding: .utf8)!:
            //: self.delegate?.func__giftUserClick(uid: self.userModel.uid)
            self.delegate?.untilRelease(uid: self.userModel.uid)
            //: self.dismiss()
            self.dataDismiss()
        //: case "Mike":
        case String(bytes: main_errorValue.map{dataNow(gender: $0)}, encoding: .utf8)!:
            //: self.delegate?.func__muteUserMickClick(uid: self.userModel.uid, mikeStatus: self.userModel.mikeStatus, position: self.userModel.mikePosition)
            self.delegate?.mickMikeTopUserMiddleClassMovePositionMute(uid: self.userModel.uid, mikeStatus: self.userModel.mikeStatus, position: self.userModel.mikePosition)
            //: self.dismiss()
            self.dataDismiss()
        //: case "@":
        case "@":
            //: self.delegate?.func__atUserClick(uid: self.userModel.uid, nickname: self.userModel.nickname)
            self.delegate?.objectNickname(uid: self.userModel.uid, nickname: self.userModel.nickname)
            //: self.dismiss()
            self.dataDismiss()
        //: case "Follow":
        case String(bytes: const_beautyStr.reversed(), encoding: .utf8)!:
            //: freshfollowUser(sender: sender)
            pastIndex(sender: sender)
        //: default: break
        default: break
        }
    }

    //: func freshfollowUser(sender: UIButton) {
    func pastIndex(sender: UIButton) {
        //: req_followUser(uid: self.userModel.uid) { succeed in
        observerCompletion(uid: self.userModel.uid) { succeed in
            //: if succeed {
            if succeed {
                //: self.followingNum.text = "\((Int(self.userModel.fansNum ) ?? 0) + 1)"
                self.followingNum.text = "\((Int(self.userModel.fansNum) ?? 0) + 1)"
                //: self.stackView.removeArrangedSubview(sender)
                self.stackView.removeArrangedSubview(sender)
                //: sender.isHidden = true
                sender.isHidden = true
                //: self.delegate?.func__followUserSucess(uid: self.userModel.uid)
                self.delegate?.sucess(uid: self.userModel.uid)
            }
        }
    }

    //: func req_followUser(uid: String, completion: ((_ succeed: Bool) -> Void)? = nil) {
    func observerCompletion(uid: String, completion: ((_ succeed: Bool) -> Void)? = nil) {
        //: var dict = [String: Any]()
        var dict = [String: Any]()
        //: dict["attentionUid"] = uid
        dict[(String(kAfterValue) + dataCountMessage.replacingOccurrences(of: "make", with: "id"))] = uid
        //: dict["source"] = "2"
        dict[(String(notiFadeUpText))] = "2"
        //: MonetaryValueThen.req_atationTool(isAttention: true, params: dict) { succeed, result, errorModel in
        MonetaryValueThen.qualityUp(isAttention: true, params: dict) { succeed, _, _ in
            //: completion?(succeed)
            completion?(succeed)
        }
    }

    //: @objc private func iconBtnClick() {
    @objc private func upBy() {
        //: TotalReactiveCompatible.share.func__pushToUserDetailVC(uid: self.userModel.uid)
        TotalReactiveCompatible.share.arrange(uid: self.userModel.uid)
        //: self.dismiss()
        self.dataDismiss()
    }
}

//: extension TalkingVoiceRoomCardView {
extension ForefrontCardView {
    //: @objc func moreBtnClick() {
    @objc func makeEvent() {
        //: var titleArray: [String] = []
        var titleArray: [String] = []
        /// 主播 & 用户在房间内
        //: if liveUserId == NowInfoThen.share.loginUserMode.userID && self.userModel.isInRoom {
        if liveUserId == NowInfoThen.share.loginUserMode.userID, self.userModel.isInRoom {
            //: titleArray.append("Kick out".localized)
            titleArray.append((String(userViewTitle.prefix(4)) + " out").localized)
            //: if self.userModel.isPartyMute {
            if self.userModel.isPartyMute {
                //: titleArray.append("Unmute".localized)
                titleArray.append((String(k_managerMsg) + userLocationStr.replacingOccurrences(of: "value", with: "u")).localized)
                //: }else {
            } else {
                //: titleArray.append("Mute".localized)
                titleArray.append((String(user_kitTitle.suffix(4))).localized)
            }
            //: if self.userModel.isOnMike {
            if self.userModel.isOnMike {
                //: titleArray.append("Kick off".localized)
                titleArray.append((String(noti_allPathViewText) + String(userNoContent)).localized)
            }
            //: if !self.userModel.inMyBlackList {
            if !self.userModel.inMyBlackList {
                //: titleArray.append("Block".localized)
                titleArray.append((String(const_toKey.suffix(5))).localized)
            }
        }
        //: titleArray.append("Report".localized)
        titleArray.append((String(showEventMsg.prefix(5)) + String(app_transitionMessage)).localized)

        //: Config.MenuCellConfig.menuCellHeight = 26
        Config.ValueTextConfig.menuCellHeight = 26
        //: let dropMenu = DropDownMenuView.pullDropDrownMenu(anchorView: moreBtn, titleArray: titleArray, imageArray: [])
        let dropMenu = IiDataSource.titleArray(anchorView: moreBtn, titleArray: titleArray, imageArray: [])
        //: dropMenu.menuBgColor = UIColor.init(hex: "#F6F6F6")!
        dropMenu.menuBgColor = UIColor(hex: (String(userSocialData)))!
        //: dropMenu.menuDelegate = self
        dropMenu.menuDelegate = self
        //: dropMenu.menuStyle = .MenuCustStyle
        dropMenu.menuStyle = .MenuCustStyle
    }

    //: func show() {
    func goldShow() {
        //: popView = TalkingPopView.init(frame: UIScreen.main.bounds)
        popView = CurrentReactiveCompatible(frame: UIScreen.main.bounds)
        //: self.frame = CGRect(x: 0, y: 0, width: ScreenWidth, height: ScreenHeight)
        self.frame = CGRect(x: 0, y: 0, width: constExplainViewMsg, height: user_deviceId)
        //: popView?.initWithView(view: self)
        popView?.draft(view: self)
        //: popView?.showInView(view: self.currentViewController()?.view ?? LineThen.getWindow())
        popView?.coverPrice(view: self.buttonController()?.view ?? LineThen.getName())
        //: popView?.isRemoveTapGes = true
        popView?.isRemoveTapGes = true
    }

    //: @objc func dismiss() {
    @objc func dataDismiss() {
        //: popView?.dismissView()
        popView?.largeView()
        //: popView = nil
        popView = nil
    }
}

//: extension TalkingVoiceRoomCardView: DropDownMenuViewDelegate {
extension ForefrontCardView: FrameViewDelegate {
    //: func didClickSelectedRow(index: Int, title: String) {
    func equalApp(index _: Int, title: String) {
        //: switch title {
        switch title {
        //: case "Kick out".localized:  // 踢出房间
        case (String(userViewTitle.prefix(4)) + " out").localized: // 踢出房间
            //: TalkingAlertShow.alert(title: nil, message: "Confirm to kick out the room?".localized, leftBtnTitle: "Cancel".localized, rightBtnTitle: "Sure".localized) {
            PileThen.downCrosshead(title: nil, message: (String(showWhiteKey) + String(app_addContent.suffix(5)) + " room?").localized, leftBtnTitle: (appTargetDefineTitle.uppercased() + noti_pathMsg.lowercased()).localized, rightBtnTitle: (String(const_progressId.suffix(4))).localized) {
                //: } rightBlock: {
            } rightBlock: {
                //: ProgressHUD.show()
                DeleteReactiveCompatible.fileAdd()
                //: TalkingVoiceRoomReqTool.req_VoiceRoomKickout(targetUid: self.userModel.uid, roomId: TalkingVoiceRoomManager.shared().partyModel.roomId) { succeed, result, errorModel in
                NameReqTool.exceptBurn(targetUid: self.userModel.uid, roomId: ConstraintEventHandler.bindTo().partyModel.roomId) { _, _, _ in
                    //: ProgressHUD.dismiss()
                    DeleteReactiveCompatible.deflector()
                    //: self.dismiss()
                    self.dataDismiss()
                }
            }
        //: case "Unmute".localized: // 解除禁言
        case (String(k_managerMsg) + userLocationStr.replacingOccurrences(of: "value", with: "u")).localized: // 解除禁言
            //: TalkingAlertShow.alert(title: nil, message: "Are you sure you want to cancel the mute?".localized, leftBtnTitle: "Cancel".localized, rightBtnTitle: "Confirm".localized) {
            PileThen.downCrosshead(title: nil, message: String(bytes: k_appDataIndexId.map{limitSize(pic: $0)}, encoding: .utf8)!.localized, leftBtnTitle: (appTargetDefineTitle.uppercased() + noti_pathMsg.lowercased()).localized, rightBtnTitle: (String(noti_contentStopName) + k_socialTouchImageData.lowercased()).localized) {
                //: } rightBlock: {
            } rightBlock: {
                //: ProgressHUD.show()
                DeleteReactiveCompatible.fileAdd()
                //: TalkingVoiceRoomReqTool.req_VoiceRoomUnMute(targetUid: self.userModel.uid, roomID:TalkingVoiceRoomManager.shared().partyModel.roomId, completion: { succeed, result, errorModel in
                NameReqTool.dataEdit(targetUid: self.userModel.uid, roomID: ConstraintEventHandler.bindTo().partyModel.roomId, completion: { succeed, _, _ in
                    //: ProgressHUD.dismiss()
                    DeleteReactiveCompatible.deflector()
                    //: if succeed {
                    if succeed {
                        //: self.func__showStatusBarSuccessMsg(showMsg: "The mute has been lifted~".localized)
                        self.beforeDemonstrate(showMsg: (String(main_pairTitleValue) + String(dataMMessageKitMsg.prefix(4)) + "ifted~").localized)
                    }
                    //: self.dismiss()
                    self.dataDismiss()
                    //: })
                })
            }
        //: case "Mute".localized: // 设置禁言
        case (String(user_kitTitle.suffix(4))).localized: // 设置禁言
            //: muteView.showView()
            muteView.stickTo()
            //: muteView.targetUid = self.userModel.uid
            muteView.targetUid = self.userModel.uid
            //: self.dismiss()
            self.dataDismiss()

        //: case "Kick off".localized: /// 踢下麦
        case (String(noti_allPathViewText) + String(userNoContent)).localized: /// 踢下麦
            //: TalkingAlertShow.alert(title: nil, message: "Confirm to kick off the seat?".localized, leftBtnTitle: "Cancel".localized, rightBtnTitle: "Sure".localized) {
            PileThen.downCrosshead(title: nil, message: (String(appWithValue) + String(noti_arrayStr) + String(k_itemKey)).localized, leftBtnTitle: (appTargetDefineTitle.uppercased() + noti_pathMsg.lowercased()).localized, rightBtnTitle: (String(const_progressId.suffix(4))).localized) {
                //: } rightBlock: {
            } rightBlock: {
                //: TalkingVoiceRoomManager.shared().voiceRoom_reqAdminChangeMike(uid: self.userModel.uid.intValue, type: 3, position: self.userModel.mikePosition)
                ConstraintEventHandler.bindTo().statusLocation(uid: self.userModel.uid.intValue, type: 3, position: self.userModel.mikePosition)
                //: self.dismiss()
                self.dataDismiss()
            }

        //: case "Report".localized:
        case (String(showEventMsg.prefix(5)) + String(app_transitionMessage)).localized:
            //: let reportView = TalkingReportAlertView(frame: UIScreen.main.bounds, type: .reportUserType, rId: self.userModel.uid)
            let reportView = WithFrameAlertView(frame: UIScreen.main.bounds, type: .reportUserType, rId: self.userModel.uid)
            //: reportView.showReportViewIn(view: nil)
            reportView.equalAttribute(view: nil)
        //: case "Block".localized:
        case (String(const_toKey.suffix(5))).localized:
            //: TalkingAlertShow.alert(title: nil, message: kMessage_blocking, leftBtnTitle: "Cancel".localized, rightBtnTitle: "OK".localized) {
            PileThen.downCrosshead(title: nil, message: appConversationValue, leftBtnTitle: (appTargetDefineTitle.uppercased() + noti_pathMsg.lowercased()).localized, rightBtnTitle: "OK".localized) {
                //: TalkingAlertShow.hideAlert()
                PileThen.obstruct()
                //: } rightBlock: {
            } rightBlock: {
                //: TalkingAlertShow.hideAlert()
                PileThen.obstruct()
                //: TalkingUserRequestManager.func__pullBlackWithUserId(uid: self.userModel.uid, isBlack: true) { succeed, result, errorModel in
                ThreadCookieThen.sum(uid: self.userModel.uid, isBlack: true) { succeed, _, _ in
                    //: if succeed == true {
                    if succeed == true {
                        //: self.func__showStatusBarSuccessMsg(showMsg: "Shielding Success".localized)
                        self.beforeDemonstrate(showMsg: (user_sinceTitle.replacingOccurrences(of: "color", with: "S") + kModelKey.lowercased() + "ng S" + dataShowText.replacingOccurrences(of: "indicator", with: "c")).localized)
                        //: self.dismiss()
                        self.dataDismiss()
                    }
                }
            }
        //: default: break
        default: break
        }
    }
}

//: extension TalkingVoiceRoomCardView {
extension ForefrontCardView {
    // 添加视图
    //: private func setupSubviews() {
    private func addMakeMagnitudery() {
        //: self.addSubview(contentView)
        self.addSubview(contentView)
        //: self.addSubview(topIcon)
        self.addSubview(topIcon)
        //: contentView.addSubview(uidLb)
        contentView.addSubview(uidLb)
        //: contentView.addSubview(nameLb)
        contentView.addSubview(nameLb)
        //: contentView.addSubview(levelIcon)
        contentView.addSubview(levelIcon)
        //: contentView.addSubview(signLb)
        contentView.addSubview(signLb)
        //: contentView.addSubview(followersLab)
        contentView.addSubview(followersLab)
        //: contentView.addSubview(followersNum)
        contentView.addSubview(followersNum)
        //: contentView.addSubview(followingLab)
        contentView.addSubview(followingLab)
        //: contentView.addSubview(followingNum)
        contentView.addSubview(followingNum)

        //: contentView.layoutIfNeeded()
        contentView.layoutIfNeeded()

        //: if self.type == .user_other {
        if self.type == .user_other {
            //: contentView.addSubview(lineView)
            contentView.addSubview(lineView)
            //: contentView.addSubview(stackView)
            contentView.addSubview(stackView)
            //: contentView.addSubview(moreBtn)
            contentView.addSubview(moreBtn)
        }
    }

    // 添加约束
    //: private func setupSubViewsConstraint() {
    private func fire() {
        //: contentView.snp.makeConstraints { make in
        contentView.snp.makeConstraints { make in
            //: make.bottom.equalToSuperview().offset(-44)
            make.bottom.equalToSuperview().offset(-44)
            //: make.leading.equalTo(10)
            make.leading.equalTo(10)
            //: make.trailing.equalTo(-10)
            make.trailing.equalTo(-10)
            //: if self.type == .user_me {
            if self.type == .user_me {
                //: make.height.equalTo(192)
                make.height.equalTo(192)
                //: }else {
            } else {
                //: make.height.equalTo(233)
                make.height.equalTo(233)
            }
        }
        //: topIcon.snp.makeConstraints { make in
        topIcon.snp.makeConstraints { make in
            //: make.centerX.equalToSuperview()
            make.centerX.equalToSuperview()
            //: make.top.equalTo(contentView).offset(-33)
            make.top.equalTo(contentView).offset(-33)
            //: make.width.height.equalTo(80)
            make.width.height.equalTo(80)
        }
        //: uidLb.snp.makeConstraints { make in
        uidLb.snp.makeConstraints { make in
            //: make.leading.top.equalTo(12)
            make.leading.top.equalTo(12)
            //: make.trailing.equalTo(topIcon.snp.leading).offset(-10)
            make.trailing.equalTo(topIcon.snp.leading).offset(-10)
        }
        //: nameLb.snp.makeConstraints { make in
        nameLb.snp.makeConstraints { make in
            //: make.top.equalTo(topIcon.snp.bottom).offset(12)
            make.top.equalTo(topIcon.snp.bottom).offset(12)
            //: make.centerX.equalToSuperview().offset(-12)
            make.centerX.equalToSuperview().offset(-12)
            //: make.width.lessThanOrEqualTo(ScreenWidth/2)
            make.width.lessThanOrEqualTo(constExplainViewMsg / 2)
            //: make.height.equalTo(18)
            make.height.equalTo(18)
        }
        //: levelIcon.snp.makeConstraints { make in
        levelIcon.snp.makeConstraints { make in
            //: make.leading.equalTo(nameLb.snp.trailing).offset(5)
            make.leading.equalTo(nameLb.snp.trailing).offset(5)
            //: make.centerY.equalTo(nameLb)
            make.centerY.equalTo(nameLb)
            //: make.size.equalTo(CGSize(width: 32, height: 14))
            make.size.equalTo(CGSize(width: 32, height: 14))
        }
        //: signLb.snp.makeConstraints { make in
        signLb.snp.makeConstraints { make in
            //: make.top.equalTo(nameLb.snp.bottom).offset(12)
            make.top.equalTo(nameLb.snp.bottom).offset(12)
            //: make.leading.equalTo(30)
            make.leading.equalTo(30)
            //: make.trailing.equalTo(-30)
            make.trailing.equalTo(-30)
            //: make.height.equalTo(38)
            make.height.equalTo(38)
        }
        //: followersNum.snp.makeConstraints { make in
        followersNum.snp.makeConstraints { make in
            //: make.top.equalTo(signLb.snp.bottom).offset(12)
            make.top.equalTo(signLb.snp.bottom).offset(12)
            //: make.centerX.equalTo(contentView).offset(-ScreenWidth/4+10)
            make.centerX.equalTo(contentView).offset(-constExplainViewMsg / 4 + 10)
            //: make.height.equalTo(18)
            make.height.equalTo(18)
        }
        //: followingNum.snp.makeConstraints { make in
        followingNum.snp.makeConstraints { make in
            //: make.top.equalTo(signLb.snp.bottom).offset(12)
            make.top.equalTo(signLb.snp.bottom).offset(12)
            //: make.centerX.equalTo(contentView).offset(ScreenWidth/4-10)
            make.centerX.equalTo(contentView).offset(constExplainViewMsg / 4 - 10)
            //: make.height.equalTo(18)
            make.height.equalTo(18)
        }
        //: followersLab.snp.makeConstraints { make in
        followersLab.snp.makeConstraints { make in
            //: make.top.equalTo(followersNum.snp.bottom).offset(5)
            make.top.equalTo(followersNum.snp.bottom).offset(5)
            //: make.centerX.equalTo(followersNum)
            make.centerX.equalTo(followersNum)
            //: make.height.equalTo(14)
            make.height.equalTo(14)
        }
        //: followingLab.snp.makeConstraints { make in
        followingLab.snp.makeConstraints { make in
            //: make.top.equalTo(followingNum.snp.bottom).offset(5)
            make.top.equalTo(followingNum.snp.bottom).offset(5)
            //: make.centerX.equalTo(followingNum)
            make.centerX.equalTo(followingNum)
            //: make.height.equalTo(14)
            make.height.equalTo(14)
        }

        //: if self.type == .user_other {
        if self.type == .user_other {
            //: lineView.snp.makeConstraints { make in
            lineView.snp.makeConstraints { make in
                //: make.top.equalTo(followersLab.snp.bottom).offset(12)
                make.top.equalTo(followersLab.snp.bottom).offset(12)
                //: make.leading.equalTo(7)
                make.leading.equalTo(7)
                //: make.trailing.equalTo(-7)
                make.trailing.equalTo(-7)
                //: make.height.equalTo(1)
                make.height.equalTo(1)
            }
            //: stackView.snp.makeConstraints { make in
            stackView.snp.makeConstraints { make in
                //: make.top.equalTo(lineView)
                make.top.equalTo(lineView)
                //: make.leading.trailing.equalToSuperview()
                make.leading.trailing.equalToSuperview()
                //: make.height.equalTo(44)
                make.height.equalTo(44)
            }
            //: moreBtn.snp.makeConstraints { make in
            moreBtn.snp.makeConstraints { make in
                //: make.trailing.equalToSuperview()
                make.trailing.equalToSuperview()
                //: make.top.equalTo(12)
                make.top.equalTo(12)
                //: make.size.equalTo(CGSize(width: 44, height: 20))
                make.size.equalTo(CGSize(width: 44, height: 20))
            }
        }
    }

    //: override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
    override func touchesBegan(_ touches: Set<UITouch>, with _: UIEvent?) {
        //: let touch = touches.first!
        let touch = touches.first!
        //: var point = touch.location(in: self.currentViewController()?.view)
        var point = touch.location(in: self.buttonController()?.view)
        //: point = contentView.layer.convert(point, from: self.currentViewController()?.view.layer)
        point = contentView.layer.convert(point, from: self.buttonController()?.view.layer)
        //: if !contentView.layer.contains(point) {
        if !contentView.layer.contains(point) {
            //: self.dismiss()
            self.dataDismiss()
        }
    }
}
