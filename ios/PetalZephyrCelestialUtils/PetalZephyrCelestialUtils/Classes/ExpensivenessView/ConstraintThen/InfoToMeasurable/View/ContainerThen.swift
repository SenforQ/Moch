...
fileprivate let main_offStr:[UInt8] = [0xd4,0xd3,0xd4,0xc9,0x95,0xde,0xd2,0xd9,0xd8,0xcf,0x87,0x94,0x9d,0xd5,0xdc,0xce,0x9d,0xd3,0xd2,0xc9,0x9d,0xdf,0xd8,0xd8,0xd3,0x9d,0xd4,0xd0,0xcd,0xd1,0xd8,0xd0,0xd8,0xd3,0xc9,0xd8,0xd9]

/*: "bth_live_follow_pre" :*/
fileprivate let app_deviceStr:[Character] = ["b","t","h","_","l","i","v","e"]
fileprivate let app_titlePath:[Character] = ["_","f","o","l","l","o","w","_","p","r","e"]

/*: "btn_live_closed_nor" :*/
fileprivate let show_modelMsg:[Character] = ["b","t","n","_","l","i","v","e","_","c","l","o","s","e","d","_","n","o","r"]

/*: "removeAttentionUid" :*/
fileprivate let kInfoName:String = "remerror"
fileprivate let main_firstMessage:String = "ntionUistatus express var permission group"
fileprivate let data_centerId:String = "pic"

/*: "attentionUid" :*/
fileprivate let noti_modeName:[Character] = ["a","t","t","e","n","t"]
fileprivate let const_pathStr:[Character] = ["i","o","n","U","i","d"]

/*: "source" :*/
fileprivate let const_modelPath:String = "m"
fileprivate let data_infoName:[Character] = ["o","u","r","c","e"]

/*: "num" :*/
fileprivate let appLastUrl:[Character] = ["n","u","m"]

/*: "99+" :*/
fileprivate let userColorFormat:[UInt8] = [0x2b,0x39,0x39]

/*: "UID: :*/
fileprivate let dataInsideContent:[Character] = ["U","I","D",":"]
...
class ContainerThen: UIView {
    ...
    private var partyModel = HeritageHandyJSON()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_offStr.map{$0^189}, encoding: .utf8)!)
    }
    ...
    private lazy var iconBgBtn: UIButton = {
        ...
        v.addTarget(self, action: #selector(toInfo), for: .touchUpInside)
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var followBtn: UIButton = {
        ...
        btn.setImage(UIImage.atomicCount85(name: (String(app_deviceStr) + String(app_titlePath))), for: .normal)
    ...
    private lazy var miniBtn: UIButton = {
        ...
        btn.setImage(UIImage.atomicCount85(name: (String(show_modelMsg))), for: .normal)
        //: btn.addTarget(self, action: #selector(miniButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(buttonGesture), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var onlineNumBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(doinglineMove), for: .touchUpInside)
    ...
    private lazy var onlineListView: LabView = {
...
extension ContainerThen {
    ...
    @objc func tailgateImageCompletion(uid: Int, isAttention: Bool, completion: ((_ succeed: Bool) -> Void)? = nil) {
        ...
        if isAttention {
            ...
            dict[(kInfoName.replacingOccurrences(of: "error", with: "o") + "veAtte" + String(main_firstMessage.prefix(7)) + data_centerId.replacingOccurrences(of: "pic", with: "d"))] = uid
            //: } else {
        } else {
            //: dict["attentionUid"] = uid
            dict[(String(noti_modeName) + String(const_pathStr))] = uid
            //: dict["source"] = "2"
            dict[(const_modelPath.replacingOccurrences(of: "m", with: "s") + String(data_infoName))] = "2"
        }
        //: MonetaryValueThen.req_atationTool(isAttention: !isAttention, params: dict) { succeed, _, _ in
...
extension ContainerThen {
    ...
    @objc private func getLoadPrevalence() {
        //: TalkingVoiceRoomReqTool.req_partyOnlineNum(roomId: partyModel.roomId) { succeed, result, _ in
        NameReqTool.afterCompletion(roomId: partyModel.roomId) { succeed, result, _ in
            //: guard succeed else { return }
            guard succeed else { return }
            //: let json = JSON(result ?? [String: Any]())
            let json = JSON(result ?? [String: Any]())
            //: self.onlineNum = json["num"].intValue
            self.onlineNum = json[(String(appLastUrl))].intValue
    ...
    private func home() {
        ...
        onlineNunTimer = Timer.scheduledTimer(timeInterval: 60, target: self, selector: #selector(getLoadPrevalence), userInfo: nil, repeats: true)
        //: onlineNunTimer?.fire()
        onlineNunTimer?.fire()
    }
...
extension ContainerThen {
    ...
    func addArray() {
        ...
        partyModel = ConstraintEventHandler.bindTo().partyModel
        //: nickNameLab.text = partyModel.streamerInfo.nickname
        nickNameLab.text = partyModel.streamerInfo.nickname
        //: uidLab.text = "UID:\(partyModel.streamerInfo.uid)"
        uidLab.text = (String(dataInsideContent)) + "\(partyModel.streamerInfo.uid)"
        ...
        icon.luculentUpwardly(urlStr: partyModel.streamerInfo.headPic, placeImg: UIImage.putReplacement(sex: partyModel.streamerInfo.sex), options: optionsInfo)
        // 是否是主播
        //: let isAnchor = (String(partyModel.streamerInfo.uid) == NowInfoThen.share.loginUid)
        let isAnchor = (String(partyModel.streamerInfo.uid) == NowInfoThen.share.loginUid)
        //: if isAnchor {
        if isAnchor {
            //: refreshFollow(isAttention: true)
            fieldMessage(isAttention: true)
            //: } else {
        } else {
            //: refreshFollow(isAttention: partyModel.streamerInfo.isAttention)
            fieldMessage(isAttention: partyModel.streamerInfo.isAttention)
        }
        // 定时更新在线人数
        //: startTimer()
        home()
    ...
    @objc private func fixingClick() {
        ...
        tailgateImageCompletion(uid: partyModel.streamerInfo.uid, isAttention: partyModel.streamerInfo.isAttention) { [weak self] succeed in
            //: guard let self = self else { return }
            guard let self = self else { return }
            //: guard succeed else { return }
            guard succeed else { return }
            ...
            self.fieldMessage(isAttention: self.partyModel.streamerInfo.isAttention)
        }
    }

    /// 最小化按钮点击事件
    //: @objc private func miniButtonClick() {
    @objc private func buttonGesture() {
        //: superview?.endEditing(true)
        superview?.endEditing(true)
        //: let view = TalkingVoiceRoomCloseView()
        let view = CardinalReactiveCompatible()
        //: view.show()
        view.dismissWithTo()
    }
    ...
    func fieldMessage(isAttention: Bool) {
    ...
    @objc private func doinglineMove() {
        //: onlineListView.showView()
        onlineListView.radiogramView()
    }
}
...
extension ContainerThen {
    ...
    private func stud() {
        ...
        frame = CGRect(x: 0, y: 0, width: constExplainViewMsg, height: appImageLimitPath + actualWidth(w: 43))
