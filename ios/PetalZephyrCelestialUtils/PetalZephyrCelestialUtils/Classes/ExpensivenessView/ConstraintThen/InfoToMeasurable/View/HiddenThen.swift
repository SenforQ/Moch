...
fileprivate let data_sizeValue:[UInt8] = [0xea,0xef,0xea,0xf5,0xa9,0xe4,0xf0,0xe5,0xe6,0xf3,0xbb,0xaa,0xa1,0xe9,0xe2,0xf4,0xa1,0xef,0xf0,0xf5,0xa1,0xe3,0xe6,0xe6,0xef,0xa1,0xea,0xee,0xf1,0xed,0xe6,0xee,0xe6,0xef,0xf5,0xe6,0xe5]

fileprivate func showTable(cur num: UInt8) -> UInt8 {
...
/*: "#FA9D33" :*/
fileprivate let constEventName:String = "image tool index style when#FA9D33"

/*: "#343338" :*/
fileprivate let user_hiddenMsg:String = "#3"
fileprivate let data_sizeTitle:String = "4sex38"
...
class HiddenThen: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_sizeValue.map{showTable(cur: $0)}, encoding: .utf8)!)
    }
...
extension HiddenThen {
    /// 刷新cell
    /// - Parameters:
    ///   - position: 麦位
    ///   - model: 麦位数据
    //: func refreshIcon(position: Int, model: TalkingMikeListItemModel) {
    func frontShow(position: Int, model: InfoToMeasurable) {
        //: guard model.uid > 0, String(model.uid) != NowInfoThen.share.loginUid else {
        guard model.uid > 0, String(model.uid) != NowInfoThen.share.loginUid else {
        ...
        let color = model.needGift ? UIColor(hex: (String(constEventName.suffix(7)))) : UIColor(hex: (user_hiddenMsg.capitalized + data_sizeTitle.replacingOccurrences(of: "sex", with: "33")))
