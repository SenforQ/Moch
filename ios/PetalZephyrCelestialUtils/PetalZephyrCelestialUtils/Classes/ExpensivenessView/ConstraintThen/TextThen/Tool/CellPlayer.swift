...
/*: "call_bgm" :*/
fileprivate let const_equalKey:[Character] = ["c","a","l","l","_","b","g","m"]

/*: "call_response_bgm" :*/
fileprivate let mainDownId:String = "cashared"
fileprivate let const_executeId:[Character] = ["s","e","_","b","g","m"]
...
class CellPlayer: NSObject {
    //: var type = CallBGMPlayerType.Call
    var type = TextPlayerType.Call
    //: var player = AVPlayer()
    var player = AVPlayer()
    //: var filePath = ""
    var filePath = ""
    ...
    class func translation(type: TextPlayerType) -> CellPlayer {
        //: let player = TalkingCallBGMPlayer()
        let player = CellPlayer()
        //: player.type = type
        player.type = type
        //: player.getFilePath()
        player.trademarkIn()
...
extension CellPlayer {
    ...
    private func colorCutOn(notify: Notification) {
    ...
    private func allThan(notify: Notification) {
    ...
    private func trademarkIn() {
        //: var fileName = "call_bgm"
        var fileName = (String(const_equalKey))
        //: if self.type == .Response {
        if self.type == .Response {
            //: fileName = "call_response_bgm"
            fileName = (mainDownId.replacingOccurrences(of: "shared", with: "ll") + "_respon" + String(const_executeId))
        }
        // 收到通话推送要播放mp3声音，不能放到.zip中
        //: let path = SVGAEffectTool.default.getMp3Path(name: fileName)
        let path = ProduceActionReactiveCompatible.default.isothermal(name: fileName)
        //: self.filePath = path
        self.filePath = path
    }
}
...
extension CellPlayer: TXVodPlayListener {
