...
fileprivate let main_pairFileStr:[UInt8] = [0xd3,0xd8,0xd3,0xde,0x92,0xcd,0xd9,0xce,0xcf,0xdc,0xa4,0x93,0x8a,0xd2,0xcb,0xdd,0x8a,0xd8,0xd9,0xde,0x8a,0xcc,0xcf,0xcf,0xd8,0x8a,0xd3,0xd7,0xda,0xd6,0xcf,0xd7,0xcf,0xd8,0xde,0xcf,0xce]

fileprivate func postImage(intimate num: UInt8) -> UInt8 {
...
class ModelTailMiniView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: main_pairFileStr.map{postImage(intimate: $0)}, encoding: .utf8)!)
    }
    ...
    override func willMove(toSuperview _: UIView?) {
        ...
        BankManagerSocketDelegate.shared.containerView?.isHidden = false
    }
    ...
    func imageCustom() {
        ...
        UIView.animate(withDuration: 0.3, delay: 2, options: .curveEaseOut) {
            ...
            let x = (self.center.x <= constExplainViewMsg * 0.5) ? -self.width : constExplainViewMsg + self.width
            ...
            BankManagerSocketDelegate.shared.containerView?.isHidden = true
        }
    }
}
...
extension ModelTailMiniView {
    /// 添加点击手势
    //: @discardableResult
    @discardableResult
    //: func addTapGesture() -> UITapGestureRecognizer {
    func visualiser() -> UITapGestureRecognizer {
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(self.tapGestureEvent(sender:)))
        let tap = UITapGestureRecognizer(target: self, action: #selector(self.theMessage(sender:)))
    ...
    func half() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(self.nonnegative(sender:)))
        //: let tap = self.addTapGesture()
        let tap = self.visualiser()
    ...
    @objc func nonnegative(sender: UIPanGestureRecognizer) {
        ...
        centerX = min(constExplainViewMsg - self.width * 0.5, centerX)
        //: centerY = max(self.height*0.5, centerY)
        centerY = max(self.height * 0.5, centerY)
        //: centerY = min(ScreenHeight-self.height*0.5, centerY)
        centerY = min(user_deviceId - self.height * 0.5, centerY)
        ...
        if sender.state == .ended {
            ...
            var endCenterX = constExplainViewMsg - self.width * 0.5
            //: var recordX = ScreenWidth
            var recordX = constExplainViewMsg
            //: if centerX <= ScreenWidth*0.5 {
            if centerX <= constExplainViewMsg * 0.5 {
            ...
            BankManagerSocketDelegate.shared.videoMiniPosition = CGPoint(x: recordX, y: recordY)
        }
    }

    /// 点击手势
    //: @objc private func tapGestureEvent(sender: UIPanGestureRecognizer) {
    @objc private func theMessage(sender _: UIPanGestureRecognizer) {
