...
fileprivate let user_makeText:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "#FFD13A" :*/
fileprivate let constCurrentValue:String = "if with cheese#FFD13A"

/*: "gift_gb_umnad_1" :*/
fileprivate let appListKey:String = "gift_gsize leading input"
fileprivate let const_mixtureId:String = "progress to return endad_1"

/*: "icon_circle_pic_default" :*/
fileprivate let data_imageFormat:String = "icon_search text title menu"
fileprivate let userColorContent:[Character] = ["c","i","r","c","l","e","_","p","i","c","_","d","e","f","a","u","l","t"]

/*: "Open lucky blind-box" :*/
fileprivate let notiSeriesTitle:String = "Open color manager"
fileprivate let show_priceMakeText:[Character] = ["l","i","n","d","-","b","o","x"]

/*: "Sent " :*/
fileprivate let app_styleData:[Character] = ["S","e","n","t"," "]

/*: "PingFangSC-Semibold" :*/
fileprivate let appToMessage:String = "action equal center hiddenPing"
fileprivate let notiContentId:[Character] = ["-","S","e","m"]
fileprivate let k_stopData:String = "ibolfirst"

/*: "%d" :*/
fileprivate let userRedAppValue:String = "%dreturn hide input"

/*: "Bombard" :*/
fileprivate let app_modelStr:String = "user if white label typeBombard"
...
class FaceView: UIView {
    ...
    var giftMsgModel = PointModelType()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_makeText.reversed(), encoding: .utf8)!)
    }
...
//        lb.numberOfLines = 1
        ...
        lb.textColor = UIColor(hex: (String(constCurrentValue.suffix(7))))
    ...
    private lazy var pluralTip: ModelItemLabel = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = ModelItemLabel()
        //: lab.isHidden = true
        lab.isHidden = true
    ...
    private lazy var countLb: ModelItemLabel = {
        //: let lab = TalkingGiftTrackItemLabel()
        let lab = ModelItemLabel()
...
extension FaceView {
    ...
    func errorObserve(model: PointModelType) {
        ...
        self.bgImage.image = UIImage.atomicCount85(name: (String(appListKey.prefix(6)) + "b_umn" + String(const_mixtureId.suffix(4))))

        //: let placeholder = UIImage.BundleImageNamed(name: "icon_circle_pic_default")
        let placeholder = UIImage.atomicCount85(name: (String(data_imageFormat.prefix(5)) + String(userColorContent)))

        //: if (model.showType == ChatGiftAnimatType.myStery.rawValue ) {
        if model.showType == AddFrameVarArg.myStery.rawValue {
            //: self.nickLb.text = "Open lucky blind-box".localized
            self.nickLb.text = (String(notiSeriesTitle.prefix(5)) + "lucky b" + String(show_priceMakeText)).localized
            //: }else {
        } else {
        ...
        self.avatarImage.luculentUpwardly(urlStr: giftMsgModel.fromHeadPic, placeImg: placeholder)
        //: self.giftImage.setUrlImage(urlStr: giftMsgModel.giftPic, placeImg: placeholder)
        self.giftImage.luculentUpwardly(urlStr: giftMsgModel.giftPic, placeImg: placeholder)

        //: self.giftName.text = "Sent ".localized + giftMsgModel.pname
        self.giftName.text = (String(app_styleData)).localized + giftMsgModel.pname
        ...
        if sendNumber > 1 {
            ...
            var font1 = UIFont(name: (String(appToMessage.suffix(4)) + "FangSC" + String(notiContentId) + k_stopData.replacingOccurrences(of: "first", with: "d")), size: 22)
            //: font1 = italicFontWith(angle: 15, fontSize: 18, fontName: font1!.fontName)
            font1 = self.angle(angle: 15, fontSize: 18, fontName: font1!.fontName)
            //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 26)
            var font2 = UIFont(name: (String(appToMessage.suffix(4)) + "FangSC" + String(notiContentId) + k_stopData.replacingOccurrences(of: "first", with: "d")), size: 26)
            //: font2 = italicFontWith(angle: 15, fontSize: 22, fontName: font2!.fontName)
            font2 = self.angle(angle: 15, fontSize: 22, fontName: font2!.fontName)
            ...
            numText = numText.equidistantLine(texts: strs, colors: colors, fonts: fonts)
        ...
        sectionDonation(giftCount: giftMsgModel.comboNum)
    }

    //: func refreshGiftCount(giftCount: Int) {
    func sectionDonation(giftCount: Int) {
        //: var font1 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font1 = UIFont(name: (String(appToMessage.suffix(4)) + "FangSC" + String(notiContentId) + k_stopData.replacingOccurrences(of: "first", with: "d")), size: 45)
        //: font1 = italicFontWith(angle: 15, fontSize: 26, fontName: font1!.fontName)
        font1 = self.angle(angle: 15, fontSize: 26, fontName: font1!.fontName)
        //: var font2 = UIFont(name: "PingFangSC-Semibold", size: 45)
        var font2 = UIFont(name: (String(appToMessage.suffix(4)) + "FangSC" + String(notiContentId) + k_stopData.replacingOccurrences(of: "first", with: "d")), size: 45)
        //: font2 = italicFontWith(angle: 15, fontSize: 38, fontName: font2!.fontName)
        font2 = self.angle(angle: 15, fontSize: 38, fontName: font2!.fontName)
        ...
        let colors = [UIColor.showViewCreate(150, 77, 20, 1), UIColor.white, UIColor.showViewCreate(150, 77, 20, 1)]
        ...
        attText = attText.equidistantLine(texts: strs, colors: colors, fonts: fonts)
        ...
        countLb.bucket(attributedText: attText)
        ...
        countLb.statisticalTable(frame: CGRect(x: x, y: countLb.origin.y, width: width, height: countLb.size.height))
    }

    /// 连击
    //: func comboHitWith(giftMsgModel: TalkingGiftMsgAnimatModel) {
    func aboutValue(giftMsgModel: PointModelType) {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(sharedItem), object: nil)
        ...
        self.perform(#selector(sharedItem), afterDelay: 3.5)
    }

    /// 展示
    //: func showGiftAnimate(model: TalkingGiftMsgAnimatModel, animate: Bool) {
    func earlyAnimate(model _: PointModelType, animate _: Bool) {
    ...
    @objc func sharedItem() {
        //: NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(animateHide), object: nil)
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(sharedItem), object: nil)
        //: giftMsgModel = TalkingGiftMsgAnimatModel()
        giftMsgModel = PointModelType()
...
extension FaceView {
    ...
    func angle(angle: CGFloat, fontSize: CGFloat, fontName: String) -> (UIFont) {
...
extension FaceView {
    ...
    private func modeForMoment() {
        ...
        font = self.angle(angle: 15, fontSize: 20, fontName: font.fontName)
        ...
        let attText = attributedString.equidistantLine(texts: [(String(app_modelStr.suffix(7))).localized], colors: [UIColor.showViewCreate(255, 232, 72, 1)], fonts: [font])
        //: pluralTip.setFrame(frame: CGRect.init(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height-20))
        pluralTip.statisticalTable(frame: CGRect(x: giftImage.right, y: 14, width: attText.size().width + 2, height: height - 20))

        //: pluralTip.setAttributedText(attributedText: attText)
        pluralTip.bucket(attributedText: attText)
        //: contentView.addSubview(pluralTip)
        contentView.addSubview(pluralTip)

        //: countLb.setFrame(frame: CGRect.init(x: giftImage.right, y: 2, width: 70, height: height))
        countLb.statisticalTable(frame: CGRect(x: giftImage.right, y: 2, width: 70, height: height))
