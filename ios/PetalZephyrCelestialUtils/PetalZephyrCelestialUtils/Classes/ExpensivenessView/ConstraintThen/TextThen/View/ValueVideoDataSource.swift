...
fileprivate let k_itemPath:[UInt8] = [0x88,0x8f,0x88,0x95,0xc9,0x82,0x8e,0x85,0x84,0x93,0xdb,0xc8,0xc1,0x89,0x80,0x92,0xc1,0x8f,0x8e,0x95,0xc1,0x83,0x84,0x84,0x8f,0xc1,0x88,0x8c,0x91,0x8d,0x84,0x8c,0x84,0x8f,0x95,0x84,0x85]

private func changeMake(lab num: UInt8) -> UInt8 {
    return num ^ 225
}
...
@objc protocol OfObjectProtocol: NSObjectProtocol {
    ///
    //: @objc optional func startScroll()
    @objc optional func comeOffScroll()

    //: @objc optional func endScroll()
    @objc optional func actionScroll()
}

//: class TalkingVideoDanmuMsgListTableView: UIView {
class ValueVideoDataSource: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_itemPath.map{changeMake(lab: $0)}, encoding: .utf8)!)
    }
...
extension ValueVideoDataSource {
    //: func startTimer() {
    func elementTimer() {
        //: stopTimer()
        endApp()
        ...
        refreshTimer = Timer.scheduledTimer(timeInterval: reloadTimeSpan, target: self, selector: #selector(destroyWithShadow), userInfo: nil, repeats: true)
    }
...
extension ValueVideoDataSource {
    ...
    func migration(msgModel: InfoHandyJSON?) {
    ...
    func modelBottom() {
        ...
        if AllHeight > dataScreenValue {
            //: if self.msgTableView.height < VideoMsgTableViewHeight {
            if self.msgTableView.height < dataScreenValue {
                ...
                self.msgTableView.height = dataScreenValue
            }
            //: } else {
        } else {
            //: var frame = self.msgTableView.frame
            ...
            frame.origin.y = dataScreenValue - AllHeight
...
extension ValueVideoDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    func bull(_ scrollView: UIScrollView) {
        //: self.delegate?.endScroll?()
        self.delegate?.actionScroll?()
    ...
    func tableView(_: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        return msgModel.msgHeight + Double(app_tableVersionPath)
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = TailView.endPath(tableView: tableView, msg: msgModel, indexPath: indexPath)
...
extension ValueVideoDataSource {
    ...
    func clubResume() {
        ...
        self.msgTableView.frame = CGRect(x: 0, y: 0, width: const_deviceKey, height: dataScreenValue)
    }
}
