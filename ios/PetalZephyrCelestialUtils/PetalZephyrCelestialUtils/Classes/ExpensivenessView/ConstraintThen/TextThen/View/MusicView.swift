...
fileprivate let user_dataAdminPath:[UInt8] = [0xc9,0xce,0xc9,0xd4,0x88,0xc3,0xcf,0xc4,0xc5,0xd2,0x9a,0x89,0x80,0xc8,0xc1,0xd3,0x80,0xce,0xcf,0xd4,0x80,0xc2,0xc5,0xc5,0xce,0x80,0xc9,0xcd,0xd0,0xcc,0xc5,0xcd,0xc5,0xce,0xd4,0xc5,0xc4]

private func emptyFrame(key num: UInt8) -> UInt8 {
    return num ^ 160
}

/*: "#171717" :*/
fileprivate let mainAddGiftText:String = "#171717"

/*: "icon_cameraoff" :*/
fileprivate let data_moreToName:[Character] = ["i"]
fileprivate let userReadyMessage:[Character] = ["c","o","n","_","c","a","m","e","r","a","o","f","f"]
...
class MusicView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        self.backgroundColor = UIColor.showViewCreate(51, 51, 51, 1)
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_dataAdminPath.map{emptyFrame(key: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var bgView: UIImageView = {
        ...
        alphaV.backgroundColor = UIColor(hex: (mainAddGiftText.capitalized))
        //: alphaV.alpha = 0.6
        alphaV.alpha = 0.6
        //: v.addSubview(alphaV)
        v.addSubview(alphaV)
    ...
    private lazy var icon: UIImageView = {
        ...
        v.image = UIImage.atomicCount85(name: (String(data_moreToName) + String(userReadyMessage)))
        //: v.isHidden = true
        v.isHidden = true
        //: return v
        return v
...
extension MusicView {
    ...
    func equalHide(urlStr: String?, isOpenCamera: AddSubordinateCameraStatus) {
...
extension MusicView: UIGestureRecognizerDelegate {
    /// 拖拽位移手势
    /// - Parameter pan: 手势
    //: @objc private func dragWindow(pan: UIPanGestureRecognizer) {
    @objc private func resolution(pan: UIPanGestureRecognizer) {
        ...
        newCenterX = min(newCenterX, constExplainViewMsg - self.frame.size.width * 0.5)
        //: newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        newCenterY = max(self.frame.size.height * 0.5, newCenterY)
        //: newCenterY = min(newCenterY, ScreenHeight - self.frame.size.height * 0.5)
        newCenterY = min(newCenterY, user_deviceId - self.frame.size.height * 0.5)
...
extension MusicView {
    ...
    private func cleaveView() {
        // 切换视角手势
        //: let tap = UITapGestureRecognizer(target: self, action: #selector(exchangeRenderContainer))
        let tap = UITapGestureRecognizer(target: self, action: #selector(cylinder))
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(resolution(pan:)))
        //: pan.delegate = self
        pan.delegate = self
