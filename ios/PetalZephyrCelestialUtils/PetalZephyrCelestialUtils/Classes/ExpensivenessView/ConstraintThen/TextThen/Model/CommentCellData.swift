...
/*: ：" :*/
fileprivate let mainIconData:[Character] = ["："]

/*: "#BAFBFF" :*/
fileprivate let kKeyMessage:String = "add add current in#BAFBFF"
...
class CommentCellData: NSObject {
    ...
    func actionObject(model: InfoHandyJSON) -> InfoHandyJSON {
        //: self.model = model
        self.model = model
        /// 普通文本消息
        //: if self.model.messageType == 1 {
    ...
    func tingDestroy() {
        ...
        let name = nameTo(text: firstStr, font: UIFont.outView(fontSize: 14), color: UIColor(hex: (String(kKeyMessage.suffix(7))))!)
        // 内容
        //: let content = getAttributed(text: self.model.content, font: UIFont.pingfangMediumFont(fontSize: 14), color: UIColor.white)
        let content = nameTo(text: self.model.content, font: UIFont.outView(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        ...
        tapContent(attribText: self.model.msgAttribText)
    }
    ...
    func kindContent() {
        ...
        let content = nameTo(text: self.model.transContent, font: UIFont.outView(fontSize: 14), color: UIColor.white)
        //: name.append(content)
        name.append(content)
        //: name.yy_paragraphStyle = paraStyle
        name.yy_paragraphStyle = paraStyle
    ...
    func isStyle() -> NSMutableParagraphStyle {
    ...
    func nameTo(text: String, font: UIFont, color: UIColor) -> NSMutableAttributedString {
...
//            attribute.yy_writingDirection = [3]
        ...
        attribute.yy_paragraphStyle = isStyle()
        //: return attribute
        return attribute
    }
    ...
    func tapContent(attribText: NSMutableAttributedString) {
        // 距离左边8  距离右边也为8
        //: let maxWidth = VideoMsgTableViewWidth - 36
        let maxWidth = const_deviceKey - 36
