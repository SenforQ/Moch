...
fileprivate let noti_automaticUrl:String = "equal add observerCach"
fileprivate let app_eventStr:String = "Effecmini name"
fileprivate let const_labFormat:[Character] = ["y"]

/*: "下载动画文件" :*/
fileprivate let user_managerMessage:[Character] = ["下","载","\u{52a8}","画","文","件"]

/*: "加载本地动画文件" :*/
fileprivate let dataVideoUrl:[Character] = ["加","载","本","地","\u{52a8}","\u{753b}","文","件"]

/*: "%@/%@/" :*/
fileprivate let show_toPlayerData:[Character] = ["%"]
fileprivate let appDataUserStr:[Character] = ["@","/","%","@","/"]

/*: "%@/%zd/" :*/
fileprivate let userLoadLockFormat:String = "%@/%zvar last in"
fileprivate let noti_dataKey:[Character] = ["d","/"]

/*: "Cache_GiftEffect/" :*/
fileprivate let notiMakeName:[Character] = ["C","a","c","h"]
fileprivate let noti_modeMsg:String = "e_Gifborder value"

/*: "%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x" :*/
fileprivate let userFilterData:[UInt8] = [0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67,0x3a,0x2f,0x2d,0x67]

private func dataLeadingYear(index num: UInt8) -> UInt8 {
    return num ^ 31
}

/*: "GJSGVA" :*/
fileprivate let user_toManagerMessage:String = "GJSGVmodel"

/*: "GJVap" :*/
fileprivate let appLeadingMessage:[Character] = ["G","J","V","a","p"]

/*: "config.json" :*/
fileprivate let const_deadlineMsg:String = "app false self else keyconfig.j"
fileprivate let app_subMsg:String = "sdata"

/*: "%d" :*/
fileprivate let show_airName:String = "%dview case"

/*: "app. :*/
fileprivate let notiChangePath:String = "app.else type window make add"

/*: .com" :*/
fileprivate let data_groupMsg:[Character] = [".","c","o","m"]

/*: "key" :*/
fileprivate let notiGameName:String = "kacrossy"

/*: "zip" :*/
fileprivate let data_sendMsg:String = "ZIP"

/*: " fail to unzip file" :*/
fileprivate let showDocumentAddTitle:[Character] = [" ","f","a","i","l"," ","t"]
fileprivate let show_nameId:String = "o unzstack image user"

/*: "RSA 解密失败" :*/
fileprivate let app_giftId:[Character] = ["R","S","A"," ","解","\u{5bc6}","失","败"]

/*: "fail to unzip script file" :*/
fileprivate let appExtraText:String = "failureail"
fileprivate let userMakeStr:[Character] = [" ","t","o"," ","u","n","z","i","p"," ","s","c","r","i","p","t"," ","f","i","l","e"]

/*: "%ld" :*/
fileprivate let const_succeedId:String = "%ldstring color top size"

/*: "gift" :*/
fileprivate let const_successLayerLabText:String = "giapp"

/*: "%@%@effect_%ld" :*/
fileprivate let kAboutId:String = "view@view@"
fileprivate let show_pageMsg:String = "make super cell aspect_%ld"

/*: "%@%@effect_%ld_unzipTest/" :*/
fileprivate let showBottomLabFormat:[Character] = ["%","@","%","@","e","f","f","e","c","t","_","%","l","d","_","u","n"]
fileprivate let notiValueMessage:String = "zapp"
fileprivate let app_voiceSumStr:String = "view out varpTest/"

/*: "%@%@effect_%ld_unzip/" :*/
fileprivate let main_makeMsg:[Character] = ["%","@","%","@","e","f","f","e","c","t","_","%","l","d","_","u"]
fileprivate let app_pathUrl:[Character] = ["n","z","i","p","/"]
...
typealias CompletionBlock = (_ success: Bool, _ effectModel: ModelMeasurable?) -> Void

//: typealias ErrorBlock = (_ error: NSError?) -> Void
typealias ErrorBlock = (_ error: NSError?) -> Void

//: class TalkingGiftAnimatTool: NSObject {
class ConstraintThen: NSObject {
    //: let TalkingGiftAnimatCacheKey  = "CacheGiftEffectCacheKey"
    let TalkingGiftAnimatCacheKey = (String(noti_automaticUrl.suffix(4)) + "eGift" + String(app_eventStr.prefix(5)) + "tCacheKe" + String(const_labFormat))

    //: @objc static let shared = TalkingGiftAnimatTool()
    @objc static let shared = ConstraintThen()
    ...
    func openBackwash(effectMsgModel: CountimateMsgModel, completion: @escaping CompletionBlock) {
        //: if checkNewVersionWithAnimatMsgModel(model: effectMsgModel) {
        if quantityimateForStop(model: effectMsgModel) {
            //: printLog(message: "下载动画文件")
            printLog(message: (String(user_managerMessage)))
            //: downloadAnimat(effectMsgModel: effectMsgModel) { [weak self] error in
            itemAsset(effectMsgModel: effectMsgModel) { [weak self] error in
                ...
                self.popComplete(effectMsgModel: effectMsgModel, complete: completion)
            }
            //: } else {
        } else {
            //: printLog(message: "加载本地动画文件")
            printLog(message: (String(dataVideoUrl)))
            //: self.loadLocalAnimatConfig(effectMsgModel: effectMsgModel, complete: completion)
            self.popComplete(effectMsgModel: effectMsgModel, complete: completion)
        }
    }
    ...
    func arrayFrom(fileName: String, model: CountimateMsgModel) -> String {
    ...
    func loadLocation() -> String {
        ...
        let subPath = (String(notiMakeName) + String(noti_modeMsg.prefix(5)) + "tEffect/")
    ...
    func successAll(filePath: String) -> String {
    ...
    func announcementKey(model: CountimateMsgModel) -> Bool {
        //: return !checkNewVersionWithAnimatMsgModel(model: model)
        return !quantityimateForStop(model: model)
    }

    //: func getNewAnimatClassName(oldName: String) -> String {
    func streetName(oldName: String) -> String {
        //: var newName = oldName
        var newName = oldName
        //: if newName.hasPrefix("GJSGVA") {
        if newName.hasPrefix((user_toManagerMessage.replacingOccurrences(of: "model", with: "A"))) {
            //: newName = String(describing: TalkingSGVAAnimationView.self)
            newName = String(describing: SwaddlingClothesView.self)
            //: } else if newName.hasPrefix("GJVap") {
        } else if newName.hasPrefix((String(appLeadingMessage))) {
            //: newName = String(describing: TalkingVapAnimationView.self)
            newName = String(describing: HiddenPlayDelegate.self)
        }
        //: return newName
        return newName
    }
...
extension ConstraintThen {
    ...
    func popComplete(effectMsgModel: CountimateMsgModel, complete: @escaping CompletionBlock) {
        ...
        let scriptPath = (scriptDirectory as NSString).appendingPathComponent((String(const_deadlineMsg.suffix(8)) + app_subMsg.replacingOccurrences(of: "data", with: "on")))
        ...
        let dic = NSDictionary.squareOn(withJsonString: configString!) ?? NSDictionary()
        //: let giftAmodel = TalkingGiftAnimatModel.init()
        let giftAmodel = ModelMeasurable()
        //: let effectModel = giftAmodel.setAnimatModek(dic: dic)
        let effectModel = giftAmodel.duringEmplace(dic: dic)
        ...
        className = streetName(oldName: className)
        //: let clsName = PodspecName + "." + className
        let clsName = k_imagePath + "." + className
    ...
    func quantityimateForStop(model: CountimateMsgModel) -> Bool {
    ...
    func itemAsset(effectMsgModel: CountimateMsgModel, errorBlock: @escaping ErrorBlock) {
        //: MonetaryValueThen.req_giftEffectDataUrl(giftUrl: effectMsgModel.effectUrl, path: downloadTmpPath(model: effectMsgModel)) { [weak self] t in
        MonetaryValueThen.exaggerateCompletion(giftUrl: effectMsgModel.effectUrl, path: bombUp(model: effectMsgModel)) { [weak self] t in
            //: guard let self = self else {return}
            guard let self = self else { return }
            //: DispatchQueue.global().async {
            DispatchQueue.global().async {
                //: self.afterDownloadGiftAnimat(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
                self.observe(effectMsgModel: effectMsgModel, isSucess: t, errorBlock: errorBlock)
            }
        }
    }

    //: func afterDownloadGiftAnimat(effectMsgModel: TalkingAnimatMsgModel, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
    func observe(effectMsgModel: CountimateMsgModel, isSucess: Bool, errorBlock: @escaping ErrorBlock) {
        //: if !isSucess {
        if !isSucess {
            //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.DownLoadFailed.rawValue))
            errorBlock(NSError(domain: (String(notiChangePath.prefix(4))) + "\(main_bottomStr)" + (String(data_groupMsg)), code: CookieHashable.DownLoadFailed.rawValue))
            //: return
            return
            ...
            let downloadPath = bombUp(model: effectMsgModel)
            //: let unzipVerifyDir = unzipVerifyDirectory(model: effectMsgModel)
            let unzipVerifyDir = handleModel(model: effectMsgModel)
            ...
            if verifyUnzipSucc {
                ...
                for filename in unzippedFiles {
                    //: if filename == "key" {
                    if filename == (notiGameName.replacingOccurrences(of: "across", with: "e")) {
                        // encrypted md5 file
                        //: keyFilePath = (unzipVerifyDir as NSString).appendingPathComponent(filename)
                        keyFilePath = (unzipVerifyDir as NSString).appendingPathComponent(filename)
                        //: } else if (filename as NSString).pathExtension == "zip" {
                    } else if (filename as NSString).pathExtension == (data_sendMsg.lowercased()) {
                        // script file
                ...
                printLog(message: (String(showDocumentAddTitle) + String(show_nameId.prefix(5)) + "ip file"))
                //: isFailed = true
                isFailed = true
                //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                errorBlock(NSError(domain: (String(notiChangePath.prefix(4))) + "\(main_bottomStr)" + (String(data_groupMsg)), code: CookieHashable.UnzipFailed.rawValue))
            }
            ...
            if !isFailed {
                ...
                do {
                    ...
                    let md5Data: Data = ClickFzrsa.betweenCustom(NSData(contentsOfFile: keyFilePath) as Data?, render: constLicenseMsg)
                    ...
                    let md5 = successAll(filePath: scriptZipFilePath)
                    ...
                    if decryptMD5 != md5, FileManager.default.fileExists(atPath: keyFilePath) {
                        ...
                        errorBlock(NSError(domain: (String(notiChangePath.prefix(4))) + "\(main_bottomStr)" + (String(data_groupMsg)), code: CookieHashable.VerifyFailed.rawValue))
                    }
                    ...
                    printLog(message: (String(app_giftId)))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.VerifyFailed.rawValue))
                    errorBlock(NSError(domain: (String(notiChangePath.prefix(4))) + "\(main_bottomStr)" + (String(data_groupMsg)), code: CookieHashable.VerifyFailed.rawValue))
            ...
            if !isFailed {
                ...
                let unzipTmpDir = unzipGreetPtolemaicSystem(model: effectMsgModel)
                ...
                //: else {
                    ...
                    printLog(message: (appExtraText.replacingOccurrences(of: "failure", with: "f") + String(userMakeStr)))
                    //: isFailed = true
                    isFailed = true
                    //: errorBlock(NSError.init(domain: "app.\(ReplaceUrlDomain).com", code: GiftAnimatUpdateError.UnzipFailed.rawValue))
                    errorBlock(NSError(domain: (String(notiChangePath.prefix(4))) + "\(main_bottomStr)" + (String(data_groupMsg)), code: CookieHashable.UnzipFailed.rawValue))
    ...
    func strength(path: String) -> Bool {
        //: if cleanFolder(path: path) {
        if imageCell(path: path) {
    ...
    func imageCell(path: String) -> Bool {
    ...
    func approximation(type: HiddenOffsetTarget) -> String {
        //: var tmpIdentify = ""
        var tmpIdentify = ""
        //: switch type {
        switch type {
        //: case .Gift:     tmpIdentify =  "gift"
        case .Gift: tmpIdentify = (const_successLayerLabText.replacingOccurrences(of: "app", with: "ft"))
        //: break
        //: default:
        default:
            //: break
    ...
    func bombUp(model: CountimateMsgModel) -> String {
        ...
        let downloadTmpPath = String(format: (kAboutId.replacingOccurrences(of: "view", with: "%") + "effect" + String(show_pageMsg.suffix(4))), NSTemporaryDirectory(), approximation(type: model.effectType), model.eid)
        //: return downloadTmpPath
        return downloadTmpPath
    }

    //: func unzipVerifyDirectory(model: TalkingAnimatMsgModel)->String {
    func handleModel(model: CountimateMsgModel) -> String {
        ...
        let unzipVerifyDirectory = String(format: (String(showBottomLabFormat) + notiValueMessage.replacingOccurrences(of: "app", with: "i") + String(app_voiceSumStr.suffix(6))), NSTemporaryDirectory(), approximation(type: model.effectType), model.eid)
        //: return unzipVerifyDirectory
        return unzipVerifyDirectory
    }

    //: func unzipTmpDirectory(model: TalkingAnimatMsgModel)->String {
    func unzipGreetPtolemaicSystem(model: CountimateMsgModel) -> String {
        ...
        let unzipTmpDirectory = String(format: (String(main_makeMsg) + String(app_pathUrl)), NSTemporaryDirectory(), approximation(type: model.effectType), model.eid)
