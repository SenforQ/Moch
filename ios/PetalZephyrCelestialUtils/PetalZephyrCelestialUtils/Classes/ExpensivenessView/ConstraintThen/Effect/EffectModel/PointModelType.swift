...
/*: "id" :*/
fileprivate let kKitUrl:[Character] = ["i","d"]

/*: "num" :*/
fileprivate let data_sexMediumPath:[Character] = ["n","u","m"]
...
@objc public enum AddFrameVarArg: Int {
    //: case normal  = 1
    case normal = 1 // 普通礼物
    //: case myStery = 2
    case myStery = 2 // 盲盒礼物
...
struct PointModelType: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func giftIncome(giftNum: Int) {
        //: self.num = giftNum
        self.num = giftNum
    }
}
...
struct HiddenModelType: HandyJSON {
    //: init() {
    init() {}
    ...
    mutating func mapping(mapper: HelpingMapper) {
        ...
        mapper <<<
            //: self.giftNum <-- "num"
            self.giftNum <-- (String(data_sexMediumPath))
    }
}
