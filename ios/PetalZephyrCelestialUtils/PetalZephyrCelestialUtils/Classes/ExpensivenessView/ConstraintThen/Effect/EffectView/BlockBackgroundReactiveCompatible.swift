...
fileprivate let user_incomeToMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "iosSvgaEffect" :*/
fileprivate let userColorContent:String = "index share start noriosSvg"
fileprivate let k_equalData:String = "CT"

/*: "iosEffect" :*/
fileprivate let const_indicatorFormat:[Character] = ["i","o","s","E","f","f","e","c","t"]

/*: "entityId" :*/
fileprivate let user_gameToMsg:String = "ENTIT"
fileprivate let dataNameStr:String = "yIdregular inside"
...
// __CLOSE_PRINT__
//
//  BlockBackgroundReactiveCompatible.swift
//  AbroadTalking
//
...
class BlockBackgroundReactiveCompatible: UIView {
    //: var currentEffectMsgModel: TalkingAnimatMsgModel?
    var currentEffectMsgModel: CountimateMsgModel?
    ...
    var animatedView: FillReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: user_incomeToMessage.reversed(), encoding: .utf8)!)
    }
...
extension BlockBackgroundReactiveCompatible {
    //: func playAnimation(infoDic: Dictionary<String, Any>) {
    func usufructuaryDic(infoDic: [String: Any]) {
        //: let dict = infoDic
        let dict = infoDic
        //: if dict.keys.isEmpty == false {
        if dict.keys.isEmpty == false {
            //: var iosEffect: String = dict["iosSvgaEffect"] as! String
            var iosEffect: String = dict[(String(userColorContent.suffix(6)) + "aEffe" + k_equalData.lowercased())] as! String
            //: if iosEffect.isEmptyString {
            if iosEffect.isEmptyString {
                //: iosEffect = dict["iosEffect"] as! String
                iosEffect = dict[(String(const_indicatorFormat))] as! String
            }

            //: let entityId = dict["entityId"] as! Int
            let entityId = dict[(user_gameToMsg.lowercased() + String(dataNameStr.prefix(3)))] as! Int
            ...
            if iosEffect.isEmptyString == false {
                ...
                let successs = quantityimateModel(msgModel: effectMsgModel)
            }
        }
    }
    ...
    func quantityimateModel(msgModel: Any?) -> Bool {
        ...
        if msgModel is CountimateMsgModel {
            //: effectMsgModel = msgModel as? TalkingAnimatMsgModel
            effectMsgModel = msgModel as? CountimateMsgModel
            //: } else if msgModel is TalkingGiftMsgAnimatModel {
        } else if msgModel is PointModelType {
            //: let giftMsgModel = msgModel as! TalkingGiftMsgAnimatModel
            let giftMsgModel = msgModel as! PointModelType
            ...
            if giftMsgModel.animationTimes > 1 {
                ...
                tmpGiftMsgModel.giftIncome(giftNum: 1) // 礼物个数设置为1，每次动效显示
                ...
                var repeatEfftect = CountimateMsgModel()
            ...
            effectMsgModel = CountimateMsgModel()
            ...
            if giftMsgModel.showType == AddFrameVarArg.myStery.rawValue || giftMsgModel.showType == AddFrameVarArg.king.rawValue {
        ...
        if self.currentEffectMsgModel != nil || paused {
            ...
            if effectMsgModel?.top ?? false && self.effectMsgModelArray.count > 0 {
                ...
                for i in 0 ..< self.effectMsgModelArray.count {
                    ...
                    let aModel: CountimateMsgModel = self.effectMsgModelArray[i] as! CountimateMsgModel
                    //: if aModel.top == false {
                    if aModel.top == false {
                        //: index = i
                        index = i
                ...
                if index == -1 {
                    ...
                    self.effectMsgModelArray.add(effectMsgModel ?? CountimateMsgModel())
                    //: } else {
                } else {
                    //: self.effectMsgModelArray.insert(effectMsgModel ?? TalkingAnimatMsgModel.init(), at: index)
                    self.effectMsgModelArray.insert(effectMsgModel ?? CountimateMsgModel(), at: index)
                }
            ...
            beauty(effectMsgModel: effectMsgModel!)
        }
...
extension BlockBackgroundReactiveCompatible {
    ...
    func arrow() {
        ...
        if self.currentEffectMsgModel == nil {
            ...
            if self.effectMsgModelArray.count > 0 {
                ...
                guard let removeModel = menuDoing() else { return }
                //: firstShow(effectMsgModel: removeModel)
                beauty(effectMsgModel: removeModel)
            }
            //: } else {
        } else {
            //: self.animatedView?.resumeAnimation()
            self.animatedView?.objectSession()
        }
    ...
    func menuDoing() -> CountimateMsgModel? {
        //: if (self.effectMsgModelArray.count == 0) {
        if self.effectMsgModelArray.count == 0 {
            //: return TalkingAnimatMsgModel.init()
            return CountimateMsgModel()
        }
        ...
        if effectMsgModel is CountimateMsgModel {
            //: let lastEffectMsgModel = effectMsgModel as! TalkingAnimatMsgModel
            let lastEffectMsgModel = effectMsgModel as! CountimateMsgModel
        ...
        return effectMsgModel as? CountimateMsgModel
    }
}
...
extension BlockBackgroundReactiveCompatible {
    ...
    func beauty(effectMsgModel: CountimateMsgModel) {
        ...
        self.delegate?.eraseMessage(effectView: self, obj: effectMsgModel)

        //: TalkingGiftAnimatTool.shared.loadEffectData(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
        ConstraintThen.shared.openBackwash(effectMsgModel: effectMsgModel, completion: { [weak self] success, giftModel in
            ...
            self.scope(effectMsgModel: effectMsgModel, success: success, effectModel: giftModel ?? ModelMeasurable())
            //: })
        })
    }
    ...
    func scope(effectMsgModel: CountimateMsgModel, success: Bool, effectModel: ModelMeasurable) {
        // 资源下载完成
        //: self.delegate?.animatViewWithDidEndLoadingResource(effectView: self, obj: effectMsgModel, success: success)
        self.delegate?.textTheory(effectView: self, obj: effectMsgModel, success: success)
        ...
        if success {
            ...
            if self.paused {
                ...
                if self.currentEffectMsgModel?.eid == effectMsgModel.eid, self.currentEffectMsgModel?.effectUrl == effectMsgModel.effectUrl {
                    ...
                    tagMaterialisation(effectMsgModel: effectMsgModel, model: effectModel)
                    //: return
                    return
                }
            }
    ...
    func responseShow(effectMsgModel: CountimateMsgModel, effectModel: ModelMeasurable) {
        //: if self.animatedView != nil {
        if self.animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.bagConk()
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.animatingSize()
        ...
        className = ConstraintThen.shared.streetName(oldName: className ?? "")
        ...
        let realClass = AnimatedViewClass as! FillReactiveCompatible.Type
...
extension BlockBackgroundReactiveCompatible: PlaceMusicThen {
    ...
    func giftCounteraction(effectItemView: FillReactiveCompatible, success: Bool) {
        ...
        self.delegate?.shortList(effectView: self, obj: effectItemView.effectMsgModel ?? CountimateMsgModel(), success: success)
    }
    ...
    func hackView(effectItemView: FillReactiveCompatible) {
        ...
        self.delegate?.fillLike(effectView: self, obj: effectMsgModel!)
    }
}
...
extension BlockBackgroundReactiveCompatible {
    ...
    func towardLabData() {
        ...
        if animatedView != nil {
            //: self.animatedView?.stopAnimating()
            self.animatedView?.bagConk() // stop animation
            //: self.animatedView?.cleanAnimating()
            self.animatedView?.animatingSize() // clean animation data
