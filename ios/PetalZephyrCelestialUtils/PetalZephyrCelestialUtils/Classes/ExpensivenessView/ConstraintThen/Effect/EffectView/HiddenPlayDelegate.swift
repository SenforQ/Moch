...
fileprivate let mainTableValue:[UInt8] = [0x16,0x11,0x16,0xb,0x57,0x1c,0x10,0x1b,0x1a,0xd,0x45,0x56,0x5f,0x17,0x1e,0xc,0x5f,0x11,0x10,0xb,0x5f,0x1d,0x1a,0x1a,0x11,0x5f,0x16,0x12,0xf,0x13,0x1a,0x12,0x1a,0x11,0xb,0x1a,0x1b]

/*: "%@.mp4" :*/
fileprivate let k_sectionName:[Character] = ["%","@",".","m","p","4"]

/*: "http" :*/
fileprivate let dataMakeMessage:[Character] = ["h","t","t","p"]
...
class HiddenPlayDelegate: FillReactiveCompatible {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainTableValue.map{$0^127}, encoding: .utf8)!)
    }
    ...
    public lazy var vapPlayerView: UIView? = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(announcementModel))
    ...
    override func lowness() {
        ...
        let path = ConstraintThen.shared.arrayFrom(fileName: self.giftEffectModel?.mainFile ?? "", model: self.effectMsgModel ?? CountimateMsgModel())

        //: if  path.count > 0 {
        if path.count > 0 {
            //: let mp4Path = String.init(format: "%@.mp4", path)
            let mp4Path = String(format: (String(k_sectionName)), path)
            ...
            self.delegate?.hackView(effectItemView: self)
        }
    }

    //: override func stopAnimating() {
    override func bagConk() {
        //: self.vapPlayerView?.stopHWDMP4()
        self.vapPlayerView?.stopHWDMP4()
    }

    //: override func cleanAnimating() {
    override func animatingSize() {
    ...
    @objc func announcementModel() {}
}

///  --- HWDMP4PlayDelegate

//: extension TalkingVapAnimationView: HWDMP4PlayDelegate {
extension HiddenPlayDelegate: HWDMP4PlayDelegate {
    ...
    func loadVapImage(withURL urlStr: String!, context _: [AnyHashable: Any]!, completion completionBlock: VAPImageCompletionBlock!) {
        ...
        DispatchQueue.main.async {
            ...
            if urlStr.hasPrefix((String(dataMakeMessage))) {
