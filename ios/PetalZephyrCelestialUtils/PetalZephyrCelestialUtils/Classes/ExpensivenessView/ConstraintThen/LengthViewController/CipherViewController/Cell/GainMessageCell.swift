...
fileprivate let noti_playerUrl:String = "TSystbegin image"
fileprivate let noti_bottomMessage:String = "geCetask establish row"
fileprivate let dataMaxUrl:String = "seIdneed white"

/*: "init(coder:) has not been implemented" :*/
fileprivate let showCellModelName:[UInt8] = [0xc4,0xc9,0xc4,0xcf,0x83,0xbe,0xca,0xbf,0xc0,0xcd,0x95,0x84,0x7b,0xc3,0xbc,0xce,0x7b,0xc9,0xca,0xcf,0x7b,0xbd,0xc0,0xc0,0xc9,0x7b,0xc4,0xc8,0xcb,0xc7,0xc0,0xc8,0xc0,0xc9,0xcf,0xc0,0xbf]

fileprivate func actualEqual(make num: UInt8) -> UInt8 {
...
/*: "icon_talk_notic" :*/
fileprivate let userAppText:String = "icoview"
fileprivate let userClubMessage:String = "iattribute"
...
@objc public protocol ButtonDelegate: NSObjectProtocol {
    /**
     *  长按消息回调
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onLongPressMessage(cell: GainMessageCell)
    func dismissCell(cell: GainMessageCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onRetryMessage(cell: GainMessageCell)
    func digitizerIcon(cell: GainMessageCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessage(cell: GainMessageCell)
    func serviceSearch(cell: GainMessageCell)
     ...
     *  @param cell 委托者，消息单元
     */
    //: func onSelectMessageAvatar(cell: GainMessageCell)
    func subHost(cell: GainMessageCell)

    /**
     *  点击消息单元中引用消息
     *  @param cell 委托者，消息单元
     */
    //: func onReplyMessage(cell: GainMessageCell)
    func soulAll(cell: GainMessageCell)
}

//: @objcMembers public class GainMessageCell: TCommonTableViewCell {
@objcMembers public class GainMessageCell: BeautyThen {
    /**
     *  信息数据类。
     *  存储了该massageCell中所需的信息。包括发送者 ID，发送者头像、信息发送状态、信息气泡图标等。
     *  messageData 详细信息请参考：Section\Chat\CellData\EqualCellData.h
     */
    //: public var messageData: EqualCellData?
    public var messageData: EqualCellData?

    /// 复用cell标识符
    //: @objc public static let TSystemMessageCell_ReuseId = "TSystemMessageCell_ReuseId"
    public static let TSystemMessageCell_ReuseId = (String(noti_playerUrl.prefix(5)) + "emMessa" + String(noti_bottomMessage.prefix(4)) + "ll_Reu" + String(dataMaxUrl.prefix(4)))

    /**
     *  协议委托
     *  负责实现 ButtonDelegate 协议中的功能。
     */
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: showCellModelName.map{actualEqual(make: $0)}, encoding: .utf8)!)
    }
    ...
    override public func compose(with data: DestabilisationCellData) {
        //: super.fill(with: data)
        super.compose(with: data)
        //: let newData: EqualCellData = data as! EqualCellData
        let newData: EqualCellData = data as! EqualCellData
    ...
    override public func updateConstraints() {
        ...
        let csize = self.messageData?.cover()
    ...
    lazy var avatarView: UIImageView = {
        ...
        let tap1 = UITapGestureRecognizer(target: self, action: #selector(sumRecognizer(recognizer:)))
    ...
    lazy var nameLabel: UILabel = {
        ...
        lb.font = UIFont.outView(fontSize: 13)
        //: lb.textColor = .gray
        lb.textColor = .gray
    ...
    public lazy var container: UIView = {
        ...
        let tap = UITapGestureRecognizer(target: self, action: #selector(startIn(recognizer:)))
        ...
        let longPress = UILongPressGestureRecognizer(target: self, action: #selector(afterPress(recognizer:)))
    ...
    lazy var retryView: UIImageView = {
        ...
        imagV.image = UIImage.atomicCount85(name: (userAppText.replacingOccurrences(of: "view", with: "n") + "_talk_not" + userClubMessage.replacingOccurrences(of: "attribute", with: "c")))
        ...
        let resendTap = UITapGestureRecognizer(target: self, action: #selector(recognizer(recognizer:)))
...
//: extension GainMessageCell {
extension GainMessageCell {
    //: @objc func onSelectMessageAvatar(recognizer: UIGestureRecognizer) {
    @objc func sumRecognizer(recognizer _: UIGestureRecognizer) {
        //: self.delegate?.onSelectMessageAvatar(cell: self)
        self.delegate?.subHost(cell: self)
    }

    //: @objc func onLongPress(recognizer: UIGestureRecognizer) {
    @objc func afterPress(recognizer: UIGestureRecognizer) {
    ...
    @objc func recognizer(recognizer _: UIGestureRecognizer) {
        ...
        if self.messageData?.status == .Msg_Status_Fail {
            ...
            self.delegate?.digitizerIcon(cell: self)
        }
    }

    //: @objc func onSelectMessage(recognizer: UIGestureRecognizer) {
    @objc func startIn(recognizer _: UIGestureRecognizer) {
