...
/*: "serialQueue" :*/
fileprivate let main_modelName:[Character] = ["s","e","r","i","a"]
fileprivate let constEditName:String = "profile normal manager temp makelQueue"

/*: "fromUids" :*/
fileprivate let const_conversationKey:[UInt8] = [0x3,0x17,0xa,0x8,0x30,0xc,0x1,0x16]

private func momentUser(time num: UInt8) -> UInt8 {
    return num ^ 101
}
...
let appDomainData = 50

//: @objcMembers
@objcMembers
//: public class DBUserInfoManager: NSObject {
public class AtReactiveCompatible: NSObject {
    ...
    private static var _instance: AtReactiveCompatible?
    //: class func shared() -> DBUserInfoManager {
    class func dropShared() -> AtReactiveCompatible {
        //: guard _instance != nil else {
        guard _instance != nil else {
            //: _instance = DBUserInfoManager()
            _instance = AtReactiveCompatible()
            // 添加退出账号通知
            //: NotificationCenter.default.addObserver(_instance!,
            NotificationCenter.default.addObserver(_instance!,
                                                   //: selector: #selector(accountLogout),
                                                   selector: #selector(imageSection),
    ...
    @objc private func imageSection() {
        ...
        AtReactiveCompatible.writePathDestroy()
    }
}
...
extension AtReactiveCompatible {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: class func cache_getUserInfo(with userIdsArr: [String], completion: @escaping ([LengthThen]?, Error?) -> Void) {
    class func executeCompletion(with userIdsArr: [String], completion: @escaping ([LengthThen]?, Error?) -> Void) {
        // 数量过多，wcdb的where语句会因过长而报错，所以做分页
        //: var newUserIdsArr = userIdsArr
        var newUserIdsArr = userIdsArr
        //: if DBUserInfoManager.shared().needReqUidsSet != nil {
        if AtReactiveCompatible.dropShared().needReqUidsSet != nil { // 合并去重
            //: newUserIdsArr = Array(DBUserInfoManager.shared().needReqUidsSet!.union(userIdsArr))
            newUserIdsArr = Array(AtReactiveCompatible.dropShared().needReqUidsSet!.union(userIdsArr))
            //: DBUserInfoManager.shared().needReqUidsSet = nil
            AtReactiveCompatible.dropShared().needReqUidsSet = nil
        }
        ...
        repeat {
            ...
            garner(with: subArr, completion: completion)
            //: location += len
            location += len
    ...
    class func clickToolCompletion(with userArr: [String], completion: @escaping ([[String: Any]]?, Error?) -> Void) {
        ...
        let serialQueue = DispatchQueue(label: (String(main_modelName) + String(constEditName.suffix(6))))
        ...
        serialQueue.async {
            ...
            for item in finalArray {
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.2) {
                    //: MonetaryValueThen.req_queryStatus(toUids: requestUidsStr) { _, list in
                    MonetaryValueThen.memberCompletion(toUids: requestUidsStr) { _, list in
                        //: defer { sema.signal() }
                        defer { sema.signal() }
    ...
    public class func magnitudeimate(message: V2TIMMessage) -> BeautyMsgTable {
        //: let key = TUISocialChatManager.checkPrimaryKey(withMsgModel: message)
        let key = SendReactiveCompatible.property(withMsgModel: message)
        //: var cacheWrap = WCDBVoiceMsgTable.db_getVoiceMsg(with: key)
        var cacheWrap = BeautyMsgTable.namePush(with: key)
        //: if cacheWrap == nil {
        if cacheWrap == nil {
            //: let info = TUISocialChatManager.packageDict(withTXMessage: message)
            let info = SendReactiveCompatible.contemporary(withTXMessage: message)
            //: cacheWrap = WCDBVoiceMsgTable.db_inserVoiceMsgToDB(info)
            cacheWrap = BeautyMsgTable.cost(info)
        }
        //: return cacheWrap!
        return cacheWrap!
    }
...
extension AtReactiveCompatible {
    ...
    //: @discardableResult
    @discardableResult
    //: class func cache_getCachedUserInfo(targetId: String) -> LengthThen? {
    class func viewText(targetId: String) -> LengthThen? {
        //: return WCDBUserInfoTable.db_getUserInfo(with: targetId)
        return ToInfoTable.nameWith(with: targetId)
    }

    /// 更新数据库【用户模型】，并发送通知
    /// - Parameter userInfo: 用户信息model
    //: class func cache_updateMsgUserInfoAndPostNotif(with userInfo: LengthThen?) {
    class func outPlay(with userInfo: LengthThen?) {
        ...
        let model = AtReactiveCompatible.viewText(targetId: userInfo.uid)
        // 自定义字段需要赋值，防止被覆盖
        ...
        ToInfoTable.videoClean(userInfo)
        //: DispatchQueue.main.async {
        DispatchQueue.main.async {
            //: NotificationCenter.default.post(name: USER_INFO_MSG_LIST_NOTIFICATION, object: [userInfo.uid: userInfo])
            NotificationCenter.default.post(name: showInformationUrl, object: [userInfo.uid: userInfo])
        }
    }
    ...
    class func lade(with targetId: String, intimate: Int, currTime: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = AtReactiveCompatible.viewText(targetId: targetId) else { return }
    ...
    class func colorGift(with targetId: String, time: Int) {
        //: guard let model = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId) else { return }
        guard let model = AtReactiveCompatible.viewText(targetId: targetId) else { return }
    ...
    class func withWrap(with targetUid: String) {
        //: WCDBUserInfoTable.db_deleteUserInfo(with: targetUid)
        ToInfoTable.deleteWith(with: targetUid)
        //: DBUserInfoManager.shared().reqedUidsSet.remove(targetUid)
        AtReactiveCompatible.dropShared().reqedUidsSet.remove(targetUid)
    }
}
...
extension AtReactiveCompatible {
    ...
    ///   - userIdsArr: 用户ids
    ///   - completion: 回调
    //: private class func inner_getUserInfo(with userIdsArr: [String], completion: @escaping ([LengthThen]?, Error?) -> Void) {
    private class func garner(with userIdsArr: [String], completion: @escaping ([LengthThen]?, Error?) -> Void) {
        //: let userModels = WCDBUserInfoTable.db_getUserInfos(with: userIdsArr)
        let userModels = ToInfoTable.voiceCount(with: userIdsArr)
        ...
        for uid in userIdsArr where !userModels.contains(where: { $0.uid == uid }) {
            ...
            if AtReactiveCompatible.dropShared().reqedUidsSet.contains(uid) == false {
        ...
        if !noDBUidArr.isEmpty {
            ...
            AtReactiveCompatible.pastTitle(with: noDBUidArr)
        }
    }
    ...
    private class func pastTitle(with userIdsArr: [String]) {
        ...
        AtReactiveCompatible.dropShared().reqedUidsSet = AtReactiveCompatible.dropShared().reqedUidsSet.union(userIdsArr)

        // 请求用户消息列表
        //: MonetaryValueThen.req_refreshMsgUserListInfo(param: ["fromUids": requestUidsStr]) { succeed, dataArr in
        MonetaryValueThen.makeCompletionShared(param: [String(bytes: const_conversationKey.map{momentUser(time: $0)}, encoding: .utf8)!: requestUidsStr]) { succeed, dataArr in
            //: guard succeed else {
            guard succeed else { // 失败还原数据
                //: DBUserInfoManager.shared().reqedUidsSet.subtract(userIdsArr)
                AtReactiveCompatible.dropShared().reqedUidsSet.subtract(userIdsArr)
                //: return
                return
            }
            // 更新数据库
            //: DBUserInfoManager.updateMsgUserInfoTable(userInfoArr: dataArr)
            AtReactiveCompatible.couvade(userInfoArr: dataArr)
            // 发送通知
            //: var retInfoDict: [String: LengthThen] = [:]
            var retInfoDict: [String: LengthThen] = [:]
            ...
            if !retInfoDict.isEmpty {
                ...
                DispatchQueue.main.async {
                    ...
                    NotificationCenter.default.post(name: showInformationUrl, object: retInfoDict)
                }
            }
        }
    }
    ...
    //: private class func updateMsgUserInfoTable(userInfoArr: [LengthThen]) {
    private class func couvade(userInfoArr: [LengthThen]) {
        //: for aWrap in userInfoArr {
        for aWrap in userInfoArr {
            //: WCDBUserInfoTable.db_updateUserInfo(aWrap)
            ToInfoTable.videoClean(aWrap)
        }
    }
}
