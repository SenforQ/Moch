...
/*: "DBUserInfoTable" :*/
fileprivate let showViewMsg:String = "index elseDBUser"
fileprivate let main_originNeedMsg:String = "InfoTablebeauty view size image"

/*: "WCDB表 :*/
fileprivate let userYouName:String = "WCDB表let raw selected"

/*: : 更新数据失败。error： :*/
fileprivate let notiItemKey:[Character] = [":"," ","更","新","数","据","失","败","。","e","r"]
fileprivate let const_starData:String = "cell change moment viewror："

/*: ." :*/
fileprivate let show_faceUrl:String = "."

/*: : 批量插入数据失败。error： :*/
fileprivate let main_headId:String = ": "
fileprivate let notiWithMessage:[Character] = ["批","量","插","\u{5165}","\u{6570}","据","失","\u{8d25}","。","e","r","r","o","r","："]
...
let user_errName = (String(showViewMsg.suffix(6)) + String(main_originNeedMsg.prefix(9)))
//: final class WCDBUserInfoTable: TableCodable {
final class ToInfoTable: TableCodable {
    /// 用户Id【主键】
    //: var uid: String = ""
    var uid: String = ""
    /// 昵称
...
// MARK: - LengthThen和WCDBUserInfoTable转换

//: extension WCDBUserInfoTable {
extension ToInfoTable {
    /// LengthThen  =>  ToInfoTable
    //: private class func mapToDBUserInfoData(_ userInfo: LengthThen) -> WCDBUserInfoTable {
    private class func dataConverter(_ userInfo: LengthThen) -> ToInfoTable {
        //: let dbData = WCDBUserInfoTable()
        let dbData = ToInfoTable()
    ...
    /// [ToInfoTable] => [LengthThen]
    //: private class func mapToUserInfoModels(_ dbDatas: [WCDBUserInfoTable]) -> [LengthThen] {
    private class func reverenceAdd(_ dbDatas: [ToInfoTable]) -> [LengthThen] {
        //: return dbDatas.map { data in
        return dbDatas.map { data in
            //: let userInfo = LengthThen()
            let userInfo = LengthThen()
...
extension ToInfoTable {
    ...
    class func deleteWith(with userId: String) {
        ...
        try? NowManager.shared.database?.run(transaction: { _ in
            ...
            let condition = ToInfoTable.Properties.uid.in(userId)
            //: try? WCDBManager.shared.database?.delete(fromTable: WCDBUserInfoTableName,
            try? NowManager.shared.database?.delete(fromTable: user_errName,
                                                     //: where: condition)
                                                     where: condition)
            //: })
        })
    ...
    /// - Parameter userId: 用户id
    /// - Returns: 模型
    //: class func db_getUserInfo(with userId: String) -> LengthThen? {
    class func nameWith(with userId: String) -> LengthThen? {
        //: let userInfos = WCDBUserInfoTable.db_getUserInfos(with: [userId])
        let userInfos = ToInfoTable.voiceCount(with: [userId])
    ...
    /// - Returns: 数据数组
    //: class func db_getUserInfos(with userIds: [String]) -> [LengthThen] {
    class func voiceCount(with userIds: [String]) -> [LengthThen] {
        //: guard userIds.count > 0 else { return [] }
        guard userIds.count > 0 else { return [] }
        //: do {
        do {
            //: let condition = WCDBUserInfoTable.Properties.uid.in(userIds)
            let condition = ToInfoTable.Properties.uid.in(userIds)
            ...
            let userInfos: [ToInfoTable]? = try NowManager.shared.database?.getObjects(on: ToInfoTable.Properties.all, fromTable: user_errName, where: condition)
            //: if userInfos?.isEmpty == false {
            if userInfos?.isEmpty == false {
                //: let models = WCDBUserInfoTable.mapToUserInfoModels(userInfos!)
                let models = ToInfoTable.reverenceAdd(userInfos!)
                //: return models
                return models
            }
            //: } catch {
    ...
    //: class func db_updateUserInfo(_ userInfo: LengthThen) {
    class func videoClean(_ userInfo: LengthThen) {
        //: let dbData = WCDBUserInfoTable.mapToDBUserInfoData(userInfo)
        let dbData = ToInfoTable.dataConverter(userInfo)
        //: try? WCDBManager.shared.database?.run(transaction: { _ in
        try? NowManager.shared.database?.run(transaction: { _ in
            // 数据库中已存在，则更新数据
            //: if WCDBUserInfoTable.db_getUserInfo(with: dbData.uid) != nil {
            if ToInfoTable.nameWith(with: dbData.uid) != nil {
                //: do {
                do {
                    //: let condition = WCDBUserInfoTable.Properties.uid == dbData.uid
                    let condition = ToInfoTable.Properties.uid == dbData.uid
                    //: try WCDBManager.shared.database?.update(table: WCDBUserInfoTableName,
                    try NowManager.shared.database?.update(table: user_errName,
                                                            //: on: WCDBUserInfoTable.Properties.all,
                                                            on: ToInfoTable.Properties.all,
                                                            //: with: dbData,
                                                            with: dbData,
                                                            //: where: condition)
                                                            where: condition)
                    ...
                    WoodLogTool.knowHow(msg: (String(userYouName.prefix(5))) + "\(user_errName)" + (String(notiItemKey) + String(const_starData.suffix(4))) + "\(error).")
                }
                //: } else {
            } else {
                // 数据库中不存在，则插入数据
                //: WCDBUserInfoTable.db_insertUserInfo(dbData)
                ToInfoTable.informationTitle(dbData)
            }
            //: })
        })
    }
    ...
    private class func informationTitle(_ userInfo: ToInfoTable) {
        //: WCDBUserInfoTable.db_insertUserInfos([userInfo])
        ToInfoTable.objectBy([userInfo])
    }

    /// 插入多条数据
    /// - Parameter userInfos: 数据数组
    //: private class func db_insertUserInfos(_ userInfos: [WCDBUserInfoTable]) {
    private class func objectBy(_ userInfos: [ToInfoTable]) {
        //: try? WCDBManager.shared.database?.run(transaction: { _ in
        try? NowManager.shared.database?.run(transaction: { _ in
            //: do {
            do {
                //: try WCDBManager.shared.database?.insert(userInfos, intoTable: WCDBUserInfoTableName)
                try NowManager.shared.database?.insert(userInfos, intoTable: user_errName)
                //: } catch {
            } catch {
                //: UploadLogTool.writeLog(msg: "WCDB表\(WCDBUserInfoTableName): 批量插入数据失败。error：\(error).")
                WoodLogTool.knowHow(msg: (String(userYouName.prefix(5))) + "\(user_errName)" + (main_headId.capitalized + String(notiWithMessage)) + "\(error).")
            }
            //: })
        })
    }
