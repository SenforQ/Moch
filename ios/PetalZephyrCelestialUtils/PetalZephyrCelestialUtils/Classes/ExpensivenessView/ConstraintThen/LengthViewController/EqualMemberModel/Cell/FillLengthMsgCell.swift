...
fileprivate let appLabTitle:[UInt8] = [0x2e,0x29,0x2e,0x33,0x6f,0x24,0x28,0x23,0x22,0x35,0x7d,0x6e,0x67,0x2f,0x26,0x34,0x67,0x29,0x28,0x33,0x67,0x25,0x22,0x22,0x29,0x67,0x2e,0x2a,0x37,0x2b,0x22,0x2a,0x22,0x29,0x33,0x22,0x23]

private func viewUser(frame num: UInt8) -> UInt8 {
    return num ^ 71
}

/*: "icon_chat_start" :*/
fileprivate let kLabMsg:String = "icon_cbottom height image return share"
fileprivate let dataFeatureFormat:[Character] = ["h","a","t","_","s","t","a","r","t"]

/*: "icon_chat_lock" :*/
fileprivate let constValueViewData:String = "icon_cself key to block"
fileprivate let dataStatusValue:[Character] = ["o","c","k"]

/*: "icon_chat_burned" :*/
fileprivate let noti_modeTitle:[Character] = ["i","c","o","n","_","c","h","a","t","_","b","u","r"]
fileprivate let dataSpringMsg:String = "onceed"

/*: "icon_chat_burned_shan" :*/
fileprivate let appValueId:[Character] = ["i","c","o"]
fileprivate let k_greetMessage:[Character] = ["n","_","c","h","a","t","_","b","u","r","n","e","d","_","s","h","a","n"]

/*: "Already burned" :*/
fileprivate let app_contentValue:[Character] = ["A","l","r","e","a","d","y"," ","b"]
fileprivate let app_warningMessage:[Character] = ["u","r","n","e","d"]
...
class FillLengthMsgCell: InfoViewCell {
    //: var videoData: AuditoryImageCellData?
    var videoData: AuditoryImageCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: appLabTitle.map{viewUser(frame: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var playImgView: UIImageView = {
        ...
        imgV.image = UIImage.atomicCount85(name: (String(kLabMsg.prefix(6)) + String(dataFeatureFormat)))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var lockImgView: UIImageView = {
        ...
        imgV.image = UIImage.atomicCount85(name: (String(constValueViewData.prefix(6)) + "hat_l" + String(dataStatusValue)))
        //: return imgV
        return imgV
        //: }()
    }()
    ...
    private lazy var burnedImgView: BlockButton = {
        ...
        imgV.setBackgroundImage(UIImage.atomicCount85(name: (String(noti_modeTitle) + dataSpringMsg.replacingOccurrences(of: "once", with: "n"))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.atomicCount85(name: (String(appValueId) + String(k_greetMessage))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(app_contentValue) + String(app_warningMessage)).localized, for: .normal)
...
extension FillLengthMsgCell {
    ...
    override func compose(with data: DestabilisationCellData) {
        ...
        self.dutationLab.text = NSDate.brimful(totalTime: duration)
        ...
        self.totalimateMake(urlStr: videoModel.coverImg, isMosaic: imgIsMosaic)
    }
    ...
    func totalimateMake(urlStr: String, isMosaic: Bool) {
        //: guard let newData = self.videoData else { return }
        guard let newData = self.videoData else { return }
        ...
        if !coverImgPath.isEmptyString, isExit { // 加载本地
            ...
            //: if isMosaic {
            if isMosaic {
                //: self.picImgView.image = UIImage.with(image, exceptCurrent: 90)
                self.picImgView.image = UIImage.with(image, exceptCurrent: 90)
                //: } else {
            } else {
            ...
            self.picImgView.sexDown(urlStr: urlStr) { [weak self] image in
