...
fileprivate let appVideoTitle:[UInt8] = [0x3e,0x39,0x3e,0x23,0x7f,0x34,0x38,0x33,0x32,0x25,0x6d,0x7e,0x77,0x3f,0x36,0x24,0x77,0x39,0x38,0x23,0x77,0x35,0x32,0x32,0x39,0x77,0x3e,0x3a,0x27,0x3b,0x32,0x3a,0x32,0x39,0x23,0x32,0x33]

/*: "View read receipts?" :*/
fileprivate let constCustomModelImageId:String = "View title i to inside"
fileprivate let kTextContent:[Character] = ["e","c","e","i","p","t","s","?"]

/*: "text" :*/
fileprivate let userNameTitle:[Character] = ["t","e","x","t"]

/*: "color" :*/
fileprivate let main_sexTempContent:[UInt8] = [0xbf,0xb3,0xb0,0xb3,0xae]

/*: "<br/>" :*/
fileprivate let user_contextFormat:[Character] = ["<","b","r","/",">"]

/*: "\n" :*/
fileprivate let user_collectionImageModelPath:[Character] = ["\n"]

/*: "<br />" :*/
fileprivate let showTableHaveData:String = "<"
fileprivate let show_costFormat:String = "br />to left style need view"

/*: "<xml>%@</xml>" :*/
fileprivate let show_rawId:[Character] = ["<","x","m","l",">","%","@","<","/","x","m","l",">"]

/*: "href" :*/
fileprivate let main_curKey:[Character] = ["h","r","e","f"]

/*: "type" :*/
fileprivate let mainUserId:[UInt8] = [0x65,0x70,0x79,0x74]

/*: "txt" :*/
fileprivate let appUpPath:String = "tsend"

/*: "padding" :*/
fileprivate let kPairUrl:String = "plimitdding"

/*: "size" :*/
fileprivate let constViewStr:String = "actionze"

/*: "maxFont" :*/
fileprivate let dataLengthContent:[Character] = ["m","a","x","F","o","n","t"]

/*: "link" :*/
fileprivate let show_giftMessage:String = "lpik"

/*: "PingFangSC-Regular" :*/
fileprivate let dataModelLayerPath:String = "if path corner path classPingFan"
fileprivate let app_normalStr:String = "background"
fileprivate let dataLengthTextFormat:[Character] = ["S","C","-","R","e","g","u","l","a","r"]

/*: "content" :*/
fileprivate let mainViewMpFormat:[UInt8] = [0x3a,0x36,0x37,0x2d,0x3c,0x37,0x2d]

private func giftValue(cur num: UInt8) -> UInt8 {
    return num ^ 89
}

/*: "%@" :*/
fileprivate let showRefreshMessage:[Character] = ["%","@"]
...
@objcMembers public class ContentThen: PriceCellData {
    ...
    override public func cover() -> CGSize {
        ...
        if self.msgModel.tips.content == (String(constCustomModelImageId.prefix(5)) + "read r" + String(kTextContent)).localized {
            //: return CGSize(width: ScreenWidth, height: 30)
            return CGSize(width: constExplainViewMsg, height: 30)
        }
        ...
        //: let rect = self.attributedString.boundingRect(with: CGSize.init(width: LineThen.getScreenWidth()-50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil )
        let rect = self.attributedString.boundingRect(with: CGSize(width: LineThen.dataWidth() - 50, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil)
    ...
    public lazy var attributedString: NSMutableAttributedString = {
        ...
        var attr = clerestory(info: self.msgModel.tips)
        //: return attr
        return attr
        //: }()
    }()
...
extension ContentThen {
    ...
    func clerestory(info: TimeDailyModel) -> NSMutableAttributedString {
...
//        let dic = NSDictionary(jsonString: extra ?? "")
        ...
        return naturalEvent(tips: content)
    }

    //: func designTextFream(tips: String) -> NSMutableAttributedString {
    func naturalEvent(tips: String) -> NSMutableAttributedString {
        ...
        _ = placeArray(msg: tips, info: attrbutedArray)
        //: self.yyLayout = designYYTextLayout(array: attrbutedArray, displayMaxWidth: LineThen.getScreenWidth()-50, para: para)
        self.yyLayout = equalHandle(array: attrbutedArray, displayMaxWidth: LineThen.dataWidth() - 50, para: para)
    ...
    func placeArray(msg: String, info: NSMutableArray) -> String {
        ...
        let pure_msg = parse(source: msg, attrbAM: contentAttrArray)
        //: for  textInfo in contentAttrArray {
        for textInfo in contentAttrArray {
            //: let textDic = textInfo as![String: Any]
            let textDic = textInfo as! [String: Any]
            //: let text: String = textDic["text"] as! String
            let text: String = textDic[(String(userNameTitle))] as! String

            //: var textColor: UIColor?
            var textColor: UIColor?

            //: var obj = textDic["color"]
            var obj = textDic[String(bytes: main_sexTempContent.map{$0^220}, encoding: .utf8)!]
            //: if obj is UIColor {
            if obj is UIColor {
                //: obj = obj as? UIColor
                obj = obj as? UIColor
            ...
            contentDict.removeObjects(forKeys: [String(bytes: main_sexTempContent.map{$0^220}, encoding: .utf8)!, (String(userNameTitle))])

            //: let attrDict = ctDictionaryForText(content: text, color: textColor)
            let attrDict = atFor(content: text, color: textColor)
    ...
    func parse(source: String, attrbAM: NSMutableArray) -> String {
        ...
        do {
            ...
            let document = try ONOXMLDocument(string: String(format: (String(show_rawId)), newSource), encoding: CFStringConvertEncodingToNSStringEncoding(CFStringBuiltInEncodings.UTF8.rawValue))
            //: let element = document.rootElement
            let element = document.rootElement
            //: parseXMLElement(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: ["color": UIColor.white])
            playAt(element: element ?? ONOXMLElement(), arrM: attrbAM, attrib: [String(bytes: main_sexTempContent.map{$0^220}, encoding: .utf8)!: UIColor.white])
            ...
            attrbAM.add([(String(userNameTitle)): newSource, String(bytes: main_sexTempContent.map{$0^220}, encoding: .utf8)!: UIColor.white])
            //: return newSource
            return newSource
        }
    }
    ...
    func playAt(element: ONOXMLElement, arrM: NSMutableArray, attrib: NSDictionary) {
        ...
        if children.count > 0 {
            ...
            if element.tag == "u" { // 下滑线
                ...
                let url: String = element.attributes[(String(main_curKey))] as! String
                //: if url.count > 0 {
                if url.count > 0 {
                    //: parentInfo["href"] = url
                    parentInfo[(String(main_curKey))] = url
                }
            }
            //: for  subElement in children {
            for subElement in children {
                //: parseXMLElement(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
                playAt(element: subElement as! ONOXMLElement, arrM: arrM, attrib: parentInfo)
            }
            ...
            if text != nil {
                ...
                //: info["text"] = text
                info[(String(userNameTitle))] = text
                //: arrM.add(info)
                arrM.add(info)
            }
        }
    ...
    func equalHandle(array: NSArray, displayMaxWidth: CGFloat, para: NSParagraphStyle?) -> YYTextLayout {
        //: let attrString = designAttribStr(array: array, paragraphStyle: para)
        let attrString = actionFor(array: array, paragraphStyle: para)
    ...
    func actionFor(array: NSArray, paragraphStyle: NSParagraphStyle?) -> NSAttributedString {
        ...
        array.enumerateObjects { obj, _, _ in
            ...
            let type = newObj[String(bytes: mainUserId.reversed(), encoding: .utf8)!] as? String
            //: var tmp = NSAttributedString.init()
            var tmp = NSAttributedString()
            //: if type  == "txt" {
            if type == (appUpPath.replacingOccurrences(of: "send", with: "xt")) {
                //: tmp = designAttribStrByTextDic(dict: newObj as NSDictionary)
                tmp = instalment(dict: newObj as NSDictionary)
                //: } else if type == "padding" {
            } else if type == (kPairUrl.replacingOccurrences(of: "limit", with: "a")) {
                //: let imageView = UIImageView.init()
                let imageView = UIImageView()
                //: imageView.size = (newObj["size"] as! NSValue).cgSizeValue
                imageView.size = (newObj[(constViewStr.replacingOccurrences(of: "action", with: "si"))] as! NSValue).cgSizeValue
                ...
                tmp = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: newObj[(String(dataLengthContent))] as! UIFont, alignment: .center)
            }
            ...
            if underline {
                ...
                if len > 0 {
                    ...
                    _ = YYTextDecoration(style: .single, width: 1.5, color: newObj[String(bytes: main_sexTempContent.map{$0^220}, encoding: .utf8)!] as? UIColor)
    ...
    func atDict(dict: NSDictionary) -> NSAttributedString {
        //: let imageView = UIImageView.init()
        let imageView = UIImageView()
        //: imageView.size = (dict["size"]as! NSValue).cgSizeValue
        imageView.size = (dict[(constViewStr.replacingOccurrences(of: "action", with: "si"))] as! NSValue).cgSizeValue
        ...
        let attachText = NSMutableAttributedString.yy_attachmentString(withContent: imageView, contentMode: .center, attachmentSize: imageView.size, alignTo: dict[(String(dataLengthContent))] as! UIFont, alignment: .center)
        //: return attachText
        return attachText
    }

    //: func designAttribStrByTextDic(dict: NSDictionary) ->NSMutableAttributedString {
    func instalment(dict: NSDictionary) -> NSMutableAttributedString {
        //: let type = dict["type"] as! String
        let type = dict[String(bytes: mainUserId.reversed(), encoding: .utf8)!] as! String
        //: if type != "link" && type != "txt" {
        if type != (show_giftMessage.replacingOccurrences(of: "pi", with: "in")) && type != (appUpPath.replacingOccurrences(of: "send", with: "xt")) {
        ...
        let color = dict[String(bytes: main_sexTempContent.map{$0^220}, encoding: .utf8)!] as? UIColor
        //: if color != nil {
        if color != nil {
        ...
        let fontSize = (dict[(constViewStr.replacingOccurrences(of: "action", with: "si"))] as? NSNumber)?.floatValue
        //: if (fontSize ?? 0 > 0) {
        if fontSize ?? 0 > 0 {
            //: let fontRef = CTFontCreateWithName("PingFangSC-Regular" as CFString, CGFloat(fontSize!), nil)
            let fontRef = CTFontCreateWithName((String(dataModelLayerPath.suffix(7)) + app_normalStr.replacingOccurrences(of: "background", with: "g") + String(dataLengthTextFormat)) as CFString, CGFloat(fontSize!), nil)
            //: attributes[kCTFontAttributeName] = fontRef
            attributes[kCTFontAttributeName] = fontRef
        }
        //: var content = (dict["content"] != nil) ? dict["content"] : ""
        var content = (dict[String(bytes: mainViewMpFormat.map{giftValue(cur: $0)}, encoding: .utf8)!] != nil) ? dict[String(bytes: mainViewMpFormat.map{giftValue(cur: $0)}, encoding: .utf8)!] : ""
    ...
    func attributes() -> NSMutableDictionary {
        ...
        let font = UIFont.labelWith(fontSize: 15)
    ...
    func atFor(content: String, color: UIColor?) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
        ...
        if color == nil {
            ...
            return scratch(content: content)
        }
        //: return ["type": "txt",
        return [String(bytes: mainUserId.reversed(), encoding: .utf8)!: (appUpPath.replacingOccurrences(of: "send", with: "xt")),
                //: "content": content,
                String(bytes: mainViewMpFormat.map{giftValue(cur: $0)}, encoding: .utf8)!: content,
                //: "color": color ?? .black]
                String(bytes: main_sexTempContent.map{$0^220}, encoding: .utf8)!: color ?? .black]
    }

    //: func ctDictionaryForText(content: String) ->NSDictionary {
    func scratch(content: String) -> NSDictionary {
        //: if content.count <= 0 {
        if content.count <= 0 {
            //: return NSDictionary.init()
            return NSDictionary()
