...
fileprivate let k_labelTitle:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "[未知消息]" :*/
fileprivate let kNumberTitle:String = "[未知消息]"

/*: "extra" :*/
fileprivate let show_keyMsg:String = "gifttra"

/*: "</at> " :*/
fileprivate let k_randomName:[Character] = ["<","/","a","t",">"]
fileprivate let mainSessionStr:[Character] = [" "]

/*: "#0084FF" :*/
fileprivate let appLabelUrl:String = "add succeed time status video#0084FF"

/*: "#FFEE4B" :*/
fileprivate let dataSuiteValue:String = "model at#FFEE4B"

/*: "<at>@[\\S\\s]+?</at>" :*/
fileprivate let show_explainGiftTitle:String = "<at>@[remove in to"
fileprivate let user_titleMsg:String = "\\S"
fileprivate let userShowCurName:[Character] = ["\\","s","]","+","?","<","/","a","t",">"]

/*: "jumpKey" :*/
fileprivate let notiTopMessage:String = "jumpKeyuser self"

/*: "user" :*/
fileprivate let showMethodText:[Character] = ["u","s","e","r"]

/*: "uid" :*/
fileprivate let mainPlayArrayId:[UInt8] = [0xaf,0xb3,0xbe]

private func directionNext(like num: UInt8) -> UInt8 {
    return num ^ 218
}

/*: "#16D073" :*/
fileprivate let dataLessImageSelectName:String = "empty name let content cell#16D073"

/*: "#F95644" :*/
fileprivate let show_countTitle:String = "push finish#F95644"
...
//: @objcMembers public class FillDeleteThen: TalkingChatMsgBaseCellData {
@objcMembers public class FillDeleteThen: PriceCellData {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_labelTitle.reversed(), encoding: .utf8)!)
    }
    ...
    override public var msgModel: ChatModel {
        ...
        set {
            ...
            if newValue.msgInfo.content.isEmptyString {
                //: contentStr = "[未知消息]"
                contentStr = (kNumberTitle.capitalized)
                //: }else {
            } else {
    ...
    public lazy var contentAttrString: NSMutableAttributedString = {
        ...
        let attr = become(msgInfoModel: self.msgModel.msgInfo)
        //: return attr
        return attr
        //: }()
    }()
    ...
    func become(msgInfoModel: DailyModel) -> NSMutableAttributedString {
        ...
        guard dic[(show_keyMsg.replacingOccurrences(of: "gift", with: "ex"))] is [String: Any] else {
            //: return NSMutableAttributedString.init(string: "")
            return NSMutableAttributedString(string: "")
        }
        ...
        let textColor = (self.direction == .MsgDirectionIncoming) ? UIColor.towardOf() : .white
        //: let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: "#0084FF")! : UIColor(hex: "#FFEE4B")!
        let atColor = (self.direction == .MsgDirectionIncoming) ? UIColor(hex: (String(appLabelUrl.suffix(7))))! : UIColor(hex: (String(dataSuiteValue.suffix(7))))!
        ...
        let regex = try! NSRegularExpression(pattern: (String(show_explainGiftTitle.prefix(6)) + user_titleMsg + String(userShowCurName)), options: [])
        ...
        for (index, value) in rangeArr.enumerated() {
            ...
            if jumpInfo[(String(notiTopMessage.prefix(7)))].stringValue == (String(showMethodText)) {
                //: uid = jumpInfo["uid"].intValue
                uid = jumpInfo[String(bytes: mainPlayArrayId.map{directionNext(like: $0)}, encoding: .utf8)!].intValue
            }
            //: if uid != -1 {
            if uid != -1 {
                ...
                attr.yy_setTextHighlight(range, color: atColor, backgroundColor: .clear, userInfo: [String(bytes: mainPlayArrayId.map{directionNext(like: $0)}, encoding: .utf8)!: uid]) { [weak self] _, _, _, _ in
    ...
    func language() -> NSMutableAttributedString {
        ...
        var color = UIColor(hex: (String(dataLessImageSelectName.suffix(7))))!
        //: if self.translateStatus == .MsgTranslate_fail {
        if self.translateStatus == .MsgTranslate_fail {
            //: color = UIColor(hex: "#F95644")!
            color = UIColor(hex: (String(show_countTitle.suffix(7))))!
        }
