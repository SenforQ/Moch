...
fileprivate let userNameKey:[UInt8] = [0x73,0x78,0x73,0x7e,0x32,0x6d,0x79,0x6e,0x6f,0x7c,0x44,0x33,0x2a,0x72,0x6b,0x7d,0x2a,0x78,0x79,0x7e,0x2a,0x6c,0x6f,0x6f,0x78,0x2a,0x73,0x77,0x7a,0x76,0x6f,0x77,0x6f,0x78,0x7e,0x6f,0x6e]

fileprivate func infoMp(touch num: UInt8) -> UInt8 {
...
/*: "all" :*/
fileprivate let show_removeBlockMessage:[Character] = ["a","l","l"]
...
class ComponentViewCell: InfoViewCell {
    //: var giftData: TalkingGroupChatGiftMsgCellData?
    var giftData: ModelReactiveCompatible?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userNameKey.map{infoMp(touch: $0)}, encoding: .utf8)!)
    }
...
extension ComponentViewCell {
    ...
    override func compose(with data: DestabilisationCellData) {
        ...
        self.giftData = data as? ModelReactiveCompatible
        ...
        if giftData.msgModel.toUser.uid == (String(show_removeBlockMessage)) { // 送礼所有人
            //: self.toUserImgView.image = UIImage.BundleImageNamed(name: giftData.msgModel.toUser.headPic)
            self.toUserImgView.image = UIImage.atomicCount85(name: giftData.msgModel.toUser.headPic)
            //: } else {
        } else {
            //: self.toUserImgView.setUrlImage(urlStr: giftData.msgModel.toUser.headPic)
            self.toUserImgView.viewFinish(urlStr: giftData.msgModel.toUser.headPic)
        }
    }
