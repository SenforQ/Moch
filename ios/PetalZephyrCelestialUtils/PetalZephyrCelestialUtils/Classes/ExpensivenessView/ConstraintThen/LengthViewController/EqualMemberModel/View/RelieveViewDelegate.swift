...
fileprivate let userMaleData:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "0/ :*/
fileprivate let userSectionTitle:[Character] = ["0","/"]

/*: "#AAAAAA" :*/
fileprivate let data_appText:[Character] = ["#","A","A","A","A","A","A"]

/*: "btn_chatsettings_unchoice" :*/
fileprivate let noti_moreValue:String = "in make path text tobtn_c"
fileprivate let dataInputKey:String = "moontting"
fileprivate let show_messageTitle:[Character] = ["s"]
fileprivate let constMediumData:[Character] = ["_","u","n","c","h","o","i","c","e"]

/*: "btn_chatsettings_choiced" :*/
fileprivate let main_sendUrl:String = "btn_clayer make target"
fileprivate let show_contentKey:[Character] = ["i","n","g","s","_","c","h","o","i","c","e","d"]

/*: "Top" :*/
fileprivate let kCollectionToValueTitle:String = "Topmodel var"

/*: "Save" :*/
fileprivate let main_tableText:String = "Savegift self style app"

/*: "Cancel" :*/
fileprivate let noti_perceptName:[Character] = ["C","a","n","c","e"]
fileprivate let mainExplainData:[Character] = ["l"]

/*: / :*/
fileprivate let userListId:[Character] = ["/"]
...
class RelieveViewDelegate: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: userMaleData.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var contentView: UIView = {
        ...
        let height = actualWidth(w: 475) + noti_limitPartyInfoUrl
        //: let v = UIView(frame: CGRect(x: 0, y: ScreenHeight-height, width: ScreenWidth, height: height))
        let v = UIView(frame: CGRect(x: 0, y: user_deviceId - height, width: constExplainViewMsg, height: height))
        //: v.backgroundColor = .white
        v.backgroundColor = .white
        //: v.Corner(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        v.hauntWithoutRadii(width: v.width, height: v.height, corners: [.topLeft, .topRight], cornerRadii: .init(width: 15, height: 15))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var textBgView: UIView = {
        ...
        v.backgroundColor = UIColor.objectVideoBurn()
    ...
    private lazy var limitLab: UILabel = {
        ...
        lab.textColor = UIColor(hex: (String(data_appText)))
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var topBtn: BlockButton = {
        ...
        btn.setImage(UIImage.atomicCount85(name: (String(noti_moreValue.suffix(5)) + "hats" + dataInputKey.replacingOccurrences(of: "moon", with: "e") + String(show_messageTitle) + String(constMediumData))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_chatsettings_choiced"), for: .selected)
        btn.setImage(UIImage.atomicCount85(name: (String(main_sendUrl.prefix(5)) + "hatsett" + String(show_contentKey))), for: .selected)
        ...
        btn.setTitle((String(kCollectionToValueTitle.prefix(3))).localized, for: .normal)
    ...
    private lazy var saveBtn: UIButton = {
        ...
        btn.setTitle((String(main_tableText.prefix(4))).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(rescue), for: .touchUpInside)
    ...
    private lazy var cancelBtn: UIButton = {
        ...
        btn.setTitle((String(noti_perceptName) + String(mainExplainData)).localized, for: .normal)
        ...
        btn.addTarget(self, action: #selector(videoItem), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
...
extension RelieveViewDelegate: YYTextViewDelegate {
...
extension RelieveViewDelegate {
    /// 保存按钮点击事件
    //: @objc private func saveButtonClick() {
    @objc private func rescue() {
        //: saveBtn.isEnabled = false
        saveBtn.isEnabled = false
        //: let isTop = topBtn.isSelected ? 1 : 2
        let isTop = topBtn.isSelected ? 1 : 2
        //: MonetaryValueThen.req_remark(content: textView.text, isTop: isTop, toUid: targetId) {
        MonetaryValueThen.giveUp(content: textView.text, isTop: isTop, toUid: targetId) {
            //: succeed, _, _ in
            succeed, _, _ in
    ...
    @objc private func buttonToDelete() {
    ...
    func actionAdd(_ content: String, isTop: Int, toUid: String) {
        //: targetId = toUid
        targetId = toUid
...
extension RelieveViewDelegate {
    ...
    private func namePlaceSubviews() {
