...
fileprivate let mainPlayerMsg:[UInt8] = [0x2a,0x2d,0x2a,0x37,0x6b,0x20,0x2c,0x27,0x26,0x31,0x79,0x6a,0x63,0x2b,0x22,0x30,0x63,0x2d,0x2c,0x37,0x63,0x21,0x26,0x26,0x2d,0x63,0x2a,0x2e,0x33,0x2f,0x26,0x2e,0x26,0x2d,0x37,0x26,0x27]

private func imageCover(corner num: UInt8) -> UInt8 {
    return num ^ 67
}

/*: "icon_chats_banner_subtract" :*/
fileprivate let k_eventText:String = "if mask to ificon_c"
fileprivate let userMakeText:String = "hsectionts"
fileprivate let constTopKey:[Character] = ["e","r","_","s","u","b","t","r","a","c","t"]

/*: "icon_chats_banner_subtract_pre" :*/
fileprivate let k_cellValue:[Character] = ["i","c","o","n","_","c","h","a","t","s","_","b","a","n","n","e","r","_","s","u","b","t","r","a"]
fileprivate let showManagerLabelUrl:String = "ct_preview app create"

/*: "bannerList" :*/
fileprivate let const_priceFormat:[Character] = ["b","a","n","n","e","r","L","i"]
fileprivate let k_modelValue:String = "emptyt"

/*: "JXBannerCellID" :*/
fileprivate let data_viewName:[Character] = ["J","X","B","a","n"]
fileprivate let app_centerData:[Character] = ["n","e","r","C"]
fileprivate let kUpData:String = "to float view heightellID"

/*: "#FFFFFF" :*/
fileprivate let constVideoLeadingBackgroundId:[Character] = ["#","F","F","F","F","F","F"]
...
protocol ThreadBannerDelegate: NSObject {
    //: func changeBannerViewFream(isMiniView: Bool)
    func smallShow(isMiniView: Bool)
}

//: class TalkingPrivateBannerView: UIView {
class ValueBannerDelegate: UIView {
    //: var isMiniView = false
    var isMiniView = false
    //: open weak var delegate: PrivateBannerDelegate?
    open weak var delegate: ThreadBannerDelegate?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainPlayerMsg.map{imageCover(corner: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var bannerListData: [SocietalTitleDivinatoryRecitalModelType] = //: return Array<SocietalTitleDivinatoryRecitalModelType>()
        .init()
    //: }()
    ...
    private lazy var bannerView: JXBanner = {
        ...
        banner.placeholderImgView.image = UIImage.labWrite()
    ...
    private lazy var closeBtn: UIButton = {
        ...
        btn.setImage(UIImage.atomicCount85(name: (String(k_eventText.suffix(6)) + userMakeText.replacingOccurrences(of: "section", with: "a") + "_bann" + String(constTopKey))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_chats_banner_subtract_pre"), for: .selected)
        btn.setImage(UIImage.atomicCount85(name: (String(k_cellValue) + String(showManagerLabelUrl.prefix(6)))), for: .selected)
...
extension ValueBannerDelegate {
    /// banner
    //: func reqBanner() {
    func someAwakeWith() {
        //: SocialRequestManager.init().req_adBanner(position: 3) { [weak self] succeed, result, errorModel in
        NameRequestManager().userCompletion(position: 3) { [weak self] _, result, _ in
            ...
            let array = JSON(result[(String(const_priceFormat) + k_modelValue.replacingOccurrences(of: "empty", with: "s"))] ?? []).arrayValue
...
extension ValueBannerDelegate {
    ...
    @objc private func totaleraction() {
        ...
        self.delegate?.smallShow(isMiniView: isMiniView)
    }

    //: func resetView(isMiniView: Bool) {
    func draftCopyImage(isMiniView: Bool) {
        //: if isMiniView {
        if isMiniView {
...
extension ValueBannerDelegate: JXBannerDelegate, JXBannerDataSource {
    ...
    func jxBanner(_: JXBannerType) -> (JXBannerCellRegister) {
        ...
        return JXBannerCellRegister(type: JXBannerCell.self, reuseIdentifier: (String(data_viewName) + String(app_centerData) + String(kUpData.suffix(5))))
    }
    ...
    func jxBanner(_: JXBannerType, cellForItemAt index: Int, cell: UICollectionViewCell) -> UICollectionViewCell {
        ...
        tempCell.imageView.luculentUpwardly(urlStr: url, placeImg: UIImage.warning())
                  ...
                  builder: JXBannerPageControlBuilder) -> JXBannerPageControlBuilder
    {
        ...
        pageControl.activeColor = UIColor(hex: (String(constVideoLeadingBackgroundId)))!
...
extension ValueBannerDelegate {
    ...
    private func setupAndWearingApparel() {
