...
fileprivate let app_sPopData:[UInt8] = [0x65,0x62,0x65,0x78,0x24,0x6f,0x63,0x68,0x69,0x7e,0x36,0x25,0x2c,0x64,0x6d,0x7f,0x2c,0x62,0x63,0x78,0x2c,0x6e,0x69,0x69,0x62,0x2c,0x65,0x61,0x7c,0x60,0x69,0x61,0x69,0x62,0x78,0x69,0x68]

/*: "icon_chat_lock" :*/
fileprivate let k_frontEditName:String = "icon_cmake if"

/*: "icon_chat_burned" :*/
fileprivate let mainFinishFormat:[Character] = ["i","c","o","n","_","c","h","a","t","_","b","u","r","n","e","d"]

/*: "icon_chat_burned_shan" :*/
fileprivate let userContainerKey:String = "isendn"
fileprivate let show_succeedPathLetData:String = "regardrne"
fileprivate let main_fileContent:String = "D"
fileprivate let mainFloorName:String = "_shanarray snap color main"

/*: "Already burned" :*/
fileprivate let appSearchResultMsg:[Character] = ["A","l","r","e","a","d","y"," "]
fileprivate let notiEqualMessage:String = "BURNED"
...
class TelevisionReactiveCompatible: InfoViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: app_sPopData.map{$0^12}, encoding: .utf8)!)
    }
    ...
    lazy var lockImagV: UIImageView = {
        ...
        imag.image = UIImage.atomicCount85(name: (String(k_frontEditName.prefix(6)) + "hat_lock"))
    ...
    lazy var burnedImagV: BlockButton = {
        ...
        imgV.setBackgroundImage(UIImage.atomicCount85(name: (String(mainFinishFormat))), for: .normal)
        //: imgV.setImage(UIImage.BundleImageNamed(name: "icon_chat_burned_shan"), for: .normal)
        imgV.setImage(UIImage.atomicCount85(name: (userContainerKey.replacingOccurrences(of: "send", with: "co") + "_chat_" + show_succeedPathLetData.replacingOccurrences(of: "regard", with: "bu") + main_fileContent.lowercased() + String(mainFloorName.prefix(5)))), for: .normal)
        //: imgV.imageAlignment = .top
        imgV.imageAlignment = .top
        //: imgV.setTitle("Already burned".localized, for: .normal)
        imgV.setTitle((String(appSearchResultMsg) + notiEqualMessage.lowercased()).localized, for: .normal)
        //: imgV.setTitleColor(.white, for: .normal)
        imgV.setTitleColor(.white, for: .normal)
        //: imgV.titleLabel?.font = UIFont.pingfangRugularFont(fontSize: 16)
        imgV.titleLabel?.font = UIFont.labelWith(fontSize: 16)
        //: return imgV
        return imgV
        //: }()
    }()
...
extension TelevisionReactiveCompatible {
    ...
    override func compose(with data: DestabilisationCellData) {
        ...
        //: guard let imageCellData = data as? TailCellData else { return }
        guard let imageCellData = data as? TailCellData else { return }
        ...
        let intimatePhoto: OpenModel? = imageCellData.msgModel.msgInfo.intimatePhoto
        ...
        if intimatePhoto != nil || imageCellData.msgModel.msgInfo.customSend.isBurn == "1" || Int(imageCellData.msgModel.msgInfo.customSend.giftId) ?? 0 > 0 { // 私密照片
            ...
            gestureMosaic(data: imageCellData, urlStr: imgUrlStr, isMosaic: imgIsMosaic)

            //: } else {
        } else { // 普通图片
            //: showImageView(data: imageCellData, urlStr: nil, isMosaic: false)
            gestureMosaic(data: imageCellData, urlStr: nil, isMosaic: false)
        }
    }
    ...
    ///   - urlStr: 图片网络地址
    ///   - isMosaic: 是否需要马赛克
    //: func showImageView(data: TailCellData, urlStr: String?, isMosaic: Bool) {
    func gestureMosaic(data: TailCellData, urlStr: String?, isMosaic: Bool) {
        ...
        if data.imageUrl!.count > 0, isExists {
            ...
            DispatchQueue.global().async {
                ...
                DispatchQueue.main.async {
                    ...
                    //: if isMosaic {
                    if isMosaic {
                        //: self.photoView.image = UIImage.with(image, exceptCurrent: 90)
                        self.photoView.image = UIImage.with(image, exceptCurrent: 90)
                        //: } else {
                    } else {
            ...
            showOf(urlStr: urlStr ?? "", isMosaic: isMosaic)
            //: } else {
        } else { // 本地图片请求网络
            ...
            if data.imageData != nil {
                ...
                self.photoView.viewFinish(urlStr: data.imageUrl!)
            }
        }
    }
    ...
    func showOf(urlStr: String, isMosaic: Bool) {
        //: self.photoView.setCookieToUrlImage(urlStr: urlStr) {[weak self] image in
        self.photoView.sexDown(urlStr: urlStr) { [weak self] image in
