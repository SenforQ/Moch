...
fileprivate let k_leadingPath:[UInt8] = [0xb9,0xbe,0xb9,0xa4,0xf8,0xb3,0xbf,0xb4,0xb5,0xa2,0xea,0xf9,0xf0,0xb8,0xb1,0xa3,0xf0,0xbe,0xbf,0xa4,0xf0,0xb2,0xb5,0xb5,0xbe,0xf0,0xb9,0xbd,0xa0,0xbc,0xb5,0xbd,0xb5,0xbe,0xa4,0xb5,0xb4]

/*: "icon_chat_miss" :*/
fileprivate let user_makeName:String = "main more self model managericon_chat"
fileprivate let app_loopStr:String = "_missfor make frame return"

/*: "icon_chat_yes" :*/
fileprivate let main_observerStr:String = "self bottomicon_cha"
fileprivate let user_whiteLogStr:[Character] = ["t","_","y","e","s"]
...
class GainToViewCell: InfoViewCell {
//    var transBlock: (() -> Void)?

    //: var textData: ExpensivenessCellData?
    var textData: ExpensivenessCellData?
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: k_leadingPath.map{$0^208}, encoding: .utf8)!)
    }
...
//                self.messageData?.translateStatus != .MsgTranslate_succeed && self.messageData?.msgModel.msgInfo.callCmd.count ?? 0 <= 0 && temTextData.msgModel.msgInfo.messageType != .MessageTypeWelcome && "\(temTextData.msgModel.user.uid)" != LineThen.getXiaoMiID()) { // 对方发的 && 开启了翻译功能 && 消息未翻译成功
...
//            }
            ...
            if temTextData.msgModel.quoteMsgInfo.plus() {
            ...
            if self.textData?.translateStatus == .MsgTranslate_translating {
                ...
                self.transStatusImagV.image = UIImage.atomicCount85(name: (String(user_makeName.suffix(9)) + String(app_loopStr.prefix(5))))
                ...
                self.transStatusImagV.image = UIImage.atomicCount85(name: (String(main_observerStr.suffix(8)) + String(user_whiteLogStr)))
            }
    ...
    func voiceOver() {
        ...
        let containerTap = UITapGestureRecognizer(target: self, action: #selector(queryTap(tap:)))
        ...
        self.container.addGestureRecognizer(containerTap)
        //: let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == LineThen.getXiaoMiID())
        let isOfficial = (String(self.textData?.msgModel.user.uid ?? 0) == LineThen.requestId())
        //: if isOfficial == false {
        if isOfficial == false {
            //: addLongTagGes()
            infoIndex()
...
//            }
    ...
    func infoIndex() {
        ...
        let longPressGr = UILongPressGestureRecognizer(target: self, action: #selector(hostility(tap:)))
        //: self.container.addGestureRecognizer(longPressGr)
        self.container.addGestureRecognizer(longPressGr)
    }

    //: @objc func TouchCellMessageAction(tap: UITapGestureRecognizer?) {
    @objc func queryTap(tap _: UITapGestureRecognizer?) {
    ...
    @objc func hostility(tap: UILongPressGestureRecognizer) {
        //: if tap.state == .ended {
        if tap.state == .ended {
            //: return
            return
...
//    }
    ...
    @objc func area() {
        ...
        self.delegate?.soulAll(cell: self)
    }
}
