...
/*: "#ffffff" :*/
fileprivate let data_modelName:String = "#ffffffas layer"

/*: "earned" :*/
fileprivate let mainHiddenStr:[Character] = ["e","a","r","n","e"]
fileprivate let constWithMessage:String = "D"

/*: "Coins in" :*/
fileprivate let user_infoKeyFormat:String = "type add try iCoins in"

/*: "Error:  :*/
fileprivate let app_sexShowLetMsg:String = "Erleading cell key view border"
fileprivate let mainContentPath:[Character] = ["r","o","r",":"," "]

/*: "#C946D2" :*/
fileprivate let dataEventKey:[Character] = ["#","C","9","4","6","D","2"]

/*: "btn_chat_floatS_userBK_nor" :*/
fileprivate let data_streamValueKey:[Character] = ["b","t","n","_","c","h","a","t","_","f","l","o","a","t","S","_","u","s"]
fileprivate let user_appHeadUrl:String = "manager self user actualerB"
fileprivate let appTotalFinishTitle:String = "return view modelK_nor"
...
// __CLOSE_PRINT__
//
//  BowlingSightTextTargetFloatingPositionView.swift
//  AbroadTalking
//
...
class BowlingSightTextTargetFloatingPositionView: UIView {
    ...
    private let TextColor = (String(data_modelName.prefix(7)))
    ...
    private func showBar() {
        ...
        if danumuMsgModel.bgPic.count > 0 {
            ...
            impressionGift(withUrl: danumuMsgModel.bgPic)
        }
        ...
        if danumuMsgModel.content.count > 0 {
            ...
            do {
                ...
                if let range = attrStr.string.range(of: (String(mainHiddenStr) + constWithMessage.lowercased())) {
                ...
                if let range = attrStr.string.range(of: (String(user_infoKeyFormat.suffix(8)))) {
                    //: attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                    attrStr.addAttributes([.foregroundColor: UIColor.white], range: NSRange(range, in: attrStr.string))
                }
                //: attrStr.addAttributes([.font: UIFont.pingfangFont(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                attrStr.addAttributes([.font: UIFont.photographicEquipment(type: .Medium, fontSize: 12)], range: NSRange(location: 0, length: attrStr.length))
                ...
                printLog(message: (String(app_sexShowLetMsg.prefix(2)) + String(mainContentPath)) + "\(error)")
            }
        }
    }

    //: func updateBackground(withUrl url: String) {
    func impressionGift(withUrl url: String) {
        ...
        self.backgroundImageView.luculentUpwardly(urlStr: url) { [weak self] _, image in
            ...
            if let image {
                ...
                if image.size.height > self.HeightOfBackgroundImageView {
                    ...
                    newImage = image.mix(with: CGSize(width: targetWidth, height: self.HeightOfBackgroundImageView))
                }
            }
    ...
    var totalTime: CGFloat {
        ...
        return (constExplainViewMsg + widthValue) * SpendTime / constExplainViewMsg
    }

    //: var totalTimeOfLine: CGFloat {
    var totalTimeOfLine: CGFloat {
        //: return widthValue * LineSpendTime / ScreenWidth
        return widthValue * LineSpendTime / constExplainViewMsg
    }

    //: var hide2ShowTime: CGFloat {
    var hide2ShowTime: CGFloat {
        //: return widthValue * SpendTime / ScreenWidth
        return widthValue * SpendTime / constExplainViewMsg
    }
    ...
    lazy var headAnimatedImageView: UIImageView = {
        ...
        h.layer.borderColor = UIColor(hex: (String(dataEventKey)))?.cgColor
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var headBackImageView: UIImageView = {
        ...
        h.image = UIImage.atomicCount85(name: (String(data_streamValueKey) + String(user_appHeadUrl.suffix(3)) + String(appTotalFinishTitle.suffix(5))))
        //: return h
        return h
        //: }()
    }()
    ...
    lazy var tapGesture: UITapGestureRecognizer = {
        ...
        let t = UITapGestureRecognizer(target: self, action: #selector(performanceShared(_:)))
        //: return t
        return t
        //: }()
    }()
...
extension BowlingSightTextTargetFloatingPositionView {
    /// 点击跳转事件
    //: @objc func tapAction(_ tapGesture: UITapGestureRecognizer) {
    @objc func performanceShared(_: UITapGestureRecognizer) {
        //: guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        guard let gameUrl = self.danumuMsgModel?.gameUrl else { return }
        //: let httpsUrl = NowInfoThen.share.appConfigMode.urlH5Domain+gameUrl
        let httpsUrl = NowInfoThen.share.appConfigMode.urlH5Domain + gameUrl
        ...
        config.clearBgColor = true
        //: TotalReactiveCompatible.share.func__pushToWebVC(urlStr: httpsUrl, webConfig: config)
        TotalReactiveCompatible.share.constraintImage(urlStr: httpsUrl, webConfig: config)
    }
}
