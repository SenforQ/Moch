...
/*: "rYMsgType" :*/
fileprivate let appStatusMsg:[Character] = ["r","Y","M","s","g","T","y"]
fileprivate let user_withNumberimateKey:String = "pgift"

/*: "GJ:CallCustom" :*/
fileprivate let kLivePath:String = "count textGJ:Ca"
fileprivate let k_toTitle:String = "STOM"
...
class SessionThen: NSObject {
    ...
    private var showView: ImageNotificationView? // 当前View
    //: private var preView: TalkingMessageNotificationView?
    private var preView: ImageNotificationView? // 旧的View
    ...
    @objc func ofName() {
        ...
        if self.preView != nil {
            ...
            self.preView?.itemThumb()
        }
        //: if self.showView != nil {
        if self.showView != nil {
            //: self.showView?.dismiss()
            self.showView?.itemThumb()
        }
    ...
    private lazy var msgArr: [CommentReactiveCompatible] = //: return [CommentReactiveCompatible]()
        .init()
    //: }()
}
...
extension SessionThen {
    /// 处理接收到的TX消息，展示消息通知
    /// - Parameters:
    ///   - msg: V2TIMMessage
    ///   - dataDict: 消息字典
    //: func onRecvNewMessage(msg: V2TIMMessage, dataDict: [String: Any]) {
    func tipOrigin(msg: V2TIMMessage, dataDict: [String: Any]) {
        ...
        guard counto() == true || petition() == true else { return }
        //: guard msg.userID != LineThen.getCustomerServiceID() && msg.userID != LineThen.getXiaoMiID() else {
        guard msg.userID != LineThen.imageConnection(), msg.userID != LineThen.requestId() else {
            //: return
            return
        }
        //: let contenType = TUISocialChatManager.messageCellType(dataDict)
        let contenType = SendReactiveCompatible.iconCamera(dataDict)
        // 提示消息
        ...
        if contenType == .TXMessageContentType_text, dataDict[(String(appStatusMsg) + user_withNumberimateKey.replacingOccurrences(of: "gift", with: "e"))] as? String == (String(kLivePath.suffix(5)) + "llCu" + k_toTitle.lowercased()) {
            //: return
            return
        }

        // 2. V2TIMMessage => CommentReactiveCompatible
        ...
        //: if let userInfo: LengthThen = DBUserInfoManager.cache_getCachedUserInfo(targetId: model.targetId) {
        if let userInfo: LengthThen = AtReactiveCompatible.viewText(targetId: model.targetId) {
    ...
    private func viewEqual() {
        ...
        let notifView = ImageNotificationView(frame: .zero)
        //: notifView.show(firstModel)
        notifView.someEqual(firstModel)
    ...
    private func counto() -> Bool {
        //: guard let currentVC = currentViewController() else { return false }
        guard let currentVC = buttonController() else { return false }
        //: if currentVC.isKind(of: TalkingVideoChatViewController.self) ||
        if currentVC.isKind(of: BeautyOfViewController.self) ||
            //: currentVC.isKind(of: TalkingVoiceChatViewController.self) {
            currentVC.isKind(of: ViewEqualObserverDelegate.self)
        {
            //: return true
            return true
        }
