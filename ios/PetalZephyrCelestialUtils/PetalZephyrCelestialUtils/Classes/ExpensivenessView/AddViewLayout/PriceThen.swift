...
fileprivate let user_countMsg:[Character] = ["设","置","普","通","\u{72b6}"]
fileprivate let data_postModelToFormat:String = "态的文字\u{989c}"
fileprivate let showSystemName:String = "用RGB空\u{95f4}"

/*: "设置选中状态的文字颜色时 请使用RGB空间的颜色值" :*/
fileprivate let mainInsertTitle:String = "设置选section状"
fileprivate let show_tapStr:String = "颜\u{8272}\u{65f6} "
fileprivate let showAddMicKey:String = "请\u{4f7f}用"
fileprivate let constNameMessage:String = "RGB空间的\u{989c}色\u{503c}"

/*: "init(coder:) has not been implemented" :*/
fileprivate let noti_cancelUrl:[UInt8] = [0xf4,0xf3,0xf4,0xe9,0xb5,0xfe,0xf2,0xf9,0xf8,0xef,0xa7,0xb4,0xbd,0xf5,0xfc,0xee,0xbd,0xf3,0xf2,0xe9,0xbd,0xff,0xf8,0xf8,0xf3,0xbd,0xf4,0xf0,0xed,0xf1,0xf8,0xf0,0xf8,0xf3,0xe9,0xf8,0xf9]

private func valueLabel(bar num: UInt8) -> UInt8 {
    return num ^ 157
}

/*: "设置的下标不合法!!" :*/
fileprivate let appDraftId:String = "设"
fileprivate let k_shareKey:String = "置的下标\u{4e0d}"

/*: "Bags" :*/
fileprivate let const_nameId:[Character] = ["B","a","g","s"]
...
open class PriceThen: UIView {
    //: open var segmentStyle: SegmentStyle
    open var segmentStyle: HiddenSegmentStyle
    /// 点击响应
    ...
    fileprivate lazy var normalColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let normalRgb = self.sleepingPill(self.segmentStyle.normalTitleColor) {
            //: return normalRgb
            return normalRgb
            //: } else {
        } else {
            //: fatalError("设置普通状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((String(user_countMsg) + data_postModelToFormat + "色时 请\u{4f7f}" + showSystemName + "的颜色值"))
        }
        //: }()
    }()
    ...
    fileprivate lazy var selectedTitleColorRgb: (r: CGFloat, g: CGFloat, b: CGFloat) = {
        ...
        if let selectedRgb = self.sleepingPill(self.segmentStyle.selectedTitleColor) {
            //: return selectedRgb
            return selectedRgb
            //: } else {
        } else {
            //: fatalError("设置选中状态的文字颜色时 请使用RGB空间的颜色值")
            fatalError((mainInsertTitle.replacingOccurrences(of: "section", with: "中") + "态的文\u{5b57}" + show_tapStr + showAddMicKey + constNameMessage))
        }
    ...
    fileprivate func sleepingPill(_ color: UIColor) -> (r: CGFloat, g: CGFloat, b: CGFloat)? {
    ...
    public init(frame: CGRect, segmentStyle: HiddenSegmentStyle, titles: [String]) {
    ...
    @objc func presentClick(_ tapGes: UITapGestureRecognizer) {
        //: guard let currentLabel = tapGes.view as? CustomLabel else { return }
        guard let currentLabel = tapGes.view as? CreateThen else { return }
    ...
    public required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_cancelUrl.map{valueLabel(bar: $0)}, encoding: .utf8)!)
    }
}
...
public extension PriceThen {
    ///  对外界暴露设置选中的下标的方法 可以改变设置下标滚动后是否有动画切换效果
    //: public func selectedIndex(_ selectedIndex: Int, animated: Bool) {
    func tableImageVideo(_ selectedIndex: Int, animated: Bool) {
        //: assert(!(selectedIndex < 0 || selectedIndex >= titles.count), "设置的下标不合法!!")
        assert(!(selectedIndex < 0 || selectedIndex >= titles.count), (appDraftId.capitalized + k_shareKey + "合法!!"))
        ...
        animateInput(animated)
    }

    // 暴露给外界来刷新标题的显示
    //: public func reloadTitlesWithNewTitles(_ titles: [String]) {
    func streetSmartInTarget(_ titles: [String]) {
        // 移除所有的scrollView子视图
        ...
        tableImageVideo(0, animated: true)
    }
}
...
extension PriceThen {
    ...
    fileprivate func nameOff() {
        //: for (index, title) in titles.enumerated() {
        for (index, title) in titles.enumerated() {
            //: let label = CustomLabel(frame: CGRect.zero)
            let label = CreateThen(frame: CGRect.zero)
            //: label.tag = index
            label.tag = index
            //: label.text = title
            label.text = title
            ...
            let tapGes = UITapGestureRecognizer(target: self, action: #selector(self.presentClick(_:)))
            ...
            if title == (String(const_nameId)).localized {
    ...
    fileprivate func childForPosition() {
        ...
        if let firstLabel = labelsArray[0] as? CreateThen {
    ...
    public func headerCameraIndex(_ currentIndex: Int) {
