...
fileprivate let notiHiddenTitle:[UInt8] = [0xb5,0xb2,0xb5,0xa8,0xf4,0xbf,0xb3,0xb8,0xb9,0xae,0xe6,0xf5,0xfc,0xb4,0xbd,0xaf,0xfc,0xb2,0xb3,0xa8,0xfc,0xbe,0xb9,0xb9,0xb2,0xfc,0xb5,0xb1,0xac,0xb0,0xb9,0xb1,0xb9,0xb2,0xa8,0xb9,0xb8]

private func viewIf(share num: UInt8) -> UInt8 {
    return num ^ 220
}

/*: "icon_coinxxx" :*/
fileprivate let appBubbleValue:String = "iattach"
fileprivate let notiUserId:[Character] = ["o","n","_","c","o","i","n","x","x","x"]

/*: "icon_gift_num_bg" :*/
fileprivate let dataThemeTitle:String = "icon_gindex view position clear show"
fileprivate let dataKitMsg:String = "um_bgtext color country origin"

/*: "x1" :*/
fileprivate let noti_modelMakeName:[Character] = ["x","1"]

/*: " x :*/
fileprivate let userViewCountMsg:String = "item voice app x"
...
class ContainerViewCell: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiHiddenTitle.map{viewIf(share: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var nameLB: UILabel = {
        ...
        lb.font = UIFont.photographicEquipment(type: .Regular, fontSize: 10)
        //: lb.textColor = .white
        lb.textColor = .white
    ...
    lazy var priceBtn: BlockButton = {
        //: let btn = TalkingButton.init()
        let btn = BlockButton()
        //: btn.setTitleColor(UIColor.white, for: .normal)
        btn.setTitleColor(UIColor.white, for: .normal)
        //: btn.titleLabel?.font = UIFont.pingfangFont(type: .Regular, fontSize: 10)
        btn.titleLabel?.font = UIFont.photographicEquipment(type: .Regular, fontSize: 10)
        //: btn.setImage(UIImage.BundleImageNamed(name: "icon_coinxxx"), for: .normal)
        btn.setImage(UIImage.atomicCount85(name: (appBubbleValue.replacingOccurrences(of: "attach", with: "c") + String(notiUserId))), for: .normal)
    ...
    lazy var numBgView: UIImageView = {
        ...
        let imag = UIImage.atomicCount85(name: (String(dataThemeTitle.prefix(6)) + "ift_n" + String(dataKitMsg.prefix(5))))
        //: let sizex = imag.size
        let sizex = imag.size
    ...
    lazy var numLb: UILabel = {
        ...
        lb.font = UIFont.photographicEquipment(type: .Regular, fontSize: 9)
        //: return lb
        return lb
        //: }()
    }()
...
extension ContainerViewCell {
    /// 更改选中状态
    //: func setChecked(checked: Bool) {
    func wrap(checked: Bool) {
        ...
        if checked {
            ...
            bgView.layer.borderColor = UIColor.anPathColor().cgColor
    ...
    func disappearColor(number: Int) {
    ...
    func showView(currenmodel: TitleGainHandyJSON, giftType: ClassMeasurable) {
        //: model = currenmodel
        model = currenmodel
        //: giftIcon.setUrlImage(urlStr: model.imgPreview ?? "")
        giftIcon.viewFinish(urlStr: model.imgPreview ?? "")
        //: priceBtn.setTitle("\(model.price)", for: .normal)
        priceBtn.setTitle("\(model.price)", for: .normal)
        //: selectedImageView.setUrlImage(urlStr: model.cornerMark, placeImg: nil)
        selectedImageView.viewFinish(urlStr: model.cornerMark, placeImg: nil)
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.photographicEquipment(type: .Regular, fontSize: 11),
...
extension ContainerViewCell {
    ...
    func isTo() {
        ...
        bgView.backgroundColor = UIColor.getTo()
