...
/*: "uid" :*/
fileprivate let userDetailMessage:[Character] = ["u","i","d"]

/*: "onlineStatus" :*/
fileprivate let noti_colorCountShowId:String = "ONLI"

/*: "isNewUser" :*/
fileprivate let showTitleData:String = "contents"
fileprivate let noti_stopKey:String = "NewUserlive true"

/*: "isTPAuth" :*/
fileprivate let k_equalSectionFormat:String = "isTPAuthin height live content"

/*: "totalIntimate" :*/
fileprivate let app_resultBackgroundStr:String = "totalIntview fail in any view"
fileprivate let constBirthdayFormat:String = "imusere"

/*: "userStatus" :*/
fileprivate let dataAddUrl:String = "userStatuser make style"
fileprivate let show_coverSectionFormat:String = "removes"

/*: "intimate" :*/
fileprivate let noti_rawChangeKey:[Character] = ["i","n"]
fileprivate let k_phoneFileFormat:String = "sizeimasizee"
...
enum SugarCookieAdditiveArithmetic: Int {
    //: case top  = 0
    case top = 0 // 置顶
    //: case normal = 1
    case normal = 1 // 默认
...
class ViewListManager {
    ...
    private var moreMsgModel: CommentReactiveCompatible = //: return CommentReactiveCompatible.getPlaceholderModel(chatType: .moreMsg)
        .tag(chatType: .moreMsg)
    //: }()

    // 初始化数据
    //: func req_initData() {
    func tempData() {
        //: self.officialArr = OndSessionConversationListener.shared.getOfficialList()
        self.officialArr = OndSessionConversationListener.shared.fast()
        //: self.topArr = OndSessionConversationListener.shared.topConvList
        self.topArr = OndSessionConversationListener.shared.topConvList
        //: self.normalArr = OndSessionConversationListener.shared.norConvList
        self.normalArr = OndSessionConversationListener.shared.norConvList
        // 更多数据
        //: req_moreMsgInitData()
        dataCorner()
    ...
    func lechery(completion: @escaping (_ succeed: Bool) -> Void) {
        //: guard self.isHalf == false else { return }
        guard self.isHalf == false else { return }
        //: OndSessionConversationListener.shared.getChatRoomData { succeed in
        OndSessionConversationListener.shared.pastMapSearch { succeed in
            //: if succeed {
            if succeed {
                //: self.chatRoomArr = OndSessionConversationListener.shared.chatRoomList
                self.chatRoomArr = OndSessionConversationListener.shared.chatRoomList
                //: completion(true)
                completion(true)
            }
    ...
    func conversationNameStyleFullPhaseOfTheMoonExperience(indexPath: IndexPath) -> CommentReactiveCompatible? {
    ...
    func constraintUser(model: CommentReactiveCompatible?) -> CommentReactiveCompatible? {
        //: guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        guard let targetId = model?.targetId, targetId.isEmpty == false else { return model }
        //: let userInfo: LengthThen? = DBUserInfoManager.cache_getCachedUserInfo(targetId: targetId)
        let userInfo: LengthThen? = AtReactiveCompatible.viewText(targetId: targetId)
    ...
    func blacklistCamera(conversationList: [CommentReactiveCompatible], finish: (() -> Void)?) {
        //: var tempArr = [String]()
        var tempArr = [String]()
        ...
        AtReactiveCompatible.executeCompletion(with: tempArr) { _, error in
    ...
    func removeFinishModel(conversationModel: CommentReactiveCompatible) {
    ...
    func postPageCompletion(completion: @escaping (_ isFinish: Bool) -> Void) {
        //: OndSessionConversationListener.shared.func__getConversationList { arr, leftMore in
        OndSessionConversationListener.shared.nothings { _, leftMore in
            //: completion(leftMore)
            completion(leftMore)
        }
    }
    ...
    lazy var officialArr: [CommentReactiveCompatible] = //: return Array<CommentReactiveCompatible>()
        .init()
    //: }()

    //: lazy var topArr: [TalkingConversationModel] = {
    lazy var topArr: [CommentReactiveCompatible] = //: return Array<CommentReactiveCompatible>()
        .init()
    //: }()

    //: lazy var normalArr: [TalkingConversationModel] = {
    lazy var normalArr: [CommentReactiveCompatible] = //: return Array<CommentReactiveCompatible>()
        .init()
    //: }()

    //: lazy var chatRoomArr: [TalkingConversationModel] = {
    lazy var chatRoomArr: [CommentReactiveCompatible] = //: return Array<CommentReactiveCompatible>()
        .init()
    //: }()

    //: lazy var intimateNorArr: [TalkingConversationModel] = {
    lazy var intimateNorArr: [CommentReactiveCompatible] = //: return Array<CommentReactiveCompatible>()
        .init()
    //: }()

    //: lazy var intimateTopArr: [TalkingConversationModel] = {
    lazy var intimateTopArr: [CommentReactiveCompatible] = //: return Array<CommentReactiveCompatible>()
        .init()
    //: }()

    //: lazy var moreMsgArr: [TalkingConversationModel] = {
    lazy var moreMsgArr: [CommentReactiveCompatible] = //: return Array<CommentReactiveCompatible>()
        .init()
    //: }()
}
...
extension ViewListManager {
    ...
    func dataCorner() {
        //: self.moreMsgArr = OndSessionConversationListener.shared.moreMsgList
        self.moreMsgArr = OndSessionConversationListener.shared.moreMsgList
    }

    /// 根据索引获取模型（更多消息）
    //: func req_moreMsgConversationModel(indexPath: IndexPath) -> TalkingConversationModel? {
    func modelUp(indexPath: IndexPath) -> CommentReactiveCompatible? {
    ...
    func someName() -> CommentReactiveCompatible {
        // 更新更多消息最新数据
        //: req_moreMsgInitData()
        dataCorner()
        ...
        moreMsgModel.unreadCount = workbox()
...
extension ViewListManager {
    ...
    func textCompletion(isFirst: Bool, completion: @escaping (_ succeed: Bool) -> Void) {
        ...
        AtReactiveCompatible.clickToolCompletion(with: data) { resultArr, _ in
            ...
            if resultArr?.count ?? 0 <= 0 { // 无数据
                ...
                resultArr?.forEach { dict in
                    ...
                    let uid = json[(String(userDetailMessage))].stringValue
                    //: let onlineStatus = json["onlineStatus"].intValue
                    let onlineStatus = json[(noti_colorCountShowId.lowercased() + "neStatus")].intValue
                    //: let isNewUser = json["isNewUser"].boolValue
                    let isNewUser = json[(showTitleData.replacingOccurrences(of: "content", with: "i") + String(noti_stopKey.prefix(7)))].boolValue
                    //: let istpAuth = json["isTPAuth"].boolValue
                    let istpAuth = json[(String(k_equalSectionFormat.prefix(8)))].boolValue
                    //: let totalIntimate = json["totalIntimate"].int ?? 0
                    let totalIntimate = json[(String(app_resultBackgroundStr.prefix(8)) + constBirthdayFormat.replacingOccurrences(of: "user", with: "at"))].int ?? 0
                    //: let userStatus = json["userStatus"].int ?? 1
                    let userStatus = json[(String(dataAddUrl.prefix(8)) + show_coverSectionFormat.replacingOccurrences(of: "remove", with: "u"))].int ?? 1
                    ...
                    self.consumerLab(uid: uid, intimate: totalIntimate)
                    //: })
                }
            }
            //: completion(true)
    ...
    func theSource(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(String(userDetailMessage))] as! String
        //: let onlineStatus = userInfo["onlineStatus"] as! Int
        let onlineStatus = userInfo[(noti_colorCountShowId.lowercased() + "neStatus")] as! Int
        //: let isNewUser = userInfo["isNewUser"] as! Bool
        let isNewUser = userInfo[(showTitleData.replacingOccurrences(of: "content", with: "i") + String(noti_stopKey.prefix(7)))] as! Bool

        /// 用户状态更新
        //: let userStatus = userInfo["userStatus"] as? Int ?? 1
        let userStatus = userInfo[(String(dataAddUrl.prefix(8)) + show_coverSectionFormat.replacingOccurrences(of: "remove", with: "u"))] as? Int ?? 1
        //: var needuserStatus = false
        var needuserStatus = false
        //: if userInfo.keys.contains("userStatus") {
        if userInfo.keys.contains((String(dataAddUrl.prefix(8)) + show_coverSectionFormat.replacingOccurrences(of: "remove", with: "u"))) {
            //: needuserStatus = true
            needuserStatus = true
        }
...
extension ViewListManager {
    ...
    func reject() {
        ...
        self.intimateTopArr.removeAll()
        //: for model in OndSessionConversationListener.shared.norConvList {
        for model in OndSessionConversationListener.shared.norConvList {
            //: if model.totalIntimate >= NowInfoThen.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= NowInfoThen.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
                let userModel = self.constraintUser(model: model)
                //: self.intimateNorArr.append(userModel!)
                self.intimateNorArr.append(userModel!)
            }
        }
        //: for model in OndSessionConversationListener.shared.topConvList {
        for model in OndSessionConversationListener.shared.topConvList {
            //: if model.totalIntimate >= NowInfoThen.share.appUserConfigMode.intimateLimit.enterTab {
            if model.totalIntimate >= NowInfoThen.share.appUserConfigMode.intimateLimit.enterTab {
                //: let userModel = self.getUserInfoFromCache(model: model)
    ...
    func spreadName(convModel: CommentReactiveCompatible, isTop: Bool) {
        //: if isTop {
        if isTop {
    ...
    func sectionStart(indexPath: IndexPath) -> CommentReactiveCompatible? {
        //: switch indexPath.section {
        switch indexPath.section {
    ...
    func consumerLab(uid: String, intimate: Int) {
        ...
        AtReactiveCompatible.lade(with: uid, intimate: intimate, currTime: Int(Date().timeIntervalSince1970))
    }

    /// 更新消息列表单个用户亲密度
    //: func update_userIntimate(userInfo: [String: Any]) -> Bool {
    func doingceCountimate(userInfo: [String: Any]) -> Bool {
        //: let uid = userInfo["uid"] as! String
        let uid = userInfo[(String(userDetailMessage))] as! String
        //: let intimate = userInfo["intimate"] as! Int
        let intimate = userInfo[(String(noti_rawChangeKey) + k_phoneFileFormat.replacingOccurrences(of: "size", with: "t"))] as! Int
        //: var updateSucceed = false
        var updateSucceed = false
        //: if let topModel = self.topArr.filter({ $0.userID == uid }).first {
        if let topModel = self.topArr.filter({ $0.userID == uid }).first {
