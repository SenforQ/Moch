...
/*: "gifts" :*/
fileprivate let data_contentTitle:String = "GIFTS"

/*: "vipGifts" :*/
fileprivate let k_priceToTitle:[Character] = ["v","i","p","G","i","f","t","s"]

/*: "packGifts" :*/
fileprivate let main_sexName:String = "packGiftsdata result index"

/*: "giftSendItem" :*/
fileprivate let user_containerMsg:[Character] = ["g","i","f","t","S"]
fileprivate let constCoverTitle:String = "econtent"
fileprivate let main_statusMessage:[Character] = ["d","I","t","e","m"]
...
// __CLOSE_PRINT__
//
//  ProjectReactiveCompatible.swift
//  AbroadTalking
//
...
class ProjectReactiveCompatible: NSObject {
    /// 是否展示背包红点
    ...
    private var cacheDict = [String: [ClassMeasurable: [Any]]]()
    // 数据锁
    ...
    private var giftType = ModelTextLiteral.normal

    //: static let share = TalkingChatGiftManager()
    static let share = ProjectReactiveCompatible()
...
extension ProjectReactiveCompatible {
    /// 发送礼物面板数据
    /// - Parameters:
    ...
    func progressCompletion(type: ModelTextLiteral = .normal, isRefresh: Bool = false, completion: @escaping () -> Void) {
        //: TalkingChatGiftManager.share.giftType = type
        ProjectReactiveCompatible.share.giftType = type
        //: if isRefresh == false {
        if isRefresh == false {
            //: let dataDict = cacheDict[type.rawValue]
            let dataDict = cacheDict[type.rawValue]
            //: let hotArr = dataDict?[GiftDataType.Hot] ?? [Any]()
            let hotArr = dataDict?[ClassMeasurable.Hot] ?? [Any]()
        ...
        if type == .groupChat {
            ...
            ProjectReactiveCompatible.share.sizeStart(completion: { isSucceed in
            ...
            ProjectReactiveCompatible.share.blunt(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            ProjectReactiveCompatible.share.dataTo(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
            ...
            ProjectReactiveCompatible.share.hiddenCompletion(completion: { isSucceed in
                //: guard isSucceed else { return }
                guard isSucceed else { return }
                //: let dataDict = self.cacheDict[type.rawValue]
                let dataDict = self.cacheDict[type.rawValue]
    ...
    private func hiddenCompletion(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = ProjectReactiveCompatible.share.giftType
        //: MonetaryValueThen.req_ChatGiftList(type: type.rawValue) { succeed, result, errorModel in
        MonetaryValueThen.completionDown(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
                //: var dataDict = [GiftDataType: [Any]]()
                var dataDict = [ClassMeasurable: [Any]]()
                // hot礼物
                //: if let array: Array = data["gifts"] as? Array<Any> {
                if let array: Array = data[(data_contentTitle.lowercased())] as? [Any] {
                    //: var hotArr = [Any]()
                    var hotArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<TitleGainHandyJSON>.deserialize(from: array as? Array) {
                        //: hotArr.append(contentsOf: datas as [Any])
                        hotArr.append(contentsOf: datas as [Any])
                    }
                    //: dataDict[GiftDataType.Hot] = hotArr
                    dataDict[ClassMeasurable.Hot] = hotArr
                }

                // vip礼物
                //: if let array: Array = data["vipGifts"] as? Array<Any> {
                if let array: Array = data[(String(k_priceToTitle))] as? [Any] {
                    //: var vipArr = [Any]()
                    var vipArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<TitleGainHandyJSON>.deserialize(from: array as? Array) {
                    ...
                    dataDict[ClassMeasurable.Vip] = vipArr
                }

                // Bags礼物
                //: if let array: Array = data["packGifts"] as? Array<Any> {
                if let array: Array = data[(String(main_sexName.prefix(9)))] as? [Any] {
                    //: var bagsArr = [Any]()
                    var bagsArr = [Any]()
                    //: if let datas = Array<TalkingRoomGiftModel>.deserialize(from: array as? Array) {
                    if let datas = Array<TitleGainHandyJSON>.deserialize(from: array as? Array) {
                    ...
                    dataDict[ClassMeasurable.Bags] = bagsArr
                }
                ...
                var firstModel = BoundaryArrModel()
                //: firstModel.num = 1
                firstModel.num = 1
                ...
                let sendItem: Array = data[(String(user_containerMsg) + constCoverTitle.replacingOccurrences(of: "content", with: "n") + String(main_statusMessage))] as! [Any]
                ...
                for i in 0 ..< sendItem.count {
                    ...
                    if let configModel = JSONDeserializer<BoundaryArrModel>.deserializeFrom(dict: sendItem[i] as? [String: Any], designatedPath: nil) {
    ...
    private func sizeStart(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = TalkingChatGiftManager.share.giftType
        let type = ProjectReactiveCompatible.share.giftType
        //: MonetaryValueThen.req_ChatRoomGiftList(type: type.rawValue) { succeed, result, errorModel in
        MonetaryValueThen.afterTo(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func trackLabel(params: [String: Any], completion: @escaping (_ data: [Any]?) -> Void) {
        //: MonetaryValueThen.req_getChatRoomMembers(params: params, completion: { succeed, result, errorModel in
        MonetaryValueThen.scrub(params: params, completion: { succeed, result, _ in
            //: if succeed {
            if succeed {
                ...
                for dict in data {
                    ...
                    if let memberModel = JSONDeserializer<EqualMemberModel>.deserializeFrom(dict: dict as? [String: Any], designatedPath: nil) {
    ...
    private func blunt(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.live
        let type = ModelTextLiteral.live
        //: MonetaryValueThen.req_LiveGiftList(type: type.rawValue) { succeed, result, errorModel in
        MonetaryValueThen.followCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    private func dataTo(completion: @escaping (_ isSucceed: Bool) -> Void) {
        //: let type = GiftViewStyle.party
        let type = ModelTextLiteral.party
        //: MonetaryValueThen.req_partyGiftList(type: type.rawValue) { succeed, result, errorModel in
        MonetaryValueThen.errorVariantCompletion(type: type.rawValue) { succeed, result, _ in
            //: if succeed {
            if succeed {
                //: let data: Dictionary = result as! Dictionary<String, Any>
                let data: Dictionary = result as! [String: Any]
    ...
    func modelLikeType(perPageCount: Int, dataType: ClassMeasurable) -> Int {
    ...
    func imageType(dataType: ClassMeasurable) -> [Any] {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
    ...
    func indexApp() -> [Any] {
        ...
        if ProjectReactiveCompatible.share.giftType == .groupChat {
            //: return chatRoomSendItems
            return chatRoomSendItems
            //: } else if TalkingChatGiftManager.share.giftType == .live || TalkingChatGiftManager.share.giftType == .party {
        } else if ProjectReactiveCompatible.share.giftType == .live || ProjectReactiveCompatible.share.giftType == .party {
    ...
    func foil(dataType: ClassMeasurable, dataArr: [Any]) {
        //: cacheLock.lock()
        cacheLock.lock()
        //: defer {
        defer {
...
extension ProjectReactiveCompatible {
    ...
    private func betweenUp() {
        ...
        NotificationCenter.default.addObserver(self,
                                               ...
                                               selector: #selector(mapPackage),
                                               //: name: GIFT_REFRESHPACKAGE_NOTIFICATION,
                                               name: const_viewMessage,
                                               //: object: nil)
                                               object: nil)
    }
    ...
    @objc private func mapPackage() {
        //: gift_clearMemoryCache(showRed: true)
        limitRed(showRed: true)
    }
}
...
extension ProjectReactiveCompatible {
    ...
    func limitRed(showRed: Bool = false) {
        //: TalkingChatGiftManager.share.showBagsRed = showRed
        ProjectReactiveCompatible.share.showBagsRed = showRed
        //: cacheDict.removeAll()
        cacheDict.removeAll()
    }
}
