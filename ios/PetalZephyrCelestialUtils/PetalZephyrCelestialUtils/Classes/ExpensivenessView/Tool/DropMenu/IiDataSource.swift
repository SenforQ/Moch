...
fileprivate let data_contentTitle:[UInt8] = [0x99,0x9e,0x99,0x84,0xd8,0x93,0x9f,0x94,0x95,0x82,0xca,0xd9,0xd0,0x98,0x91,0x83,0xd0,0x9e,0x9f,0x84,0xd0,0x92,0x95,0x95,0x9e,0xd0,0x99,0x9d,0x80,0x9c,0x95,0x9d,0x95,0x9e,0x84,0x95,0x94]

private func setOut(transform num: UInt8) -> UInt8 {
    return num ^ 240
}

/*: "SHBDropMenuCell" :*/
fileprivate let main_scaleMsg:String = "SHBDropMeresult in color"
fileprivate let main_makeId:String = "mode"
fileprivate let show_userText:String = "uCellreturn message fail response succeed"

/*: "title" :*/
fileprivate let constExplainName:String = "titldirection"
...
public class IiDataSource: UIView {
    ...
    var menuBgColor: UIColor? {
        ...
        //: }
    } // = Config.EraseView.menuBgColor
    // 线条颜色
    //: var lineColor = Config.MenuCellConfig.lineColor
    var lineColor = Config.ValueTextConfig.lineColor
    // cell高度
    //: public var menuCellHeight = Config.MenuCellConfig.menuCellHeight
    public var menuCellHeight = Config.ValueTextConfig.menuCellHeight
    /**
     *  table最大高度限制
     *  默认：5 * cellHeight
     */
    //: var menuMaxHeight = Config.MenuCellConfig.menuMaxHeight
    var menuMaxHeight = Config.ValueTextConfig.menuMaxHeight
    /**
     *  小三角高度
     *  45°等腰三角形
     */
    ...
    public var menuStyle = Config.initialMenuStyle ?? IngredientMenuStyle.MenuDarkStyle {
        //: didSet {
        didSet {
            //: switch menuStyle {
            switch menuStyle {
    ...
    public class func titleArray(anchorView: UIView, titleArray: [String], imageArray: [UIImage]) -> IiDataSource {
        ...
        let menuView = beforeHeadBottom(anchorView: anchorView, menuArray: menuArray)
        //: return menuView
        return menuView
    }

    //: public class func pullDropDrownMenu(anchorView: UIView, menuArray: [DropMenuModel]) -> DropDownMenuView {
    public class func beforeHeadBottom(anchorView: UIView, menuArray: [CenterMenuModel]) -> IiDataSource {
        //: let window = LineThen.getWindow()
        let window = LineThen.getName()
        //: let menuView = DropDownMenuView()
        let menuView = IiDataSource()
...
//        menuView.refreshUI()
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_contentTitle.map{setOut(transform: $0)}, encoding: .utf8)!)
    }

    //: private func makeContentView() -> UIView {
    private func pub() -> UIView {
        //: let view  = UIView()
        let view = UIView()
        //: return view
        return view
    ...
    private func colorDoing() -> UITableView {
    ...
    private func miniUi() {
        ...
        mTable.register(MenuCell.self, forCellReuseIdentifier: (String(main_scaleMsg.prefix(9)) + main_makeId.replacingOccurrences(of: "mode", with: "n") + String(show_userText.prefix(5))))
...
extension IiDataSource: UITableViewDelegate, UITableViewDataSource {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: (String(main_scaleMsg.prefix(9)) + main_makeId.replacingOccurrences(of: "mode", with: "n") + String(show_userText.prefix(5))), for: indexPath) as! MenuCell
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        selectionAction?(indexPath.row, cellModel.title ?? (constExplainName.replacingOccurrences(of: "direction", with: "e")))
        //: animateRemoveView()
        voiceOf()

        //: menuDelegate?.didClickSelectedRow(index: indexPath.row, title: cellModel.title ?? "")
        menuDelegate?.equalApp(index: indexPath.row, title: cellModel.title ?? "")
    }
}
