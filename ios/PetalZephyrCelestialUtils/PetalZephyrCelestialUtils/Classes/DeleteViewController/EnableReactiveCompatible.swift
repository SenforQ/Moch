...
fileprivate let app_toTitle:[Character] = ["U","s","e","r","D","e","t","a","i","l","T","a","g"]
fileprivate let noti_selectedFormat:String = "Meif resume adjustment data"
fileprivate let userSuiteId:[Character] = ["C","e","l","l","I","D"]

/*: "UserDetailTagInterestCellID" :*/
fileprivate let userCenterPath:String = "social return self maximum placeUserD"
fileprivate let show_sectionMessage:String = "TagInsound text view"
fileprivate let appAddPath:String = "try stroke make right totCellID"

/*: "AIHobbyCellID" :*/
fileprivate let k_viewFormat:[Character] = ["A","I","H","o","b","b","y","C","e","l"]
fileprivate let userMaxId:String = "lIDelse true view until"
...
class EnableReactiveCompatible: TailRecognizerDelegate {
    ...
    private var tableHeightModel = ValueReactiveCompatible() // 记录分区高度
    ...
    private lazy var tableView: UITableView = {
        ...
        tableView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: noti_liveFormat, right: 0)
        ...
        tableView.register(AntiphonyProfileCell.self, forCellReuseIdentifier: AntiphonyProfileCell.className())
        //: tableView.register(TalkingUserDetailTagCell.self, forCellReuseIdentifier: "UserDetailTagMeCellID")
        tableView.register(AtTagCell.self, forCellReuseIdentifier: (String(app_toTitle) + String(noti_selectedFormat.prefix(2)) + String(userSuiteId)))
        //: tableView.register(TalkingUserDetailTagCell.self, forCellReuseIdentifier: "UserDetailTagInterestCellID")
        tableView.register(AtTagCell.self, forCellReuseIdentifier: (String(userCenterPath.suffix(5)) + "etail" + String(show_sectionMessage.prefix(5)) + "teres" + String(appAddPath.suffix(7))))
        //: tableView.register(TalkingUserDetailGiftCell.self, forCellReuseIdentifier: TalkingUserDetailGiftCell.className())
        tableView.register(QuantityView.self, forCellReuseIdentifier: QuantityView.className())
        //: tableView.register(TalkingUserDetailTagCell.self, forCellReuseIdentifier: "AIHobbyCellID")
        tableView.register(AtTagCell.self, forCellReuseIdentifier: (String(k_viewFormat) + String(userMaxId.prefix(3))))
        //: return tableView
        return tableView
        //: }()
    }()
...
extension EnableReactiveCompatible: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cellType: BeautyCaseIterable = self.sectionArr[indexPath.section]
        //: switch cellType {
        switch cellType {
        //: case .profileType:
        case .profileType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: AntiphonyProfileCell.className(), for: indexPath) as! AntiphonyProfileCell
            //: cell.configWithDataModel(userModel: userInfoModel)
            cell.accumulationBy(userModel: userInfoModel)
            //: self.setProfileCellHeight(signAttribute: cell.signLab.attributedText ?? NSAttributedString.init(string: ""))
            self.putDoing(signAttribute: cell.signLab.attributedText ?? NSAttributedString(string: ""))
            //: return cell
            return cell
        ...
        case .aboutMeType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: (String(app_toTitle) + String(noti_selectedFormat.prefix(2)) + String(userSuiteId)), for: indexPath) as! AtTagCell
            //: cell.configTagCellWithDataModel(type: TUserDetailCellType.aboutMeType, userModel: userInfoModel)
            cell.conversation(type: BeautyCaseIterable.aboutMeType, userModel: userInfoModel)
        ...
        case .intersetsType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: (String(userCenterPath.suffix(5)) + "etail" + String(show_sectionMessage.prefix(5)) + "teres" + String(appAddPath.suffix(7))), for: indexPath) as! AtTagCell
            //: cell.configTagCellWithDataModel(type: TUserDetailCellType.intersetsType, userModel: userInfoModel)
            cell.conversation(type: BeautyCaseIterable.intersetsType, userModel: userInfoModel)
            //: cell.collectionLayoutSubviewBlock = {[weak self] collectH in
            cell.collectionLayoutSubviewBlock = { [weak self] collectH in
                //: guard let self = self else { return }
                guard let self = self else { return }
        ...
        case .giftType:
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: QuantityView.className(), for: indexPath) as! QuantityView
...
extension EnableReactiveCompatible {
    ...
    private func putDoing(signAttribute: NSAttributedString) {
        ...
        var labelH = signAttribute.boundingRect(with: CGSize(width: constExplainViewMsg - 58, height: CGFloat(MAXFLOAT)), options: [.usesLineFragmentOrigin, .usesFontLeading], context: nil).size.height
...
extension EnableReactiveCompatible: JXPagingViewListViewDelegate {
...
extension EnableReactiveCompatible {
    ...
    func giftEqual(_ giftArr: [BuildTransformable],
                   //: _ sectionArr: [TUserDetailCellType],
                   _ sectionArr: [BeautyCaseIterable],
                   //: _ userModel: TalkingUserInfoModel) {
                   _ userModel: GainDetailsMeasurable)
    {
