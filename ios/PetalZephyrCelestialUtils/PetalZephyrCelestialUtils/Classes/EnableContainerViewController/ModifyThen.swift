...
fileprivate let userFrameTitle:[UInt8] = [0x5b,0x5c,0x5b,0x46,0x1a,0x51,0x5d,0x56,0x57,0x40,0x8,0x1b,0x12,0x5a,0x53,0x41,0x12,0x5c,0x5d,0x46,0x12,0x50,0x57,0x57,0x5c,0x12,0x5b,0x5f,0x42,0x5e,0x57,0x5f,0x57,0x5c,0x46,0x57,0x56]

private func blockHalfTo(file num: UInt8) -> UInt8 {
    return num ^ 50
}
...
class ModifyThen: TailRecognizerDelegate {
    //: var tranConfig: BasePresentTransition!
    var tranConfig: GainPresentTransition!
    //: init() {
    init() {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: self.tranConfig = BasePresentTransition.init(target: self)
        self.tranConfig = GainPresentTransition(target: self)
    }

    //: required init?(coder: NSCoder) {
    required init?(coder _: NSCoder) {
        //: fatalError("init(coder:) has not been implemented")
        fatalError(String(bytes: userFrameTitle.map{blockHalfTo(file: $0)}, encoding: .utf8)!)
    }
}

//: class BasePresentTransition: NSObject {
class GainPresentTransition: NSObject {
    ...
    var frameOfPresentedView = CGRect(x: 0, y: 0, width: constExplainViewMsg, height: user_deviceId * 0.6)
    ...
    func numerousness(corners: UIRectCorner, radius: CGFloat) {
...
extension GainPresentTransition: UIViewControllerTransitioningDelegate {
    ...
    func presentationController(forPresented presented: UIViewController, presenting: UIViewController?, source _: UIViewController) -> UIPresentationController? {
        ...
        let res = BaseModelRecognizerDelegate(transition: self, presentedViewController: presented, presentingViewController: presenting)
        //: return res
        return res
    }
}
...
extension GainPresentTransition: UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if toVC == targetVC {
            ...
            pushColor(transitionContext: transitionContext)
            //: } else {
        } else {
            //: dismissTransition(transitionContext: transitionContext)
            tilter(transitionContext: transitionContext)
        }
    }

    //: private func presentTransition(transitionContext: UIViewControllerContextTransitioning) {
    private func pushColor(transitionContext: UIViewControllerContextTransitioning) {
        //: let toVC = transitionContext.viewController(forKey: .to)
        let toVC = transitionContext.viewController(forKey: .to)
    ...
    private func tilter(transitionContext: UIViewControllerContextTransitioning) {
...
class BaseModelRecognizerDelegate: UIPresentationController, UIGestureRecognizerDelegate {
    //: var transition: BasePresentTransition!
    var transition: GainPresentTransition!

    //: init(transition: BasePresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    init(transition: GainPresentTransition, presentedViewController: UIViewController, presentingViewController: UIViewController?) {
    ...
    lazy var visualView: UIView = {
        ...
        view.frame = containerView?.bounds ?? CGRect(x: 0, y: 0, width: constExplainViewMsg, height: user_deviceId)
