...
fileprivate let noti_viewMaxNameMessage:[UInt8] = [0x64,0x65,0x74,0x6e,0x65,0x6d,0x65,0x6c,0x70,0x6d,0x69,0x20,0x6e,0x65,0x65,0x62,0x20,0x74,0x6f,0x6e,0x20,0x73,0x61,0x68,0x20,0x29,0x3a,0x72,0x65,0x64,0x6f,0x63,0x28,0x74,0x69,0x6e,0x69]

/*: "btn_live_toast_bg" :*/
fileprivate let k_detailName:[Character] = ["b","t","n","_","l","i","v","e","_","t","o","a","s","t","_","b","g"]

/*: "btn_live_toast_arrow" :*/
fileprivate let mainLineText:[Character] = ["b","t","n","_","l","i","v","e","_","t","o","a","s","t","_"]
fileprivate let notiFinishDeviceKey:String = "aroundow"
...
var data_topValue = false

//: class TalkingLiveTabToast: UIView {
class TheoryThen: UIView {
    // 气泡间距
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: noti_viewMaxNameMessage.reversed(), encoding: .utf8)!)
    }
    ...
    private lazy var bubbleImgView: UIImageView = {
        ...
        var img = UIImage.atomicCount85(name: (String(k_detailName))).resizableImage(withCapInsets: bubbleInsets, resizingMode: .stretch)
        //: imgV.image = img
        imgV.image = img
        //: return imgV
        return imgV
    ...
    private lazy var arrowImgView: UIImageView = {
        ...
        imgV.image = UIImage.atomicCount85(name: (String(mainLineText) + notiFinishDeviceKey.replacingOccurrences(of: "round", with: "rr")))
        //: return imgV
        return imgV
        //: }()
    }()
...
extension TheoryThen {
    /// 展示视图，5s后自动移除
    //: func show(superView: UIView) {
    func superior(superView: UIView) {
        //: guard LIVETABTOASTISSHOW == false else { return }
        guard data_topValue == false else { return }
        ...
        //: guard NowInfoThen.share.appUserConfigMode.liveTabTips.count > 0 else { return }
        guard NowInfoThen.share.appUserConfigMode.liveTabTips.count > 0 else { return }
        //: LIVETABTOASTISSHOW = true
        data_topValue = true

        //: textLab.text = NowInfoThen.share.appUserConfigMode.liveTabTips
        textLab.text = NowInfoThen.share.appUserConfigMode.liveTabTips
        //: let maxWidth = ScreenWidth - 48
        let maxWidth = constExplainViewMsg - 48
