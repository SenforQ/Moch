...
/*: "%@%@" :*/
fileprivate let showGiftContent:String = "%@%@"

/*: "/TXCache" :*/
fileprivate let main_gestureStr:String = "/TXCachebackground class model plus on"
...
protocol HearingThen: NSObject {
    //: func func__playerStatus(player: TalkingVideoPlayerManager, status: VideoPlayerStatus)
    func comment(player: VideoTitlePlayListener, status: CenterPlayerStatus)

    //: func func__playerDuration(player: TalkingVideoPlayerManager, duration: Int, currentTime: Int)
    func itemTime(player: VideoTitlePlayListener, duration: Int, currentTime: Int)

    //: func func__playerProgress(player: TalkingVideoPlayerManager, progress: CGFloat)
    func row(player: VideoTitlePlayListener, progress: CGFloat)
}

//: class TalkingVideoPlayerManager: NSObject {
class VideoTitlePlayListener: NSObject {
    //: var isPlaying = false
    var isPlaying = false
    //: var isMute = false
    var isMute = false
    ...
    open weak var delegate: HearingThen?
    //: var playStatus: VideoPlayerStatus?
    var playStatus: CenterPlayerStatus?
    //: static let shared = TalkingVideoPlayerManager()
    static let shared = VideoTitlePlayListener()
    ...
    lazy var player: TXVodPlayer = {
        ...
        let cachePath = NSString(format: "%@%@", NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first!, (String(main_gestureStr.prefix(8)))) as String
    ...
    lazy var timer: Timer = {
        ...
        let timer = Timer.scheduledTimer(timeInterval: 0.25, target: self, selector: #selector(giftBlock), userInfo: nil, repeats: true)
...
extension VideoTitlePlayListener {
    //: func playerWithUrlAndVideoView(url: String, view: UIView) {
    func showWorldView(url: String, view: UIView) {
        //: let newUrl = url.replacingOccurrences(of: UrlDomin.urlAes256Decrypt(), with: ReplaceUrlDomain)
        let newUrl = url.replacingOccurrences(of: dataSessionUrl.increase(), with: main_bottomStr)
    ...
    @objc func giftBlock() {
        ...
        if duration() > 0 {
            ...
            self.delegate?.row(player: self, progress: CGFloat(currentTime() / duration()))
        }
    }

    //: func setRenderMode(renderMode: EnumTypeRenderMode) {
    func namePosition(renderMode: CommentRenderMode) {
    ...
    func magnitudeervalAdd(time: Float) {
    ...
    func muteSample(bEnable: Bool) {
        //: var isMute = bEnable
        var isMute = bEnable
        //: if TalkingSocketManager.shared.isTarget == true {
        if BankManagerSocketDelegate.shared.isTarget == true { // 音视频通话时静音
            //: isMute = true
            isMute = true
        }
    ...
    func elementGift() {
        ...
        if self.playStatus != .Pausing {
            ...
            self.delegate?.comment(player: self, status: self.playStatus!)
        }
    }

    //: func isPlayings() -> Bool {
    func componentView() -> Bool {
...
extension VideoTitlePlayListener: TXVodPlayListener {
    ...
    func onPlayEvent(_: TXVodPlayer!, event EvtID: Int32, withParam _: [AnyHashable: Any]!) {
        ...
        case PLAY_EVT_PLAY_PROGRESS.rawValue:
            ...
            self.delegate?.itemTime(player: self, duration: Int(duration), currentTime: Int(currentTime))
