...
/*: "id" :*/
fileprivate let appDismissText:[Character] = ["i","d"]
...
struct EqualAddMeasurable: HandyJSON {
    //: var uid: String?
    var uid: String?
    //: var sex: String?
    var sex: String?
    ...
    var pic = [InfoMeasurable]()
    //: var momentType: String?
    var momentType: String?
    //: var videoInfo: TalkingMomentVideoModel?
    var videoInfo: MomentVideoBeautyModelMeasurable?
...
enum CountSubscriptType: String {
    //: case nones  = "0"
    case nones = "0"
    //: case Photo = "1"
    case Photo = "1"
...
struct MomentVideoBeautyModelMeasurable: HandyJSON {
    //: var videoUrl = ""
    var videoUrl = ""
    //: var imgUrl = ""
    var imgUrl = ""
    ...
    var status: PointStatusClusterLiteral?
}

///
//: enum MomentVideoStatus: String {
enum PointStatusClusterLiteral: String {
    //: case NoPass = "-1"   // 未通过
    case NoPass = "-1" // 未通过
    //: case Audit = "0"     // 审核中
    case Audit = "0" // 审核中
...
extension EqualAddMeasurable {
    //: mutating func caculateItemHeight(isPost: Bool = false) {
    mutating func anima(isPost: Bool = false) {
        ...
        if content!.count > 0 {
            ...
            textHeight = Int(LineationReactiveCompatible.viewCon(text: content!, size: CGSize(width: constExplainViewMsg - 54, height: CGFloat(MAXFLOAT)), font: UIFont.photographicEquipment(type: .Regular, fontSize: 17), lineNumber: lineNumber).height + (lineNumber == 0 ? 20 : 0))
        }

        //: if momentType == MomentType.Photo.rawValue || momentType == MomentType.Live.rawValue {
        if momentType == CountSubscriptType.Photo.rawValue || momentType == CountSubscriptType.Live.rawValue {
            //: if pic.count==1 {
            if pic.count == 1 {
                //: photesHeight = 212
                photesHeight = 212
