...
/*: "id" :*/
fileprivate let noti_waitStr:String = "itip"

/*: "%@ reply %@：%@" :*/
fileprivate let dataKindPath:[Character] = ["%","@"," ","r","e","p","l","y"," ","%","@","：","%","@"]

/*: " %@：%@" :*/
fileprivate let app_sizeIconText:String = " %title：%title"
...
extension VideoModelType {
    //: mutating func caculateReplyItemHeight() {
    mutating func highVideo() {
        //: var tempContent = ""
        var tempContent = ""
        //: if replyUser.count>0 {
        if replyUser.count > 0 {
            //: tempContent = String(format: "%@ reply %@：%@", nickname, replyUser, content)
            tempContent = String(format: (String(dataKindPath)), nickname, replyUser, content)
            //: } else {
        } else {
        ...
        let sizeheight = Int(LineationReactiveCompatible.viewCon(text: tempContent, size: CGSize(width: constExplainViewMsg - 82, height: CGFloat(MAXFLOAT)), font: UIFont.photographicEquipment(type: .Regular, fontSize: 14), lineNumber: 0).height)
