...
fileprivate let show_photoMessage:[UInt8] = [0x9f,0x98,0x9f,0x82,0xde,0x95,0x99,0x92,0x93,0x84,0xcc,0xdf,0xd6,0x9e,0x97,0x85,0xd6,0x98,0x99,0x82,0xd6,0x94,0x93,0x93,0x98,0xd6,0x9f,0x9b,0x86,0x9a,0x93,0x9b,0x93,0x98,0x82,0x93,0x92]

private func letImage(top num: UInt8) -> UInt8 {
    return num ^ 246
}
...
class ModelThen: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: show_photoMessage.map{letImage(top: $0)}, encoding: .utf8)!)
    }
    ...
    private lazy var progressView: ResplendenceReactiveCompatible = {
        ...
        let v = ResplendenceReactiveCompatible(frame: self.frame, progressBgColor: self.progressBgColor, progressPlayedColor: self.progressPlayedColor, progressCachedColor: self.progressCachedColor)
    ...
    private lazy var panGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(deadline(gesture:)))
        //: return v
        return v
        //: }()
    }()
    ...
    private lazy var tapGesture: UIPanGestureRecognizer = {
        ...
        let v = UIPanGestureRecognizer(target: self, action: #selector(atGesture(gesture:)))
        //: return v
        return v
        //: }()
    }()
...
extension ModelThen {
    ...
    @objc func deadline(gesture: UIPanGestureRecognizer) {
        // 移动的距离
    ...
    @objc func atGesture(gesture: UIPanGestureRecognizer) {
