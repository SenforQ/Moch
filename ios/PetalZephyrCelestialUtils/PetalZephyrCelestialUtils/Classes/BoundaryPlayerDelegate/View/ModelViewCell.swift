...
fileprivate let dataTableValue:[UInt8] = [0x1,0x6,0x1,0x1c,0x40,0xb,0x7,0xc,0xd,0x1a,0x52,0x41,0x48,0x0,0x9,0x1b,0x48,0x6,0x7,0x1c,0x48,0xa,0xd,0xd,0x6,0x48,0x1,0x5,0x18,0x4,0xd,0x5,0xd,0x6,0x1c,0xd,0xc]

private func pingShared(button num: UInt8) -> UInt8 {
    return num ^ 104
}
...
class ModelViewCell: UICollectionViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: dataTableValue.map{pingShared(button: $0)}, encoding: .utf8)!)
    }
    ...
    func snapPlay() {
        ...
        let doubletap = UITapGestureRecognizer(target: self, action: #selector(front))
...
extension ModelViewCell: UIScrollViewDelegate {
    //: @objc func imageViewClick(tap: UIGestureRecognizer) {
    @objc func cellTap(tap _: UIGestureRecognizer) {
        //: if sblock != nil {
        if sblock != nil {
            //: sblock()
            sblock()
    ...
    @objc func front(tap: UIGestureRecognizer) {
    ...
    func ad(url: String) {
        ...
        imageView.viewFinish(urlStr: url)
    }

    //: func configImage(image: UIImage) {
    func configSumeraction(image: UIImage) {
