...
fileprivate let kMBackgroundData:[UInt8] = [0x88,0x8f,0x88,0x95,0xc9,0x82,0x8e,0x85,0x84,0x93,0xdb,0xc8,0xc1,0x89,0x80,0x92,0xc1,0x8f,0x8e,0x95,0xc1,0x83,0x84,0x84,0x8f,0xc1,0x88,0x8c,0x91,0x8d,0x84,0x8c,0x84,0x8f,0x95,0x84,0x85]

private func talkData(up num: UInt8) -> UInt8 {
    return num ^ 225
}
...
class EqualCookieView: UICollectionView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: kMBackgroundData.map{talkData(up: $0)}, encoding: .utf8)!)
    }

    //: func configModel(model: TalkingMomentModel) {
    func simulation(model: EqualAddMeasurable) {
        //: cunrrenModel = model
        cunrrenModel = model
        //: self.reloadData()
        self.reloadData()
...
extension EqualCookieView: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, insetForSectionAt _: Int) -> UIEdgeInsets {
        ...
        if cunrrenModel.pic.count == 4 {
            ...
            return UIEdgeInsets(top: 0, left: 0, bottom: 0, right: constExplainViewMsg - 200 - 30)
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, sizeForItemAt _: IndexPath) -> CGSize {
        ...
        if cunrrenModel.pic.count == 1 {
            ...
            var width = isMyhost ? (constExplainViewMsg - 30 - 24 - 30) / 3 : (constExplainViewMsg - 30 - 24) / 3
    ...
    func collectionView(_: UICollectionView, layout _: UICollectionViewLayout, referenceSizeForHeaderInSection _: Int) -> CGSize {
        ...
        return CGSize(width: constExplainViewMsg, height: 1)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: TailThen = collectionView.dequeueReusableCell(withReuseIdentifier: TailThen.className(), for: indexPath) as! TailThen
        //: let model: TalkingMomentPhotoModel = cunrrenModel.pic[indexPath.row]
        let model: InfoMeasurable = cunrrenModel.pic[indexPath.row]
        //: cell.configUrl(url: model.thumbUrl ?? "")
        cell.afterPlay(url: model.thumbUrl ?? "")
    ...
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let vc = MomentScreenVc(momentModel: cunrrenModel, index: indexPath.row, type: .normal)
...
extension EqualCookieView {
    ...
    private func viewSubviews() {
        ...
        register(TailThen.self, forCellWithReuseIdentifier: TailThen.className())
        //: dataSource = self
        dataSource = self
        //: delegate = self
        delegate = self
