...
/*: "News Feed" :*/
fileprivate let kNameDayCaseFormat:String = "user"
fileprivate let constOnMsg:[Character] = ["e","w","s"," ","F","e","e","d"]

/*: "There's no feed yet." :*/
fileprivate let noti_dataKey:String = "There\'"
fileprivate let app_insertMessage:String = "feed succeed manager let public"
fileprivate let dataUpKey:[Character] = ["y","e","t","."]

/*: "The post has been deleted ~" :*/
fileprivate let constImageMsg:[Character] = ["T","h","e"," ","p"]
fileprivate let userAppLineAsFormat:String = "let content leading forost h"
fileprivate let user_nameMakeText:String = "een view format"
fileprivate let app_clickUrl:String = "ted ~asset app"
...
class ViewDataSource: TailRecognizerDelegate {
    //: private var pageIndex = 0
    private var pageIndex = 0
    //: private var dataArr: [TalkingRemindListModel] = []
    private var dataArr: [HiddenTargetListModel] = []
    ...
    override func viewDidLoad() {
        ...
        title = (kNameDayCaseFormat.replacingOccurrences(of: "user", with: "N") + String(constOnMsg)).localized
        //: setupSubviews()
        straitAndNarrowDisplay()
    ...
    lazy var emptyView: EmptyView = {
        ...
        style.TipsTitle = (noti_dataKey + "s no " + String(app_insertMessage.prefix(5)) + String(dataUpKey)).localized
    ...
    lazy var tableView: UITableView = {
        ...
        table.register(OpenThen.self, forCellReuseIdentifier: OpenThen.className())
        //: return table
        return table
        //: }()
    }()
...
extension ViewDataSource {
    ...
    private func buildVerbally() {
        //: TalkingMomentRequestTool.req_getRemindList(page: pageIndex) { data in
        HeritageRequestTool.appImage(page: pageIndex) { data in
...
extension ViewDataSource: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: OpenThen.className(), for: indexPath) as! OpenThen
        //: cell.refreshCell(dataArr[indexPath.row])
        cell.somebody(dataArr[indexPath.row])
        //: return cell
        return cell
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if model.status == 0 {
            ...
            addMsg(showMsg: (String(constImageMsg) + String(userAppLineAsFormat.suffix(5)) + "as b" + String(user_nameMakeText.prefix(4)) + "dele" + String(app_clickUrl.prefix(5))).localized)
            //: return
            return
        }
        ...
        let detailVC = DetailReactiveCompatible(model: detailModel)
...
extension ViewDataSource {
    ...
    private func straitAndNarrowDisplay() {
