...
fileprivate let mainHideTitle:[UInt8] = [0x8c,0x8b,0x8c,0x91,0xcd,0x86,0x8a,0x81,0x80,0x97,0xdf,0xcc,0xc5,0x8d,0x84,0x96,0xc5,0x8b,0x8a,0x91,0xc5,0x87,0x80,0x80,0x8b,0xc5,0x8c,0x88,0x95,0x89,0x80,0x88,0x80,0x8b,0x91,0x80,0x81]

/*: "Save successfully" :*/
fileprivate let notiMessageKey:String = "Savelive lab let"
fileprivate let user_matchValue:String = "cmoressfully"

/*: / :*/
fileprivate let user_buttonSendPath:[Character] = ["/"]
...
class MomentScreenVc: DropReactiveCompatible {
    ...
    init(momentModel: EqualAddMeasurable, index: Int, type: AddPhotoType) {
        ...
        if WithLanguageManager.shared.direction == .rightToLeft {
            ...
            DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 0.3) {
                ...
                self.collecView.contentOffset = CGPoint(x: curren * Int(constExplainViewMsg), y: 0)
    ...
    init(images: [UIImage], index: Int, type: AddPhotoType) {
        //: super.init(nibName: nil, bundle: nil)
        super.init(nibName: nil, bundle: nil)
        //: myType = type
        myType = type
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainHideTitle.map{$0^229}, encoding: .utf8)!)
    }
    ...
    override func viewDidLoad() {
        ...
        courseOfAction(modle: currenModel)
//        if myType == .normal {
//            setDownloadView()
//        }
    }
    ...
    lazy var collecView: UICollectionView = {
        ...
        flayout.itemSize = CGSize(width: constExplainViewMsg, height: user_deviceId)
        ...
        view.frame = CGRect(x: 0, y: 0, width: constExplainViewMsg, height: user_deviceId)
        ...
        view.contentOffset = CGPoint(x: pageIndex * Int(constExplainViewMsg), y: 0)
        //: if myType == .normal {
        if myType == .normal {
            //: view.contentSize = CGSize(width: (currenModel.pic.count )*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (currenModel.pic.count) * Int(constExplainViewMsg), height: 0)
            //: } else {
        } else {
            //: view.contentSize = CGSize(width: (photos.count)*(Int(ScreenWidth)), height: 0)
            view.contentSize = CGSize(width: (photos.count) * Int(constExplainViewMsg), height: 0)
        }
        //: view.register(TalkingMomentPhotosPreviewCell.self, forCellWithReuseIdentifier: TalkingMomentPhotosPreviewCell.className())
        view.register(ModelViewCell.self, forCellWithReuseIdentifier: ModelViewCell.className())
...
//    lazy var downloadBtn: BlockButton = {
//        let btn = BlockButton.init()
...
//        LengthPermissionTool.func__openAlbumServiceWithBlock(true) { [weak self] (isOpen: Bool) in
//            if isOpen {
//                guard let self = self else {return}
//                guard  self.pageIndex >= 0 && self.pageIndex < self.currenModel.pic.count  else { return }
//                let model: InfoMeasurable = self.currenModel.pic[self.pageIndex]
...
//    }
    ...
    func usufructuary(image: UIImage) {
        ...
        PHPhotoLibrary.shared().performChanges({
            ...
            if isSuccess {
                ...
                DispatchQueue.main.async {
                    ...
                    self.beforeDemonstrate(showMsg: (String(notiMessageKey.prefix(4)) + " suc" + user_matchValue.replacingOccurrences(of: "more", with: "e")).localized)
                }
                //: } else {
            } else {
...
extension MomentScreenVc: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    //: func hideTopView() {
    func calendarWeek() {
...
//        downloadBtn.isHidden = isHideTopView
    ...
    func compartmentView() {
        //: if  currenModel.sex==NowInfoThen.share.loginUserMode.sex {
        if currenModel.sex == NowInfoThen.share.loginUserMode.sex {
            //: return
            return
        }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let cell: ModelViewCell = collectionView.dequeueReusableCell(withReuseIdentifier: ModelViewCell.className(), for: indexPath) as! ModelViewCell
        //: if myType == .normal {
        if myType == .normal {
            //: let model: TalkingMomentPhotoModel = currenModel.pic[indexPath.row]
            let model: InfoMeasurable = currenModel.pic[indexPath.row]
            //: if model.imgUrl?.count ?? 0 > 1 {
            if model.imgUrl?.count ?? 0 > 1 {
                //: cell.configUrl(url: model.imgUrl! )
                cell.ad(url: model.imgUrl!)
                //: }else {
            } else {
                //: cell.configImage(image: model.imagePic ?? UIImage.init())
                cell.configSumeraction(image: model.imagePic ?? UIImage())
            }

            //: } else {
        } else {
            //: cell.configImage(image: photos[indexPath.row])
            cell.configSumeraction(image: photos[indexPath.row])
        }
    ...
    func collectionView(_: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt _: IndexPath) {
        ...
        let tcell: ModelViewCell = cell as! ModelViewCell
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let currentIndex = Int(offSetWidth / constExplainViewMsg)
        //: if myType == .normal {
        if myType == .normal {
