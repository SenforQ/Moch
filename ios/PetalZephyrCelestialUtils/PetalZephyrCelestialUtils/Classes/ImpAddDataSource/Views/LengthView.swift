...
fileprivate let mainLabelRenderStr:[UInt8] = [0x70,0x75,0x70,0x7b,0x2f,0x6a,0x76,0x6b,0x6c,0x79,0x41,0x30,0x27,0x6f,0x68,0x7a,0x27,0x75,0x76,0x7b,0x27,0x69,0x6c,0x6c,0x75,0x27,0x70,0x74,0x77,0x73,0x6c,0x74,0x6c,0x75,0x7b,0x6c,0x6b]

fileprivate func userColor(lab num: UInt8) -> UInt8 {
...
/*: "icon_rank_coin" :*/
fileprivate let mainActualKey:[UInt8] = [0x2b,0x21,0x2d,0x2c,0x1d,0x30,0x23,0x2c,0x29,0x1d,0x21,0x2d,0x2b,0x2c]

private func attentionEffectTitle(now num: UInt8) -> UInt8 {
    return num ^ 66
}

/*: "icon_lounge" :*/
fileprivate let mainSocialUrl:String = "info appicon_"
fileprivate let userTopValue:[Character] = ["l"]
fileprivate let dataImageMessage:[Character] = ["o","u","n","g","e"]

/*: "icon_rank_coin2" :*/
fileprivate let k_tagMsg:String = "count fillicon_"
fileprivate let app_moreFormat:String = "succeednk"
...
class LengthView: UITableViewCell {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: mainLabelRenderStr.map{userColor(lab: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var rankLabel = UILabel().then {
        ...
        $0.textColor = UIColor.someRegard()
        //: $0.font = .pingfangFont(type: .Semibold, fontSize: 20)
        $0.font = .photographicEquipment(type: .Semibold, fontSize: 20)
    }
    ...
    lazy var coinIconBtn = BlockButton().then {
        //: $0.setImage(UIImage.BundleImageNamed(name: "icon_rank_coin"), for: .normal)
        $0.setImage(UIImage.atomicCount85(name: String(bytes: mainActualKey.map{attentionEffectTitle(now: $0)}, encoding: .utf8)!), for: .normal)
        //: $0.setTitleColor(UIColor.white, for: .normal)
        $0.setTitleColor(UIColor.white, for: .normal)
        //: $0.titleLabel?.font = .pingfangFont(type: .Medium, fontSize: 15)
        $0.titleLabel?.font = .photographicEquipment(type: .Medium, fontSize: 15)
        ...
        $0.setBackgroundImage(UIImage.gradientMore(colors: UIColor.showCorner(), size: CGSize(width: actualWidth(w: 80), height: actualWidth(w: 33))), for: .normal)
    ...
    lazy var loungeImgV = UIImageView().then {
        ...
        $0.image = UIImage.atomicCount85(name: (String(mainSocialUrl.suffix(5)) + String(userTopValue) + String(dataImageMessage)))
    }
}
...
extension LengthView {
    ...
    func byError(model: MusicItemModel, index: Int) {
        ...
        headPic.viewFinish(urlStr: model.headPic)
    ...
    func optimize(type: Int) {
        //: let imageStr = type == 0 ? "icon_rank_coin":"icon_rank_coin2"
        let imageStr = type == 0 ? String(bytes: mainActualKey.map{attentionEffectTitle(now: $0)}, encoding: .utf8)! : (String(k_tagMsg.suffix(5)) + app_moreFormat.replacingOccurrences(of: "succeed", with: "ra") + "_coin2")
        //: coinIconBtn.setImage(UIImage.BundleImageNamed(name: imageStr), for: .normal)
        coinIconBtn.setImage(UIImage.atomicCount85(name: imageStr), for: .normal)
    }
}
