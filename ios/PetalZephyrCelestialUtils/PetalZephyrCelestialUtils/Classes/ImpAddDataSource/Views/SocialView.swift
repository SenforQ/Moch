...
fileprivate let data_timeKey:[UInt8] = [0x39,0x3e,0x39,0x24,0x78,0x33,0x3f,0x34,0x35,0x22,0x6a,0x79,0x70,0x38,0x31,0x23,0x70,0x3e,0x3f,0x24,0x70,0x32,0x35,0x35,0x3e,0x70,0x39,0x3d,0x20,0x3c,0x35,0x3d,0x35,0x3e,0x24,0x35,0x34]

private func viewWarning(fill num: UInt8) -> UInt8 {
    return num ^ 80
}

/*: "Not yet~" :*/
fileprivate let main_backTitle:[Character] = ["N","o","t"," ","y"]
fileprivate let k_bagUrl:[Character] = ["e","t","~"]

/*: "#8C7AFF" :*/
fileprivate let showPartyId:String = "#8C7AFFindex head"
...
class SocialView: UIView {
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: data_timeKey.map{viewWarning(fill: $0)}, encoding: .utf8)!)
    }
    ...
    lazy var dataArray: [MusicItemModel] = [] {
        //: didSet {
        didSet {
    ...
    lazy var ranktableView = UITableView(frame: self.bounds, style: .plain).then {
        ...
        $0.register(AppReactiveCompatible.self, forCellReuseIdentifier: AppReactiveCompatible.className())
        //: $0.register(SocialRankListViewCell.self, forCellReuseIdentifier: SocialRankListViewCell.className())
        $0.register(LengthView.self, forCellReuseIdentifier: LengthView.className())
    }
    ...
    lazy var tableHeaderView = ModelInfoReactiveCompatible(frame: CGRect(x: 0, y: 0, width: constExplainViewMsg, height: actualHeight(h: 220 + 14))).then {
        //: $0.backgroundColor = .clear
        $0.backgroundColor = .clear
        //: $0.index = self.type
        $0.index = self.type
    ...
    lazy var tableBackgrourdView = SkinnyThen().then {
        //: $0.backgroundColor = .white
        $0.backgroundColor = .white
        //: $0.desLab.text = "Not yet~".localized
        $0.desLab.text = (String(main_backTitle) + String(k_bagUrl)).localized
        //: $0.desLab.textColor = UIColor(hex: "#8C7AFF")
        $0.desLab.textColor = UIColor(hex: (String(showPartyId.prefix(7))))
...
extension SocialView: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: AppReactiveCompatible.className(), for: indexPath) as! AppReactiveCompatible
            //: return cell
            return cell
        }
        ...
        let cell = tableView.dequeueReusableCell(withIdentifier: LengthView.className(), for: indexPath) as! LengthView
        //: let model = dataArray[indexPath.row+3]
        let model = dataArray[indexPath.row + 3]
        //: cell.updateRankCell(model: model, index: indexPath.row+3)
        cell.byError(model: model, index: indexPath.row + 3)
        //: cell.updateIcon(type: self.type)
        cell.optimize(type: self.type)
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 0 {
            ...
            let cell = tableView.dequeueReusableCell(withIdentifier: AppReactiveCompatible.className()) as! AppReactiveCompatible
...
extension SocialView: JXSegmentedListContainerViewListDelegate {
