...
fileprivate let notiGroupStr:[UInt8] = [0xa1,0xa6,0xa1,0xbc,0xe0,0xab,0xa7,0xac,0xad,0xba,0xf2,0xe1,0xe8,0xa0,0xa9,0xbb,0xe8,0xa6,0xa7,0xbc,0xe8,0xaa,0xad,0xad,0xa6,0xe8,0xa1,0xa5,0xb8,0xa4,0xad,0xa5,0xad,0xa6,0xbc,0xad,0xac]

/*: "btn_video_minimize" :*/
fileprivate let show_totalValue:String = "btn_if true gift index"
fileprivate let k_bringMsg:[Character] = ["o","_","m","i","n","i","m","i","z","e"]

/*: "btn_video_drop_nor" :*/
fileprivate let user_hairFormat:String = "burn varbtn_vi"
fileprivate let main_addTitle:String = "process imagedrop_no"
fileprivate let const_hiddenData:String = "source"

/*: "btn_video_turn_nor" :*/
fileprivate let userToUrl:[Character] = ["b","t","n","_","v","i","d","e","o"]
fileprivate let showColorTitle:String = "image"
fileprivate let appModelData:String = "leadingrn"

/*: "btn_video_turn_pre" :*/
fileprivate let data_mainViewUrl:String = "let to image countbtn_"
fileprivate let k_toCookieStatusStr:[Character] = ["_","t","u","r","n","_","p","r","e"]

/*: "Switch" :*/
fileprivate let noti_greenBorderName:[Character] = ["S","w","i","t","c","h"]

/*: "btn_video_start_nor" :*/
fileprivate let kMakeTableUrl:[Character] = ["b","t","n","_","v","i","d","e","o","_","s"]
fileprivate let constAppearData:String = "first make hidden let labtart_nor"

/*: "btn_video_start_pre" :*/
fileprivate let data_thenStr:[Character] = ["b","t","n","_","v"]
fileprivate let mainPlayerTakePath:String = "viewdeo"
fileprivate let noti_atLabelTitle:[Character] = ["r","t","_","p","r","e"]

/*: "Camera On" :*/
fileprivate let data_messageId:[Character] = ["C","a","m","e","r","a"," ","O","n"]

/*: "00:00" :*/
fileprivate let noti_cellName:String = "00:00"

/*: "Camera must be on" :*/
fileprivate let data_pathContent:String = "make let make model matchCame"
fileprivate let showDraftStr:String = " "
fileprivate let userControlName:String = "button to return letbe on"

/*: "Camera Off" :*/
fileprivate let main_giftModelFalsePath:String = "Cametext type succeed transform"
fileprivate let mainColorMsg:String = "to var center from returnra Off"

/*: "%02i:%02i" :*/
fileprivate let dataModelMsg:String = "%02i:%02gift lab face none false"
fileprivate let main_addKey:String = "theme"
...
class PositionCellView: UIView {
    //: weak var delegate: Talking1v1VideoInteractionViewDelegate?
    weak var delegate: TwoThreadObjectProtocol?
    //: private var chatModel = TalkingVideoChatModel()
    private var chatModel = NameChatModel() // 引用类型，整个视频通话共用同一个model
    ...
    required init?(coder _: NSCoder) {
        ...
        fatalError(String(bytes: notiGroupStr.map{$0^200}, encoding: .utf8)!)
    }
    ...
    private lazy var touchHidenBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(stateCheck), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var minimizeBtn: UIButton = {
        ...
        btn.setImage(UIImage.atomicCount85(name: (String(show_totalValue.prefix(4)) + "vide" + String(k_bringMsg))), for: .normal)
        //: btn.addTarget(self, action: #selector(minimizeButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(vox), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var hangupBtn: UIButton = {
        ...
        btn.setImage(UIImage.atomicCount85(name: (String(user_hairFormat.suffix(6)) + "deo_" + String(main_addTitle.suffix(7)) + const_hiddenData.replacingOccurrences(of: "source", with: "r"))), for: .normal)
        //: btn.addTarget(self, action: #selector(hangupButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(stateWith), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var rotateCameraBtn: UIButton = {
        ...
        btn.setImage(UIImage.atomicCount85(name: (String(userToUrl) + showColorTitle.replacingOccurrences(of: "image", with: "_") + appModelData.replacingOccurrences(of: "leading", with: "tu") + "_nor")), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_video_turn_pre"), for: .selected)
        btn.setImage(UIImage.atomicCount85(name: (String(data_mainViewUrl.suffix(4)) + "video" + String(k_toCookieStatusStr))), for: .selected)
        //: btn.addTarget(self, action: #selector(rotateCameraButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(titleFull), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var rotateCameraDesLab: UILabel = {
        ...
        lab.text = (String(noti_greenBorderName)).localized
        //: return lab
        return lab
        //: }()
    }()
    ...
    private lazy var openCameraBtn: UIButton = {
        ...
        btn.setImage(UIImage.atomicCount85(name: (String(kMakeTableUrl) + String(constAppearData.suffix(8)))), for: .normal)
        //: btn.setImage(UIImage.BundleImageNamed(name: "btn_video_start_pre"), for: .selected)
        btn.setImage(UIImage.atomicCount85(name: (String(data_thenStr) + mainPlayerTakePath.replacingOccurrences(of: "view", with: "i") + "_sta" + String(noti_atLabelTitle))), for: .selected)
        //: btn.addTarget(self, action: #selector(openCameraButtonClick), for: .touchUpInside)
        btn.addTarget(self, action: #selector(velleity), for: .touchUpInside)
        //: return btn
        return btn
        //: }()
    }()
    ...
    private lazy var openCameraDesLab: UILabel = {
        ...
        lab.text = (String(data_messageId)).localized
        //: return lab
        return lab
        //: }()
    }()
...
extension PositionCellView {
    /// 展示视图状态
    //: private func showViewsViaStage() {
    private func disappearModel() {
        ...
        self.timeLab.text = (noti_cellName.capitalized)
        //: startTalkCount()
        sizeSum()
        //: touchHiddenTimer()
        alongTimer()
    ...
    func primaryColourSuspend(view: UIView) {
    ...
    @objc private func stateWith() {
        //: uploadRecord.uploadRecordEvent(eventID: clickVideoCancelButton)
        dataMatchStr.isometric(eventID: dataDevicePath)
        //: closeViewClick()
        beginEqual()
    }
    ...
    @objc private func clickSub() {
        //: self.delegate?.interactionView_shouldAcceptCall()
        self.delegate?.actionOrientation()
    }
    ...
    @objc private func equalObserver() {
        //: acceptButtonClick()
        clickSub()
    ...
    @objc private func titleFull() {
        //: if SenseTime_Use == false, self.openCameraBtn.isSelected == true {
        if notiAppPath == false, self.openCameraBtn.isSelected == true { // 腾讯美颜 && 摄像头关闭时
            //: self.func__showStatusBarErrorMsg(showMsg: "Camera must be on".localized)
            self.addMsg(showMsg: (String(data_pathContent.suffix(4)) + "ra must" + showDraftStr.capitalized + String(userControlName.suffix(5))).localized)
            //: return
            return
        }
        ...
        self.delegate?.makeByFront(isFront: !self.rotateCameraBtn.isSelected)
    }
    ...
    @objc private func velleity() {
        ...
        self.openCameraDesLab.text = self.openCameraBtn.isSelected ? (String(main_giftModelFalsePath.prefix(4)) + String(mainColorMsg.suffix(6))).localized : (String(data_messageId)).localized
        //: self.delegate?.interactionView_shouldForbiddenCamera(forbidden: self.openCameraBtn.isSelected)
        self.delegate?.tabForbidden(forbidden: self.openCameraBtn.isSelected)
    }

    /// 点击展示/隐藏通话中的按钮
    //: @objc private func changeInteractionShowState() {
    @objc private func stateCheck() {
...
extension PositionCellView {
    ...
    private func alongTimer() {
        //: invalidateIdleTimer()
        feel()
        //: self.perform(#selector(hideInteractionView), afterDelay: 10)
        self.perform(#selector(pathRelease), afterDelay: 10)
    }
    ...
    private func feel() {
        ...
        NSObject.cancelPreviousPerformRequests(withTarget: self, selector: #selector(pathRelease), object: nil)
    }

    /// 延时到时隐藏视图
    //: @objc private func hideInteractionView() {
    @objc private func pathRelease() {
...
extension PositionCellView {
    ...
    private func videoConstraint() {
        ...
        hangupBtn.snp.makeConstraints { make in
            ...
            make.bottom.equalTo(-noti_limitPartyInfoUrl - 53)
        ...
        timeLab.snp.makeConstraints { make in
            ...
            make.top.equalTo(appImageLimitPath + 20)
        }
    }
