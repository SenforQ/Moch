...
// __CLOSE_PRINT__
//
//  AdjournWonderBoyModelType.swift
//  AbroadTalking
//
...
struct AdjournWonderBoyModelType: HandyJSON {
    //: var activity: SocialPeriodModel?
    var activity: CookieMeasurable?
    //: var charm: SocialPeriodModel?
    var charm: CookieMeasurable?
}

//: struct SocialPeriodModel: HandyJSON {
struct CookieMeasurable: HandyJSON {
    //: var yesterday: SocialResModel?
    var yesterday: DeleteModelType?
    //: var today: SocialResModel?
    var today: DeleteModelType?
    //: var weekly: SocialResModel?
    var weekly: DeleteModelType?
}

// 毫无意义的一层，等后端后期调整
//: struct SocialResModel: HandyJSON {
struct DeleteModelType: HandyJSON {
    //: var res: [SocialRankItemModel] = []
    var res: [MusicItemModel] = []
}

//: struct SocialRankItemModel: HandyJSON {
struct MusicItemModel: HandyJSON {
    //: var uid = 0
    var uid = 0
    //: var value = 0
    var value = 0
